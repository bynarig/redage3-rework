...
const props = withDefaults(defineProps<{
  popupData?: { title: string; elements: string | [string, string][] } | null
}>(), {
  popupData: () => ({ title: '', elements: [] }),
})

const elements = ref<[string, string][]>(
  typeof props.popupData?.elements === 'string'
    ? JSON.parse(props.popupData.elements)
    : (props.popupData?.elements as [string, string][]) ?? []
)
...
const handleKeyUp = (event: KeyboardEvent) => {
  ...
  switch (keyCode) {
    case 13:
      onSelected(elements.value[listId.value]?.[1] ?? 'null')
      break
    case 38:
