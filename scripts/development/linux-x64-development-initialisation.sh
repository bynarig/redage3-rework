...
echo "Starting Linux Setup for RAGE:MP..."

# Check for curl (essential for NVM and .NET installation)
if ! command -v curl > /dev/null 2>&1; then
    echo "Error: 'curl' is not installed. Please install it (e.g., 'sudo apt install curl' or 'sudo yum install curl') and try again."
    exit 1
fi
...
if ! command -v git > /dev/null 2>&1; then
    echo "Error: 'git' is not installed. Please install git (e.g., 'sudo apt install git' or 'sudo yum install git') and try again."
    exit 1
fi
