...
namespace NeptuneEvo.VehicleModel
{
    public class AirAutoRoom : Script
    {
        ...
        private static readonly List<string> AirCarList = new List<string>
        {
            "akula",
            "annihilator",
            "annihilator2",
        ...
        private static readonly float NpcBuyRotation = 169.37247f;

        //
        public static Vector3 NpcSpawnPosition = new Vector3(-750.6089, -1510.7106, 5.003792);

        private static readonly float NpcSpawnRotation = -22.12805f;

        //
        public static string NpcName = "npc_airshop";

        private static readonly List<string> Airs = new List<string>
        {
            "Havok",
            "Seasparrow2",
            "Seasparrow",
        ...
        private static int SelectAirPosition;

        private static readonly List<(Vector3, Vector3)> AirPositions = new List<(Vector3, Vector3)>
        {
        ...
        public void Event_ResourceStart()
        {
            ...
            Repository.CreateQuest("csb_reporter", NpcBuyPosition, NpcBuyRotation,
                title: "~y~NPC~w~ Продавец воздушного транспорта", colShapeEnums: ColShapeEnums.AirAutoRoom);
            //
            Main.CreateBlip(new Main.BlipData(602, "HeliPort", NpcSpawnPosition, 60, true));
            Repository.CreateQuest("s_m_y_airworker", NpcSpawnPosition, NpcSpawnRotation,
                title: "~y~NPC~w~ Работник Аэропорта", colShapeEnums: ColShapeEnums.AirSpawn);
        }
        ...
        public static void Perform(ExtPlayer player)
        {
            try
            {
                ...
                CarRoom.OpenCarromMenuGos(player, Airs);
            }
            catch (Exception e)
            {
        ...
        public static void OpenAirSpawn(ExtPlayer player, int index)
        {
            ...
            foreach (var number in vehiclesNumber)
            {
                ...
                if (BusinessManager.BusProductsData.ContainsKey(vehicleData.Model))
                    price = Wallet.GetPriceToVip(player, BusinessManager.BusProductsData[vehicleData.Model].Price);
