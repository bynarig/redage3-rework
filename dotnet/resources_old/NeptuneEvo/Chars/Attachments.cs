...
{
    internal class Attachments : Script
    {
        ...
        public static void AddAttachment(ExtPlayer player, uint attachmentHash, bool send = true)
        {
            try
            {
                ...
                if (sessionData != null && !sessionData.Attachments.Contains(attachmentHash))
                {
                    ...
                    if (send)
                        Trigger.SetSharedData(player, "attachmentsData",
                            JsonConvert.SerializeObject(sessionData.Attachments));
                }
            }
            catch (Exception e)
            {
                Log.Write($"AddAttachment Exception: {e}");
            }
        }
        ...
        public static bool HasAttachment(ExtPlayer player, uint attachmentHash)
        {
            ...
            {
                Log.Write($"HasAttachment Exception: {e}");
                return false;
            }
        }
        ...
        public void staticAttachmentsAdd(ExtPlayer player, string hash)
        {
            ...
            {
                Log.Write($"staticAttachmentsAdd Exception: {e}");
            }
        }
        ...
        public void staticAttachmentsSave(ExtPlayer player, string model, int BoneId, string sPos, string sRot)
        {
            try
            {
                var pos = JsonConvert.DeserializeObject<Vector3>(sPos);
                var rot = JsonConvert.DeserializeObject<Vector3>(sRot);
                using (var saveCoords = new StreamWriter("attachments.txt", true, Encoding.UTF8))
                {
                    Thread.CurrentThread.CurrentCulture = new CultureInfo("en-US");

                    var rand = new Random().Next(1, 9);
                    saveCoords.Write(
                        $"mp.attachments.register('{model}-{rand}', '{model}', {BoneId}, new mp.Vector3({pos.X}, {pos.Y}, {pos.Z}), new mp.Vector3({rot.X}, {rot.Y}, {rot.Z}), true);\r\n");
                    saveCoords.Close();
            ...
            {
                Log.Write($"staticAttachmentsSave Exception: {e}");
            }
        }
        ...
        public void staticAttachmentsRemove(ExtPlayer player, string hash)
        {
            ...
            {
                Log.Write($"staticAttachmentsRemove Exception: {e}");
            }
        }
        ...
        public void CMD_NAttachments(ExtPlayer player, string id, int boneName)
        {
            ...
            {
                Log.Write($"CMD_NAttachments Exception: {e}");
            }
        }
        ...
        public void CMD_animplayer(ExtPlayer player, string text)
        {
            ...
            {
                Log.Write($"CMD_animFlag Exception: {e}");
            }
        }
        ...
        public void CMD_findAnim(ExtPlayer player, int flag)
        {
            ...
            {
                Log.Write($"CMD_findAnim Exception: {e}");
            }
        }
