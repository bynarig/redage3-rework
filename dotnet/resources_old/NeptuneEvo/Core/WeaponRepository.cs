...
{
    internal class WeaponRepository : Script
    {
        ...
        private static readonly List<int> CheckWeapons = new List<int>
        {
            -1045183535, // Revolver
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (DataRow Row in result.Rows)
                    BusinessesLastSerial.Add(Convert.ToInt32(Row["id"]), Convert.ToInt32(Row["lastserial"]));
            }
            catch (Exception e)
            {
        ...
        public static string GetSerial(bool isFraction, int id, bool organization = false)
        {
            try
            {
                if (isFraction)
                {
                    ...
                    var fractionType = Manager.FractionTypes[id];
                    if (fractionType == FractionsType.Mafia || fractionType == FractionsType.Gangs ||
                        fractionType == FractionsType.Nongov ||
                        fractionType == FractionsType.Bikers) return $"{1000 + id}xxxxx";
        ...
        public static string GetAirdropSerial()
        {
            try
            {
                var serial = 100000 + AirdropLastSerial;
                AirdropLastSerial++;
        ...
        public static int GiveWeapon(ExtPlayer player, ItemId type, string serial, int count = 1)
        {
            try
            {
                ...
                var weaponHP = CustomDamage.WeaponsHP.ContainsKey(type) ? CustomDamage.WeaponsHP[type] : 100;
                if (characterData == null) return -1;
                if (Chars.Repository.isFreeSlots(player, type) != 0) return -1;
                return Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", type, count,
                    $"{serial}_{weaponHP}");
            }
            catch (Exception e)
            {
                Log.Write($"GiveWeapon Exception: {e}");
                return -1;
            }
        }
        ...
        public static void SaveWeaponsDB()
        {
            try
            {
                foreach (var dict in BusinessesLastSerial)
                    ...
                    {
                        Log.Write($"SaveWeaponsDB Foreach Exception: {e}");
                    }
            ...
            {
                Log.Write($"SaveWeaponsDB Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_playerRemove(ExtPlayer player)
        {
            try
            {
                ...
                if (Item.ItemId != ItemId.Ball && Item.ItemId != ItemId.Snowball)
                    return;
                //TakeWeapon(player);
            ...
            {
                Log.Write($"RemoteEvent_playerRemove Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_playerReload(ExtPlayer player, int ammoInClip)
        {
            try
            {
                ...
                if (ammoInClip >= WeaponsClipsMax[Item.ItemId]) return;
                var wAmmoType = WeaponsAmmoTypes[Item.ItemId];
                var wItemStruct = Chars.Repository.isItem(player, "inventory", wAmmoType);
                var ammoLefts = wItemStruct == null ? 0 : wItemStruct.Item.Count;
                if (ammoLefts == 0) return;
                var ammo = ammoLefts < WeaponsClipsMax[Item.ItemId] - ammoInClip
                    ? ammoLefts
                    : WeaponsClipsMax[Item.ItemId] - ammoInClip;
                Chars.Repository.RemoveIndex(player, wItemStruct.Location, wItemStruct.Index, ammo);
                Trigger.ClientEvent(player, "client.weapon.give", (int)GetHash(Item.ItemId.ToString()), ammo, true,
                    Item.ItemId);
            }
            catch (Exception e)
            {
                Log.Write($"RemoteEvent_playerReload Exception: {e}");
            }
        }
        ...
        public static void RemoveAmmo(ExtPlayer player, int hash, int ammoInClip)
        {
            try
            {
                ...
                var wName = ((Hash)hash).ToString();
                var ItemId = (ItemId)Enum.Parse(typeof(ItemId), wName);
                if (!WeaponsAmmoTypes.ContainsKey(ItemId)) return;
                var aType = WeaponsAmmoTypes[ItemId];
                var allAmmoCount = Chars.Repository.isFreeSlots(player, aType, ammoInClip, false);
                if (allAmmoCount == -1)
                {
                    Chars.Repository.ItemsDrop(player, new InventoryItemData(0, aType, ammoInClip));
                }
                else
                {
                    if (ammoInClip > WeaponsClipsMax[ItemId]) ammoInClip = WeaponsClipsMax[ItemId];
                    if (allAmmoCount == 0)
                        Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", aType,
                            ammoInClip, isInfo: false); //если влезают все патроны
                    else if (allAmmoCount > 0)
                        Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", aType,
                            allAmmoCount, isInfo: false);
                    if (allAmmoCount > 0)
                    {
            ...
            {
                Log.Write($"RemoteEvent_playerAmmoIn Exception: {e}");
            }
        }
        ...
        public static void RemoveHands(ExtPlayer player)
        {
            ...
            {
                Log.Write($"RemoveHands Exception: {e}");
            }
        }
        ...
        public static void TakeWeapon(ExtPlayer player) // Пока что думаем как реализовать instant переменную.
        {
            try
            {
                ...
                var ItemStruct = sessionData.ActiveWeap;
                if (ItemStruct.Item == null || ItemStruct.Index == -1) return;
                var Item = ItemStruct.Item.ItemId;
                if (Item == ItemId.Debug) return;

                var ItemInfo = Chars.Repository.ItemsInfo[Item];
                if (ItemInfo.functionType == newItemType.Weapons || ItemInfo.functionType == newItemType.MeleeWeapons)
                {
                    ...
                    Commands.RPChat("sme", player,
                        "убрал" + (characterData.Gender ? "" : "а") + $" {Chars.Repository.ItemsInfo[Item].Name}");
                ...
                {
                    var isUseItem = Chars.Repository.ItemsHands(player, ItemStruct.Location, ItemStruct.Index,
                        ItemStruct.Item);
                    if (isUseItem == 1)
                        Commands.RPChat("sme", player,
                            "убрал" + (characterData.Gender ? "" : "а") + $" {Chars.Repository.ItemsInfo[Item].Name}");
            ...
            {
                Log.Write($"TakeWeapon Exception: {e}");
            }
        }
        ...
        private static void OnStartTimerWeaponUpdate(ExtPlayer player, int nextSqlId)
        {
            ...
            {
                Log.Write($"OnStartTimerWeaponUpdate Exception: {e}");
            }
        }
        ...
        public static void PlayerKickAntiCheat(ExtPlayer player, byte reason, bool type)
        {
            try
            {
                ...
                if (!type)
                {
                    Trigger.SendToAdmins(1,
                        LangFunc.GetText(LangType.Ru, DataName.WasKickedAC, player.Name, player.Value, reas));
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                        LangFunc.GetText(LangType.Ru, DataName.YouWasKickedAc), 30000);
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                        LangFunc.GetText(LangType.Ru, DataName.AcIfUsure), 30000);
                ...
                else
                {
                    ...
                    Trigger.SendToAdmins(1,
                        "!{#DF5353}" + LangFunc.GetText(LangType.Ru, DataName.AutoACBan2, player.Name, reas));
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                        LangFunc.GetText(LangType.Ru, DataName.ReasonCheats), 30000);
            ...
            {
                Log.Write($"PlayerKickAntiCheat Exception: {e}");
        ...
        public void OnPlayerWeaponSwitch(ExtPlayer player, WeaponHash oldWeapon, WeaponHash newWeapon)
        {
            try
            {
                ...
                if (!characterData.IsAlive || !CheckWeapons.Contains((int)newWeapon) ||
                ...
                var wName = ((Hash)newWeapon).ToString();
                var itemname = (ItemId)Enum.Parse(typeof(ItemId), wName);
                var ItemStruct = Chars.Repository.isItem(player, "fastSlots", itemname);
                if (ItemStruct == null) PlayerKickAntiCheat(player, 1, false);
            }
            catch (Exception e)
            {
                Log.Write($"OnPlayerWeaponSwitch Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_changeWeapon(ExtPlayer player, int key)
        {
            try
            {
                ...
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (characterData.DemorganTime >= 1 || sessionData.DeathData.InDeath ||
                    sessionData.AntiAnimDown) return;
                key--;
                var Item = Chars.Repository.GetItemData(player, "fastSlots", key);
                if (Item.ItemId == ItemId.Debug && key != 4) return;
                var ItemInfo = Chars.Repository.ItemsInfo[Item.ItemId];
                if (key == 4)
                {
                    var _hItem = Chars.Repository.GetItemData(player, "accessories", 1);
                    if (Item.ItemId != ItemId.Mask && _hItem.ItemId != ItemId.Mask) return;
                    if (Item.ItemId == ItemId.Mask)
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouPutOnMask), 3000);
                    else
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouPutOffMask), 3000);
                ...
                if (ItemInfo.functionType == newItemType.Weapons || ItemInfo.functionType == newItemType.MeleeWeapons)
                {
                    ...
                    if (characterData.LVL == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantDoTimeWeapon), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantDoKPZ), 3000);
                        return;
                    }

                    var index = sessionData.ActiveWeap.Index;
                    TakeWeapon(player);
                    sessionData.ActiveWeap = new ItemStruct("", -1, null);
                    if (index == key) return;
                    var wHash = GetHash(Item.ItemId.ToString());
                    if (WeaponsAmmoTypes.ContainsKey(Item.ItemId))
                    {
                        var wAmmoType = WeaponsAmmoTypes[Item.ItemId];
                        var wItemStruct = Chars.Repository.isItem(player, "inventory", wAmmoType);
                        var ammoLefts = wItemStruct == null ? 0 : wItemStruct.Item.Count;
                        var ammoin = ammoLefts == 0 || ammoLefts < WeaponsClipsMax[Item.ItemId]
                            ? ammoLefts
                            : WeaponsClipsMax[Item.ItemId];
                        if (ammoin != 0)
                            Chars.Repository.RemoveIndex(player, wItemStruct.Location, wItemStruct.Index, ammoin);
                    ...
                    {
                        if (Item.ItemId == ItemId.Snowball)
                            Trigger.ClientEvent(player, "client.weapon.give", (int)wHash, 10, false, Item.ItemId);
            ...
            {
                Log.Write($"RemoteEvent_changeWeapon Exception: {e}");
            }
        }
