...
using FractionClothingSets = NeptuneEvo.Fractions.FractionClothingSets;
using Manager = NeptuneEvo.Organizations.Manager;
using Repository = NeptuneEvo.Players.Phone.Messages.Repository;

namespace NeptuneEvo.Core
{
    internal class Admin : Script
    {
        public static readonly nLog Log = new nLog("Core.Admin");
        public static bool IsServerStoping;
        public static bool TimeChanged = false;
        public static short[] SetTime = new short[3] { 0, 0, 0 };

        public static Dictionary<string, (string, string, int)> ChangeNameQueue =
            new Dictionary<string, (string, string, int)>(); // CurName, (newName, admName, admLvl)

        public static Dictionary<ExtPlayer, (int, string, int)> SetLeaderQueue =
            new Dictionary<ExtPlayer, (int, string, int)>(); // targetId (fracid, admName, admLvl)

        public static Dictionary<ExtPlayer, (string, int)>
            SendCreatorQueue = new Dictionary<ExtPlayer, (string, int)>(); // targetId (admName, admLvl)

        public static Dictionary<int, (string, string, int, DateTime)> GlobalQueue =
            new Dictionary<int, (string, string, int, DateTime)>(); // QueueID (globaltext, admName, admLvl, globalexp)
...

        {
...

            {
                ...
                Trigger.SendToAdmins(2,
                    $"{ChatColors.StrongOrange}[A] Запрос на выдачу лидерки для {player.Name} ({player.Value}) удалён в связи с {text}.");
            }
...

            {
                ...
                Trigger.SendToAdmins(2,
                    $"{ChatColors.StrongOrange}[A] Запрос на смену внешности для {player.Name} ({player.Value}) удалён в связи с {text}.");
            }
        }
        ...
        public static void AdminLog(int myadm, string message)
        {
            ...
            {
                Log.Write($"AdminLog Exception: {e}");
            }
        }

        public static void AdminsLog(int myadm, string message, byte levels_to = 2, string color = "#FFB833",
            bool highRankActionHide = true, byte hideAdminLevel = 8)
        {
            try
            {
                if (myadm >= hideAdminLevel && highRankActionHide)
                    return; // Не логировать в чат действия от спец.администратора и выше
            ...
            {
                Log.Write($"AdminsLog Exception: {e}");
            }
        }
        ...
        public static void ErrorLog(string message)
        {
            ...
            {
                Log.Write($"ErrorLog Exception: {e}");
            }
        }
        ...
        public static void onPlayerDeathHandler(ExtPlayer player, ExtPlayer entityKiller, uint weapon, Vector3 pos)
        {
            try
            {
                ...
                if (killerCharacterData != null)
                {
                    ...
                    sessionData.DeathData.LastDeath =
                        $"{DateTime.Now.ToString("s")} | {entityKiller.Name} ({entityKiller.Value}) убил {player.Name} с {weapon}";
                    GameLog.Kills($"player({killerCharacterData.UUID})", weapon.ToString(),
                        $"player({characterData.UUID})", $"{pos.X} {pos.Y} {pos.Z}");

                    var killerSessionData = entityKiller.GetSessionData();
                    if (killerSessionData != null && killerSessionData.KillData.DamageDisabled)
                    {
                        killerSessionData.KillData.Count++;
                        if (killerSessionData.KillData.Count == 1)
                            Notify.Send(entityKiller, NotifyType.Info, NotifyPosition.BottomCenter,
                                "Если Вы убьёте еще 1 человека до получения 1 уровня, то будете кикнуты", 10000);
        ...
        {
            Trigger.ClientEvent(player, "hud.kill", killer != null ? killer.Id : -1, victim != null ? victim.Id : -1,
                weapon);
        }
        ...
        public static void sendRedbucks(ExtPlayer player, ExtPlayer target, int amount)
        {
            try
            {
                ...
                UpdateData.RedBucks(target, amount, "Отправка RB");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.RbOutcome, target.Name, amount), 3000);
                ...
                Repository.AddSystemMessage(target, (int)DefaultNumber.RedAge,
                    LangFunc.GetText(LangType.Ru, DataName.RbIncome, amount, player.Name), DateTime.Now);
            }
            catch (Exception e)
            {
                Log.Write($"sendRedbucks Exception: {e}");
            }
        }
        ...
        public static void giveFreeCase(ExtPlayer player, ExtPlayer target, byte caseid)
        {
            try
            {
                ...
                Chars.Repository.AddNewItemWarehouse(target, ItemId.Case0 + caseid);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выдали {target.Name} ({target.Value}) открытие одного Free Case #{caseid}", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы получили возможность от {player.Name} открыть один Free Case #{caseid} в меню Бесплатная-Рулетка!",
                    3000);
                GameLog.Admin(player.Name, $"givecase{caseid}", target.Name);
            }
            catch (Exception e)
            {
                Log.Write($"giveFreeCase Exception: {e}");
            }
        }
        ...
        public static void stopServer(ExtPlayer sender, string reason = "Происходит плановая перезагрузка сервера!")
        {
            ...
            {
                Log.Write($"stopServer Exception: {e}");
            }
        }
        ...
        public static async void SaveServer()
        {
            try
            {
                ...
                var vehiclesLocalData = Entities.Vehicles.All.Cast<ExtVehicle>()
                    ...
                    .Where(v => v.VehicleLocalData.Access == VehicleAccess.Garage ||
                                v.VehicleLocalData.Access == VehicleAccess.Personal)
                    .ToList();
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        foreach (var foreachPlayer in Character.Repository.GetPlayers())
                            ...
                            {
                                Log.Write($"saveDatabase Foreach #1 Exception: {e}");
                            }
                        ...
                        {
                            if (!VehicleData.LocalData.Repository.IsVehicleToNumber(VehicleAccess.Personal, number))
                                continue;
                        ...
                        await Manager.SaveOrganizations(db);
                    ...
                    {
                        Log.Write($"saveDatabase Exception: {e}");
                    }
                });
            }
            catch (Exception e)
        ...
        public static void stopServer(string Admin = "server",
            string reason = "Происходит плановая перезагрузка сервера!")
        {
            try
            {
                ...
                foreach (var foreachPlayer in Entities.Players.All.Cast<ExtPlayer>())
                    Players.Disconnect.Repository.OnPlayerDisconnect(foreachPlayer, DisconnectionType.Kicked, reason,
                        false);
                ...
                Trigger.SetTask(async () =>
                {
                    ...
                    HouseManager.SavingHouses(true);
        ...
        private static async Task SaveAllPlayersServer()
        {
            ...
            do
            {
                ...
                isSave = Entities.Players.All.Cast<ExtPlayer>()
        ...
        public static void saveCoords(ExtPlayer player, string msg)
        {
            try
            {
                ...
                //pos.Z -= 1.12f;
                var rot = NAPI.Entity.GetEntityRotation(player);
                ...
                using (var saveCoords = new StreamWriter("coords.txt", true, Encoding.UTF8))
                {
                    saveCoords.Write(
                        $"{msg}: new Vector3({pos.X}, {pos.Y}, {pos.Z}), new Vector3({rot.X}, {rot.Y}, {rot.Z})\r\n");
                    saveCoords.Close();
                }
            }
            catch (Exception e)
            {
                Log.Write($"saveCoords Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У человека уже есть админ. прав", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы Выдали админ. права игроку {target.Name}", 3000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"{player.Name} Выдал Вам админ. права", 3000);
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) выдал {target.Name} админ. права 1 уровня", 1, "#FFB833", false);
                GameLog.Admin($"{player.Name}", AdminCommands.Setadmin, $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"setPlayerAdminGroup Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "Игрок был онлайн, поэтому offdeladmin заменён на deladmin", 3000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "У человека нет админ. прав", 3000);
                            return;
                        }
                        ...
                        if (character.Adminlvl >= characterData.AdminLVL)
                        {
                            Trigger.SendToAdmins(3,
                                "!{#FF0000}" +
                                $"[A] {sessionData.Name} ({sessionData.Value}) попытался снять {targetName} (offline), который имеет выше уровень администратора.");
                            return;
                        }

                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            $"Вы забрали права администратора у {targetName}", 3000);
                        AdminsLog(characterData.AdminLVL,
                            $"{sessionData.Name} ({sessionData.Value}) снял с {targetName} админ. права", 1, "#FFB833",
                            false);
                    ...
                    {
                        Log.Write($"OffdelPlayerAdminGroup SetTask Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"OffdelPlayerAdminGroup Exception: {e}");
            }
        }
        ...
        public static void delPlayerAdminGroup(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете забрать админ. права у себя", 3000);
                    return;
                }
                ...
                if (targetCharacterData.AdminLVL >= characterData.AdminLVL)
                {
                    Trigger.SendToAdmins(3,
                        "!{#FF0000}" +
                        $"[A] {player.Name} ({player.Value}) попытался снять {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                    return;
                }

                if (targetCharacterData.AdminLVL < 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У человека нет админ. прав",
                        3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы забрали права у администратора {target.Name}", 3000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"{player.Name} забрал у Вас админ. права", 3000);
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) снял с {target.Name} админ. права",
                    1, "#FFB833", false);
                GameLog.Admin($"{player.Name}", "delAdmin", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"delPlayerAdminGroup Exception: {e}");
            }
        }
        ...
        public static void setPlayerAdminRank(ExtPlayer player, ExtPlayer target, int rank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете установить себе ранг", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Человек не является администратором!", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете изменить уровень прав у этого администратора", 3000);
                    return;
                }

                if (rank < 1 || rank >= characterData.AdminLVL)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Невозможно выдать такой ранг",
                        3000);
                    return;
                }

                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы выдали игроку {target.Name} {rank} уровень админ. прав", 3000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"{player.Name} выдал Вам {rank} уровень админ. прав", 3000);
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) выдал {target.Name} {rank} уровень админ. прав", 1, "#FFB833",
                    false);
            ...
            {
                Log.Write($"setPlayerAdminRank Exception: {e}");
            }
        }
        ...
        public static void offSetPlayerAdminRank(ExtPlayer player, string targetName, int rank)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "Игрок был онлайн, поэтому offarank заменён на arank", 3000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            $"Вы выдали игроку {targetName} {rank} уровень админ. прав", 3000);
                        AdminsLog(characterData.AdminLVL,
                            $"{sessionData.Name} ({sessionData.Value}) выдал {targetName} {rank} уровень админ. прав",
                            1, "#FFB833", false);
                    ...
                    {
                        Log.Write($"offSetPlayerAdminRank SetTask Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"offSetPlayerAdminRank Exception: {e}");
            }
        }
        ...
        public static void setPlayerVipLvl(ExtPlayer player, ExtPlayer target, int rank, ushort days)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Невозможно выдать такой уровень ВИП аккаунта", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Максимальный срок VIP для выдачи - 3 года (1095 дней)", 3000);
                    return;
                }
                ...
                if (targetAccountData == null) return;
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы выдали человеку {target.Name} {Group.GroupNames[rank]}", 3000);
                EventSys.SendCoolMsg(target, "Администрация", "Выдача VIP",
                    $"Администратор {player.Name} выдал Вам {Group.GroupNames[rank]}", "", 7000);
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) выставил {target.Name} статус {Group.GroupNames[rank]} на {days} дней.");
                targetAccountData.VipLvl = rank;
                if (targetAccountData.VipDate > DateTime.Now)
                    targetAccountData.VipDate = targetAccountData.VipDate.AddDays(days);
                else targetAccountData.VipDate = DateTime.Now.AddDays(days);
                GameLog.Admin($"{player.Name}", $"setVipLvl({rank},{days})", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"setPlayerVipLvl Exception: {e}");
            }
        }
        ...
        public static void setFracLeader(ExtPlayer player, ExtPlayer target, int fractionId)
        {
            try
            {
                ...
                var fractionData = Fractions.Manager.GetFractionData(fractionId);
                if (fractionData != null)
                {
                    ...
                    {
                        AdminsLog(characterData.AdminLVL,
                            $"{player.Name} ({player.Value}) выдал лидерку({fractionId}) {target.Name} ({target.Value})");
                        ...
                        EventSys.SendCoolMsg(target, "Администрация", "Лидерка",
                            $"Вы стали лидером фракции {Fractions.Manager.GetName(fractionId)}", "", 7000);
                        //Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы стали лидером фракции {Manager.GetName(fractionId)}", 3000);


                        if (FractionClothingSets.FractionMainCloakrooms.ContainsKey(fractionId))
                        {
                            Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.StartWorkDay), 3000);
                            Fractions.Manager.SetSkin(target);
                        }
                    }
                    else if (characterData.AdminLVL >= 5 ||
                             (SetLeaderQueue.ContainsKey(target) && SetLeaderQueue[target].Item1 == fractionId))
                    {
                        if (characterData.AdminLVL <= 4)
                        {
                            ...
                            AdminsLog(characterData.AdminLVL,
                                $"{player.Name} ({player.Value}) подтвердил запрос выдачи лидерки администратора {SetLeaderQueue[target].Item2}");
                            AdminsLog(SetLeaderQueue[target].Item3,
                                $"{SetLeaderQueue[target].Item2} выдал лидерку({fractionId}) {target.Name} ({target.Value})");
                            GameLog.Admin($"{player.Name}", $"setFracLeaderAccept({fractionId})",
                                $"{SetLeaderQueue[target].Item2}");
                            GameLog.Admin($"{SetLeaderQueue[target].Item2}", $"setFracLeader({fractionId})",
                                $"{target.Name}");
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                "Вы подтвердили запрос на выдачу лидерки!", 3000);
                        }
                        else
                        {
                            AdminsLog(characterData.AdminLVL,
                            ...
                            GameLog.Admin($"{player.Name}", $"setFracLeader({fractionId})", $"{target.Name}");
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                $"Вы поставили {target.Name} на лидерство {Fractions.Manager.GetName(fractionId)}",
                                3000);
                        }
                    ...
                    else
                    {
                        if (SetLeaderQueue.ContainsKey(target) && SetLeaderQueue[target].Item1 != fractionId)
                        {
                            SetLeaderQueue.Remove(target);
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Старый запрос на выдачу лидерки для этого персонажа удалён", 3000);
                        }

                        SetLeaderQueue.Add(target, (fractionId, player.Name, characterData.AdminLVL));
                        Trigger.SendToAdmins(2,
                            "!{#FFB833}" +
                            $"[A] Запрос от {player.Name} ({player.Value}) на выдачу лидерки({fractionId}) для {target.Name} ({target.Value}). Чтобы подтвердить действие - введите: /setleader {target.Value} {fractionId}");
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"setFracLeader Exception: {e}");
            }
        }
        ...
        public static void delFracLeader(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                var fractionData = Fractions.Manager.GetFractionData(targetMemberFractionData.Id);
                if (fractionData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У человека нет фракции", 3000);
                    return;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Человек не является лидером",
                        3000);
                    return;
                }
                ...
                EventSys.SendCoolMsg(target, "Администрация", "Лидерка",
                    $"{player.Name.Replace('_', ' ')} снял Вас с поста лидера фракции", "", 7000);
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы сняли {target.Name.Replace('_', ' ')} с поста лидера фракции", 3000);
                //Chars.Repository.PlayerStats(target);
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) снял лидерку {target.Name} ({target.Value})");
                GameLog.Admin($"{player.Name}", "delFracLeader", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"delFracLeader Exception: {e}");
            }
        }
        ...
        public static void delJob(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetCharacterData.WorkID != 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Человек должен быть не в рабочей форме", 3000);
                        return;
                    }
                    ...
                    //Chars.Repository.PlayerStats(target);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"{player.Name.Replace('_', ' ')} снял трудоустройство с Вашего персонажа", 3000);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Вы сняли {target.Name.Replace('_', ' ')} с трудоустройства", 3000);
                    //Chars.Repository.PlayerStats(target);
                    AdminsLog(characterData.AdminLVL,
                        $"{player.Name} ({player.Value}) уволил с работы {target.Name} ({target.Value})");
                    GameLog.Admin($"{player.Name}", "delJob", $"{target.Name}");
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У человека нет работы", 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"delJob Exception: {e}");
            }
        }
        ...
        public static void delFrac(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Человек является лидером фракции", 3000);
                    return;
                }
                ...
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Администратор {player.Name.Replace('_', ' ')} выгнал Вас из фракции", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы выгнали {target.Name.Replace('_', ' ')} из фракции", 3000);
                //Chars.Repository.PlayerStats(target);
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) выкинул из фракции {target.Name} ({target.Value})");
                GameLog.Admin($"{player.Name}", "delFrac", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"delFrac Exception: {e}");
            }
        }
        ...
        public static void giveBankMoney(ExtPlayer player, int bankid, int amount)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (!Bank.Accounts.ContainsKey(bankid))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Игрок с таким ID/номером банк.счёта не найден", 3000);
                    return;
                }

                var data = Bank.Accounts[bankid];
                if (data.Type != 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Банковский счёт, который Вы пытаетесь вписать - не принадлежит человеку.", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выдали человеку {data.Holder} {amount}$ на банковский счёт {bankid}", 3000);
            ...
            {
                Log.Write($"giveBankMoney Exception: {e}");
            }
        }
        ...
        public static void giveMoney(ExtPlayer player, ExtPlayer target, int amount)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выдали человеку {target.Name} {amount}$", 3000);
                Repository.AddSystemMessage(target, (int)DefaultNumber.Bank,
                    LangFunc.GetText(LangType.Ru, DataName.MoneyIncome, amount), DateTime.Now);
            }
            catch (Exception e)
            {
                Log.Write($"giveMoney Exception: {e}");
            }
        }
        ...
        public static void OffGiveMoney(ExtPlayer player, string name, int amount)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "Игрок был онлайн, поэтому offgivemoney заменён на givemoney", 3000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"Вы установили {name} {money}$ (+{amount}$)", 3000);
                    ...
                    {
                        Log.Write($"OffGiveMoney SetTask Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"OffGiveMoney Exception: {e}");
            }
        }
        ...
        public static void mutePlayer(ExtPlayer player, ExtPlayer target, int time, string reason)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете дать мут больше, чем на 10080 минут", 3000);
                    return;
                }
                ...
                Trigger.SendPunishment(
                    $"{CommandsAccess.AdminPrefix}{player.Name} выдал мут игроку {target.Name}({target.Value}) на {time} {deTimeMsg}. Причина: {reason}",
                    target);
                GameLog.Admin($"{player.Name}", $"mutePlayer({time}{deTimeMsg}, {reason})", $"{target.Name}");
                Repository.AddSystemMessage(target, (int)DefaultNumber.RedAge,
                    $"{player.Name} выдал Вам мут на {time} {deTimeMsg}. Причина: {reason}", DateTime.Now);
            }
            catch (Exception e)
            {
                Log.Write($"mutePlayer Exception: {e}");
            }
        }
        ...
        public static void unmutePlayer(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetCharacterData.Unmute >= 0)
                {
                    ...
                    Trigger.SendPunishment(
                        $"{CommandsAccess.AdminPrefix}{player.Name} снял мут с игрока {target.Name}({target.Value})");
                    GameLog.Admin($"{player.Name}", "unmutePlayer", $"{target.Name}");
                    Repository.AddSystemMessage(target, (int)DefaultNumber.RedAge, $"{player.Name} снял с Вас мут.",
                        DateTime.Now);
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У игрока нет мута", 1500);
                }
            }
            catch (Exception e)
            {
                Log.Write($"unmutePlayer Exception: {e}");
            }
        }
        ...
        public static void OffMutePlayer(ExtPlayer player, string targetName, int time, string reason)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "Игрок был онлайн, поэтому offmute заменён на mute", 3000);
                    return;
                }
                ...
                var targetuuid = Main.PlayerUUIDs[targetName];
                var split = targetName.Split('_');

                Character.Save.Repository.SaveUnMute(targetuuid, firstTime);

                Trigger.SendPunishment(
                    $"{CommandsAccess.AdminPrefix}{player.Name} выдал мут игроку {targetName} в оффлайне на {time} {deTimeMsg}. Причина: {reason}");
                GameLog.Admin($"{player.Name}", $"mutePlayer({time}{deTimeMsg}, {reason})", $"{targetName}");
            }
            catch (Exception e)
            {
                Log.Write($"OffMutePlayer Exception: {e}");
            }
        }
        ...
        public static void OffUnMutePlayer(ExtPlayer player, string targetName)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "Игрок был онлайн, поэтому offunmute заменён на unmute", 3000);
                    return;
                }
                ...
                Trigger.SendPunishment(
                    $"{CommandsAccess.AdminPrefix}{player.Name} снял мут с игрока {targetName} в оффлайне.");
                GameLog.Admin($"{player.Name}", "unmutePlayer", $"{targetName}");
            }
            catch (Exception e)
            {
                Log.Write($"OffUnMutePlayer Exception: {e}");
            }
        }
        ...
        public static void getRb(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetAccountData == null) return;
                var targetRb = targetAccountData.RedBucks;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"У игрока {target.Name} ({target.Value}) - {targetRb} RedBucks", 5000);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) узнал количество RB у {target.Name} ({target.Value}) - {targetRb}");
            }
            catch (Exception e)
            {
                Log.Write($"getRb Exception: {e}");
            }
        }
        ...
        public static void getVip(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetAccountData == null) return;
                var targetVip = targetAccountData.VipLvl;
                var targetVipDate = targetAccountData.VipDate;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"У игрока {target.Name} ({target.Value}) - VIP {targetVip} уровня до {targetVipDate} ", 5000);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) узнал VIP-статус {target.Name} ({target.Value}) - {targetVip}");
            }
            catch (Exception e)
            {
                Log.Write($"getRb Exception: {e}");
        ...
        public static void getLogin(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Логин игрока {target.Name} ({target.Value}) - {targetlogin}", 5000);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) проверил логин {target.Name} ({target.Value}) - {targetlogin}");
            }
            catch (Exception e)
            {
                Log.Write($"getLogin Exception: {e}");
            }
        }
        ...
        public static bool CheckMe(ExtPlayer player, byte type)
        {
            try
            {
                ...
                var alvl = characterData.AdminLVL;
                var limit = alvl >= 1 && alvl <= 5 ? 5 : 10;
                switch (type)
                {
                    case 0:
                        ...
                        {
                            Trigger.SendToAdmins(3,
                                $"{ChatColors.Red}[A] {player.Name} ({player.Value}) был снят системой безопасности за превышение лимита наказаний");
                            BanMe(player, 1);
                            return false;
                        }
        ...
        public static void banPlayer(ExtPlayer player, ExtPlayer target, int time, string reason, bool isSBan = false)
        {
            try
            {
                ...
                if (targetCharacterData == null) return;
                if (isSBan && !CommandsAccess.CanUseCmd(player, AdminCommands.Sban)) return;
                ...
                if (tadmlvl == 9)
                {
                    Trigger.SendToAdmins(1,
                        "!{#FF0000}" +
                        $"[A] {player.Name} ({player.Value}) попытался забанить {target.Name} ({target.Value}).");
                    BanMe(player, 0);
                }
                else if (tadmlvl != 0 && tadmlvl >= characterData.AdminLVL)
                {
                    Trigger.SendToAdmins(3,
                        $"{ChatColors.StrongOrange}[A] {player.Name} ({player.Value}) забанил {target.Name} ({target.Value}) и был забанен системой.");

                    Character.BindConfig.Repository.DeleteAdmin(target);
                    Character.BindConfig.Repository.DeleteAdmin(player);

                    Ban.Online(target, DateTime.MaxValue, false, reason, player.Name);
                    Ban.Online(player, DateTime.MaxValue, false,
                        $"Забанен системой за бан администратора {target.Name}", "server");

                    Notify.Send(target, NotifyType.Warning, NotifyPosition.Center,
                        $"Вы заблокированы навсегда администратором {player.Name}.", 30000);
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                        $"Вы заблокированы навсегда системой за бан администратора {target.Name}.", 30000);

                    GameLog.Ban(characterData.UUID, targetCharacterData.UUID, target.GetLogin(), DateTime.MaxValue,
                        reason, false);
                    GameLog.Ban(-2, characterData.UUID, accountData.Login, DateTime.MaxValue,
                        $"Забанен системой за бан администратора {target.Name}", false);
                ...
                else
                {
...

                    {
                        if (isSBan)
                            AdminsLog(characterData.AdminLVL,
                                $"{player.Name} забанил игрока {target.Name}({target.Value}) без лишнего шума навсегда. Причина: {reason}",
                                1, "#FFB833", false);

                        else
                            Trigger.SendPunishment(
                                $"{CommandsAccess.AdminPrefix}{player.Name} заблокировал персонажа {target.Name}({target.Value}) навсегда. Причина: {reason}",
                                target);
                    }
...

                    {
                        if (isSBan)
                            AdminsLog(characterData.AdminLVL,
                                $"{player.Name} забанил игрока {target.Name}({target.Value}) без лишнего шума на {time} дней. Причина: {reason}",
                                1, "#FFB833", false);

                        else
                            Trigger.SendPunishment(
                                $"{CommandsAccess.AdminPrefix}{player.Name} заблокировал персонажа {target.Name}({target.Value}) на {time} дней. Причина: {reason}.",
                                target);
                    }
                    ...
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.Center,
                        $"Вы заблокированы до {unbanTime.Day} {unbanTime.ToString("MMMM")} {unbanTime.Year}г. {unbanTime.Hour}:{unbanTime.Minute}:{unbanTime.Second} (UTC+3).",
                        30000);
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.Center, $"Причина: {reason}", 30000);

                    GameLog.Ban(characterData.UUID, targetCharacterData.UUID, target.GetLogin(), unbanTime, reason,
                        false);
                    target.Kick(reason);
                }
            }
            ...
            {
                Log.Write($"banPlayer Exception: {e}");
            }
        }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        if (ban != null)
                        {
                            ...
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.Center, $"Игрок уже в {hard}бане.",
                                3000);
                            return;
                        }
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                                    $"Вы забанили логин {login} в оффлайне с причиной {reason}.", 3000);

                                var unbanTime = DateTime.Now.AddDays(time);
                                ...
                                {
                                    AdminLog(characterData.AdminLVL,
                                        $"{player.Name} ({player.Value}) в оффлайне забанил логин({login}) {target.Name} ({target.Value}) по причине {reason}");

                                    Notify.Send(target, NotifyType.Warning, NotifyPosition.Center,
                                        $"Вы заблокированы до {unbanTime.Day} {unbanTime.ToString("MMMM")} {unbanTime.Year}г. {unbanTime.Hour}:{unbanTime.Minute}:{unbanTime.Second} (UTC+3).",
                                        30000);

                                    Notify.Send(target, NotifyType.Warning, NotifyPosition.Center, $"Причина: {reason}",
                                        30000);
                                ...
                                {
                                    AdminLog(characterData.AdminLVL,
                                        $"{player.Name} ({player.Value}) в оффлайне забанил логин({login}) по причине {reason}");
                                }
                            }
                            ...
                            {
                                Log.Write($"banLoginPlayer NAPI.Task Exception: {e}");
                            }
                        });
                    }
                    ...
                    {
                        Log.Write($"banLoginPlayer Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"banLoginPlayer Exception: {e}");
            }
        }
        ...
        public static void unbanLoginPlayer(ExtPlayer player, string login)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "Такого логина не найдено в системе.", 3000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                $"{login} не находится в бане!", 3000);
                            return;
                        }
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            {
                                AdminLog(characterData.AdminLVL,
                                    $"{player.Name} ({player.Value}) разбанил логин({login})");

                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    "Логин разблокирован!", 3000);
                            }
                            catch (Exception e)
                            {
                                Log.Write($"unbanLoginPlayer Task.Run Exception: {e}");
                            }
                        });
                    }
                    catch (Exception e)
            ...
            {
                Log.Write($"unbanLoginPlayer Exception: {e}");
            }
        }
        ...
        public static void hardbanPlayer(ExtPlayer player, ExtPlayer target, int time, string reason)
        {
            try
            {
                ...
                else if (tadmlvl != 0 && tadmlvl >= characterData.AdminLVL)
                {
                    ...
                    GameLog.Ban(-2, AUUID, accountData.Login, DateTime.MaxValue,
                ...
                else
                {
                    ...
                    if (time >= 3650)
                        Trigger.SendPunishment(
                            $"{CommandsAccess.AdminPrefix}{player.Name} выдал пожизненный банхаммер игроку {target.Name}({target.Value}). Причина: {reason}",
                            target);
                    else
                        Trigger.SendPunishment(
                            $"{CommandsAccess.AdminPrefix}{player.Name} выдал банхаммер игроку {target.Name}({target.Value}) на {time} дней. Причина: {reason}",
                            target);
                    Ban.Online(target, unbanTime, true, reason, player.Name);
                    EventSys.SendCoolMsg(target, "Администрация", "БАНХАММЕР",
                        $"Вы получили банхаммер до {unbanTime.ToString()}. Причина: {reason}. До новых встреч!", "",
                        15000);
            ...
            {
                Log.Write($"hardbanPlayer Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                $"HWID персонажа совпадает с забаненным аккаунтом {ban.Account}", 3000);
                            return;
                        }

                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "HWID персонажа не найден в бане", 3000);
                    }
                    catch (Exception e)
                    {
                        Log.Write($"isHardPlayer Task.Run Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"isHardPlayer Exception: {e}");
            }
        }
        ...
        public static void offBanPlayer(ExtPlayer player, string name, int time, string reason, bool isHard = false)
        {
            try
            {
                ...
                if (isHard && !CommandsAccess.CanUseCmd(player, AdminCommands.Offhardban)) return;
                ...
                if (target.IsCharacterData())
                {
...

                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                            "Игрок был онлайн, поэтому offhardban был заменён на hardban", 3000);
                    }
                    else
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                ...
                                if (targetCharacter.Adminlvl == 9)
                                {
                                    Trigger.SendToAdmins(1,
                                        "!{#FF0000}" +
                                        $"[A] {player.Name} ({player.Value}) попытался забанить {prefix} {name} (offline).");
                                    BanMe(player, 0);
                                    return;
                                }
                                ...
                                {
                                    var login = Main.GetLoginFromUUID(targetCharacter.Uuid);
                                    ...
                                    Trigger.SendToAdmins(3,
                                        $"{ChatColors.StrongOrange}[A] {player.Name} ({player.Value}) забанил {name} в оффлайне и был забанен системой.");
                                    ...
                                    Ban.Online(player, DateTime.MaxValue, isHard,
                                        $"Забанен системой за бан администратора {name}", "server");


                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                                        $"Вы заблокированы навсегда системой за офф{prefix}бан администратора {name}.",
                                        30000);


                                    if (login != null)
                                        GameLog.Ban(characterData.UUID, targetCharacter.Uuid, login, DateTime.MaxValue,
                                            reason, isHard);

                                    else
                                        GameLog.Ban(characterData.UUID, targetCharacter.Uuid, "-", DateTime.MaxValue,
                                            reason, isHard);


                                    GameLog.Ban(-2, characterData.UUID, accountData.Login, DateTime.MaxValue,
                                        $"Забанен системой за офф{prefix}бан администратора {name}", isHard);
                                ...
                                if (isHard)
                                {
                                    if (time >= 3650)
                                        Trigger.SendPunishment(
                                            $"{CommandsAccess.AdminPrefix}{player.Name} выдал пожизненный банхаммер игроку {name} в оффлайне. Причина: {reason}",
                                            target);
                                    else
                                        Trigger.SendPunishment(
                                            $"{CommandsAccess.AdminPrefix}{player.Name} выдал банхаммер игроку {name} в оффлайне на {time} дней. Причина: {reason}",
                                            target);
                                }
                                else
                                {
                                    if (time >= 3650)
                                        Trigger.SendPunishment(
                                            $"{CommandsAccess.AdminPrefix}{player.Name} заблокировал персонажа {name} в оффлайне навсегда. Причина: {reason}");
                                    else
                                        Trigger.SendPunishment(
                                            $"{CommandsAccess.AdminPrefix}{player.Name} заблокировал персонажа {name} в оффлайне на {time} дней. Причина: {reason}");
                                }


                                Ban.OfflineBanToNickName(name, unbanTime, isHard, reason, player.Name);

                                var login1 = Main.GetLoginFromUUID(targetCharacter.Uuid);

                                if (login1 != null)
                                    GameLog.Ban(characterData.UUID, targetCharacter.Uuid, login1, unbanTime, reason,
                                        isHard);
                                else
                                    GameLog.Ban(characterData.UUID, targetCharacter.Uuid, "-", unbanTime, reason,
                                        isHard);
                            }
                            catch (Exception e)
                            {
                                Log.Write($"offBanPlayer NAPI.Task.Run Exception: {e}");
                            }
                        });
                    }
                    catch (Exception e)
                    {
                        Log.Write($"offBanPlayer NAPI.Task.Run 1 Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"offBanPlayer Exception: {e}");
            }
        }
                    ...
                    try
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    "Игрок разблокирован!", 3000);

                                GameLog.Admin($"{player.Name}", "unban", $"{name}");
                            }
                            catch (Exception e)
                            {
                                Log.Write($"unbanPlayer NAPI.Task.Run Exception: {e}");
                            }
                        });
                    }
                    catch (Exception e)
                    {
                        Log.Write($"unbanPlayer Task.Run Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"unbanPlayer Exception: {e}");
            }
        }
                    ...
                    try
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    "С игрока снят хардбан!", 3000);
                            }
                            catch (Exception e)
                            {
                                Log.Write($"unhardbanPlayer NAPI.Task.Run Exception: {e}");
                            }
                        });
                    }
                    catch (Exception e)
                    {
                        Log.Write($"unhardbanPlayer Task.Run Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"unhardbanPlayer Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (ban != null)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы разблокировал IP адрес.", 3000);
                    });
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Этот IP не заблокирован.",
                        3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"unbanIp Exception: {e}");
            }
        }
        ...
        public static void kickPlayer(ExtPlayer player, ExtPlayer target, string reason, bool isSilence)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                    if (targetCharacterData.AdminLVL >= characterData.AdminLVL)
                    {
                        Trigger.SendToAdmins(3,
                            "!{#FF0000}" +
                            $"[A] {player.Name} ({player.Value}) попытался кикнуть {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                        return;
                    }

                if (isSilence && Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                    Trigger.SendToAdmins(1,
                        $"{ChatColors.Red}[A] {player.Name} ({player.Value}) был снят системой за причину в наказании: {reason}");
                    Character.BindConfig.Repository.DeleteAdmin(player);
                ...
                {
                    Trigger.SendPunishment(
                        $"{CommandsAccess.AdminPrefix}{player.Name} кикнул игрока {target.Name}({target.Value}). Причина: {reason}",
                        target);
                ...
                else
                {
                    Trigger.SendToAdmins(1,
                        "!{#FFB833}" +
                        $"[A] {player.Name} кикнул игрока {target.Name}({target.Value}) без лишнего шума.");
            ...
            {
                Log.Write($"kickPlayer Exception: {e}");
            }
        }
        ...
        public static void warnPlayer(ExtPlayer player, ExtPlayer target, string reason)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= characterData.AdminLVL)
                {
                    Trigger.SendToAdmins(3,
                        "!{#FF0000}" +
                        $"[A] {player.Name} ({player.Value}) попытался предупредить {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                    return;
                }
                ...
                if (target.GetFractionId() > 0)
                    Fractions.Table.Logs.Repository.AddLogs(target, FractionLogsType.UnInvite,
                ...
                Trigger.SendPunishment(
                    $"{CommandsAccess.AdminPrefix}{player.Name} выдал предупреждение игроку {target.Name}({target.Value}). | {targetCharacterData.Warns}/3. Причина: {reason}",
                    target);

                if (targetCharacterData.Warns >= 3)
                {
                    var unbanTime = DateTime.Now.AddMinutes(43200);
                ...
                Repository.AddSystemMessage(target, (int)DefaultNumber.RedAge,
                    $"{player.Name} выдал Вам WARN | {targetCharacterData.Warns}/3. Причина: {reason}", DateTime.Now);
                target.Kick("Предупреждение");
            }
            catch (Exception e)
            {
                Log.Write($"warnPlayer Exception: {e}");
            }
        }
        ...
        public static void ReviveMe(ExtPlayer player)
        {
            ...
            {
                Log.Write($"ReviveMe Exception: {e}");
            }
        }
        ...
        public static void reviveTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                var id = target.Value;
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) возродил игрока {target.Name} ({target.Value})", 2, "#636363");
                GameLog.Admin($"{player.Name}", "revivePlayer", $"{target.Name}");
                EventSys.SendCoolMsg(target, "Администрация", "Реанимация",
                    $"Администратор {player.Name} реанимировал Вас", "", 7000);
            ...
            {
                Log.Write($"reviveTarget Exception: {e}");
            }
        }
        ...
        public static void healTarget(ExtPlayer player, ExtPlayer target, int hp)
        {
            try
            {
                ...
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) выставил HP({hp}) {target.Name} ({target.Value})", 1, "#636363",
                    hideAdminLevel: 9);
                NAPI.Player.SetPlayerHealth(target, hp);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Администратор {player.Name} изменил Вам уровень здоровья на {hp}.", 3000);
                GameLog.Admin($"{player.Name}", $"healPlayer({hp})", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"healTarget Exception: {e}");
            }
        }
        ...
        public static void armorTarget(ExtPlayer player, ExtPlayer target, int ar)
        {
            try
            {
                ...
                {
                    Chars.Repository.AddNewItem(target, $"char_{targetCharacterData.UUID}", "inventory",
                        ItemId.BodyArmor, 1, ar.ToString());
                    AdminLog(characterData.AdminLVL,
                        $"{player.Name} ({player.Value}) выдал Armor({ar}) {target.Name} ({target.Value})");
                    GameLog.Admin($"{player.Name}", $"armorPlayer({ar})", $"{target.Name}");
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У игрока уже есть броня в инвентаре.", 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"armorTarget Exception: {e}");
            }
        }
        ...
        public static void checkGodmode(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) проверил на GM {target.Name} ({target.Value})");
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        if (!target.IsCharacterData()) return;
                        if (target.Health >= targetHealth)
                        {
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                $"У {target.Name} ({target.Value}) возможно есть GM", 3000);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                $"У {target.Name} ({target.Value}) нет GM", 3000);
                            target.Health = targetHealth;
                        }
                    }
                    catch (Exception e)
                    {
                        Log.Write($"checkGodmode Task Exception: {e}");
                    }
                }, 500);
            ...
            {
                Log.Write($"checkGodmode Exception: {e}");
            }
        }
        ...
        public static void slapPlayer(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) подкинул {target.Name} ({target.Value})");
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Администратор {player.Name} подкинул Вас", 3000);
                GameLog.Admin($"{player.Name}", "Slap", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"slapPlayer Exception: {e}");
            }
        }
        ...
        public static void checkMoney(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                var bankAcc = Bank.Get(Main.PlayerBankAccs[target.Name]);
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                    $"У {target.Name} {Wallet.Format(targetCharacterData.Money)}$ | Bank: {Wallet.Format(bankMoney)}",
                    3000);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) проверил денежные средства {target.Name} ({target.Value})");
                GameLog.Admin($"{player.Name}", "checkMoney", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"checkMoney Exception: {e}");
            }
        }
        ...
        public static void teleportTargetToPlayer(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 6)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок с таким ID не найден",
                            3000);
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter,
                            $"{player.Name} ({player.Value}) попытался телепортировать Вас к себе (/metp)", 3000);
                        return;
                    }
                }

                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) телепортировал к себе {target.Name} ({target.Value})", 1,
                    "#636363");
                GameLog.Admin($"{player.Name}", $"metp({player.Position.X}, {player.Position.Y}, {player.Position.Z})",
                    $"{target.Name}");
                NAPI.Entity.SetEntityPosition(target, player.Position);
                Trigger.Dimension(target, UpdateData.GetPlayerDimension(player));
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы телепортировали {target.Name} к себе", 3000);
                EventSys.SendCoolMsg(target, "Телепорт", "Телепорт", $"{player.Name} телепортировал Вас к себе", "",
                    7000);
                //Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"{player.Name} телепортировал Вас к себе", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"teleportTargetToPlayer Exception: {e}");
            }
        }
        ...
        public static void freezeTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (!target.IsCharacterData()) return;
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) заморозил {target.Name} ({target.Value})");
                Trigger.ClientEvent(target, "freeze", true);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы заморозили игрока {target.Name}",
                    3000);
                GameLog.Admin($"{player.Name}", "freeze", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"freezeTarget Exception: {e}");
            }
        }
        ...
        public static void unFreezeTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (!target.IsCharacterData()) return;
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) разморозил {target.Name} ({target.Value})");
                Trigger.ClientEvent(target, "freeze", false);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы разморозили игрока {target.Name}", 3000);
                GameLog.Admin($"{player.Name}", "unfreeze", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"unFreezeTarget Exception: {e}");
            }
        }
        ...
        public static void giveTargetGun(ExtPlayer player, ExtPlayer target, string weapon, string serial)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Серийный номер состоит из 9 символов", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Неверный ввод серийного номера!", 3000);
                    return;
                }

                var wType = (ItemId)Enum.Parse(typeof(ItemId), weapon);
                if (wType == ItemId.CarCoupon) return;
                if ((Chars.Repository.StrongWeapons.Contains(wType) || Chars.Repository.HeavyWeapons.Contains(wType)) &&
                    characterData.AdminLVL <= 5) return;
                if (characterData.AdminLVL >= 8 || (characterData.AdminLVL <= 7 &&
                                                    (Chars.Repository.ItemsInfo[wType].functionType ==
                                                     newItemType.Weapons ||
                                                     Chars.Repository.ItemsInfo[wType].functionType ==
                                                     newItemType.MeleeWeapons)))
                {
                    ...
                    AdminLog(characterData.AdminLVL,
                        $"{player.Name} ({player.Value}) выдал оружие ({weapon}|{serial}) {target.Name} ({target.Value})");
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Вы выдали игроку {target.Name} оружие ({weapon})", 3000);
                    GameLog.Admin($"{player.Name}", $"giveGun({weapon},{serial})", $"{target.Name}");
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Доступно с 8 уровня администрирования.", 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"giveTargetGun Exception: {e}");
            }
        }
        ...
        public static void giveTargetCar(ExtPlayer player, ExtPlayer target, string vehicle)
        {
            try
            {
                ...
                Chars.Repository.AddNewItem(target, $"char_{targetCharacterData.UUID}", "inventory", ItemId.CarCoupon,
                    1, vehicle);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) выдал купон на машину ({vehicle}) {target.Name} ({target.Value})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы выдали игроку {target.Name} купон на машину ({vehicle})", 3000);
                GameLog.Admin($"{player.Name}", $"giveCarC({vehicle})", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"giveTargetCar Exception: {e}");
            }
        }
        ...
        public static void giveTargetSkin(ExtPlayer player, ExtPlayer target, string pedModel)
        {
            try
            {
                ...
                if (pedModel.Equals("-1"))
                {
                    if (targetSessionData.AdminSkin)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            "Вы восстановили игроку внешность", 3000);
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игроку не меняли внешность",
                            3000);
                    }
                }
                else
                {
                    ...
                    if (pedHash != 0)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            $"Вы сменили игроку {target.Name} внешность на ({pedModel})", 3000);
                    }
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Внешности с таким названием не было найдено", 3000);
                        }
                        else
                        {
                            targetSessionData.AdminSkin = true;
            ...
            {
                Log.Write($"giveTargetSkin Exception: {e}");
            }
        }
        ...
        public static void giveTargetClothes(ExtPlayer player, ExtPlayer target, string weapon, string serial)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Серийный номер состоит из 6-12 символов", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Этой командой можно выдавать только предметы одежды", 3000);
                    return;
                }
                ...
                {
                    Log.Write("giveTargetClothes ERROR");
                }

                if (Chars.Repository.AddNewItem(target, $"char_{targetCharacterData.UUID}", "inventory", wType, 1,
                        serial) == -1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У игрока недостаточно места в инвентаре", 3000);
                    return;
                }

                GameLog.Admin($"{player.Name}", $"giveClothes({weapon},{serial})", $"{target.Name}");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы выдали игроку {target.Name} одежду ({weapon})", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"giveTargetClothes Exception: {e}");
            }
        }
        ...
        public static void takeTargetGun(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы забрали у игрока {target.Name} всё оружие", 3000);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) забрал все оружия {target.Name} ({target.Value})");
                GameLog.Admin($"{player.Name}", "takeGuns", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"takeTargetGun Exception: {e}");
            }
        }
        ...
        public static void adminSMS(ExtPlayer player, ExtPlayer target, string message)
        {
            try
            {
                ...
                Trigger.SendChatMessage(target,
                    $"{ChatColors.Report}Администратор {player.Name} ({player.Value}): {message}");
                Trigger.SendToAdmins(characterData.AdminLVL,
                    $"{ChatColors.Report}[A][ASMS] {player.Name} ({player.Value}) для {target.Name} ({target.Value}): {message}");
            ...
            {
                Log.Write($"adminSMS Exception: {e}");
            }
        }
        ...
        public static void checkKill(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"У {target.Name} нет данных последней смерти.", 2500);
                    return;
                }

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    targetSessionData.DeathData.LastDeath, 7000);
                AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) проверил данные последней смерти {target.Name} ({target.Value})");
                GameLog.Admin($"{player.Name}", "checkKill", $"{target.Name}");
            }
            catch (Exception e)
            {
                Log.Write($"checkKill Exception: {e}");
            }
        }
        ...
        public static void adminChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                message = characterData.AdminLVL >= 6
                    ? $"{ChatColors.AdminChat}[A] {ChatColors.HAdmin}{player.Name}{ChatColors.AdminChat} ({player.Value}): {message}"
                    : $"{ChatColors.AdminChat}[A] {ChatColors.LAdmin}{player.Name}{ChatColors.AdminChat} ({player.Value}): {message}";
            ...
            {
                Log.Write($"adminChat Exception: {e}");
            }
        }
        ...
        public static void adminGlobal(ExtPlayer player, string message)
        {
            try
            {
                ...
                if (characterData.AdminLVL <= 6)
                    ...
                    {
                        Trigger.SendToAdmins(1,
                            $"{ChatColors.Red}[A] {player.Name} ({player.Value}) был снят системой за причину в глобале: {message}");
                        Character.BindConfig.Repository.DeleteAdmin(player);
                        return;
                    }
                ...
                if (characterData.AdminLVL >= 6)
                {
                    NAPI.Chat.SendChatMessageToAll(
                        $"{CommandsAccess.AdminPrefixChat}{player.Name.Replace('_', ' ')}: {message}");
                    GameLog.Admin($"{player.Name}", $"global({message})", "null");
                    EventSys.SendPlayersToEvent("Администрация", $"{player.Name}", $"{message}", "", 10000);
                }
                else
                {
                    GlobalQueue.Add(GlobalID,
                        (message, player.Name, characterData.AdminLVL, DateTime.Now.AddSeconds(60)));
                    Trigger.SendToAdmins(2,
                        "!{#FFB833}" +
                        $"[A] Запрос от {player.Name} ({player.Value}) на global({message}). Чтобы подтвердить действие - введите: /accept {GlobalID}");
                    GlobalID++;
                }
            }
            catch (Exception e)
            {
                Log.Write($"adminGlobal Exception: {e}");
            }
        }
        ...
        public static void adminGlobalAccept(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "К сожалению, запроса подтверждения global с таким ID не существует.", 3000);
                    return;
                }
                ...
                if (GlobalQueue[id].Item4 < DateTime.Now)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Время выполнения запроса истекло (60 секунд).", 3000);
                    return;
                }
                ...
                AdminsLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) подтвердил запрос на global({GlobalQueue[id].Item2})");
                NAPI.Chat.SendChatMessageToAll(
                    $"{CommandsAccess.AdminPrefixChat}{GlobalQueue[id].Item2.Replace('_', ' ')}: {GlobalQueue[id].Item1}");
            ...
            {
                Log.Write($"adminGlobalAccept Exception: {e}");
            }
        }
        ...
        public static void sendPlayerToDemorgan(ExtPlayer admin, ExtPlayer target, int time, string reason)
        {
            try
            {
                ...
                {
                    Trigger.SendToAdmins(1,
                        $"{ChatColors.Red}[A] {admin.Name} ({admin.Value}) был снят системой за причину в наказании: {reason}");
                ...
                Trigger.SendPunishment(
                    $"{CommandsAccess.AdminPrefix}{admin.Name} посадил в деморган {target.Name}({target.Value}) на {time} {deTimeMsg}. Причина: {reason}",
                    target);
                ...
                //
                if (targetSessionData.TimersData.ArrestTimer != null)
                    Timers.Stop(targetSessionData.TimersData.ArrestTimer);
                ...
                target.Eval(
                    "mp.game.audio.playSoundFrontend(-1, \"Deliver_Pick_Up\", \"HUD_FRONTEND_MP_COLLECTABLE_SOUNDS\", true);");
                target.SetSharedData("HideNick", true);
                //
                NAPI.Player.SetPlayerHealth(target, 3);
                Chars.Repository.RemoveAllWeapons(target, true, true, true);
                ...
                EventSys.SendCoolMsg(target, "Администрация", $"{admin.Name}",
                    $"Отправил Вас в деморган на {time} {deTimeMsg}. Причина: {reason}", "", 15000);
                //
            }
            catch (Exception e)
            {
                Log.Write($"sendPlayerToDemorgan Exception: {e}");
            }
        }
        ...
        public static void releasePlayerFromDemorgan(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Игрок не находится в спец.тюрьме.", 3000);
                    return;
                }

                targetCharacterData.DemorganTime = 1;
                Trigger.SendPunishment(
                    $"{CommandsAccess.AdminPrefix}{player.Name} выпустил из деморгана {target.Name} ({target.Value})");
                ...
                EventSys.SendCoolMsg(target, "Администрация", $"{player.Name}",
                    "Выпустил Вас из деморгана. Приятной игры и больше не нарушайте! :)", "", 15000);
            }
            catch (Exception e)
            {
                Log.Write($"releasePlayerFromDemorgan Exception: {e}");
            }
        }
        ...
        public static void timer_demorgan(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (characterData.DemorganTime-- <= 0) FractionCommands.freePlayer(player, true);
            }
            catch (Exception e)
            {
                Log.Write($"timer_demorgan Exception: {e}");
            }
        }
                        ...
                        if (characterData.DemorganTime <= 0)
                            NAPI.Task.Run(() =>
                            {
                                try
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                        "Mute был снят, не нарушайте больше!", 3000);
                                }
                                catch (Exception e)
                                {
                                    Log.Write($"timer_mute Task Exception: {e}");
                                }
                            });
                        else
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                "Mute был снят, но воспользоваться голосовым чатом Вы сможете после истечения срока другого наказания.",
                                10000);
                    }
            }
            catch (Exception e)
            {
                Log.Write($"timer_mute Exception: {e}");
            }
        }
        /*
        ...
        */
        ...
        public void ClientEvent_SaveEspState(ExtPlayer player, byte state)
        {
            ...
            {
                Log.Write($"ClientEvent_SaveEspState Exception: {e}");
            }
        }
        ...
        public static void RespawnFractionCar(ExtVehicle vehicle)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        var loader = vehicleLocalData.LoaderMats;
                        var loaderSessionData = loader.GetSessionData();

                        if (loaderSessionData != null)
                        {
                            Notify.Send(loader, NotifyType.Warning, NotifyPosition.BottomCenter,
                                "Загрузка материалов отменена, так как машина покинула чекпоинт", 3000);
            ...
            {
                Log.Write($"RespawnFractionCar Exception: {e}");
            }
        }
        ...
        public static void CMD_Cnum(ExtPlayer player, ExtPlayer target, string a)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (!target.IsCharacterData()) return;
                if (characterData.AdminLVL <= 8 || !Roullete.Roulette.ContainsValue(a) ||
                    !Roullete.PlayerData.ContainsKey(target)) return;
                if (Roullete.RouletteTables.Count <= Roullete.PlayerData[target].SelectedTable) return;
                var table = Roullete.RouletteTables[Roullete.PlayerData[target].SelectedTable];
                if (table.Process) return;
                var pair = Roullete.Roulette.FirstOrDefault(p => p.Value.Equals(a));
            ...
            {
                Log.Write($"CMD_Cnum Exception: {e}");
            }
        }
        ...
        public static void CMD_Chnum(ExtPlayer player, int a)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (characterData.AdminLVL <= 8 || a < 1 || a > 6 || Horses.curentScreen != 0) return;
                Horses.WinHorse = a;
        ...
        public static void CMD_Cinum(ExtPlayer player, int a, byte b, byte c)
        {
            try
            {
                ...
                var target = Main.GetPlayerByID(a);
                ...
                if (b >= Chars.Repository.RouletteCasesData.Count ||
                    Chars.Repository.RouletteCasesData[b].RouletteItemsData.Count <= c) return;
                var p = Chars.Repository.RouletteCasesData[b].RouletteItemsData[c];
                if (p == null) return;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"{Chars.Repository.RouletteCasesData[b].Name} - {p.Name}", 1000);
