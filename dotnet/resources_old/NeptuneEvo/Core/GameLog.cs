...
namespace NeptuneEvo.Core
{
    public class GameLog
    {
        ...
        private static readonly List<List<string>> CmdQueue = new List<List<string>>();
        ...
        public static void FracLog(int Frac, int Me, int He, string Me1, string He1, string Action)
        {
            ...
            cmd.Add(
                $"'{DateTime.Now.ToString("s")}','{Manager.FractionNames[Frac]}',{Me},{He},'{Me1}','{He1}','{Action}'");
            CmdQueue.Add(cmd);
        }
        ...
        public static void Ban(int Admin, int Player, string Login, DateTime Until, string Reason, bool isHard)
        {
            ...
            cmd.Add(
                $"'{DateTime.Now.ToString("s")}',{Admin},{Player},'{Login}','{Until.ToString("s")}','{Reason}',{isHard}");
            CmdQueue.Add(cmd);
        }
        ...
        public static void Disconnected(int Uuid, int Id, string Reason, string login)
        {
            ...
            if (ticks > 0)
            {
                ...
                if (time > 0)
                    HelperThread.AddEvent("session_time", Repository.GetEmailToLogin(login), "", time);
            }
        }
        ...
        public static void EventLogUpdate(string AdmName, int MembCount, string WinName, uint Reward, string Time,
            int RewardLimit, ushort MemLimit, string EvName)
        {
            ...
            cmd.Add("`eventslog`");
            cmd.Add(
                $"`AdminClosed`='{AdmName}',`Members`={MembCount},`Winner`='{WinName}',`Reward`={Reward},`Ended`='{Time}',`RewardLimit`={RewardLimit}");
