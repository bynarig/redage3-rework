    ...
    internal class DrivingSchool : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                Repository.CreateQuest("mp_m_securoguard_01", enterSchool, -60.65183f,
                    title: "~y~NPC~w~ Офицер Бенсон\nВыдача лицензий", colShapeEnums: ColShapeEnums.DriveSchool);
        ...
        public static void startDrivingCourse(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    case 0:
                        if (UpdateData.CanIChange(player, Main.LicPrices[0], true) != 255) return;
                        characterData.Licenses[0] = true;
                        Wallet.Change(player, -Main.LicPrices[0]);
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucBuyMotoLic), 3000);
                        //Repository.PlayerStats(player);
                        return;
                    case 1:
                        if (UpdateData.CanIChange(player, Main.LicPrices[1], true) != 255) return;
                        characterData.Licenses[1] = true;
                        qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage7, 2, true);
                        qMain.UpdateQuestsComplete(player, Zdobich.QuestName, (int)zdobich_quests.Stage7, true);
                        Wallet.Change(player, -Main.LicPrices[1]);
                        //fractionData.Money += Main.LicPrices[3];
                        //GameLog.Money($"player({characterData.UUID})", $"frac(6)", Main.LicPrices[3], $"buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucBuyVehLic), 3000);
                        //Repository.PlayerStats(player);
                        return;
                    case 2:
                        if (UpdateData.CanIChange(player, Main.LicPrices[2], true) != 255) return;
                        characterData.Licenses[2] = true;
                        Wallet.Change(player, -Main.LicPrices[2]);
                        //fractionData.Money += Main.LicPrices[3];
                        //GameLog.Money($"player({characterData.UUID})", $"frac(6)", Main.LicPrices[3], $"buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucBuyGruzLic), 3000);
                        //Repository.PlayerStats(player);
                        return;
                    /*case 0:
                        try
                    ...
                    case 2:
                        ...
                        return;*/
                    case 3:
                        if (UpdateData.CanIChange(player, Main.LicPrices[3], true) != 255) return;
                        characterData.Licenses[3] = true;
                        Wallet.Change(player, -Main.LicPrices[3]);
                        fractionData.Money += Main.LicPrices[3];
                        GameLog.Money($"player({characterData.UUID})", "frac(6)", Main.LicPrices[3], "buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucBuySeaLic), 3000);
                        //Repository.PlayerStats(player);
                        return;
                    case 4:
                        if (UpdateData.CanIChange(player, Main.LicPrices[4], true) != 255) return;
                        if (characterData.LVL < 20)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.LicMustBe20), 3000);
                            return;
                        }

                        characterData.Licenses[4] = true;
                        Wallet.Change(player, -Main.LicPrices[4]);
                        fractionData.Money += Main.LicPrices[4];
                        GameLog.Money($"player({characterData.UUID})", "frac(6)", Main.LicPrices[4], "buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucBuyHeliLic), 3000);
                        //Repository.PlayerStats(player);
                        return;
                    case 5:
                        if (UpdateData.CanIChange(player, Main.LicPrices[5], true) != 255) return;
                        if (characterData.LVL < 20)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.LicMustBe20), 3000);
