...
namespace NeptuneEvo.Core
{
    public class SafeMain : Script
    {
        ...
        public static void onPlayerDeathHandler(ExtPlayer player, ExtPlayer entityKiller, uint weapon)
        {
            try
            {
                ...
                var Bags = Repository.GetItemData(player, "accessories", 8).ItemId;
                if (Bags == ItemId.BagWithMoney || Bags == ItemId.BagWithDrill)
                    Repository.ItemsDropToIndex(player, "accessories", 8);
                if (sessionData.TempSafeID == -1) return;
                var safe = Safes.FirstOrDefault(s => s.ID == sessionData.TempSafeID);
                if (safe == null) return;
                safe.Occupier = null;
            }
            catch (Exception e)
        ...
        public static void CreateSafe(int i, Vector3 position, float rotation, int minamount, int maxamount,
            string address)
        {
            try
            {
                var new_safe = new Safe(i, position, rotation, minamount, maxamount, address);
                Safes.Add(new_safe);
                ...
                {
                    CommandText =
                        "INSERT INTO safes (minamount, maxamount, pos, address, rotation) VALUES (@val0,@val1,@val2,@val3,@val4)"
                };
            ...
            {
                Log.Write($"CreateSafe Exception: {e}");
            }
        }
        ...
        public static void RemoveSafe(int ID)
        {
            try
            {
                var safe = Safes.FirstOrDefault(s => s.ID == ID);
                if (safe == null) return;
                safe.Destroy(true);
                Safes.Remove(safe);
                using var cmd = new MySqlCommand
            ...
            {
                Log.Write($"RemoveSafe Exception: {e}");
            }
        }
        ...
        public static void startSafeDoorCracking(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У Вас нет дрели для взлома",
                        3000);
                    return;
                }
                ...
                if (isOpen)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Дверь в хранилище уже открыта",
                        3000);
                    return;
                }

                if (timer != null) return;
                Repository.RemoveIndex(player, "accessories", 8);
                isCracking = true;
                safeDrill = NAPI.Object.CreateObject(-443429795, new Vector3(253.9534, 225.2, 102.22),
                    new Vector3(0, 0, -18), 255, 0);
                label = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel("~r~8:00", new Vector3(253.9534, 225.2, 102.22),
                    4F, 0.3F, 0, new Color(255, 255, 255));
                secondsLeft = 480;
                timer = Timers.Start("DoorCracking", 1000, () => updateDoorCracking(), true);
                canBeClosed = false;
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.CITY,
                    "Кто-то пытается взломать дверь в хранилище мэрии.");
                ...
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.FIB,
                    "Кто-то пытается взломать дверь в хранилище мэрии.");
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.ARMY,
                    "Кто-то пытается взломать дверь в хранилище мэрии.");
            }
            catch (Exception e)
            {
                Log.Write($"startSafeDoorCracking Exception: {e}");
            }
        }
        ...
        private static void updateDoorCracking()
        {
            try
            {
                ...
                if (secondsLeft == 0)
                {
                    ...
                    {
                        Log.Write($"updateDoorCracking #2 Exception: {e}");
                    }

                    isCracking = false;
                    Timers.StartOnce("bankTimer", 600000, () => { canBeClosed = true; });
            ...
            {
                Log.Write($"updateDoorCracking Exception: {e}");
            }
        }
        ...
        public void onPlayerEnterVehicle(ExtPlayer player, ExtVehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                var Bags = Repository.GetItemData(player, "accessories", 8).ItemId;
                if ((Bags == ItemId.BagWithDrill || Bags == ItemId.BagWithMoney) &&
                    player.VehicleSeat == (int)VehicleSeat.Driver)
                {
                    VehicleManager.WarpPlayerOutOfVehicle(player);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        $"Вы не можете сесть на водительское место с {Repository.ItemsInfo[Bags].Name}.", 3000);
                }

                if (!characterData.Gender)
                {
                    var PlayerTopUp = Repository.GetItemData(player, "accessories", 5);
            ...
            {
                Log.Write($"onPlayerEnterVehicle Exception: {e}");
            }
        }
        ...
        public void SafeCracker_Init()
        {
            try
            {
                ...
                CustomColShape.CreateCylinderColShape(new Vector3(-2.1323678, -1821.9778, 29.543238), 1.5F, 2, 0,
                    ColShapeEnums.BlackMarket);

                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("Нажмите \n'Взаимодействие'\n~g~Caleb Baker"),
                    new Vector3(-2.1323678, -1821.9778, 29.543238), 5F, 0.8F, 0, new Color(255, 255, 255));

                for (var b = 0; b < salutePoints.Count; b++)
                {
                    PedSystem.Repository.CreateQuest("ig_ramp_hic", salutePoints[b], 105.93525f,
                        title: "~y~NPC~w~ Сергей\nПродавец фейерверков", colShapeEnums: ColShapeEnums.SaluteShop);
                ...
                foreach (DataRow Row in result.Rows)
                    try
                    {
                        var safePos = JsonConvert.DeserializeObject<Vector3>(Row["pos"].ToString());
                        var safeRot = Convert.ToSingle(Row["rotation"]);

                        var safe = new Safe(i, safePos, safeRot, Convert.ToInt32(Row["minamount"]),
                            Convert.ToInt32(Row["maxamount"]), Row["address"].ToString());
                        Safes.Add(safe);
                        safe.Create();
                        i++;
                    }
                    ...
                    {
                        Log.Write($"SafeCracker_Init Foreach Exception: {e}");
                    }
            }
            catch (Exception e)
            {
                Log.Write($"SafeCracker_Init Exception: {e}");
            }
        }
        ...
        public static void openSafe(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (stage == 2)
                    {
                        safe.SafeLoot = SafeRNG.Next(safe.MinAmount * Main.ServerSettings.MoneyMultiplier,
                            safe.MaxAmount * Main.ServerSettings.MoneyMultiplier);
                        safe.SetDoorOpen(true);
                        safe.Occupier = null;
                        Repository.Remove(player, $"char_{characterData.UUID}", "inventory", ItemId.Lockpick);
                        Trigger.ClientEvent(player, "dial", "close");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы успешно взломали сейф",
                            2000);
                        player.Eval(
                    ...
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"Вы подобрали {stage} из 3 паролей", 2000);
                        player.Eval(
            ...
            {
                Log.Write($"openSafe Exception: {e}");
            }
        }
        ...
        public static void OnHouseSafe(ExtPlayer player, int _)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Взлом возможен только в маске", 3000);
                        return;
                    }

                    if (safe.Occupier != null && NAPI.Player.GetPlayerFromHandle(safe.Occupier) != null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Этот сейф уже взламывают",
                            3000);
                        return;
                    }

                    var fracId = player.GetFractionId();
                    if (Manager.FractionTypes[fracId] != FractionsType.Gangs &&
                        Manager.FractionTypes[fracId] != FractionsType.Bikers)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Доступно только для банд",
                            5000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Для взлома нужна отмычка. Купить её можно на черном рынке. (GPS-Прочее)", 5000);
                        return;
                    }

                    if (DateTime.Now < NextRobbery && NowRobberyID != safe.ID)
                    {
                        var ticks = NextRobbery.Ticks - DateTime.Now.Ticks;
                        if (ticks <= 0) return;
                        var g = new DateTime(ticks);
                        if (g.Hour >= 1)
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                $"Попробуйте через {g.Hour}:{g.Minute}:{g.Second}", 3000);
                        else
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                $"Попробуйте через {g.Minute}:{g.Second}", 3000);
                        return;
                    }

                    var nearestPlayers = Main.GetPlayersInRadiusOfPosition(player.Position, 7);
                    var gangsters = 0;
                    foreach (var foreachPlayer in nearestPlayers)
                    {
                        ...
                        if (Manager.FractionTypes[foreachMemberFractionData.Id] == FractionsType.Gangs ||
                            Manager.FractionTypes[foreachMemberFractionData.Id] == FractionsType.Bikers) gangsters++;
                    }

                    if (gangsters == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Для начала нужен минимум еще 1 человек.", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        "С минуты на минуту сюда прибудут копы.", 3000);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.POLICE,
                        "!{#F08080}[F] " + $"Сейф №{safe.ID} по адресу {safe.Address} пытаются взломать.", true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.SHERIFF,
                        "!{#F08080}[F] " + $"Сейф №{safe.ID} по адресу {safe.Address} пытаются взломать.", true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.FIB,
                        "!{#F08080}[F] " + $"Сейф №{safe.ID} по адресу {safe.Address} пытаются взломать.", true);
                    ...
                    if (DateTime.Now >= safe.BlipSet)
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            {
                                Log.Write($"interactSafe Task Exception: {e}");
                            }
                        }, 900000);
                    }
                    ...
                    if (player.HasSharedData("IS_MASK") && !player.GetSharedData<bool>("IS_MASK"))
                    {
                        var wantedLevel = new WantedLevel(4, LangFunc.GetText(LangType.Ru, DataName.Police),
                            DateTime.Now, "Ограбление сейфа");
            ...
            {
                Log.Write($"OnHouseSafe Exception: {e}");
            }
        }

        [Interaction(ColShapeEnums.HouseSafe, true)]
        ...
        public static void OutHouseSafe(ExtPlayer player, int index)
        {
            var sessionData = player.GetSessionData();
            if (sessionData == null) return;
            var safe = Safes.FirstOrDefault(s => s.ID == index);
        ...
        public static void MoneyFlow(ExtPlayer player)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                var Bags = Repository.GetItemData(player, "accessories", 8);
                if (Bags.ItemId != ItemId.BagWithMoney) return;
                Repository.RemoveIndex(player, "accessories", 8);
                Wallet.Change(player, (int)(Convert.ToInt32(Bags.Data) * 0.97));
                GameLog.Money("server", $"player({characterData.UUID})", (int)(Convert.ToInt32(Bags.Data) * 0.97),
                    "moneyFlow");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы отмыли {(int)(Convert.ToInt32(Bags.Data) * 0.97)}$. Человек забрал {(int)(Convert.ToInt32(Bags.Data) * 0.03)}$ за свои услуги",
                    3000);
            }
            catch (Exception e)
            {
                Log.Write($"MoneyFlow Exception: {e}");
            }
        }
        ...
        public static void SafeCracker_Disconnect(ExtPlayer player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (Repository.GetItemData(player, "accessories", 8).ItemId == ItemId.BagWithMoney)
                    Repository.ItemsDropToIndex(player, "accessories", 8);

                var safe = Safes.FirstOrDefault(s => s.Occupier == player);
                if (safe != null)
                {
                    Repository.Remove(player, $"char_{characterData.UUID}", "inventory", ItemId.Lockpick);
                    safe.Occupier = null;
            ...
            {
                Log.Write($"SafeCracker_Disconnect Exception: {e}");
            }
        }
        ...
        public void SafeCracker_Exit()
        {
            try
            {
                foreach (var safe in Safes) safe.Destroy();
                Safes.Clear();
            }
            catch (Exception e)
            {
                Log.Write($"SafeCracker_Exit Exception: {e}");
            }
        }
        ...
        public static void CMD_CreateSafe(ExtPlayer player, int id, float distance, int minamount, int maxamount,
            string address)
        {
            try
            {
                ...
                var position = XYInFrontOfPoint(player.Position, player.Rotation.Z, distance) -
                               new Vector3(0.0, 0.0, 0.25);
        ...
        public static void OnBlackMarket(ExtPlayer player)
        {
            try
            {
                ...
                var ListItems = new List<int> { 0, 1, 2, 3, 4, 5, 6, 7, 69, 78, 79, 80, 81 };

                var _JsonData = new List<Manager.FracMatsData>();
            ...
            {
                Log.Write($"OnBlackMarket Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                switch (index)
                {
                    case 0:
                        ...
                        return;
                    case 1:
                        if (Repository.isFreeSlots(player, ItemId.BagWithDrill) != 0) return;
                        if (UpdateData.CanIChange(player, Main.BlackMarketDrill, true) != 255) return;
                        Repository.ChangeAccessoriesItem(player, 8, "", true, ItemId.BagWithDrill);
                        Repository.AccessoriesUse(player, 8);
                        ClothesComponents.UpdateClothes(player);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketDrill,
                            "buyMavr(drill)");
                        Wallet.Change(player, -Main.BlackMarketDrill);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили сумку с дрелью для ограблений", 3000);
                        return;
                    case 2:
                        if (characterData.Money < Main.BlackMarketLockPick)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Недостаточно денег",
                                3000);
                            return;
                        }

                        if (Repository.isFreeSlots(player, ItemId.Lockpick) != 0) return;
                        Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Lockpick);
                        Wallet.Change(player, -Main.BlackMarketLockPick);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketLockPick,
                            "buyMavr(lockpick)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили отмычку для замков", 3000);
                        return;
                    case 3:
                        if (UpdateData.CanIChange(player, Main.BlackMarketArmyLockPick, true) != 255) return;
                        if (Repository.isFreeSlots(player, ItemId.ArmyLockpick) != 0) return;
                        Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.ArmyLockpick);
                        Wallet.Change(player, -Main.BlackMarketArmyLockPick);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketArmyLockPick,
                            "buyMavr(armylockpick)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили военную отмычку", 3000);
                        return;
                    case 4:
                        if (UpdateData.CanIChange(player, Main.BlackMarketCuffs, true) != 255) return;
                        if (Repository.isFreeSlots(player, ItemId.Cuffs) != 0) return;
                        Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Cuffs);
                        Wallet.Change(player, -Main.BlackMarketCuffs);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketCuffs,
                            "buyMavr(cuffs)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы купили стяжки для рук",
                            3000);
                        return;
                    case 5:
                        if (UpdateData.CanIChange(player, Main.BlackMarketPocket, true) != 255) return;
                        if (Repository.isFreeSlots(player, ItemId.Pocket) != 0) return;
                        Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Pocket);
                        Wallet.Change(player, -Main.BlackMarketPocket);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketPocket,
                            "buyMavr(pocket)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили мешок на голову", 3000);
                        return;
                    case 6:
                        if (characterData.WantedLVL == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Вы не находитесь в розыске", 3000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.BlackMarketWanted, true) != 255) return;
                        Wallet.Change(player, -Main.BlackMarketWanted);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketWanted,
                            "buyMavr(wanted)");
                    ...
                    case 7:
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CONFIRM_BUY_BODYARMOUR",
                            "Вы действительно хотите скрафтить бронежилет, стомостью 150 материалов?");
                        return;
                    case 69:
                        if (!sessionData.CuffedData.Cuffed)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "На Вас нет наручников",
                                3000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.BlackMarketUnCuff, true) != 255) return;
                        Wallet.Change(player, -Main.BlackMarketUnCuff);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketUnCuff,
                            "buyMavr(uncuff)");
                    ...
                    case 78:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "У Вас уже есть лицензия на оружие", 3000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.BlackMarketGunLic, true) != 255) return;
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketGunLic,
                            "buyMavr(gunlic)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили лицензию на оружие", 3000);
                    ...
                    case 79:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "У Вас уже есть мед. карта", 3000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.BlackMarketMedCard, true) != 255) return;
                        Wallet.Change(player, -Main.BlackMarketMedCard);
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackMarketMedCard,
                            "buyMavr(medcard)");
                        qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage31, 2, true);
                        qMain.UpdateQuestsComplete(player, Zdobich.QuestName, (int)zdobich_quests.Stage31, true);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы купили мед. карту",
                            3000);
                        characterData.Licenses[7] = true;
                        return;
                    case 80:
                        if (Repository.isItem(player, "inventory", ItemId.QrFake) != null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouAlreadyHaveQrMavr), 5000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.BlackQrFake, true) != 255) return;

                        if (Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.QrFake) ==
                            -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили поддельный QR-код", 5000);
                        return;
                    case 81:
                        if (Repository.isItem(player, "inventory", ItemId.RadioInterceptor) != null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "У вас уже есть Радиоперехватчик.", 5000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.BlackRadioInterceptord, true) != 255) return;

                        if (Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                ItemId.RadioInterceptor) == -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoSpaceInventory), 3000);
                            return;
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", Main.BlackRadioInterceptord,
                            "buyMavr(RadioInterceptor)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы купили Радиоперехватчик", 5000);
                        return;
                }
            }
            catch (Exception e)
            {
                Log.Write($"callback_moneyflow Exception: {e}");
            }
        }
        ...
        public static void OnSaluteShop(ExtPlayer player)
        {
            try
            {
                if (!player.IsCharacterData()) return;
                if (Manager.FractionDataMats.Count == 0) return;
                var ListItems = new List<int> { 70, 71, 72, 73 };
            ...
            {
                Log.Write($"OnSaluteShop Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                switch (index)
                {
                    case 70:
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", Main.PricesSettings.FireworkPrices[0],
                            "buySaluteShop(1)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы купили фейерверк",
                            3000);
                    ...
                    case 71:
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", Main.PricesSettings.FireworkPrices[1],
                            "buySaluteShop(2)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы купили фейерверк",
                            3000);
                        BattlePass.Repository.UpdateReward(player, 28);
                    ...
                    case 72:
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", Main.PricesSettings.FireworkPrices[2],
                            "buySaluteShop(3)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы купили фейерверк",
                            3000);
                        BattlePass.Repository.UpdateReward(player, 28);
                    ...
                    case 73:
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", Main.PricesSettings.FireworkPrices[3],
                            "buySaluteShop(4)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы купили фейерверк",
                            3000);
                        BattlePass.Repository.UpdateReward(player, 28);
        ...
        public static void OpenSafedoorMenu(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OpenSafedoorMenu Exception: {e}");
            }
        }
        ...
        private static void callback_safedoor(ExtPlayer player, object listItem)
        {
                ...
                switch (listItem)
                {
                    case "change":
                        ...
                        if (memberFractionData != null &&
                            memberFractionData.Id == (int)Fractions.Models.Fractions.CITY &&
                            memberFractionData.Rank >= 19)
                        {
                            if (isCracking)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
            ...
            {
                Log.Write($"callback_safedoor Exception: {e}");
            }
        }
    ...
    public class Safe
    {
        [JsonIgnore] private ExtColShape colShape;

        [JsonIgnore] private Object DoorObject;

        [JsonIgnore] public ExtTextLabel Label;

        [JsonIgnore] private int RemainingSeconds;

        [JsonIgnore] public int SafeLoot;

        [JsonIgnore] private string Timer;
        ...
        [JsonIgnore] public bool IsOpen { get; private set; }

        [JsonIgnore] public List<int> LockAngles { get; } = new List<int>();

        [JsonIgnore] public ExtPlayer Occupier { get; set; }

        [JsonIgnore] public Object Object { get; private set; }

        [JsonIgnore] public ExtBlip Blip { get; set; }

        [JsonIgnore] public DateTime BlipSet { get; set; } = DateTime.Now;

        public void Create()
        {
            try
            {
                Object = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("v_ilev_gangsafe"), Position,
                    new Vector3(0.0, 0.0, Rotation), 255, 0);
                DoorObject = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("v_ilev_gangsafedoor"), Position,
                    new Vector3(0.0, 0.0, Rotation), 255, 0);
                colShape = CustomColShape.CreateCylinderColShape(Position, 1.25f, 1.0f, 0, ColShapeEnums.HouseSafe, ID);

                Label = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Сейф"),
                    Position + new Vector3(0, 0, 1.05), 5f, 0.65f, 0, new Color(255, 255, 255));

                for (var i = 0; i < 3; i++) LockAngles.Add(SafeMain.SafeRNG.Next(0, 361));
            }
            catch (Exception e)
            {
                SafeMain.Log.Write($"Create Exception: {e}");
            }
        }
        ...
        public void Loot(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveSumka), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SafeNoMoney), 3000);
                    return;
                }
                ...
                {
                    var lefts = Convert.ToInt32(Bags.Data);
                    if (lefts == SafeMain.MaxMoneyInBag)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SumkaFullMoney), 3000);
                        return;
                    }
                    ...
                    lefts += money;
                    Bags.Data = $"{lefts}";
                    Repository.SetItemData(player, "accessories", 8, Bags, true);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.InSumkeLefts, lefts), 3000);
                }
                else
                {
                    if (Repository.isFreeSlots(player, ItemId.BagWithMoney) != 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.EstSumkaIliDrill), 3000);
                        return;
                    }

                    Repository.ChangeAccessoriesItem(player, 8, $"{money}", true, ItemId.BagWithMoney);
                    Repository.AccessoriesUse(player, 8);
                    ClothesComponents.UpdateClothes(player);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouGetSumka, money), 3000);
                }
            ...
            {
                SafeMain.Log.Write($"Loot Exception: {e}");
            }
        }
        ...
        public void Countdown()
        {
            try
            {
                ...
                {
                    Label.Text = "~w~Сейф";
                    for (var i = 0; i < 3; i++) LockAngles[i] = SafeMain.SafeRNG.Next(10, 351);
                    SetDoorOpen(false);
                }
                else
                {
                    var time = TimeSpan.FromSeconds(RemainingSeconds);
                    Label.Text = string.Format("~r~Сейф ~n~~w~{0:D2}:{1:D2}:{2:D2}", time.Hours, time.Minutes,
                        time.Seconds);
                    Label.Text += $"\n~r~{SafeLoot}$";
                }
            }
            ...
            {
                SafeMain.Log.Write($"Countdown Exception: {e}");
            }
        }
        ...
        public void SetDoorOpen(bool is_open)
        {
            try
            {
                ...
                if (is_open)
                {
                    ...
                    Timer = Timers.Start(1000, () => { Countdown(); }, true);
                }
                else
                {
                    SafeLoot = 0;
            ...
            {
                SafeMain.Log.Write($"SetDoorOpen Exception: {e}");
            }
        }
        ...
        public void Destroy(bool check_players = false)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        SafeMain.Log.Write($"Destroy Task Exception: {e}");
                    }
                });
            ...
            {
                SafeMain.Log.Write($"Destroy Exception: {e}");
            }
        }
    }
}