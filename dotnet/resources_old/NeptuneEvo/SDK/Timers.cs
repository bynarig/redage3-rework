...
namespace NeptuneEvo.SDK
{
    ...
    public static class Timers
    {
        ...
        private static void Logic()
        {
            try
            {
                while (true)
                    try
                    {
                        foreach (var TimerId in TimersData.Keys.ToList())
                            try
                            {
                                if (!TimersData.ContainsKey(TimerId)) continue;
                                var timer = TimersData[TimerId];
                                if (timer != null && !timer.isFinished) timer.Elapsed();
                                else if (timer != null && timer.isFinished && TimersData.ContainsKey(timer.ID))
                                    TimersData.TryRemove(timer.ID, out _);
                            }
                            catch (Exception e)
                            {
                                Log.Write($"Logic Foreach Exception: {e}");
                            }
                    ...
                    {
                        Log.Write($"Logic While Exception: {e}");
                    }
            }
            catch (Exception e)
            {
                Log.Write($"Logic Exception: {e}");
            }
        }
        ...
        public static string Start(int interval, Action action, bool isnapitask = false)
        {
            try
            {
                ...
                var newtimer = new nTimer(action, id, interval, isnapitask_: isnapitask);
                lock (TimersData)
                {
        ...
        public static string Start(string id, int interval, Action action, bool isnapitask = false)
        {
            ...
            {
                Log.Write($"Start({id}) Exception: {e}");
                return null;
            }
        }
        ...
        public static string StartOnce(int interval, Action action, bool isnapitask = false)
        {
            ...
            {
                var id = Guid.NewGuid().ToString();
                var newtimer = new nTimer(action, id, interval, true, isnapitask_: isnapitask);
                lock (TimersData)
                {
                    TimersData.TryAdd(id, newtimer);
                }
            ...
            {
                Log.Write($"StartOnce Exception: {e}");
                return null;
            }
        }
        ...
        public static string StartOnce(string id, int interval, Action action, bool isnapitask = false)
        {
            ...
            {
                Log.Write($"StartOnce({id}) Exception: {e}");
                return null;
            }
        }
        ...
        public static string StartTask(int interval, Action action)
        {
            ...
            {
                var id = Guid.NewGuid().ToString();
                var newtimer = new nTimer(action, id, interval, false, true);
                lock (TimersData)
                {
                    TimersData.TryAdd(id, newtimer);
                }
            ...
            {
                Log.Write($"StartTask Exception: {e}");
                return null;
            }
        }
        ...
        public static string StartTask(string id, int interval, Action action)
        {
            ...
            {
                Log.Write($"StartTask({id}) Exception: {e}");
                return null;
            }
        }
        ...
        public static string StartOnceTask(int interval, Action action)
        {
            ...
            {
                var id = Guid.NewGuid().ToString();
                var newtimer = new nTimer(action, id, interval, true, true);
                lock (TimersData)
                {
                    TimersData.TryAdd(id, newtimer);
                }
            ...
            {
                Log.Write($"StartOnceTask Exception: {e}");
                return null;
            }
        }
        ...
        public static string StartOnceTask(string id, int interval, Action action)
        {
            ...
            {
                Log.Write($"StartOnceTask({id}) Exception: {e}");
                return null;
            }
        }
        ...
        {
            var timers_ = "";
            foreach (var t in TimersData.Values)
            {
                var state = t.isFinished ? "stopped" : "active";
                timers_ += $"{t.ID}:{state} ";
            }
    ...
    public class nTimer
    {
        ...
        public void Elapsed()
        {
                ...
                if (Next <= DateTime.Now)
                {
                    ...
                    if (isTask)
                        Task.Factory.StartNew(() =>
                        {
                            ...
                            {
                                Timers.Log.Write(
                                    $"Elapsed({ID},MS:{MS},ONCE:{isOnce},From:{action.Target}) Task #1 Exception: {e}");
                            }
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            {
                                Timers.Log.Write(
                                    $"Elapsed({ID},MS:{MS},ONCE:{isOnce},From:{action.Target}) Task #2 Exception: {e}");
                            }
                        });
                    else action.Invoke();
            ...
            {
                Timers.Log.Write($"Elapsed({ID},MS:{MS},ONCE:{isOnce},From:{action.Target}) Exception: {e}");
            }
        }
    }
}