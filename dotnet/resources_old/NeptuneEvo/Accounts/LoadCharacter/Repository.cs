    ...
    internal class Repository
    {
        ...
        public static async void Load(ExtPlayer player, DateTime TestSpeedLoad, bool isReg = false)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.Center,
                        LangFunc.GetText(LangType.Ru, DataName.CharDataGetError), 5000);
                    return;
                }
                ...
                foreach (var charData in accountData.Chars.ToList())
                {
                    if (charData > 0 && characters.FirstOrDefault(c => c.Uuid == charData) == null)
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CharDelete, charData), 3000);
                ...
                foreach (var character in characters)
                    try
                    {
                        if (accountData.Chars.Contains(character.Uuid))
                        {
                            ...
                            var memberOrganizationData =
                                Organizations.Manager.GetOrganizationMemberData(character.Uuid);
                    ...
                    {
                        Log.Write($"LoadSlots({accountData.Login}) Foreach Exception: {e}");
                    }

                returnData.Add("charsSlot", accountData.Chars);
                returnData.Add("chars", charsData);

                Trigger.ClientEvent(player, "toslots", JsonConvert.SerializeObject(returnData),
                    JsonConvert.SerializeObject(customizationsData), JsonConvert.SerializeObject(clothesData),
                ...
                if (isReg)
                    HelperThread.AddEvent("register", accountData.Email, accountData.Ga);
                else
                    HelperThread.AddEvent("login", accountData.Email, accountData.Ga);
            ...
            catch (Exception e)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.Center,
                    LangFunc.GetText(LangType.Ru, DataName.CharDataGetError), 5000);
                Log.Write($"LoadSlots Exception: {e}");
            }
        }
    }
}