...
namespace NeptuneEvo.Table.Tasks
{
    public class Repository
    {
        ...
        public static IReadOnlyDictionary<TableTaskId, TableTaskData> TasksList =
            new Dictionary<TableTaskId, TableTaskData>
            {
                {
        ...
        private static readonly int MaxTableTask = 3;

        public static IReadOnlyDictionary<Fractions.Models.Fractions, TableTaskId[]> FractionTaskIds =
            new Dictionary<Fractions.Models.Fractions, TableTaskId[]>
            {
                {
            ...
            try
            {
                ...
                switch (award.Type)
                {
                    case BattlePassRewardType.Item:
                        ...
                        GameLog.Money("system", $"player({player.GetUUID()})", 1,
                            $"TSGiveBonus({award.ItemId},{award.Data})");
                        if (isMessage)
                    ...
                    try
                    {
                        ...
                        if (taskIds.Count > 0)
                            for (var i = 0; i < MaxTableTask; i++)
                                if (tableTasksPlayerData.Length <= i || isUpdate || tableTasksPlayerData[i] == null ||
                                    tableTasksPlayerData[i].Time != time)
                                {
                                    if (!isUpdate && tableTasksPlayerData[i] != null &&
                                        _taskIds.Contains(tableTasksPlayerData[i].Id))
                                    {
                                    ...
                                    var id = Horses.Shuffle(taskIds)[0];
        ...
        public static TableTaskPlayerData[] GetData(TableTaskPlayerData[] tasks, TableTaskPlayerData[] staticTasks,
            bool isUpdate = false)
        {
            try
            {
        ...
        public static void UpdateOrg()
        {
            try
            {
                ...
                foreach (var fractionData in Manager.Fractions.Values)
                {
                ...
                {
                    if (!Manager.AllMembers.ContainsKey(fractionId))
                        continue;

                    foreach (var fractionMemberData in Manager.AllMembers[fractionId].ToList())
                    {
                        ...
                        foreach (var taskData in fractionMemberData.TasksData)
                        {
                            ...
                            if (!TasksList.ContainsKey(taskData.Id))
                                continue;

                            var fractionTasksData =
                                successTableTask[(int)fracId].FirstOrDefault(t => t.Id == taskData.Id);
                            ...
                            var task = TasksList[taskData.Id];
                            ...
                            {
                                Trigger.ClientEvent(player, "client.battlepass.missionComplite",
                                    "Фракционное задание выполнено", $"{task.Name}");
                                foreach (var award in task.Awards)
                                    GiveBonus(player, award);
                            }
                        }

                        fractionMemberData.TasksData =
                            GetData(JsonConvert.SerializeObject(fractionData.TasksData), ids, true);
                            ...
                            foreach (var award in task.Awards)
                                ...
                                else if (award.Type == BattlePassRewardType.Item)
                                {
                                    if (award.Count > 1)
                                    {
                                        for (var i = 0; i < award.Count; i++)
                                            if (CustomDamage.WeaponsHP.ContainsKey((ItemId)award.ItemId))
                                                Chars.Repository.AddNewItemWarehouseThread(db, uuid,
                                                    (ItemId)award.ItemId, 1,
                                                    $"{GetSerial()}_{CustomDamage.WeaponsHP[(ItemId)award.ItemId]}");
                                            else if ((ItemId)award.ItemId == ItemId.BodyArmor)
                                                Chars.Repository.AddNewItemWarehouseThread(db, uuid,
                                                    (ItemId)award.ItemId, 1, "100");
                                            else
                                                Chars.Repository.AddNewItemWarehouseThread(db, uuid,
                                                    (ItemId)award.ItemId, 1, award.Data);
                                    }
                                    else
                                    {
                                        if (CustomDamage.WeaponsHP.ContainsKey((ItemId)award.ItemId))
                                            Chars.Repository.AddNewItemWarehouseThread(db, uuid, (ItemId)award.ItemId,
                                                1, $"{GetSerial()}_{CustomDamage.WeaponsHP[(ItemId)award.ItemId]}");
                                        else if ((ItemId)award.ItemId == ItemId.BodyArmor)
                                            Chars.Repository.AddNewItemWarehouseThread(db, uuid, (ItemId)award.ItemId,
                                                1, "100");
                                        else
                                    ...
                                    GameLog.Money("system", $"player({uuid})", 1,
                                        $"TSGiveBonusOffline({award.ItemId},{award.Data})");
                                }
                        }
                    }
                    catch (Exception e)
