...
using DonatePack = NeptuneEvo.SDK.Models.DonatePack;
...
namespace NeptuneEvo
{
    public class Main : Script
    {
        ...
        public static int PlayersAtOnce;

        // Characters
        public static List<int> UUIDs = new List<int>(); // characters UUIDs

        public static ConcurrentDictionary<string, int>
            RefCodes = new ConcurrentDictionary<string, int>(); // characters RefCodes

        public static ConcurrentDictionary<int, string>
            PlayerNames = new ConcurrentDictionary<int, string>(); // character uuid - character name

        public static ConcurrentDictionary<string, int>
            PlayerBankAccs = new ConcurrentDictionary<string, int>(); // character name - character bank

        public static ConcurrentDictionary<string, int>
            PlayerUUIDs = new ConcurrentDictionary<string, int>(); // character name - character uuid

        public static List<ExtPlayer> Characters = new List<ExtPlayer>();

        public static ConcurrentDictionary<int, ExtPlayer>
            PlayerIdToEntity = new ConcurrentDictionary<int, ExtPlayer>();
        ...
        public static ConcurrentDictionary<string, List<int>>
            Usernames = new ConcurrentDictionary<string, List<int>>(); // usernames
        ...
        public static ConcurrentDictionary<string, string>
            LoginToEmail = new ConcurrentDictionary<string, string>(); // client's accounts

        public static readonly string[] stringDefaultBlock =
        {
            "даун", "daun", "д а у н", "d a u n", "d.a.u.n", "д.а.у.н",
            "пидр", "пiдр", "педер", "пидар", "пидор", "педик", "педрик", "педрил", "п и д а р", "п и д о р", "pidor",
            "pedik", "pidrila", "pidoras", "pidaras", "p i d o r", "pedor", "p.i.d.o.r", "п.и.д.о.р", "п.и.д.а.р",
            "нигер", "ниггер", "нига", "нигга", "ниgа", "ниggа", "негр", "нeгр", "nigg", "niga", "niger", "nigger",
            "negr", "n i g g a", "n i g a", "n e g r", "н.е.г.р", "н.и.г.е.р", "n.e.g.r",
        ...
        public static readonly string[] stringGlobalBlock =
        {
            ...
            "пидор", "пизда", "туз", "малафья", "гомик", "мудила", "пилотка", "манда", "анус", "вагина", "путана",
            "пидрила", "шалава", "хуила", "машонка", "елда",
            "lox", "loh",
            "xyёв", "xyй", "xyя", "xуе", "xуй", "xую", "хуйня",
            "zaeb",
            "пизд", "ахуел", "ахуеть",
            "бздение", "бздеть", "бздех", "бздецы", "бздит", "бздло", "бзднуть", "бздун", "бздюха", "бздюшка",
            "бздюшко",
            "вафел", "вафлёр", "взъеб", "взьеб", "въеб", "выеб",
            "гавно", "гавню", "гамно", "гандон", "гнид", "говенка", "говеный", "говешка", "говна", "говне", "говни",
            "говно",
            "говню", "говня", "гондон",
            "дебил", "доебываться", "долбоеб", "долбоёб", "долбоящер", "дрисня", "дрист", "дроч", "даун", "daun",
            "д а у н", "d a u n", "d.a.u.n", "д.а.у.н",
            "дура",
            ...
            "падла", "падонки", "падонок", "паскуда", "педер", "пидар", "пидор", "педик", "педрик", "педрил", "пезде",
            "пезди", "пезд", "п и д а р", "п и д о р", "pidor", "pedik", "pidrila", "pidoras", "pidaras", "p i d o r",
            "pedor", "p.i.d.o.r", "п.и.д.о.р", "п.и.д.а.р",
        ...
        public static readonly string[] LicWords =
        {
            "A",
            "B",
            "C",
        ...
        public static ConcurrentDictionary<string, BonusCodesData> BonusCodes =
            new ConcurrentDictionary<string, BonusCodesData>();

        public static ConcurrentDictionary<string, PromoCodesData> PromoCodes =
            new ConcurrentDictionary<string, PromoCodesData>();
        ...
        private static bool IsStartGameMode;
        ...
        public Main()
        {
            try
            {
                        ...
                        foreach (var player in NAPI.Pools.GetAllPlayers().Cast<ExtPlayer>())
                            if (player.Dimension == 0 && player.CharacterData == null && player.Position.X != 0)
                            {
                                Trigger.SendToAdmins(1,
                                    $"~r~ Игрок {player.Name} ({player.Id}) {player.Address} автоматически отпритонен и кикнут с сервера.");
                                player.Kick();
                            }
                    }
                });
                ...
                Entities.Players.CreateEntity = netHandle => new ExtPlayer(netHandle);
                Entities.Vehicles.CreateEntity = netHandle => new ExtVehicle(netHandle);
                Entities.Peds.CreateEntity = netHandle => new ExtPed(netHandle);
                Entities.Objects.CreateEntity = netHandle => new ExtObject(netHandle);
                Entities.Colshapes.CreateEntity = netHandle => new ExtColShape(netHandle);
                Entities.Blips.CreateEntity = netHandle => new ExtBlip(netHandle);
                Entities.Checkpoints.CreateEntity = netHandle => new ExtCheckpoint(netHandle);
                Entities.DummyEntities.CreateEntity = netHandle => new ExtDummyEntity(netHandle);
                Entities.Markers.CreateEntity = netHandle => new ExtMarker(netHandle);
                Entities.Pickups.CreateEntity = netHandle => new ExtPickup(netHandle);
                Entities.TextLabels.CreateEntity = netHandle => new ExtTextLabel(netHandle);
                ...
                var connectionStrings = new List<IConnectionStringSettings>();
                ...
                DataConnection.WriteTraceLine = (message, category, level) =>
                {
                    if (IsStartGameMode && Thread.CurrentThread.Name == "Main")
                        ...
                        {
                            Timers.Log.Write($"SaveMainDB Exception: {e}");
                        }
                };
        ...
        public static DateTime CompileDate { get; } =
            new FileInfo(Assembly.GetExecutingAssembly().Location).LastWriteTime;

        public static int WeekInfo { get; set; } =
        ...
        public static void LoadServerSettings()
        {
            ...
            Manager.FractionDataMats[70].Price = $"{PricesSettings.FireworkPrices[0]}$";
            Manager.FractionDataMats[71].Price = $"{PricesSettings.FireworkPrices[1]}$";
            Manager.FractionDataMats[72].Price = $"{PricesSettings.FireworkPrices[2]}$";
            Manager.FractionDataMats[73].Price = $"{PricesSettings.FireworkPrices[3]}$";

            Manager.FractionDataMats[74].Price = $"{PricesSettings.InstrumentPrices[0]}$";
            Manager.FractionDataMats[75].Price = $"{PricesSettings.InstrumentPrices[1]}$";
            Manager.FractionDataMats[76].Price = $"{PricesSettings.InstrumentPrices[2]}$";
            Manager.FractionDataMats[77].Price = $"{PricesSettings.InstrumentPrices[3]}$";
            ...
            if (jobsMinLvl.Count == 0)
                Settings.Save("jobsMinLvl", WorkManager.JobsMinLvl);
            else
                WorkManager.JobsMinLvl = jobsMinLvl;
            ...
            if (isBlock && WhiteList.Logins.Count == 0)
                foreach (var foreachPlayer in Entities.Players.All.Cast<ExtPlayer>())
                {
                    ...
                    Trigger.ClientEvent(foreachPlayer, "client.init",
                        ...
                        DonateSettings.Multiplier,
                        DonateSettings.Convert,
                        ServerSettings.IsMerger);
        ...
        {
            NAPI.Blip.CreateBlip(blipData.id, blipData.position, blipData.scale, (byte)blipData.color, blipData.name,
                shortRange: blipData.shortRange);
        }

        public static string BlockSymbols(string check)
        {
            for (var i = check.IndexOfAny(stringBlock); i >= 0;)
            {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                CreateBlip(new BlipData(304, LangFunc.GetText(LangType.Ru, DataName.Theather),
                    new Vector3(683.739, 570.40, 130.46), 7, true));
                CreateBlip(new BlipData(455, LangFunc.GetText(LangType.Ru, DataName.Yacht),
                    new Vector3(-2070.151, -1023.134, 11.9), 81, true));
                //CreateBlip(new BlipData(311, LangFunc.GetText(LangType.Ru, DataName.Gym), new Vector3(-1202.281, -1568.798, 3.488338), 4, true));

                CreateBlip(new BlipData(93, "Пляжные вечеринки", new Vector3(-1497.7688, -1484.525, 5.7608714), 43,
                    true)); // todo LangFunc
                CreateBlip(new BlipData(766, "Остров", new Vector3(5024.207, -5122.8962, 3.701595), 43,
                    true)); // todo LangFunc
                CreateBlip(new BlipData(93, "Пляжные вечеринки", new Vector3(-1705.88, -970.7998, 8.648665), 43,
                    true)); // todo LangFunc
                ...
                {
                    CommandText =
                        "SELECT `login`,`socialclub`,`email`,`hwid`,`character1`,`character2`,`character3`,`characters` FROM `accounts`"
                };
                ...
                if (result != null)
                {
                    ...
                    foreach (DataRow Row in result.Rows)
                        ...
                        {
                            Log.Write($"onResourceStart Foreach#5 Exception: {e}");
                        }
                ...
                {
                    Log.Write("DB `accounts` return null result", nLog.Type.Warn);
                }

                using var cmdCharacters = new MySqlCommand
                {
                    CommandText =
                        "SELECT `uuid`,`firstname`,`lastname`,`sim`,`lvl`,`exp`,`fraction`,`fractionlvl`,`money`,`bank`,`adminlvl`,`refcode` FROM `characters`"
                };

                var fractionPlayers = new List<List<object>>();
                var isFractionTable = true;

                using var resultCharacters = MySQL.QueryRead(cmdCharacters);
                    ...
                    foreach (DataRow Row in resultCharacters.Rows)
                        try
                        {
                            ...
                            if (adminlvl > 0)
                            {
                                ...
                                {
                                    CommandText =
                                        "SELECT `socialclub` FROM `accounts` WHERE `character1`=@val0 OR `character2`=@val1 OR `character3`=@val2 OR `characters` LIKE @val3"
                                };
                                ...
                                using var result2 = MySQL.QueryRead(cmdAccounts);
                        ...
                        {
                            Log.Write($"onResourceStart Foreach#1 Exception: {e}");
                        }
                ...
                {
                    Log.Write("DB `characters` return null result", nLog.Type.Warn);
                }
                ...
                foreach (var number in Enum.GetValues(typeof(DefaultNumber)))
                ...
                var time = 3600 - DateTime.Now.Minute * 60 - DateTime.Now.Second;
        ...
        public static string GetLoginFromUUID(int uuid)
        {
            ...
            {
                Log.Write($"ReturnLoginFromUUID Exception: {e}");
                return null;
            }
        }
        ...
        public static (byte, float) GetPlayerJobLevelBonus(sbyte jobId, int points)
        {
            try
            {
                switch (jobId)
                {
                    case 0: //electrecian
                        ...
                        if (points >= 2100 && points <= 4499) return (2, 1.1f);
                        if (points >= 4500 && points <= 7499) return (3, 1.12f);
                        if (points >= 7500 && points <= 14999) return (4, 1.15f);
                        if (points >= 15000) return (5, 1.2f);
                        return (0, 1);
                    case 1: //lawnmower
                        if (points >= 2000 && points <= 5999) return (1, 1.05f);
                        if (points >= 6000 && points <= 11999) return (2, 1.1f);
                        if (points >= 12000 && points <= 19999) return (3, 1.12f);
                        if (points >= 20000 && points <= 39999) return (4, 1.15f);
                        if (points >= 40000) return (5, 1.2f);
                        return (0, 1);
                    case 2: //postman
                        if (points >= 200 && points <= 599) return (1, 1.05f);
                        if (points >= 600 && points <= 1199) return (2, 1.1f);
                        if (points >= 1200 && points <= 1999) return (3, 1.12f);
                        if (points >= 2000 && points <= 3999) return (4, 1.15f);
                        if (points >= 4000) return (5, 1.2f);
                        return (0, 1);
                    case 3: //taxi
                        if (points >= 25 && points <= 74) return (1, 1);
                        if (points >= 75 && points <= 199) return (2, 1);
                        if (points >= 200 && points <= 499) return (3, 1);
                        if (points >= 500 && points <= 999) return (4, 1);
                        if (points >= 1000) return (5, 1);
                        return (0, 1);
                    case 4: //bus
                        if (points >= 3000 && points <= 9999) return (1, 1.05f);
                        if (points >= 10000 && points <= 19999) return (2, 1.1f);
                        if (points >= 20000 && points <= 34999) return (3, 1.12f);
                        if (points >= 35000 && points <= 69999) return (4, 1.15f);
                        if (points >= 70000) return (5, 1.2f);
                        return (0, 1);
                    case 5: //mech
                        if (points >= 10 && points <= 24) return (1, 1);
                        if (points >= 25 && points <= 49) return (2, 1);
                        if (points >= 50 && points <= 99) return (3, 1);
                        if (points >= 100 && points <= 249) return (4, 1);
                        if (points >= 250) return (5, 1);
                        return (0, 1);
                    case 6: //trucker
                        if (points >= 30 && points <= 89) return (1, 1);
                        if (points >= 90 && points <= 199) return (2, 1);
                        if (points >= 200 && points <= 349) return (3, 1);
                        if (points >= 350 && points <= 699) return (4, 1);
                        if (points >= 700) return (5, 1);
                        return (0, 1);
                    case 7: //collector
                        if (points >= 150 && points <= 399) return (1, 1.05f);
                        if (points >= 400 && points <= 899) return (2, 1.1f);
                        if (points >= 900 && points <= 1499) return (3, 1.12f);
                        if (points >= 1500 && points <= 2999) return (4, 1.15f);
                        if (points >= 3000) return (5, 1.2f);
                        return (0, 1);
                    default: return (0, 1);
                }
            }
            ...
            {
                Log.Write($"GetPlayerJobLevelBonus Exception: {e}");
                return (0, 1);
            }
        }
        ...
        public static int[] GetPlayerJobsNextLevel(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData == null) return new[] { 700, 2000, 200, 25, 3000, 10, 30, 150 };
                ...
                for (var i = 0; i <= 7; i++)
                    ...
                    {
                        var points = characterData.JobSkills[i];
                        var correctValue = 250;
            ...
            {
                Log.Write($"GetPlayerJobsNextLevel Exception: {e}");
                return new[] { 700, 2000, 200, 25, 3000, 10, 30, 150 };
            }
        }
        ...
        private static int GetTime(int addDay = 0, bool isRealTime = false)
        {
            ...
            Console.WriteLine(new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, isRealTime ? dateTime.Hour : 0,
                isRealTime ? dateTime.Minute : 0, isRealTime ? dateTime.Second : 0));
            return (int)new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, isRealTime ? dateTime.Hour : 0,
                isRealTime ? dateTime.Minute : 0, isRealTime ? dateTime.Second : 0).Subtract(
        ...
        private void enviromentChangeTrigger()
        {
            ...
            {
                Log.Write($"enviromentChangeTrigger Exception: {e}");
            }
        }
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    if (compensation != null)
                    {
                        ...
                        {
                            GameLog.Money("system", $"player({characterData.UUID})", compensation.Money,
                                $"Compensation({compensation.AutoId})");
                            Wallet.Change(player, compensation.Money);
                        }

                        if (compensation.Donate > 0)
                        {
                            UpdateData.RedBucks(player, compensation.Donate,
                                $"получил компенсацию в виде ({compensation.Donate})");
                            GameLog.Money("system", $"player({characterData.UUID})", compensation.Donate,
                                "CompensationDonate");
                        }

                        if (compensation.ItemID != (int)ItemId.Debug)
                        {
                            Chars.Repository.AddNewItemWarehouse(player, (ItemId)compensation.ItemID, 1,
                                compensation.Data);
                            GameLog.Money("system", $"player({characterData.UUID})", 1,
                                $"CompensationItem({compensation.ItemID},{compensation.Data})");
                        }
                ...
                {
                    Log.Write($"Compensation Exception: {e}");
                }
            });
        }
        ...
        private static void OnlineReward(ExtPlayer player, byte hours, int redbuckses)
        {
            try
            {
                ...
                UpdateData.RedBucks(player, redbuckses, "Вознаграждение за дневной онлайн");
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge,
                    LangFunc.GetText(LangType.Ru, DataName.BonusEverdayOnline, hours, redbuckses), DateTime.Now);
                ...
                EventSys.SendCoolMsg(player, "Система", "Награда за онлайн",
                    $"{LangFunc.GetText(LangType.Ru, DataName.BonusEverdayOnline, hours, redbuckses)}", "", 10000);
                if (!characterData.Achievements[16])
                {
                    ...
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.RbBonus1), 30000);
                    Notify.SendToKey(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.RbBonus2), 30000, 21);
                }
            }
            catch (Exception e)
            {
                Log.Write($"OnlineReward Exception: {e}");
            }
        }
        ...
        private void playedMinutesTrigger()
        {
            try
            {
                Trigger.SetTask(async () =>
                {
                    ...
                    {
                        Log.Write($"playedMinutesTrigger Task Exception: {e}");
                    }
                });
                var now = DateTime.Now;

                if (now.Hour == 22 && now.Minute == 0)
                    HelperThread.AddEvent("server_online", DateTime.Now.ToString("s"), "-",
                        Character.Repository.GetPlayers().Count);
                    ...
                    if (now.Hour == 6)
                    {
                        if (now.Minute == 0)
                        {
                            NAPI.Chat.SendChatMessageToAll(
                                "!{#FF0000}[AUTO RESTART] Дорогие игроки, в 06:15 произойдёт автоматическая перезагрузка сервера.");
                        }
                        else if (now.Minute == 5)
                        {
                            NAPI.Chat.SendChatMessageToAll(
                                "!{#FF0000}[AUTO RESTART] Дорогие игроки, напоминаем, что в 06:15 произойдёт автоматическая перезагрузка сервера.");
                        }
                        else if (now.Minute == 10)
                        {
                            if (ServerNumber != 0)
                            {
                                Blackjack.BlackJackWorking = false;
                                Roullete.RouletteWorking = false;
                                Spin.SpinsWorking = false;
                                Horses.HorsesWorking = false;
                            }
                        ...
                        else if (now.Minute == 15 || now.Minute == 16)
                        {
                            ...
                            {
                                NAPI.Chat.SendChatMessageToAll(
                                    "!{#FF0000}[AUTO RESTART] Дорогие игроки, сейчас произойдёт автоматическая перезагрузка сервера, сервер будет доступен вновь примерно в течение пары минут.");
                                Admin.stopServer(
                                    "Происходит автоматическая перезагрузка, будем рады видеть Вас снова через пару минут! :)");
                            }
                        }
                    }
                ...
                var IsNewWeek = dayofweek != EverydayAward.InitDayOfWeek && dayofweek == DayOfWeek.Monday;
                ...
                if (IsNewWeek)
                {
                    WeekInfo = CultureInfo.CurrentCulture.Calendar.GetWeekOfYear(DateTime.Now,
                ...
                if (dayofweek == DayOfWeek.Wednesday || dayofweek == DayOfWeek.Friday || dayofweek == DayOfWeek.Sunday)
                {
                    if (now.Hour == 19 && now.Minute == 45)
                    {
                        ...
                        ;
                        EventSys.SendPlayersToEvent("MATWAR", "Война за материалы!",
                            "В 20:00 начнется война за материалы! Фракции, приготовьтесь!", "", 15000);
                    }
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    if (foreachCharacterData.DemorganTime <= 0)
                        switch (foreachCharacterData.Time.TodayTime)
                        {
                            case 120:
                                ...
                                break;
                            case 180:
                                Chars.Repository.AddNewItemWarehouse(foreachPlayer, ItemId.Case0);
                                ...
                                if (!foreachCharacterData.Achievements[17])
                                {
                                    ...
                                    Notify.SendToKey(foreachPlayer, NotifyType.Alert, NotifyPosition.BottomCenter,
                                        "В донат-меню в разделе Рулетка Вам стал доступен Бесплатный Ежедневный кейс. Бесплатно открыть его можно 1 раз в день, отыграв 3 часа.",
                                        30000, 21);
                                }
                            ...
                            case 240:
                                ...
                                break;
                            case 300:
                                Chars.Repository.AddNewItemWarehouse(foreachPlayer, ItemId.Case1);
                                ...
                                if (!foreachCharacterData.Achievements[23])
                                {
                                    ...
                                    Notify.SendToKey(foreachPlayer, NotifyType.Alert, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.Case0get), 30000, 21);
                                }

                                break;
                            case 480:
                                OnlineReward(foreachPlayer, 8, 4);
                                Chars.Repository.AddNewItemWarehouse(foreachPlayer, ItemId.Case2);
                                if (!foreachCharacterData.Achievements[24])
                                {
                                    foreachCharacterData.Achievements[24] = true;
                                    Notify.SendToKey(foreachPlayer, NotifyType.Alert, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.Case1get), 30000, 21);
                                }
                    ...
                    {
                        UpdateData.RedBucks(foreachPlayer, 500, "Вознаграждение за месячный онлайн");
                        Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.Case2get), 5000);
                    }

                    if (foreachAccountData.VipLvl > 0 && foreachAccountData.VipDate <= DateTime.Now)
                    {
                        foreachAccountData.VipLvl = 0;
                        Notify.Send(foreachPlayer, NotifyType.Alert, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.VipIsOver), 3000);
                    }

                    try
                    {
                        if (foreachSessionData.RentData != null && !foreachSessionData.RentData.IsJob &&
                            now > foreachSessionData.RentData.Date)
                        {
                            Rentcar.OnReturnVehicle(foreachPlayer);
                            Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.RentOver), 3000);
                        }
                    }
                    catch (Exception e)
                    {
                        Log.Write($"Rent Vehicle Delete Exception: {e}");
                    }
                }
            ...
            {
                Log.Write($"playedMinutesTrigger Exception: {e}");
            }
        }
                    ...
                    lock (BonusCodes)
                    {
                        if (accountData.BonusCodes.Contains(text))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.BonusUsed), 5000);
                            return;
                        }

                        var pcdata = BonusCodes[text];
                        if (pcdata.UsedLimit == 0 || pcdata.UsedTimes < pcdata.UsedLimit)
                        {
                            Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", ItemId.Present);
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.OpenGift), 3000);
                            if (characterData.Gender)
                                Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.OpenedGift));
                            else Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.OpenedaGift));
                            accountData.BonusCodes.Add(text);
                            GameLog.Money("server", $"player({characterData.UUID})", pcdata.RewardMoney,
                                $"BonusReward({text})");
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, pcdata.RewardMessage,
                                15000);
                            pcdata.UsedTimes++;
                            ...
                            {
                                Wallet.Change(player, (int)pcdata.RewardMoney);
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    $"[BONUS] +{pcdata.RewardMoney}$", 15000);
                            }
                            ...
                            if (pcdata.RewardVipLvl != 0)
                            {
                                if (accountData.VipLvl == 0)
                                {
                                    switch (pcdata.RewardVipLvl)
                                    {
                                        case 1:
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.Ru, DataName.BronzeBonus,
                                                    pcdata.RewardVipDays), 15000);
                                            break;
                                        case 2:
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.Ru, DataName.SilverBonus,
                                                    pcdata.RewardVipDays), 15000);
                                            break;
                                        case 3:
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.Ru, DataName.GoldBonus, pcdata.RewardVipDays),
                                                15000);
                                            break;
                                        case 4:
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.Ru, DataName.PlatinumBonus,
                                                    pcdata.RewardVipDays), 15000);
                                            break;
                                        default:
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.Ru, DataName.ErrorBonus,
                                                    pcdata.RewardVipDays), 15000);
                                            break;
                                    }
                                    ...
                                    GameLog.Money("system", $"player({characterData.UUID})", 0,
                                        $"bonusVIP({accountData.VipLvl}lvl, {pcdata.RewardVipDays}d, стало до {accountData.VipDate.ToString("s")})");
                                }
                                ...
                                else
                                {
                                    ...
                                    GameLog.Money("system", $"player({characterData.UUID})", 0,
                                        $"bonusVIP({accountData.VipLvl}lvl, {days}d, стало до {accountData.VipDate.ToString("s")})");
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.DaysDop, days), 15000);
                                }
                            ...
                            if (characterData.Gender)
                            {
                                if (pcdata.RewardItemsMale.Count != 0)
                                {
                                    foreach (var item in pcdata.RewardItemsMale)
                                        ...
                                        {
                                            Log.Write($"CheckMyBonusCode Foreach #1 Exception: {e}");
                                        }

                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.BonusItems,
                                            pcdata.RewardItemsMale.Count), 15000);
                                }
                            }
                            else
                            {
                                if (pcdata.RewardItemsFemale.Count != 0)
                                {
                                    foreach (var item in pcdata.RewardItemsFemale)
                                        ...
                                        {
                                            Log.Write($"CheckMyBonusCode Foreach #2 Exception: {e}");
                                        }

                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.BonusItems,
                                            pcdata.RewardItemsFemale.Count), 15000);
                                }
                            }
                            ...
                            {
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    $"[BONUS] +{pcdata.RewardExp} EXP", 15000);
                                UpdateData.Exp(player, pcdata.RewardExp);
                            }
                        }
                        else if (pcdata.UsedLimit != 0 && pcdata.UsedTimes >= pcdata.UsedLimit)
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.BonusEnd, pcdata.UsedLimit), 3000);
                        }
                    }
                else
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoBonusMore), 5000);
            }
            catch (Exception e)
            {
                Log.Write($"CheckMyBonusCode Exception: {e}");
            }
        }
        ...
        public static void CheckMyPromoCode(ExtPlayer player, bool sendstats = true)
        {
            try
            {
                ...
                {
                    var promo = accountData.PromoCodes[0];
                    if (PromoCodes.ContainsKey(promo))
                    {
                        lock (PromoCodes)
                        {
                            accountData.PresentGet = true;
                            var pcdata = PromoCodes[promo];
                            if (pcdata.RewardLimit == 0 || pcdata.RewardReceived < pcdata.RewardLimit)
                            {
                                ...
                                if (pcdata.RewardMoney != 0) Wallet.Change(player, (int)pcdata.RewardMoney);
                                if (pcdata.RewardVipLvl != 0)
                                {
                                    if (accountData.VipLvl == 0)
                                    {
                                        ...
                                        accountData.VipDate = DateTime.Now.AddDays(pcdata.RewardVipDays);
                                        GameLog.Money("system", $"player({characterData.UUID})", 0,
                                            $"promoCodeVIP({accountData.VipLvl}lvl, {pcdata.RewardVipDays}d, стало до {accountData.VipDate.ToString("s")})");
                                    }
                                    else if (accountData.VipLvl == pcdata.RewardVipLvl)
                                    {
                                        accountData.VipDate = accountData.VipDate.AddDays(pcdata.RewardVipDays);
                                    ...
                                    else
                                    {
                                        ...
                                        accountData.VipDate = accountData.VipDate.AddDays(days);
                                        GameLog.Money("system", $"player({characterData.UUID})", 0,
                                            $"promoCodeVIP({accountData.VipLvl}lvl, {days}d, стало до {accountData.VipDate.ToString("s")})");
                                        Trigger.SendChatMessage(player,
                                            $"Так как у Вас уже был VIP статус, то вместо его замены Вам было начислено дополнительно {days} дней использования!");
                                    }
                                    //if (sendstats) Chars.Repository.PlayerStats(player);
                                }

                                if (pcdata.RewardItems.Count != 0)
                                    foreach (var item in pcdata.RewardItems)
                                        try
                                        {
                                            WeaponRepository.GiveWeapon(player, item.ItemId, item.Data, item.Count);
                                        }
                                        ...
                                        {
                                            Log.Write($"CheckMyPromoCode Foreach #1 Exception: {e}");
                                        }

                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    pcdata.RewardMessage, 15000);
                                if (pcdata.CreatorUUID != 0)
                                {
                                    GameLog.Money("server", $"player({pcdata.CreatorUUID})", 500,
                                        $"PromoOwnerReward({promo})");
                                    var isGiven = false;
                                    var target = GetPlayerByUUID((int)pcdata.CreatorUUID);
                                    if (target != null)
                                    {
                                        Wallet.Change(target, 500);
                                        Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.NagradaLvlPromo, player.Name), 5000);
                                        isGiven = true;
                                    }
                            ...
                            {
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.BonusEnd, pcdata.RewardLimit), 3000);
                            }
                        }
                    }
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.BonusOver), 7000);
                    }
                }
                ...
                if (accountData.RefferalId != 0 && !accountData.RefPresentGet)
                    ...
                    {
                        GameLog.Money("server", $"player({characterData.UUID})", 5000,
                            $"RefReward({accountData.RefferalId})");
                        accountData.RefPresentGet = true;
                        Wallet.Change(player, 1500);
                        Chars.Repository.AddNewItemWarehouse(player, ItemId.Case0);
                        Chars.Repository.AddNewItemWarehouse(player, ItemId.Case1);
                        Chars.Repository.AddNewItemWarehouse(player, ItemId.Case2);
                        UpdateData.RedBucks(player, 100, "Подарок за ввод реф-кода");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.RefBonus, characterData.LVL), 15000);
                        var target = GetPlayerByUUID(accountData.RefferalId);
                        ...
                        Trigger.SetTask(async () =>
                        {
                            try
                            {
                                ...
                                for (var i = 0; i < Chars.Repository.RefferalsData.Length; i++)
                                    if (Chars.Repository.RefferalsData[i].Count <= count)
                                        refferalData = Chars.Repository.RefferalsData[i];
                                if (refferalData == null)
                                    return;

                                GameLog.Money("server", $"player({accountData.RefferalId})", refferalData.Money,
                                    $"RefOwnerReward({count})");
                                var targetAccountData = target.GetAccountData();
                                if (targetAccountData != null)
                                {
                                    var successText = LangFunc.GetText(LangType.Ru, DataName.RefBonusGet,
                                        sessionData.Name);
                                    if (refferalData.VipId > 0)
                                    {
                                        successText +=
                                            $" {Group.GroupNames[refferalData.VipId]} ({refferalData.VipDays} Дней);";
                                        Chars.Repository.UpdateVipStatus(target, refferalData.VipId,
                                            refferalData.VipDays, false, true);
                                    }

                                    if (refferalData.FreeCoin)
                                    {
                                        successText += " Бесплатная прокрутка;";
                                        Chars.Repository.AddNewItemWarehouse(target, ItemId.Case0);
                                        Trigger.ClientEvent(target, "client.roullete.updateCase", 1);
                                    }

                                    if (refferalData.Money > 0)
                                    {
                                        successText += $" Игровая валюта (+{Wallet.Format(refferalData.Money)}$);";
                                        Wallet.Change(target, +refferalData.Money);
                                    }

                                    if (refferalData.RB > 0)
                                    {
                                        successText += $" RedBucks (+{Wallet.Format(refferalData.RB)}RB);";
                                        UpdateData.RedBucks(target, refferalData.RB, "Бонус за рефферала.");
                                    }

                                    //Chars.Repository.PlayerStats(RefferalClient);
                                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, successText,
                                        5000);
                                ...
                                {
                                    var login = GetLoginFromUUID(accountData.RefferalId);
                                    ...
                                    if (refferalData.VipId > 0)
                                    {
                                        if (accountData.VipLvl > 0)
                                        {
                                            var unixTime = TimeSpan.FromTicks(vipDate.Ticks - DateTime.Now.Ticks)
                                                .TotalSeconds;
                                            if (vipLvl < refferalData.VipId)
                                            {
                                                var addTime = unixTime / (refferalData.VipId + 1 - vipLvl);
                                                vipDate = DateTime.Now.AddSeconds(
                                                    refferalData.VipDays * 86400 + addTime);
                                                vipLvl = refferalData.VipId;
                                            }
                                            else if (vipLvl > refferalData.VipId)
                                            {
                                                var addTime = unixTime / (vipLvl - (refferalData.VipId - 1));
                                                vipDate = DateTime.Now.AddSeconds(
                                                    refferalData.VipDays * 86400 + addTime);
                                                vipLvl = refferalData.VipId;
                                            }
                                            ...
                                            {
                                                vipDate = vipDate.AddDays(refferalData.VipDays);
                                            }
                                        }
                                        else
                                        {
                                        ...
                                        GameLog.Money("server", $"player({refferalId})", 1,
                                            $"RefVIP({vipLvl}lvl, стало до: {vipDate.ToString("s")})");
                                    }
                ...
                if (accountData.PromoCodes[0] == "noref" && accountData.RefferalId == 0 && !accountData.PresentGet &&
                    !accountData.RefPresentGet)
                    Trigger.SendChatMessage(player,
                        "~r~Вы не указывали промокод или реф.код при регистрации, но всё еще можете активировать любой из промокодов через телефон (M -> Подарок).");
            }
            catch (Exception e)
            {
                Log.Write($"CheckMyPromoCode Exception: {e}");
            }
        }
        ...
        public static void payDayTrigger(bool withlottery)
        {
            try
            {
                ...
                if (withlottery)
                    try
                    {
                        ...
                        {
                            // Конец этой игры
                            var winnumb = rnd.Next(1, Lottery.LotteryBought.Count() + 1);
                            var winuuid = Lottery.LotteryBought[(uint)winnumb];
                            if (PlayerNames.ContainsKey(winuuid)) // Если персонаж победителя еще существует
                            {
                                var target = GetPlayerByUUID(winuuid);
                                if (target != null)
                                {
                                    Wallet.Change(target, (int)Lottery.Price);
                                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.LotteryWinner), 5000);
                                }
                                else
                                {
                                    Character.Save.Repository.AddMoney(winuuid, (int)Lottery.Price);
                                }

                                GameLog.Money("server", $"player({winuuid})", Lottery.Price,
                                    $"LotteryWin({Lottery.ID})");

                                NAPI.Chat.SendChatMessageToAll(
                                    $"~g~[ЛОТЕРЕЯ] {PlayerNames[winuuid]} сорвал джекпот в размере {Wallet.Format(Lottery.Price)}$ с шансом {(Lottery.LotteryBought.Where(p => p.Value == winuuid).Count() * 100.0 / Lottery.LotteryBought.Count()).ToString("0.##")}%.");
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.LotteryTicketSold,
                                    Lottery.LotteryBought.Count(), winnumb));

                                // Старт следующей игры
                                Lottery.ID += 1;
                                Lottery.Price = 0;
                                Lottery.LotteryBought = new Dictionary<uint, int>();
                                Trigger.SetTask(async () =>
                                {
                                    try
                                    {
                            ...
                            {
                                NAPI.Chat.SendChatMessageToAll(
                                    "~g~[ЛОТЕРЕЯ] Из-за технической ошибки, розыгрыш будет проведён через час.");
                            }
                        }
                    }
                    catch (Exception e)
                    {
                        Log.Write($"payDayTrigger #1 Exception: {e}");
                    }
                ...
                foreach (var familyZone in Organizations.FamilyZones.Repository.FamilyZones.Values.ToList())
                {
                    ...
                    familyZonesSalary[familyZone.OrganizationId] +=
                        Organizations.FamilyZones.Repository.FamilyZoneMoney;
                }
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    try
                    {
                        ...
                        if (foreachCharacterData.HotelID != -1)
                            if (foreachCharacterData.HotelLeft-- <= 0)
                            {
                                ...
                                Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.HotelLeftNoMoney), 15000);
                            }
                        ...
                        if (foreachCharacterData.LastHourMin < 15)
                        {
                            ...
                            Notify.Send(foreachPlayer, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.MustPlay15MinForPd), 3000);
                            continue;
                        }

                        if (foreachAccountData.VipLvl == 3)
                        {
                            UpdateData.RedBucks(foreachPlayer, 2, string.Empty);
                            Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.Platinum2RB), 3000);
                        }

                        if (foreachAccountData.VipLvl == 4)
                        {
                            UpdateData.RedBucks(foreachPlayer, 5, string.Empty);
                            Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.Diamond5RB), 3000);
                        }
                        ...
                        {
                            payment += Convert.ToInt32((100 - foreachOrganizationData.Salary) / 100f *
                                                       familyZonesSalary[foreachOrganizationData.Id]);
                                ...
                                case FractionsType.Bikers:
                                    ...
                                    if (foreachCharacterData.LVL < 30)
                                        payment += Convert.ToInt32(
                                            PricesSettings.PosobieNew * ServerSettings.MoneyMultiplier +
                                    ...
                                    //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы получили пособие по безработице в виде {MoneySystem.Wallet.Format(payment)}$", 3000);
                                        ...
                                        payment += Convert.ToInt32(
                                            PricesSettings.PosobieOld * ServerSettings.MoneyMultiplier +
                                            Donate.AddPayment(foreachAccountData.VipLvl) *
                                            ServerSettings.MoneyMultiplier);
                                    ...
                                    Wallet.Change(foreachPlayer, payment);
                                    break;
                        ...
                        switch (type)
                        {
                            case 0:
                                ...
                                break;
                            case 1:
                                GameLog.Money("server", $"player({foreachCharacterData.UUID})", payment,
                                    "payday(exp,noallowance)");
                                break;
                            case 2:
                                GameLog.Money("server", $"player({foreachCharacterData.UUID})", payment,
                                    "payday(exp,allowance)");
                                break;
                            case 3:
                                GameLog.Money("server", $"player({foreachCharacterData.UUID})", payment,
                                    "payday(exp,payday)");
                        ...
                        UpdateData.Exp(foreachPlayer,
                            1 * Group.GroupEXP[foreachAccountData.VipLvl] * ServerSettings.ExpMultiplier, payment);
                    ...
                    {
                        Log.Write($"payDayTrigger Foreach #1 Exception: {e}");
                    }

                //

                foreach (var familyZoneSalary in familyZonesSalary)
                {
                    ...
                    var amount = Convert.ToInt32(foreachOrganizationData.Salary / 100f * familyZoneSalary.Value *
                                                 familyZonesPlayers[foreachOrganizationData.Id]);
                    foreachOrganizationData.Money += amount;

                    var newMulti = foreachOrganizationData.MoneyMultiplier();
                    if (lastMulti != newMulti)
                        Organizations.Manager.UpdateForMembers(foreachOrganizationData.Id,
                            OrganizationOfficeTypeUpdate.Money, newMulti);
                ...
                foreach (var biz in BusinessManager.BizList.Values)
                    try
                    {
                        ...
                        if (biz.Mafia != -1)
                        {
                            var mafiaFractionData = Manager.GetFractionData(biz.Mafia);
                            if (mafiaFractionData != null)
                                mafiaFractionData.Money += MafiaForBiz;
                        }

                        var tax = Convert.ToInt32(biz.SellPrice / 100f * biz.Tax);
                        biz.Zatratq += tax;
                        ...
                        if (PlayerNames.Values.Contains(owner) && PlayerUUIDs.ContainsKey(owner))
                        {
                            ...
                            if (characterData != null)
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.BizSlet), 3000);
                            ...
                            {
                                var targetUuid = PlayerUUIDs[owner];
                                    ...
                                    try
                                    {
                                        ...
                                        if (character != null)
                                        {
                                            ...
                                            await db.Characters
                                                .Where(c => c.Uuid == character.Uuid)
                                                .Set(c => c.Money,
                                                    character.Money + Convert.ToInt32(biz.SellPrice * 0.8))
                            ...
                            GameLog.Money("server", $"player({PlayerUUIDs[biz.Owner]})",
                                Convert.ToInt32(biz.SellPrice * 0.8), $"bizTax({biz.ID})");
                        }
                    ...
                    {
                        Log.Write($"payDayTrigger Foreach #2 Exception: {e}");
                    }
                ...
                foreach (var house in HouseManager.Houses)
                    try
                    {
                        ...
                        var tax = Convert.ToInt32(house.Price / 100f * HouseManager.HouseTax);
                        if (!Bank.Accounts.TryGetValue(house.BankID, out var houseBalance))
                            continue;
                        ...
                        if (accountData != null && characterData != null)
                        {
                            ...
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NalogPokaPoka), 3000);
                        ...
                        {
                            if (house.Type != 7)
                                GameLog.Money("server", $"player({PlayerUUIDs[owner]})", price,
                                    $"houseTax({house.ID})");
                            else
                    ...
                    {
                        Log.Write($"payDayTrigger Foreach #3 Exception: {e}");
                    }
                ...
                if (DateTime.Now.Hour == 0)
                {
                    ...
                    fractionData = Manager.GetFractionData((int)Fractions.Models.Fractions.SHERIFF);
                    if (fractionData != null)
                ...
                foreach (var point in GangsCapture.GangPoints.Values)
                {
            ...
            {
                Log.Write($"payDayTrigger Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (!AutoRestart)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.Center,
                        LangFunc.GetText(LangType.Ru, DataName.AutoRRoff), 1500);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.Center,
                        LangFunc.GetText(LangType.Ru, DataName.AutoRRon), 1500);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_AutoRestart Exception: {e}");
            }
        }
        ...
        public void event_stats(ExtPlayer player, string name)
        {
            ...
            {
                Log.Write($"event_stats Exception: {e}");
            }
        }
        ...
        public static void GetMemory(ExtPlayer player, string name)
        {
            ...
            {
                Log.Write($"GetMemory Exception: {e}");
            }
        }
        ...
        public static void GetStats()
        {
            ...
            {
                if (!IsDebugEvents) return;
                using (var save = new StreamWriter("StatsClientToServer.txt", true, Encoding.UTF8))
                {
                    save.Write($"{DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss")}:\n\n");
                    foreach (var stats in StatsClientToServer) save.Write($"{stats.Key}: {stats.Value} Count\n");
                    save.Write("-----------------------------------------------");
                    save.Close();
                    StatsClientToServer.Clear();
                }

                using (var save = new StreamWriter("StatsServerToClient.txt", true, Encoding.UTF8))
                {
                    save.Write($"{DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss")}:\n\n");
                    foreach (var stats in StatsServerToClient) save.Write($"{stats.Key}: {stats.Value} Count\n");
                    save.Write("-----------------------------------------------");
                    save.Close();
        ...
        {
            public BonusCodesData(ulong used, ulong limit, string msg, byte exp, uint money, byte vip, ushort days,
                List<InventoryItemData> items, List<InventoryItemData> items2)
            {
                UsedTimes = used;
                UsedLimit = limit;
                RewardMessage = msg;
        ...
        {
            public PromoCodesData(uint uuid, ulong used, ulong received, ulong limit, string msg, uint money, byte vip,
                ushort days, List<InventoryItemData> items, double donate, string donatel, ulong donate2)
            {
                CreatorUUID = uuid;
                UsedTimes = used;
        ...
        public static int[]
            LicPrices = new int[6]
            {
                200, 500, 1500, 10000, 20000, 20000
        ...
        {
            Players.Disconnect.Repository.OnPlayerDisconnect(player, type, reason);
        }
        ...
        public static void RagempCheatDetected(ExtPlayer player, int cheatCode)
        {
            try
            {
                ...
                Trigger.SendPunishment(LangFunc.GetText(LangType.Ru, DataName.AutoACBan, player.Name, cheatCode),
                    player);
                var until = DateTime.Now.AddDays(999);
                Ban.Online(player, until, true, $"Cheats({cheatCode})", "AntiCheat");
                Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                    LangFunc.GetText(LangType.Ru, DataName.YouBannedPerm), 30000);
                Notify.Send(player, NotifyType.Warning, NotifyPosition.Center,
                    LangFunc.GetText(LangType.Ru, DataName.ReasonCheats, cheatCode), 30000);
                GameLog.Ban(0, characterData.UUID, player.GetLogin(), until,
                    LangFunc.GetText(LangType.Ru, DataName.ReasonCheats, cheatCode), true);
                player.Kick();
            }
            catch (Exception e)
            {
                Log.Write($"RagempCheatDetected Exception: {e}");
            }
        }
        ...
        public void ClientEvent_Kick(ExtPlayer player)
        {
            ...
            {
                Log.Write($"ClientEvent_Kick Exception: {e}");
                if (player != null) player.Kick();
            }
        }
        ...
        public void ClientEvent_KeyInsert(ExtPlayer player)
        {
            ...
            try
            {
                ...
                Trigger.SendToAdmins(5,
                    LangFunc.GetText(LangType.Ru, DataName.InsertTrigger, player.Name, player.Value,
                        characterData.LVL));
            }
            catch (Exception e)
            {
                Log.Write($"ClientEvent_KeyInsert Exception: {e}");
            }
        }
        ...
        public void ClientEvent_TryCatch(ExtPlayer player, string path, string callback, string msg)
        {
            ...
            {
                Log.Write($"ClientEvent_TryCatch Exception: {e}");
            }
        }
        ...
        public void ClientEvent_SyncWP(ExtPlayer player, float X, float Y)
        {
            try
            {
                ...
                var target = (ExtPlayer)driver; // Test
                if (!target.IsCharacterData()) return;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.VoditeluPeredalDannie), 3000);
                BattlePass.Repository.UpdateReward(player, 126);
                Trigger.ClientEvent(target, "syncWP", X, Y, 0);
            }
            catch (Exception e)
            {
                Log.Write($"ClientEvent_SyncWP Exception: {e}");
            }
        }
        ...
        public void UpdateAFKStatus(ExtPlayer player, bool status = false)
        {
            ...
            {
                Log.Write($"UpdateAFKStatus Exception: {e}");
            }
        }
        ...
        public void ClientEvent_SyncWP(ExtPlayer player)
        {
            try
            {
                ...
                if (!player.IsInVehicle ||
                    (Manager.FractionTypes[fractionId] != FractionsType.Gov && characterData.AdminLVL == 0)) return;
                var veh = (ExtVehicle)player.Vehicle;
                if (veh == null || !veh.Exists) return;
                if (NAPI.Vehicle.GetVehicleDriver(veh) != player) return;
                if (veh.GetSharedData<bool>("SIRENSOUND"))
                {
                    Trigger.ClientEventInDimension(UpdateData.GetPlayerDimension(player), "VehStream_SetSirenSound",
                        veh, false);
                    veh.SetSharedData("SIRENSOUND", false);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.WeeWeeOn), 1000);
                }
                else
                {
                    Trigger.ClientEventInDimension(UpdateData.GetPlayerDimension(player), "VehStream_SetSirenSound",
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.WeeWeeOff), 1000);
                }
            }
            catch (Exception e)
            {
        ...
        public void VehStream_updateSirenStatus(ExtPlayer player, ExtVehicle veh, bool status)
        {
            ...
            {
                Log.Write($"VehStream_updateSirenStatus Exception: {e}");
            }
        }
        ...
        public static void ClientEvent_Spawn(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (characterData.ArrestTime > 0)
                {
                    ...
                    {
                        sessionData.TimersData.ArrestTimer =
                            Timers.Start(1000, () => FractionCommands.arrestTimer(player));
                ...
                else if (characterData.DemorganTime > 0)
                {
                    if (sessionData.TimersData.ArrestTimer == null)
                    {
                        ...
                        NAPI.Entity.SetEntityPosition(player,
                            Admin.DemorganPositions[rnd.Next(55)] + new Vector3(0, 0, 1.5));
                ...
                else
                {
                    switch (id)
                    {
                        ...
                        case 2:
                            ...
                            else if (characterData.HotelID != -1)
                            {
                                player.Position = Hotel.HotelEnters[characterData.HotelID] + new Vector3(0, 0, 1.12);
                            }
                            else
                            {
                                player.Position = characterData.SpawnPos;
            ...
            {
                Log.Write($"ClientEvent_Spawn Exception: {e}");
            }
        }
        ...
        public static bool IHaveDemorgan(ExtPlayer player, bool withsmg = false)
        {
            try
            {
                ...
                if (characterData.DemorganTime >= 1)
                {
                    if (withsmg)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.IHaveDemorgan), 4000);
                    return true;
                }
            ...
            {
                Log.Write($"IHaveDemorgan Exception: {e}");
                return true;
            }
        }
        ...
        public void StartPushVehicle(ExtPlayer player, ExtVehicle vehicle)
        {
            try
            {
                ...
                if (vehicle == null || player.Position.DistanceTo(vehicle.Position) > 3 ||
                    characterData.ArrestTime >= 1) return;
            ...
            {
                Log.Write($"StartPushVehicle Exception: {e}");
            }
        }
        ...
        public void StopPushVehicle(ExtPlayer player)
        {
            ...
            {
                Log.Write($"StopPushVehicle Exception: {e}");
            }
        }
        ...
        public void ClientEvent_setStock(ExtPlayer player, string stock)
        {
            ...
            {
                Log.Write($"ClientEvent_setStock Exception: {e}");
            }
        }
        ...
        public void ClientEvent_inputCallback(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                if (IHaveDemorgan(player, true))
                {
                    if (sessionData.SellItemData.Seller != null)
                    {
                        ...
                        if (targetSessionData != null && target != player)
                            targetSessionData.SellItemData = new SellItemData();
                    }

                    if (sessionData.TicketsData.Target != null)
                    {
                        var target = sessionData.TicketsData.Target;
                        sessionData.TicketsData = new TicketsData();
                        var targetSessionData = target.GetSessionData();
                        if (targetSessionData != null && target != player)
                            targetSessionData.TicketsData = new TicketsData();
                    }
                ...
                var callback = arguments[0].ToString();
                ...
                switch (callback)
                {
                    ...
                    case "fuelcontrol_news":
                        ...
                        switch (callback)
                        {
                            case "fuelcontrol_city":
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.FuelcontrolCity, limit), 3000);
                                break;
                            case "fuelcontrol_police":
                                fractionData = Manager.GetFractionData((int)Fractions.Models.Fractions.POLICE);
                                if (fractionData == null)
                                ...
                                if (fractionData.FuelLeft > limit) fractionData.FuelLeft = limit;
                                //Stocks.fracStocks[(int)Fractions.Models.Fractions.SHERIFF].FuelLeft = limit;
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.FuelcontrolPolice, limit), 3000);
                                break;
                            case "fuelcontrol_ems":
                                fractionData = Manager.GetFractionData((int)Fractions.Models.Fractions.EMS);
                                if (fractionData == null)
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.FuelcontrolEms, limit), 3000);
                                break;
                            case "fuelcontrol_fib":
                                fractionData = Manager.GetFractionData((int)Fractions.Models.Fractions.FIB);
                                if (fractionData == null)
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.FuelcontrolFib, limit), 3000);
                                break;
                            case "fuelcontrol_army":
                                fractionData = Manager.GetFractionData((int)Fractions.Models.Fractions.ARMY);
                                if (fractionData == null)
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.FuelcontrolArmy, limit), 3000);
                                break;
                            case "fuelcontrol_news":
                                fractionData = Manager.GetFractionData((int)Fractions.Models.Fractions.LSNEWS);
                                if (fractionData == null)
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.FuelcontrolNews, limit), 3000);
                                break;
                        }
                    ...
                    case "player_offerhousesell":
                        ...
                        HouseManager.OfferHouseSell(player, target, limit);
                        return;
                    case "buy_drugs":
                        limit = 0;
                        if (text.Contains('.')) text = text.Replace(".", null);
                    ...
                    case "mayor_take":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoCityhallGov, Cityhall.canGetMoney), 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoMoneyGov), 3000);
                            return;
                        }

                        Bank.Change(characterData.Bank, limit);
                    ...
                    case "mayor_put":
                        ...
                        if (!Bank.Change(characterData.Bank, -limit))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    ...
                    case "take_stock":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.ServerCantAccept), 3000);
                            return;
                        }
                    ...
                    case "streetrace":
                        ...
                        {
                            Trigger.ClientEvent(player, "openInput", "Уличная гонка",
                                LangFunc.GetText(LangType.Ru, DataName.InputBet0100k), 6, "streetrace");
                            return;
                        }
                    ...
                    case "sellcar":
                        ...
                        var number = sessionData.SellItemData.Number;
                        ...
                        if (house != null)
                        {
                            ...
                            if (garage == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.PlayerNoGarage), 3000);
                                return;
                            }

                            var vehiclesCount = VehicleManager.GetVehiclesCarCountToPlayer(target.Name);
                            if (vehiclesCount >= GarageManager.GarageTypes[garage.Type].MaxCars)
                            {
                                sessionData.SellItemData = new SellItemData();
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.PlayerMaxVehGarage), 3000);
                                return;
                            }
                        }
                        else
                        {
                            ...
                            if (vehiclesCount >= GarageManager.MaxGarageCars)
                            {
                                sessionData.SellItemData = new SellItemData();
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.PlayerMaxVeh), 3000);
                                return;
                            }
                        }
                        ...
                        if (vehicleData == null)
                        {
                            sessionData.SellItemData = new SellItemData();
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VehNotExists), 3000);
                            return;
                        }
                        ...
                        if (vehicleData.Holder != player.Name)
                        {
                            sessionData.SellItemData = new SellItemData();
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouNeVladeetVehTrade), 3000);
                            return;
                        }

                        var targetSessionData = target.GetSessionData();
                        if ((targetSessionData.SellItemData.Seller != null ||
                             targetSessionData.SellItemData.Buyer != null) && Chars.Repository.TradeGet(target))
                        {
                            sessionData.SellItemData = new SellItemData();
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        ...
                        var vName = vehicleData.Model;
                        EventSys.SendCoolMsg(player, "Предложение", "Покупка",
                            $"{LangFunc.GetText(LangType.Ru, DataName.BuycarOffer, target.Name, vName, number, Wallet.Format(limit))}",
                            "", 5000);
                        ...
                        Trigger.ClientEvent(target, "openDialog", "BUY_CAR",
                            LangFunc.GetText(LangType.Ru, DataName.BuycarOffered, player.Name, vName, number,
                                Wallet.Format(limit)));
                        return;
                    case "weaptransfer":
                        limit = 0;
                    ...
                    case "extend_hotel_rent":
                        ...
                        Hotel.ExtendHotelRent(player, limit);
                        return;
                    case "sell_festive":
                        if (text.Contains('.')) text = text.Replace(".", null);
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.AmountIncorrect), 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NumberNotDelitsya), 3000);
                            return;
                        }

                        var count = Chars.Repository.getCountItem($"char_{characterData.UUID}", Festive.EventCoins,
                            false);
                        if (limit > count)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        ...
                        Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", Festive.EventCoins,
                            limit);
                        var prefix = "";
                        if (limit == 2 || limit == 3 || limit == 4) prefix = "а";
                        else if (limit > 4) prefix = "ов";
                        var rb = Convert.ToInt32(limit / 3);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucTradeCoins, limit, prefix, rb), 3000);
                        UpdateData.RedBucks(player, rb,
                            LangFunc.GetText(LangType.Ru, DataName.ExchangeCoins, Festive.EventCoinsName));
                        break;
                    case "rentname":
                        if (sessionData.TentIndex == -1) return;

                        var index = sessionData.TentIndex;
                        ...
                        Trigger.SendToAdmins(1,
                            LangFunc.GetText(LangType.Ru, DataName.TentPostavil, player.Name, player.Value, text));
                        break;
                    case "sell_tent":
                        ...
                        var Item = Chars.Repository.GetItemData(player, inventoryTentData.ArrayName,
                            inventoryTentData.Index);
                        ...
                        if (tentData.isBlack && Item.ItemId != ItemId.BodyArmor && Item.ItemId != ItemId.Drugs &&
                            Item.ItemId != ItemId.Material && ItemInfo.functionType != newItemType.Weapons &&
                            ItemInfo.functionType != newItemType.MeleeWeapons &&
                            ItemInfo.functionType != newItemType.Ammo)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.CantSellItem), 3000);
                            return;
                        }

                        if (inventoryTentData.Value == Item.Count)
                        {
                            if (!Chars.Repository.InventoryMaxSlots.ContainsKey(Location) ||
                                (slotId = Chars.Repository.AddItem(player, locationName, Location, Item,
                                    Chars.Repository.GetMaxSlots(player, Location))) == -1)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            ...
                            Chars.Repository.SetItemData(player, inventoryTentData.ArrayName, inventoryTentData.Index,
                                new InventoryItemData(), true);
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.SellItemTent,
                                    Chars.Repository.ItemsInfo[Item.ItemId].Name, limit), 10000);
                            return;
                        }

                        if ((slotId = Chars.Repository.AddNewItem(player, locationName, Location, Item.ItemId,
                                inventoryTentData.Value, Item.Data,
                                MaxSlots: Chars.Repository.GetMaxSlots(player, Location), price: limit)) == -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoSpaceInventory), 3000);
                            return;
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SellItemTent,
                                Chars.Repository.ItemsInfo[Item.ItemId].Name, limit), 10000);
                        Chars.Repository.RemoveIndex(player, inventoryTentData.ArrayName, inventoryTentData.Index,
                            inventoryTentData.Value);
                        break;
                    case "make_ped_name":
                        if (string.IsNullOrEmpty(text))
                        {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.Name36ogran), 3000);
                            return;
                        }
                    ...
                    case "player_ticketsum":
                        ...
                        Trigger.ClientEvent(player, "openInput", "Выписать штраф (причина)", "Причина", 50,
                            "player_ticketreason");
                        break;
                    case "player_ticketreason":
                        var tddata = sessionData.TicketsData;
                        if (string.IsNullOrEmpty(text))
                        {
                            tddata.Target = null;
                            tddata.Price = 0;
            ...
            {
                Log.Write($"ClientEvent_inputCallback Exception: {e}");
            }
        }
        ...
        public static async Task SaveDoorsControl(ServerBD db)
        {
            ...
            {
                Log.Write($"SaveDoorsControl Exception: {e}");
            }
        }
        ...
        public void DoorControlState(ExtPlayer player, string door)
        {
            try
            {
                ...
                {
                    case "pd":
                        if (fractionId != (int)Fractions.Models.Fractions.POLICE &&
                            fractionId != (int)Fractions.Models.Fractions.FIB) return;
                        if (!player.IsFractionAccess(RankToAccess.DoorControl)) return;
                        break;
                    case "fib":
                        ...
                        break;
                    case "gov":
                        if (fractionId != (int)Fractions.Models.Fractions.CITY &&
                            fractionId != (int)Fractions.Models.Fractions.FIB) return;
                        if (!player.IsFractionAccess(RankToAccess.DoorControl)) return;
                        break;
                    case "news":
                    ...
                    case "lcn":
                        ...
                        break;
                    case "ems":
                        if (fractionId != (int)Fractions.Models.Fractions.EMS &&
                            fractionId != (int)Fractions.Models.Fractions.FIB) return;
                        break;
                    case "jud":
                        if (fractionId != (int)Fractions.Models.Fractions.CITY &&
                        ...
                        break;
                    case "sheriff":
                        if (fractionId != (int)Fractions.Models.Fractions.SHERIFF &&
                            fractionId != (int)Fractions.Models.Fractions.FIB) return;
                        if (!player.IsFractionAccess(RankToAccess.DoorControl)) return;
                        break;
                    case "army":
                        if (fractionId != (int)Fractions.Models.Fractions.ARMY &&
                            fractionId != (int)Fractions.Models.Fractions.FIB) return;
                        if (!player.IsFractionAccess(RankToAccess.DoorControl)) return;
                        break;
                    case "fam":
                ...
                if (DoorsControl[door])
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DoorsClosed), 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DoorsOpened), 3000);

                Trigger.ClientEventForAll("doorControl", door, DoorsControl[door]);
            }
            catch (Exception e)
            {
                Log.Write($"DoorControlState Exception: {e}");
            }
        }
        ...
        public void ClientEvent_engineCarPressed(ExtPlayer player)
        {
            ...
            {
                Log.Write($"ClientEvent_engineCarPressed Exception: {e}");
            }
        }
        ...
        public void ClientEvent_lockCarPressed(ExtPlayer player)
        {
            try
            {
                ...
                var id = CustomColShape.GetDataToEnum(player, ColShapeEnums.EnterHouse);
                ...
                if (id != (int)ColShapeData.Error)
                {
                    ...
                    if (house != null &&
                        (sessionData.Name == house.Owner || house.Roommates.ContainsKey(sessionData.Name)))
                    {
                        house.SetLock(!house.Locked);
                        if (house.Locked)
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.HouseClosed), 3000);
                            BattlePass.Repository.UpdateReward(player, 125);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.HouseOpened), 3000);
            ...
            {
                Log.Write($"ClientEvent_lockCarPressed Exception: {e}");
            }
        }
        ...
        public void RemoteEvent_acceptPressed(ExtPlayer player)
        {
            try
            {
                ...
                if (DateTime.Now >= sessionData.RequestData.Time)
                {
                    ...
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.OfferTimeout), 1500);
            ...
            {
                Log.Write($"RemoteEvent_acceptPressed Exception: {e}");
            }
        }
        ...
        public void RemoteEvent_cancelPressed(ExtPlayer player)
        {
            try
            {
                ...
                switch (sessionData.RequestData.Request)
                {
                    case "acceptPass":
                        if (characterData.Gender)
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.HePassNo));
                        else Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.ShePassNo));
                        break;
                    case "acceptLics":
                        if (characterData.Gender)
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.HeLicNo));
                        else Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.SheLicNo));
                        break;
                    case "HANDSUP":
                        if (characterData.Gender)
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.NoHandsUp));
                        else Commands.RPChat("sme", player, LangFunc.GetText(LangType.Ru, DataName.SheNoHandsUp));
                        break;
                }

                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.CancelOffer), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"RemoteEvent_cancelPressed Exception: {e}");
            }
        }
        ...
        public void RemoteEvent_DialogCallback(ExtPlayer player, string callback, bool yes)
        {
            try
            {
                ...
                if (yes)
                {
                    ...
                    switch (callback)
                    {
                        case "RepairMyVeh":
                            ...
                            {
                                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.NoMoneyMech), 3000);
                                return;
                            }
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.RepairPayed), 3000);
                        ...
                        case "FORGET_FRIENDS":
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.ForgotFriends), 3000);
                            return;
                        case "PAY_MEDKIT":
                        ...
                        case "DICE_PLAY":
                        {
                            if (sessionData.DiceData.Target != null)
                            {
                                var diceplayer = sessionData.DiceData.Target;
                                var diceplayerCharacterData = diceplayer.GetCharacterData();
                                if (diceplayerCharacterData == null)
                                {
                                    sessionData.DiceData = new DiceData();
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.OpponentNotFound), 3000);
                                }
                                else
                                {
                                    var diceplayerSessionData = diceplayer.GetSessionData();
                                    var dicemon = sessionData.DiceData.Money;
                                    if (characterData.Money < dicemon)
                                    {
                                        ...
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.NoMoney), 3000);
                                        Notify.Send(diceplayer, NotifyType.Error, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.PlayerNotEnoughMoney), 3000);
                                        return;
                                    }
                                    ...
                                    Notify.Send(diceplayer, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.PlayerAcceptOffer, player.Value), 3000);
                        ...
                        case "RENTCARI_DATETIME":
                            ...
                            {
                                var rentCar = sessionData.RentData;
                                if (rentCar.Date >= DateTime.Now.AddMinutes(60 * 8)) // 8 часа максимально
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.AlreadyRentMaxTime), 8000);
                                    return;
                                }

                                var price = Rentcar.GetRentCarCash(accountData.VipLvl, characterData.LVL,
                                    rentCar.Price);
                                if (!Bank.Change(characterData.Bank, -price, false))
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.NoBankMoney), 5000);
                                    return;
                                }
                                ...
                                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Rent,
                                    LangFunc.GetText(LangType.Ru, DataName.SuccessfullRentExpand, rentCar.Date),
                                    DateTime.Now);
                            }
                        ...
                        case "BUY_CAR":
                        {
                            ...
                            GameLog.Money($"player({characterData.UUID})", $"player({sellerCharacterData.UUID})", price,
                                $"buyCar({vehicleData.Model}, {number})");
                            var sellerhouse = HouseManager.GetHouse(seller);
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                $"Вы купили {vehicleData.Model} ({number}) за {Wallet.Format(price)}$ у {seller.Name}",
                                3000);
                            Notify.Send(seller, NotifyType.Success, NotifyPosition.BottomCenter,
                                $"{player.Name} купил у Вас {vehicleData.Model} ({number}) за {Wallet.Format(price)}$",
                                3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                        ...
                        case "REMOVE_SNUM":
                        {
                            var num = sessionData.SMSNum;
                            if (!characterData.Contacts.ContainsKey(num))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.NoContacts, num), 4000);
                                return;
                            }

                            Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.DelContact, num), 4000);
                        ...
                        case "INVITED_FRAC":
                        {
                            try
                            {
                                if (sessionData.InviteData.Fraction != -1)
                                {
                                    ...
                                    EventSys.SendCoolMsg(player, "Фракция", "Добро пожаловать!",
                                        LangFunc.GetText(LangType.Ru, DataName.JoinFrac,
                                            Manager.FractionNames[fractionId]), "", 8000);

                                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.AcceptJoinFrac, player.Name), 3000);
                                    //EventSys.SendCoolMsg(target,"Фракция", "Приглашение",  LangFunc.GetText(LangType.Ru, DataName.AcceptJoinFrac, player.Name), "", 8000); 
                                    GameLog.FracLog(fractionId, target.GetUUID(), characterData.UUID, target.Name,
                                        player.Name, "invite");
                                    Fractions.Table.Logs.Repository.AddLogs(target, FractionLogsType.Invite,
                                        LangFunc.GetText(LangType.Ru, DataName.AcceptedFrac, player.Name,
                                            characterData.UUID));
                                    Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.Invite,
                                        LangFunc.GetText(LangType.Ru, DataName.InvitedFrac, target.Name,
                                            target.GetUUID()));
                                }
                                else
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.InvitedOffline), 3000);
                                }
                            }
                            catch
                            {
                                Log.Write("RemoteEvent_DialogCallback23 ERROR");
                            }
                        ...
                        case "INVITED_ORG":
                        {
                            try
                            {
                                if (sessionData.InviteData.Organization != -1)
                                {
                                    ...
                                    EventSys.SendCoolMsg(player, "Организация", "Добро пожаловать!",
                                        LangFunc.GetText(LangType.Ru, DataName.JoinFrac, organizationData.Name), "",
                                        8000);
                                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.AcceptedOrg, player.Name), 3000);
                                    //EventSys.SendCoolMsg(target,"Организация", "Приглашение",  LangFunc.GetText(LangType.Ru, DataName.AcceptedOrg, player.Name), "", 8000); 
                                    Organizations.Table.Logs.Repository.AddLogs(target, OrganizationLogsType.Invite,
                                        LangFunc.GetText(LangType.Ru, DataName.AcceptedFrac, player.Name,
                                            characterData.UUID));
                                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.Invite,
                                        LangFunc.GetText(LangType.Ru, DataName.InvitedFrac, target.Name,
                        ...
                        case "REPAIR_CAR":
                            AutoMechanic.mechanicPay(player);
                            return;
                        case "FUEL_CAR":
                            AutoMechanic.mechanicPayFuel(player);
                            return;
                        case "HOUSE_SELL":
                        ...
                        case "CAR_SELL_TOGOV":
                        {
                            ...
                            {
                                var number = sessionData.CarSellGov;
                                var vehicleData = VehicleManager.GetVehicleToNumber(number);
                                sessionData.CarSellGov = null;
                                if (vehicleData == null)
                                {
                                ...
                                if (BusinessManager.BusProductsData.ContainsKey(vehicleData.Model))
                                    switch (accountData.VipLvl)
                                    {
                                        ...
                                        case 1: // Bronze
                                        case 2: // Silver
                                            price = Convert.ToInt32(BusinessManager.BusProductsData[vehicleData.Model]
                                                .Price * 0.4);
                                            break;
                                        case 3: // Gold
                                            price = Convert.ToInt32(BusinessManager.BusProductsData[vehicleData.Model]
                                ...
                                Wallet.Change(player, price);
                                GameLog.Money("server", $"player({characterData.UUID})", price,
                                    $"carSell({vehicleData.Model}, {number})");
                                ...
                                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                                    LangFunc.GetText(LangType.Ru, DataName.YouSellCar, vehicleData.Model, number,
                                        Wallet.Format(price)), DateTime.Now);
                        ...
                        case "CONFIRM_BUY_BODYARMOUR":
                            ...
                            if (fractionId == (int)Fractions.Models.Fractions.None ||
                                Manager.GovIds.Contains(fractionId))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.CrimeBuyBronik), 3000);
                                return;
                            }

                            if (Chars.Repository.itemCount(player, "inventory", ItemId.BodyArmor) >=
                                Chars.Repository.maxItemCount)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            ...
                            var mItem = Chars.Repository.isItem(player, "inventory", ItemId.Material);
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.Bronik50Crafted), 3000);
                            return;
                        ...
                        case "buy_tent":
                            ...
                            Chars.Repository.ItemBuy(player, inventoryTentData.ArrayName, inventoryTentData.Index,
                                inventoryTentData.Value);
                            return;
                        case "sell_pet":
                            ...
                            switch (weddingData.type)
                            {
                                case 0:
                                    ...
                                    if (targetCharacterData == null)
                                        return;
                                    if (targetCharacterData.WeddingUUID != 0)
                                        return;
                                    ...
                                    if (weddingData.typeSurname == 0)
                                        Trigger.ClientEvent(weddingData.player, "openDialog", "Boda",
                                            LangFunc.GetText(LangType.Ru, DataName.YouWantWedding, player.Name,
                                                characterData.LastName));
                                    else
                                        Trigger.ClientEvent(weddingData.player, "openDialog", "Boda",
                                            LangFunc.GetText(LangType.Ru, DataName.YouWantWeddingTo, player.Name));
                                    break;
                                case 1:
                                    targetCharacterData = weddingData.player.GetCharacterData();
                                    if (targetCharacterData != null &&
                                        player.Position.DistanceTo(weddingData.player.Position) <= 5f)
                                        Wedding.OnDivorcio(player);
                                    break;
                                case 2:
                                    Wedding.OnDivorcio(player, true);
                                    break;
                            }
                        ...
                        case "Boda":
                            ...
                            if (bodaData == null)
                                return;
                            Wedding.OnBoda(bodaData.player, player, bodaData.typeSurname);
                            return;
                        case "DelObjects":
                            if (!CommandsAccess.CanUseCmd(player, AdminCommands.DelObjects)) return;

                            if (!Chars.Repository.ItemsData.ContainsKey(sessionData.DelObjects)) return;

                            Chars.Repository.RemoveAll(sessionData.DelObjects);
                ...
                else
                {
                    ...
                    switch (callback)
                    {
                        ...
                        case "PM_LIC":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.SucCancelBuyLic), 3000);
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelBuyLic), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            break;
                        case "BUSINESS_BUY":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelBuyBiz), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelBuyBiz), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "FUEL_CAR":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelZapravka), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelFuel), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "REPAIR_CAR":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelRepairVeh), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelRepairVeh), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "HOUSE_SELL":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelBuyHome), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelBuyHome), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "PAY_HEAL":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelHeal), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelHeal), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "PAY_MEDKIT":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelMedkit), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelMedkit), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "BUY_CAR":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelVeh), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelVeh), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "DICE_PLAY":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelDice), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.DiceData = new DiceData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelDice), 3000);
                            sellerSessionData.DiceData = new DiceData();
                            sessionData.DiceData = new DiceData();
                            return;
                        case "ROOM_INVITE":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelHomeGo), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelHomeGo), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "BUS_RENT":
                        ...
                        case "TAXI_PAY":
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouCancelPayBus), 3000);
                            if (sellerSessionData == null)
                            {
                                sessionData.SellItemData = new SellItemData();
                                return;
                            ...
                            Notify.Send(seller, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerCancelPayBus), 3000);
                            sellerSessionData.SellItemData = new SellItemData();
                            sessionData.SellItemData = new SellItemData();
                            return;
                        case "TICKET":
            ...
            {
                Log.Write($"RemoteEvent_DialogCallback Exception: {e}");
            }
        }
        ...
        public void ClientEvent_playerPressCuffBut(ExtPlayer player)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (characterData.AdminLVL == 0 && !player.IsFractionAccess(RankToAccess.Cuff, false) &&
                    !player.IsOrganizationAccess(RankToAccess.Cuff)) return;
                FractionCommands.playerPressCuffBut(player);
            }
            catch (Exception e)
            {
                Log.Write($"ClientEvent_playerPressCuffBut Exception: {e}");
            }
        }
        ...
        public void ClientEvent_cuffUpdate(ExtPlayer player)
        {
            ...
            {
                Log.Write($"ClientEvent_cuffUpdate Exception: {e}");
            }
        }
        ...
        public void CMD_BUILD(ExtPlayer player)
        {
            ...
            {
                Log.Write($"CMD_BUILD Exception: {e}");
            }
        }
        ...
        public static List<ExtPlayer> GetPlayersInRadiusOfPosition(Vector3 position, float radius,
            uint dimension = 39999999)
        {
            try
            {
            ...
            {
                Log.Write($"GetPlayersInRadiusOfPosition Exception: {e}");
        ...
        public static ExtPlayer GetNearestPlayer(ExtPlayer player, int radius)
        {
            try
            {
                ...
                foreach (var foreachPlayer in players)
                {
                    ...
                    if (player.Position.DistanceTo(foreachPlayer.Position) <
                        player.Position.DistanceTo(nearestPlayer.Position)) nearestPlayer = foreachPlayer;
                }
            ...
            {
                Log.Write($"GetNearestPlayer Exception: {e}");
                return null;
            }
        }
        ...
        public static ExtPlayer GetPlayerByID(int id)
        {
            ...
            {
                var target = PlayerIdToEntity[id];
        ...
        {
            if (PlayerUUIDToPlayerId.ContainsKey(UUID)) return GetPlayerByID(PlayerUUIDToPlayerId[UUID]);
            return null;
        }
        ...
        public static void PlayerEnterInterior(ExtPlayer player, Vector3 pos)
        {
            ...
            {
                Log.Write($"PlayerEnterInterior Exception: {e}");
            }
        }
        ...
        public static void OnAntiAnim(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OnAntiAnim Exception: {e}");
            }
        }
        ...
        public static void OffAntiAnim(ExtPlayer player)
        {
            try
            {
                ...
                var playerPhoneMeta = sessionData.VoiceData;
            ...
            {
                Log.Write($"OffAntiAnim Exception: {e}");
            }
        }
        ...
        public static void OpenMayorMenu(ExtPlayer player)
        {
            try
            {
                ...
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.MoneyKazna, fractionData.Money),
                    "info"));
                frameList.List.Add(
                    new ListData(LangFunc.GetText(LangType.Ru, DataName.KaznaSobrano, Cityhall.lastHourTax), "info2"));
            ...
            {
                Log.Write($"OpenMayorMenu Exception: {e}");
            }
        }
        ...
        private static void callback_mayormenu(ExtPlayer player, object listItem) /// Никитос Чини 
        {
            try
            {
                ...
                {
                    case "take":
                        Trigger.ClientEvent(player, "openInput", "Получить деньги из казны", "Количество", 6,
                            "mayor_take");
                        return;
                    case "put":
                        Trigger.ClientEvent(player, "openInput", "Положить деньги в казну", "Количество", 6,
                            "mayor_put");
                        return;
                    case "fuelcontrol":
            ...
            {
                Log.Write($"callback_mayormenu Exception: {e}");
            }
        }
        ...
        public static void OpenFuelcontrolMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (fractionData != null)
                    frameList.List.Add(new ListData(
                        $"Мэрия. Осталось сегодня: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_city"));
                ...
                if (fractionData != null)
                    frameList.List.Add(new ListData(
                        $"Полиция. Осталось сегодня: {fractionData.FuelLeft}/{fractionData.FuelLimit}$",
                        "info_police"));
                ...
                if (fractionData != null)
                    frameList.List.Add(new ListData(
                        $"EMS. Осталось сегодня: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_ems"));
                ...
                if (fractionData != null)
                    frameList.List.Add(new ListData(
                        $"FIB. Осталось сегодня: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_fib"));
                ...
                if (fractionData != null)
                    frameList.List.Add(new ListData(
                        $"Армия. Осталось сегодня: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_army"));
                ...
                if (fractionData != null)
                    frameList.List.Add(new ListData(
                        $"News. Осталось сегодня: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_news"));
            ...
            {
                Log.Write($"OpenFuelcontrolMenu Exception: {e}");
            }
        }
        ...
        private static void callback_fuelcontrol(ExtPlayer player, object listItem) /// Никитос Чини 
        {
            try
            {
                ...
                {
                    case "set_city":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SetLimit),
                            LangFunc.GetText(LangType.Ru, DataName.FuelLimitM), 5, "fuelcontrol_city");
                        return;
                    case "set_police":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SetLimit),
                            LangFunc.GetText(LangType.Ru, DataName.FuelLimitP), 5, "fuelcontrol_police");
                        return;
                    case "set_ems":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SetLimit),
                            LangFunc.GetText(LangType.Ru, DataName.FuelLimitE), 5, "fuelcontrol_ems");
                        return;
                    case "set_fib":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SetLimit),
                            LangFunc.GetText(LangType.Ru, DataName.FuelLimitF), 5, "fuelcontrol_fib");
                        return;
                    case "set_army":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SetLimit),
                            LangFunc.GetText(LangType.Ru, DataName.FuelLimitA), 5, "fuelcontrol_army");
                        return;
                    case "set_news":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SetLimit),
                            LangFunc.GetText(LangType.Ru, DataName.FuelLimitN), 5, "fuelcontrol_news");
                        return;
                }
            }
            catch (Exception e)
            {
                Log.Write($"callback_fuelcontrol Exception: {e}");
            }
        }
    ...
    public class Trigger : Script
    {
        public static void SendChatMessage(ExtPlayer player, string message)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"SendChatMessage Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"SendChatMessage Exception: {e}");
            }
        }
        ...
        public static void Kick(ExtPlayer player)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"Kick Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"Kick Exception: {e}");
            }
        }
        ...
        public static void SetSharedData(Entity entity, string key, object value)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"SetSharedData Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"SetSharedData Exception: {e}");
            }
        }

        public static void PlayAnimation(ExtPlayer player, string animDict, string animName, int flag,
            bool share = true, string attachmentName = null)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"PlayAnimation Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"PlayAnimation Exception: {e}");
            }
        }

        public static void TaskPlayAnim(ExtPlayer player, string animDict, string animName, int flag = 3,
            bool clearTasks = false, string attachmentName = null)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                ClientEventInRange(player.Position, 250f, "taskPlayAnim", player, animDict, animName, flag, clearTasks,
                    attachmentName);
            }
            catch (Exception e)
            {
                Main.Log.Write($"PlayAllAnimation Exception: {e}");
            }
        }
        ...
        public static void StopAnimation(ExtPlayer player, bool force = true)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"StopAnimation Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"StopAnimation Exception: {e}");
            }
        }
        ...
        public static void Position(ExtPlayer player, Vector3 pos)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"Player Position Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"Player Position Exception: {e}");
            }
        }
        ...
        public static void Position(ExtVehicle vehicle, Vector3 pos)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"Vehicle Position Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"Vehicle Position Exception: {e}");
            }
        }
        ...
        public static void SendToAdmins(int minLVL, string message, byte soundnotify = 0)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"SendToAdmins Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"SendToAdmins Exception: {e}");
            }
        }
            ...
            try
            {
                if (Thread.CurrentThread.Name == "Main")
                {
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        if (foreachCharacterData.AdminLVL >= 1 || foreachSessionData.Punishments)
                            foreachPlayer.SendChatMessage(message);
                    }
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"SendPunishment Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"SendPunishment Exception: {e}");
            }
        }
        ...
        public static void ClientEvent(ExtPlayer player, string eventName, params object[] args)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"ClientEvent Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"ClientEvent({eventName}) Exception: {e}");
            }
        }
        ...
        public static void UniqueDimension(Entity entity)
        {
            try
            {
                ...
                {
                    if (entity == null) return;
                    var newDimension = (uint)(1500 + entity.Id);
                    NAPI.Entity.SetEntityDimension(entity, newDimension);

                    if (entity.Type == EntityType.Player)
                    {
                        var playerEntity = (ExtPlayer)entity;
                        var sessionData = playerEntity.GetSessionData();
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"Player Dimension Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"Player Dimension Exception: {e}");
            }
        }
        ...
        public static void DamageDisable(ExtPlayer player, bool trigger)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"DamageDisable Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"DamageDisable Exception: {e}");
            }
        }
        ...
        public static void ClientEventInRange(Vector3 pos, float range, string eventName, params object[] args)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"ClientEventInRange Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"ClientEventInRange Exception: {e}");
            }
        }
        ...
        public static void ClientEventForAll(string eventName, params object[] args)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"ClientEventForAll Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"ClientEventForAll Exception: {e}");
            }
        }
        ...
        public static void ClientEventInDimension(uint dim, string eventName, params object[] args)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"ClientEventInDimension Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"ClientEventInDimension Exception: {e}");
            }
        }
        ...
        public static void ClientEventToPlayers(ExtPlayer[] players, string eventName, params object[] args)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Main.Log.Write($"ClientEventToPlayers Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Main.Log.Write($"ClientEventToPlayers Exception: {e}");
            }
        }
        ...
        public static void SetMainTask(Action action)
        {
            if (Thread.CurrentThread.Name == "Main")
            {
                ...
                {
                    Timers.Log.Write($"SetMainTask Exception: {e}");
                }
            ...
            NAPI.Task.Run(() =>
            {
                ...
                {
                    Timers.Log.Write($"SetMainTask Task Exception: {e}");
                }
            });
        }
        ...
        public static void SetTask(Action action)
        {
            if (Thread.CurrentThread.Name != "Main")
            {
                ...
                {
                    Timers.Log.Write($"SetTask Exception: {e}");
                }
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                {
                    Timers.Log.Write($"SetTask Task Exception: {e}");
                }
        ...
        public static void SetAsyncTask(Func<Task> action)
        {
            if (Thread.CurrentThread.Name != "Main")
            {
                ...
                {
                    Timers.Log.Write($"SetAsyncTask Exception: {e}");
                }
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                {
                    Timers.Log.Write($"SetAsyncTask Task Exception: {e}");
                }
            }, CancellationToken.None, TaskCreationOptions.LongRunning, TaskScheduler.Default);
        }
    }
