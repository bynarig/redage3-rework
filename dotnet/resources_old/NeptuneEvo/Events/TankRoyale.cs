...
{
    internal class TankRoyale : Script
    {
        ...
        private static readonly float[] TR_Stages = new float[5] { 400f, 330f, 250f, 195f, 100f };
        private static readonly Vector3 TR_Center = new Vector3(978.3693, -3090.363, 5.9007626);

        private static readonly Dictionary<int, Vector3> TR_Vehicles = new Dictionary<int, Vector3>
        {
        ...
        public static void TanksLobbiesStartInterval()
        {
            ...
            {
                Log.Write($"TanksLobbiesStartInterval Exception: {e}");
            }
        }
        ...
        public static void CheckLobby(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 1,
                    JsonConvert.SerializeObject(LobbyList.Values), "tanks");
            }
            catch (Exception e)
            {
                Log.Write($"CheckLobby Exception: {e}");
            }
        }

        [RemoteEvent("tanks_createLobby_server")]
        public static void CreateLobby(ExtPlayer player, string lobby_name, int lobby_price, string lobby_password,
            int gameMap)
        {
            try
            {
                ...
                if (sessionData.InTanksLobby >= 0) OutTanksLobbyZone(player, false);
                ...
                LobbyList[player.Value].TR_Timer =
                    Timers.Start("TR_Tick", 30000, () => TankRoyale_Tick(player.Value), true);

                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NowWaitOthers), 5000);
                Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 3,
                    JsonConvert.SerializeObject(new int[3] { LobbyList[player.Value].TR_Players.Count, 2, 30 }),
                    "tanks", LobbyList[player.Value].GameStartSeconds);
            }
            catch (Exception e)
            {
                Log.Write($"CreateLobby Exception: {e}");
            }
        }
        ...
        public static void DeathCheck(ExtPlayer player)
        {
            try
            {
                ...
                var lobbyId = sessionData.InTanksLobby;
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId) &&
                    LobbyList[lobbyId].TR_Players.ContainsKey(player) && LobbyList[lobbyId].TR_State == 2)
                {
                    player.Health = 100;
                    player.Armor = 0;
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouLeftFromMp), 3000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void PlayerExitFromTR(ExtPlayer player, bool dc = false, bool restoremoney = false,
            bool checkwinner = false, bool killlist = false)
        {
            try
            {
                var characterData = player.GetCharacterData();
            ...
            {
                Log.Write($"PlayerExitFromTR Exception: {e}");
            }
        }
        ...
        public static void RegisterAccept(ExtPlayer player, int index, string password)
        {
            try
            {
                ...
                {
                    Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 3,
                        JsonConvert.SerializeObject(new int[3] { LobbyList[index].TR_Players.Count, 2, 30 }), "tanks",
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.TankRoyaleMaxP), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.RegisterRoyaleCost,
                            Wallet.Format(LobbyList[index].LobbyPrice)), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GameIsOn), 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.TankSucRegister), 5000);
            ...
            {
                Log.Write($"RegisterAccept Exception: {e}");
            }
        }
        ...
        public static void TanksUpdateServerPlayersInLobbyMenuList(ExtPlayer player, int state)
        {
            ...
            {
                Log.Write($"TanksUpdateServerPlayersInLobbyMenuList Exception: {e}");
            }
        }
                    ...
                    foreach (var foreachPlayer in PlayersInLobbyMenu)
                        if (foreachPlayer.IsCharacterData())
                            Trigger.ClientEvent(foreachPlayer, "airsoft_updateLobbyList_client",
                                JsonConvert.SerializeObject(LobbyList), "tanks");
                }
            }
            catch (Exception e)
            {
        ...
        public static void UpdateTanksLobbyStats(int index)
        {
            try
            {
                if (LobbyList.ContainsKey(index) && LobbyList[index].TR_State == 1 &&
                    LobbyList[index].TR_Players.Count >= 1)
                    foreach (var foreachPlayer in LobbyList[index].TR_Players.Keys)
                        if (foreachPlayer.IsCharacterData())
                            Trigger.ClientEvent(foreachPlayer, "airsoft_lobbyMenuHandler", 3,
                                JsonConvert.SerializeObject(new int[3] { LobbyList[index].TR_Players.Count, 2, 30 }),
                                "tanks", LobbyList[index].GameStartSeconds);
            }
            catch (Exception e)
            {
                Log.Write($"UpdateTanksLobbyStats Exception: {e}");
            }
        }
        ...
        public static void OutTanksLobbyZone(ExtPlayer player, bool needMsg = true)
        {
            ...
            {
                Log.Write($"OutTanksLobbyZone Exception: {e}");
            }
        }
        ...
        public static void OnVehicleDamageHandler(ExtVehicle vehicle, float bodyHealthLoss, float engineHealthLoss)
        {
            try
            {
                foreach (var lobby in LobbyList)
                {
                    var PlayerInfo = LobbyList[lobby.Key].TR_Players.FirstOrDefault(p => p.Value.Vehicle == vehicle);
                    if (lobby.Key >= 0 && LobbyList.ContainsKey(lobby.Key) && LobbyList[lobby.Key].TR_State == 2 &&
                        LobbyList[lobby.Key].Vehicles.Contains(vehicle))
                    {
                        if (PlayerInfo.Value == default(TRClass) || PlayerInfo.Value.Vehicle == null) return;
                        PlayerInfo.Value.Health -= 1;
                        var target = PlayerInfo.Key;
                            ...
                            if (target.Health <= 7 || PlayerInfo.Value.Health <= 5)
                            {
                                ...
                                if (target != null)
                                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.YouLeftFromMp), 5000);

                                lock (LobbyList[lobby.Key].TR_Players.Keys)
                                {
                                    var count = LobbyList[lobby.Key].TR_Players.Count;
                                    if (count == 0)
                                    {
                                        if (LobbyList[lobby.Key].TR_Players.Keys.Count >= 1)
                                        {
                                            var Players = LobbyList[lobby.Key].TR_Players.Keys.ToList();
                                    ...
                                    if (count == 1) Winner(lobby.Key);
                                    else if (count >= 2)
                                        foreach (var foreachPlayer in LobbyList[lobby.Key].TR_Players.Keys)
                                        {
                                            if (!foreachPlayer.IsCharacterData()) continue;
                                            Trigger.SendChatMessage(foreachPlayer,
                                                $"~o~[Tank Royale] В игре осталось {LobbyList[lobby.Key].TR_Players.Count} танков!");
                                        }
                                }
                            }
                        }
            ...
            {
                Log.Write($"OnVehicleDamageHandler Exception: {e}");
            }
        }
        ...
        private static void Winner(int index)
        {
            try
            {
                ...
                GameLog.Money("system", $"player({winnerCharacterData.UUID})", LobbyList[index].TR_Bank,
                    "TankRoyaleWinner");
                if (LobbyList[index].TR_Players.Keys.Count >= 1)
                {
                    var Players = LobbyList[index].TR_Players.Keys.ToList();
                    foreach (var foreachPlayer in Players)
                    {
                        if (!foreachPlayer.IsCharacterData()) continue;
                        Trigger.SendChatMessage(foreachPlayer,
                            $"~o~[Tank Royale] Победитель {winner.Name.Replace('_', ' ')} выжил с {LobbyList[index].TR_Players[winner].Health}HP и получил {Wallet.Format(LobbyList[index].TR_Bank)}$.");
                    }
                }
        ...
        private static void EndGame(int index, bool withplayers = true)
        {
            try
            {
                ...
                if (LobbyList[index].TR_Marker != null && LobbyList[index].TR_Marker.Exists)
                    LobbyList[index].TR_Marker.Delete();
                LobbyList[index].TR_Marker = null;
                if (LobbyList[index].TR_LMarker != null && LobbyList[index].TR_LMarker.Exists)
                    LobbyList[index].TR_LMarker.Delete();
        ...
        private static void DeleteAllVehicles(int index)
        {
            ...
            {
                Log.Write($"DeleteAllVehicles Exception: {e}");
            }
        }
        ...
        private static void TankRoyale_Tick(int index)
        {
            try
            {
                ...
                switch (LobbyList[index].TR_State)
                {
                    ...
                    case 1:
                        if (LobbyList[index].TR_TimerTicks == 19) // 9:30
                        {
                            ...
                            foreach (var foreachPlayer in players)
                            {
                                ...
                                if (!SafeZones.IsSafeZone(foreachSessionData.InsideSafeZone,
                                        SafeZones.ZoneName.newarena))
                                    Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                        "Если Вы будете вне Зелёной Зоны мероприятия, то не попадёте на мероприятие.",
                                        3000);
                                else if (foreachPlayer.IsInVehicle)
                                    Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                        "Если Вы будете сидеть в машине, то не попадёте на мероприятие.", 3000);
                            }
                        }
                        else if (LobbyList[index].TR_TimerTicks == 20) // 10
                        {
                            if (LobbyList[index].TR_Players.Count >= 2)
                            {
                                ...
                                foreach (var foreachPlayer in players)
                                {
                                    ...
                                    {
                                        Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.MpLeftCuzFar), 6000);
                                        LobbyList[index].TR_Players.Remove(foreachPlayer);
                                        foreachSessionData.InTanksLobby = -1;
                                        continue;
                                    }
                                    ...
                                    if (foreachPlayer.IsInVehicle)
                                    {
                                        Notify.Send(foreachPlayer, NotifyType.Error, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.MpLeftCuzVeh), 6000);
                                        LobbyList[index].TR_Players.Remove(foreachPlayer);
                                        foreachSessionData.InTanksLobby = -1;
                                    }
                                    else if (UpdateData.CanIChange(foreachPlayer, LobbyList[index].LobbyPrice, true) !=
                                             255)
                                    {
                                        LobbyList[index].TR_Players.Remove(foreachPlayer);
                                        foreachSessionData.InTanksLobby = -1;
                                    ...
                                    else
                                    {
                                        ...
                                        GameLog.Money($"player({foreachCharacterData.UUID})", "system",
                                            LobbyList[index].LobbyPrice, "TankRoyaleRegister");
                                        Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.TargetLastAlive), 3000);
                                        Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.LoseIfDamage), 5000);
                                    }
                                }
                                ...
                                if (LobbyList[index].TR_Players.Count >= 2)
                                {
                                    ...
                                    LobbyList[index].TR_ZoneTimer = Timers.Start(1000, () =>
                                    {
                                        if (LobbyList[index].TR_State == 2)
                                        {
                                            ...
                                            foreach (var foreachPlayer in players)
                                                if (foreachPlayer.Position.DistanceTo2D(TR_Center) >
                                                    TR_Stages[LobbyList[index].TR_Stage] / 2)
                                                    if (LobbyList[index].TR_Players.ContainsKey(foreachPlayer))
                                                        OnVehicleDamageHandler(
                                                            LobbyList[index].TR_Players[foreachPlayer].Vehicle, 0f, 0f);
                                        }
                                    }, true);
                                    LobbyList[index].TR_Marker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                        new Vector3(), new Vector3(), TR_Stages[0], new Color(255, 0, 0, 50),
                                        dimension: 10);
                                    ...
                                    lock (LobbyList[index].TR_Players.Keys)
                                    {
                                        foreach (var foreachPlayer in LobbyList[index].TR_Players.Keys)
                                        {
                                            ...
                                            var veh = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey("rhino"),
                                                TR_Vehicles[id], 0f, 0, 0, number, numb: id, acc: VehicleAccess.Tank,
                                                petrol: 200, engine: true, dimension: 10);
                                            ...
                                            LobbyList[index].TR_Players[foreachPlayer].Health =
                                                !LobbyList[index].TR_RandomGun ? 50 : 100;
                                            LobbyList[index].Vehicles.Add(veh);
                                            Trigger.ClientEvent(foreachPlayer, "setIntoVehicle", veh,
                                                VehicleSeat.Driver - 1);
                                            if (!foreachCharacterData.Achievements[27])
                                            {
                                                foreachCharacterData.Achievements[27] = true;
                                                Notify.Send(foreachPlayer, NotifyType.Alert,
                                                    NotifyPosition.BottomCenter,
                                                    LangFunc.GetText(LangType.Ru, DataName.HowToJump), 5000);
                                            }
                                    ...
                                    Timers.StartOnce(2000, () =>
                                    {
                                        foreach (var foreachPlayer in LobbyList[index].TR_Players.Keys)
                                        {
                                            ...
                                            var vehicleLocalData = LobbyList[index].TR_Players[foreachPlayer].Vehicle
                                                .GetVehicleLocalData();
                                            if (foreachPlayer.Position.DistanceTo2D(TR_Center) > TR_Stages[0] / 2)
                                            {
                                                LobbyList[index].TR_Players[foreachPlayer].Vehicle.Position =
                                                    TR_Vehicles[vehicleLocalData.Number];
                                                Trigger.ClientEvent(foreachPlayer, "setIntoVehicle",
                                                    LobbyList[index].TR_Players[foreachPlayer].Vehicle,
                                                    VehicleSeat.Driver - 1);
                                            }
                                        }
                                    }, true);
                                ...
                                else
                                {
                                    if (LobbyList[index].TR_Players.Keys.Count >= 1)
                                    {
                                        ...
                                        foreach (var foreachPlayer in Players)
                                        {
                                            if (!foreachPlayer.IsCharacterData()) continue;
                                            Trigger.SendChatMessage(foreachPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.TooNotManyPlayers));
                                        }
                                    }
                    ...
                    case 2:
                        ...
                        {
                            case 21: // 10:30
                                LobbyList[index].TR_LMarker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[1], new Color(0, 255, 0, 50),
                                    dimension: 10);
                                lock (LobbyList[index].TR_Players.Keys)
                                {
                                    foreach (var foreachPlayer in LobbyList[index].TR_Players.Keys)
                                    {
                                        ...
                                        Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.RedCircleHp), 10000);
                                    }
                                }
                            ...
                            case 22: // 11
                                ...
                                LobbyList[index].TR_Marker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[1], new Color(255, 0, 0, 50),
                                    dimension: 10);
                                break;
                            case 23: // 11:30
                                if (LobbyList[index].TR_LMarker != null && LobbyList[index].TR_LMarker.Exists)
                                ...
                                LobbyList[index].TR_LMarker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[2], new Color(0, 255, 0, 50),
                                    dimension: 10);
                                break;
                            case 24: // 12
                                if (LobbyList[index].TR_Marker != null && LobbyList[index].TR_Marker.Exists)
                                ...
                                LobbyList[index].TR_Marker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[2], new Color(255, 0, 0, 50),
                                    dimension: 10);
                                break;
                            case 25: // 12:30
                                if (LobbyList[index].TR_LMarker != null && LobbyList[index].TR_LMarker.Exists)
                                ...
                                LobbyList[index].TR_LMarker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[3], new Color(0, 255, 0, 50),
                                    dimension: 10);
                                break;
                            case 26: // 13
                                if (LobbyList[index].TR_Marker != null && LobbyList[index].TR_Marker.Exists)
                                ...
                                LobbyList[index].TR_Marker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[3], new Color(255, 0, 0, 50),
                                    dimension: 10);
                                break;
                            case 27: // 13:30
                                if (LobbyList[index].TR_LMarker != null && LobbyList[index].TR_LMarker.Exists)
                                ...
                                LobbyList[index].TR_LMarker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[4], new Color(0, 255, 0, 50),
                                    dimension: 10);
                                break;
                            case 28: // 14
                                if (LobbyList[index].TR_Marker != null && LobbyList[index].TR_Marker.Exists)
                                ...
                                LobbyList[index].TR_Marker = (ExtMarker)NAPI.Marker.CreateMarker(28, TR_Center,
                                    new Vector3(), new Vector3(), TR_Stages[4], new Color(255, 0, 0, 50),
                                    dimension: 10);
                                break;
                            case 40: // 20
                                if (LobbyList[index].TR_Marker != null && LobbyList[index].TR_Marker.Exists)
                                ...
                                if (LobbyList[index].TR_Players.Keys.Count >= 1)
                                {
                                    ...
                                    foreach (var foreachPlayer in Players)
                                    {
                                        if (!foreachPlayer.IsCharacterData()) continue;
                                        Trigger.SendChatMessage(foreachPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.NoWinnerWin));
                                    }
                                }
            ...
            {
                Log.Write($"TankRoyale_Tick Exception: {e}");
            }
        }

        private class LobbyData
        {
            [JsonIgnore] public byte GameStartSeconds = 120; // Closed, Register, OnGoing

            public int TR_Bank;

            [JsonIgnore] public ExtMarker TR_LMarker;

            [JsonIgnore] public ExtMarker TR_Marker;

            [JsonIgnore] public Dictionary<ExtPlayer, TRClass> TR_Players = new Dictionary<ExtPlayer, TRClass>();

            [JsonIgnore] public bool TR_RandomGun; // False = default, True = SuperGun

            [JsonIgnore] public int TR_Stage;

            [JsonIgnore] public byte TR_State; // Closed, Register, OnGoing

            [JsonIgnore] public string TR_Timer;

            [JsonIgnore] public int TR_TimerTicks;

            [JsonIgnore] public string TR_ZoneTimer;

            [JsonIgnore] public List<ExtVehicle> Vehicles = new List<ExtVehicle>();
