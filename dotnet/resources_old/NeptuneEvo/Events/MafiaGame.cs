...
namespace NeptuneEvo.Core
{
    ...
    {
        [JsonIgnore] public bool doctorHealedYourself;

        [JsonIgnore] public int doctorLastHealNumber = -1;

        [JsonIgnore] public sbyte doctorLastHealNumberNightStatus;

        [JsonIgnore] public List<int> mafiaGame_toKickList = new List<int>();

        [JsonIgnore] public int mafiaGameDiscussionStatus = -1;

        [JsonIgnore]
        public Dictionary<int, MafiaGamePlayerData> MafiaGamePlayersInfo = new Dictionary<int, MafiaGamePlayerData>();

        [JsonIgnore] public int mafiaGamePlayerTurn;

        [JsonIgnore] public string mafiaGameTimer;

        [JsonIgnore] public int mafiaGameVoteStatus;

        [JsonIgnore] public List<ExtPlayer> MafiaPlayers = new List<ExtPlayer>();

        [JsonIgnore] public int totalMafia;

        [JsonIgnore] public int totalPlayers;

        [JsonIgnore] public int wasFucked = -1;

        [JsonIgnore] public int wasHealed = -1;

        [JsonIgnore] public int wasKilled = -1;
        ...
        [JsonIgnore] public int MaxPlayers { get; set; } = 15;
    ...
    internal class MafiaGamePlayerData
    {
        public int PlayerId;
    ...
    internal class MafiaGame : Script
    {
        ...
        private static readonly Dictionary<int, string> role_names = new Dictionary<int, string>
        {
            [1] = "Мафия",
            [2] = "Комиссар",
            [3] = "Доктор",
        ...
        private static readonly List<List<(Vector3, float)>> MafiaGamePlayersPositions =
            new List<List<(Vector3, float)>>
            {
        ...
        public static void MafiaLobbiesStartInterval()
        {
            ...
            {
                Log.Write($"MafiaLobbiesStartInterval Exception: {e}");
            }
        }
        ...
        public static void MafiaUpdateServerPlayersInLobbyMenuList(ExtPlayer player, int state)
        {
            ...
            {
                Log.Write($"MafiaUpdateServerPlayersInLobbyMenuList Exception: {e}");
            }
        }
        ...
        public static void CheckMafiaLobby(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 1,
                    JsonConvert.SerializeObject(LobbyList.Values), "mafia");
            }
            catch (Exception e)
            {
                Log.Write($"CheckMafiaLobby Exception: {e}");
            }
        }
        ...
        public static void UpdateMafiaLobbyStats(int index)
        {
            try
            {
                if (LobbyList[index].GameStatus <= 1 && LobbyList[index].MafiaPlayers.Count > 0)
                    foreach (var foreachPlayer in LobbyList[index].MafiaPlayers)
                        if (foreachPlayer.IsCharacterData())
                            Trigger.ClientEvent(foreachPlayer, "airsoft_lobbyMenuHandler", 3,
                                JsonConvert.SerializeObject(new int[3]
                                    { LobbyList[index].MafiaPlayers.Count, 6, LobbyList[index].MaxPlayers }), "mafia",
                                LobbyList[index].GameStartSeconds);
            }
            catch (Exception e)
            {
                Log.Write($"UpdateMafiaLobbyStats Exception: {e}");
            }
        }
        ...
        public static void OutMafiaLobbyZone(ExtPlayer player)
        {
            try
            {
                ...
                var lobbyId = sessionData.InMafiaLobby;
                if (lobbyId >= 0)
                    if (LobbyList.ContainsKey(lobbyId) && LobbyList[lobbyId].GameStatus <= 1 &&
                        LobbyList[lobbyId].MafiaPlayers.Contains(player))
                    {
                        ...
                        {
                            UpdateMafiaLobbyStats(lobbyId);
                        }
                    }
            }
            catch (Exception e)
            {
                Log.Write($"OutMafiaLobbyZone Exception: {e}");
            }
        }

        [RemoteEvent("mafia_createLobby_server")]
        public static void CreateMafiaLobby(ExtPlayer player, string lobby_name, int lobby_price, string lobby_password,
            int gameMap)
        {
            try
            {
                ...
                if (characterData.Money < lobby_price)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GameStartPrice, lobby_price), 3000);
                    return;
                }

                if (sessionData.InMafiaLobby >= 0) LeaveMafiaGame(player);

                LobbyList.Add(player.Value,
                    new LobbyData(player.Value, lobby_name.Length < 1 ? player.Name.Split("_")[0] : lobby_name,
                        lobby_price, lobby_password, gameMap));
                ...
                LobbyList[player.Value].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 120000,
                    () => mafiaGameTimerFunction(player.Value), true); // change to 120000

                Wallet.Change(player, -Math.Abs(lobby_price));
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    "Чтобы игра началась, нужно от 6 участников в лобби. Если вы покините зону, то игра будет отменена.",
                    5000);
                Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 3,
                    JsonConvert.SerializeObject(new int[3]
                        { LobbyList[player.Value].MafiaPlayers.Count, 6, LobbyList[player.Value].MaxPlayers }), "mafia",
                    LobbyList[player.Value].GameStartSeconds);
            }
            catch (Exception e)
            {
                Log.Write($"CreateMafiaLobby Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_joinMafiaLobby(ExtPlayer player, int index, string password)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MaxPlayersOn), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoGameStarted), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GameIsOn), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GameStartCost, LobbyList[index].LobbyPrice), 5000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.GameReg), 8000);
            ...
            {
                Log.Write($"joinMafiaLobby Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_voteMafiaGame(ExtPlayer player, int target_id)
        {
            try
            {
                ...
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantVoteYourself), 3000);
                        return;
                    }

                    if (player.Value == target_id && player.GetSharedData<int>("mafiaGameRole") == 3 &&
                        LobbyList[lobbyId].doctorHealedYourself)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AlreadyHealYourself), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoPlayerGame), 3000);
                        return;
                    }
                    ...
                    else if (LobbyList[lobbyId].mafiaGameVoteStatus == 6 &&
                             player.GetSharedData<int>("mafiaGameRole") == 2)
                    {
                        foreach (var MafiaPlayer in LobbyList[lobbyId].MafiaPlayers)
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                    if (MafiaPlayer.Value == target_id)
                                    {
                                        if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                            SendTextInfoForPlayer(player,
                                                LangFunc.GetText(LangType.Ru, DataName.PlayerIsMafia), 10000);
                                        else
                                            SendTextInfoForPlayer(player,
                                                LangFunc.GetText(LangType.Ru, DataName.PlayerNotMafia), 10000);
                                    }
                            }
                            catch (Exception e)
                            {
                                Log.Write($"voteMafiaGame Foreach #2 Exception: {e}");
                            }
                    ...
                    else if (LobbyList[lobbyId].mafiaGameVoteStatus == 7 &&
                             player.GetSharedData<int>("mafiaGameRole") == 3)
                    {
                        if (LobbyList[lobbyId].doctorLastHealNumber == target_id &&
                            LobbyList[lobbyId].doctorLastHealNumberNightStatus > 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouHealedPlayerNight), 3000);
                            return;
                        }
                        ...
                        foreach (var MafiaPlayer in LobbyList[lobbyId].MafiaPlayers)
                            ...
                            {
                                Log.Write($"voteMafiaGame Foreach #3 Exception: {e}");
                            }
                    ...
                    else if (LobbyList[lobbyId].mafiaGameVoteStatus == 8 &&
                             player.GetSharedData<int>("mafiaGameRole") == 4)
                    {
                        foreach (var MafiaPlayer in LobbyList[lobbyId].MafiaPlayers)
                            try
                            {
                            ...
                            {
                                Log.Write($"voteMafiaGame Foreach #4 Exception: {e}");
                            }
            ...
            {
                Log.Write($"voteMafiaGame Exception: {e}");
            }
        }
        ...
        private static void mafiaGameTimerFunction(int index)
        {
            try
            {
                ...
                if (LobbyList[index].GameStatus == 1)
                {
                    ...
                    {
                        foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                            try
                            {
                                ...
                                {
                                    Notify.Send(MafiaPlayer, NotifyType.Error, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.NoPlayersMafia), 3000);
                            ...
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #1 Exception: {e}");
                            }
                    ...
                    else
                    {
                        ...
                        foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    var playerRoleIndex = 5;
                                    var roleChance = new Random().Next(1, 5);
                                    playerNumber += 1;

                                    if ((roleChance == 1 && mafiaLimit > 0) ||
                                        (playerNumber == LobbyList[index].MafiaPlayers.Count - 2 && mafiaLimit > 0))
                                    {
                                        playerRoleIndex = 1;
                                        mafiaLimit -= 1;
                                    }
                                    else if ((roleChance == 2 && commissarLimit > 0) ||
                                             (playerNumber == LobbyList[index].MafiaPlayers.Count - 2 &&
                                              commissarLimit > 0))
                                    {
                                        playerRoleIndex = 2;
                                        commissarLimit -= 1;
                                    }
                                    else if ((roleChance == 3 && doctorLimit > 0) ||
                                             (playerNumber == LobbyList[index].MafiaPlayers.Count - 2 &&
                                              doctorLimit > 0))
                                    {
                                        playerRoleIndex = 3;
                                    ...
                                    else if ((roleChance == 4 && courtesanLimit > 0) ||
                                             (playerNumber == LobbyList[index].MafiaPlayers.Count - 2 &&
                                              courtesanLimit > 0))
                                    {
                                        playerRoleIndex = 4;
                                    ...
                                    if (!LobbyList[index].MafiaGamePlayersInfo.ContainsKey(MafiaPlayer.Value))
                                        LobbyList[index].MafiaGamePlayersInfo.Add(MafiaPlayer.Value,
                                            new MafiaGamePlayerData(MafiaPlayer.Value, MafiaPlayer.Name,
                                                playerRoleIndex));

                                    Trigger.ClientEvent(MafiaPlayer, "freeze", true);
                                    Trigger.Dimension(MafiaPlayer, 150);
                                    NAPI.Entity.SetEntityPosition(MafiaPlayer,
                                        MafiaGamePlayersPositions[LobbyList[index].LobbyMap][playerNumber - 1].Item1);
                                    NAPI.Entity.SetEntityRotation(MafiaPlayer,
                                        new Vector3(0, 0,
                                            MafiaGamePlayersPositions[LobbyList[index].LobbyMap][playerNumber - 1]
                                                .Item2));

                                    Animations.AnimationPlay(MafiaPlayer,
                                        LobbyList[index].LobbyMap == 0 ? "3_16" : "1_2");
                            ...
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #2 Exception: {e}");
                            }

                        LobbyList[index].GameStatus = 2;
                        LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 145000,
                            () => mafiaGameTimerFunction(index), true); // change to 145000
                    }
                }
                else if (LobbyList[index].GameStatus == 2)
                {
                    ...
                    foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                        ...
                        {
                            if (MafiaPlayer.IsCharacterData() &&
                                LobbyList[index].MafiaGamePlayersInfo.ContainsKey(MafiaPlayer.Value))
                            {
                                SendTextInfoForPlayer(MafiaPlayer,
                                    $"Ваша роль в этой игре: {role_names[MafiaPlayer.GetSharedData<int>("mafiaGameRole")]}",
                                    10000);
                        ...
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #3 Exception: {e}");
                        }

                    LobbyList[index].GameStatus = 3;
                    LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 10000,
                        () => mafiaGameTimerFunction(index), true);
                }
                else if (LobbyList[index].GameStatus == 3) // Первая ночь
                {
                    if (LobbyList[index].totalMafia < 2)
                    {
                        foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                            try
                            {
                                if (MafiaPlayer.IsCharacterData() && LobbyList[index].MafiaGamePlayersInfo
                                        .ContainsKey(MafiaPlayer.Value))
                                {
                                    Trigger.ClientEvent(MafiaPlayer, "mafia_startSoundSpeech", "night_mafia");
                                    Trigger.ClientEvent(MafiaPlayer, "setTimeCmd", 0, 0, 0);

                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                            JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true,
                                            false, false, LangFunc.GetText(LangType.Ru, DataName.MafiaKillsMaf),
                                            LangFunc.GetText(LangType.Ru, DataName.YouHave30Sec));
                                }
                            }
                            catch (Exception e)
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #4 Exception: {e}");
                            }
                                        ...
                                        .ContainsKey(MafiaPlayer.Value))
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                    {
                                        ...
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                            JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true,
                                            false, false, LangFunc.GetText(LangType.Ru, DataName.MafiaKillsMaf),
                                            LangFunc.GetText(LangType.Ru, DataName.YouHave1Min));

                                        SendTextInfoForPlayer(MafiaPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.YouHave1MinDiscuss), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #5 Exception: {e}");
                            }

                        LobbyList[index].GameStatus = 4;
                        LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 60000,
                            () => mafiaGameTimerFunction(index), true);
                    }
                }
                        ...
                        try
                        {
                            if (MafiaPlayer.IsCharacterData() &&
                                ...
                                if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                {
                                    ...
                                    SendTextInfoForPlayer(MafiaPlayer,
                                        LangFunc.GetText(LangType.Ru, DataName.MafiaTakeChoice), 10000);
                                }
                        }
                        catch (Exception e)
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #7 Exception: {e}");
                        }

                    LobbyList[index].GameStatus = 5;
                    LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer",
                        LobbyList[index].totalMafia > 1 ? 15000 : 30000, () => mafiaGameTimerFunction(index), true);
                }
                else if (LobbyList[index].GameStatus == 5)
                {
                    if (LobbyList[index].mafiaGame_toKickList.Count > 0)
                    {
                        var mostValues = new List<(int, int)>();

                        for (var i = 0; i < LobbyList[index].MafiaPlayers.Count; i++)
                        {
                            if (LobbyList[index].mafiaGame_toKickList.Count < 1) break;

                            var most = LobbyList[index].mafiaGame_toKickList.GroupBy(x => x)
                        ...
                        if (mostValues.Count < 1 ||
                            (mostValues.Count > 1 && mostValues[0].Item2 == mostValues[1].Item2))
                            LobbyList[index].wasKilled = -1;
                    ...
                    foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                        ...
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #10 Exception: {e}");
                        }

                    LobbyList[index].GameStatus = 6;
                    LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 3000,
                        () => mafiaGameTimerFunction(index), true);
                }
                        ...
                        foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                            try
                            {
                                        ...
                                        .ContainsKey(MafiaPlayer.Value))
                                {
                                    ...
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                            JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), false,
                                            true, false, LangFunc.GetText(LangType.Ru, DataName.TryToFindMaf),
                                            LangFunc.GetText(LangType.Ru, DataName.YouHave30Sec));
                                        SendTextInfoForPlayer(MafiaPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.YouCanProverit), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #13 Exception: {e}");
                            }

                        LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 30000,
                            () => mafiaGameTimerFunction(index), true);
                    }
                    else
                    {
                            ...
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 3)
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                            JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), false,
                                            true, false, LangFunc.GetText(LangType.Ru, DataName.MafYouCanHeal),
                                            LangFunc.GetText(LangType.Ru, DataName.YouHave30Sec));
                                        SendTextInfoForPlayer(MafiaPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.MafTryHeal), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #15 Exception: {e}");
                            }
                                ...
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 3)
                                    {
                                        if (LobbyList[index].wasHealed == -1)
                                            SendTextInfoForPlayer(MafiaPlayer,
                                                "К сожалению, время истекло. Вы никого не вылечили.");
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 4)
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                            JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), false,
                                            true, false, LangFunc.GetText(LangType.Ru, DataName.MafSexGo),
                                            LangFunc.GetText(LangType.Ru, DataName.YouHave30Sec));
                                        SendTextInfoForPlayer(MafiaPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.MafSexTry), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #17 Exception: {e}");
                            }
                            ...
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 3)
                                    {
                                        if (LobbyList[index].wasHealed == -1)
                                            SendTextInfoForPlayer(MafiaPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.MafHealTimeOut));
                ...
                else if (LobbyList[index].GameStatus == 9)
                {
                    foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 4 &&
                                    LobbyList[index].wasFucked == -1)
                                    SendTextInfoForPlayer(MafiaPlayer,
                                        LangFunc.GetText(LangType.Ru, DataName.MafSexTimeout));
                        ...
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #19 Exception: {e}");
                        }

                    LobbyList[index].GameStatus = 10;
                    LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 5000,
                        () => mafiaGameTimerFunction(index), true);
                }
                else if (LobbyList[index].GameStatus == 10)
                {
                    ...
                    if (LobbyList[index].wasFucked > -1 &&
                        LobbyList[index].MafiaGamePlayersInfo.ContainsKey(LobbyList[index].wasFucked))
                    {
                        if (LobbyList[index].MafiaGamePlayersInfo[LobbyList[index].wasFucked].PlayerRole == 3)
                            LobbyList[index].wasHealed = -1;
                        else if (LobbyList[index].MafiaGamePlayersInfo[LobbyList[index].wasFucked].PlayerRole == 1 &&
                                 LobbyList[index].totalMafia <= 1) LobbyList[index].wasKilled = -1;
                    }
                    ...
                    foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                ...
                                if (LobbyList[index].wasKilled > -1 &&
                                    LobbyList[index].wasKilled != LobbyList[index].wasHealed)
                                {
                                    SendTextInfoForPlayer(MafiaPlayer,
                                        LangFunc.GetText(LangType.Ru, DataName.MafDied, LobbyList[index].wasKilled),
                                        10000);

                                    if (MafiaPlayer.Value == LobbyList[index].wasKilled)
                                        playerToKick = MafiaPlayer;
                                    Repository.UpdateReward(MafiaPlayer, 131);
                                }
                                else
                                {
                                    SendTextInfoForPlayer(MafiaPlayer,
                                        LangFunc.GetText(LangType.Ru, DataName.MafNoDied), 10000);
                                }
                            }
                        }
                        catch (Exception e)
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #21 Exception: {e}");
                        }
                    ...
                    if (playerToKick != null)
                    {
                        ...
                        if (playerToKick.IsCharacterData() && foreachSessionData != null)
                        {
                            ...
                            if (LobbyList[index].totalMafia >=
                                LobbyList[index].MafiaPlayers.Count - LobbyList[index].totalMafia)
                            {
                ...
                {
                    var turnPlayerValue = 0;
                    ExtPlayer turnPlayer = null;

                    if (LobbyList[index].mafiaGameDiscussionStatus > 0 &&
                        LobbyList[index].MafiaPlayers.Count > LobbyList[index].mafiaGamePlayerTurn)
                    ...
                    foreach (var pMafia in LobbyList[index].MafiaGamePlayersInfo)
                        if (LobbyList[index].mafiaGameDiscussionStatus <= 0 && pMafia.Value.PlayerLife)
                            pMafia.Value.PlayerMute = false;
                        else pMafia.Value.PlayerMute = true;

                    if (LobbyList[index].mafiaGameDiscussionStatus > 0 && turnPlayer != null &&
                        turnPlayer.IsCharacterData() &&
                        LobbyList[index].MafiaGamePlayersInfo.ContainsKey(turnPlayer.Value))
                    {
                    ...
                    foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                if (LobbyList[index].mafiaGameDiscussionStatus >= LobbyList[index].MafiaPlayers.Count)
                                {
                                    ...
                                    SendTextInfoForPlayer(MafiaPlayer,
                                        LangFunc.GetText(LangType.Ru, DataName.EveryHave30Sec), 10000);
                                }

                                if (LobbyList[index].mafiaGameDiscussionStatus > 0)
                                {
                                    Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                        JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, false,
                                        true, LangFunc.GetText(LangType.Ru, DataName.DayMafiaVote),
                                        LangFunc.GetText(LangType.Ru, DataName.VseHave30Sec));

                                    if (MafiaPlayer == turnPlayer)
                                        Trigger.ClientEvent(MafiaPlayer, "mafia_updateMicroStatus", 2);
                                    else Trigger.ClientEvent(MafiaPlayer, "mafia_updateMicroStatus", 3);

                                    Notify.Send(MafiaPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.OcheredChela, turnPlayerValue), 3000);
                                }
                                else
                                {
                                    Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                        JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, false,
                                        true, LangFunc.GetText(LangType.Ru, DataName.DayVoteing),
                                        LangFunc.GetText(LangType.Ru, DataName.VseHave1Min));
                                    Trigger.ClientEvent(MafiaPlayer, "mafia_updateMicroStatus", 2);
                                }
                            }
                        }
                        ...
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #23 Exception: {e}");
                        }
                ...
                else if (LobbyList[index].GameStatus == 12)
                {
                    ...
                    foreach (var pMafia in LobbyList[index].MafiaGamePlayersInfo) pMafia.Value.PlayerMute = true;
                    ...
                    foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                ...
                                Trigger.ClientEvent(MafiaPlayer, "setTimeCmd", 12, 0, 0);
                                Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu",
                                    JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, true,
                                    true, LangFunc.GetText(LangType.Ru, DataName.DayPlayerChoice),
                                    LangFunc.GetText(LangType.Ru, DataName.VseHave15Sec));
                                Trigger.ClientEvent(MafiaPlayer, "mafia_updateMicroStatus", 1);
                                SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.Ru, DataName.VoteJail15),
                                    10000);
                            }
                        }
                        catch (Exception e)
                        {
                            Log.Write($"mafiaGameTimerFunction Foreach #25 Exception: {e}");
                        }

                    LobbyList[index].GameStatus = 13;
                    LobbyList[index].mafiaGameTimer = Timers.StartOnce("mafiaGameTimer", 15000,
                        () => mafiaGameTimerFunction(index), true);
                }
                else if (LobbyList[index].GameStatus == 13)
                {
                    if (LobbyList[index].mafiaGame_toKickList.Count > 0)
                    {
                        ...
                        if (mostValues.Count < 1 ||
                            (mostValues.Count > 1 && mostValues[0].Item2 == mostValues[1].Item2)) mostCommon = -1;
                        ...
                        if (mostCommon > -1)
                        {
                            foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                                try
                                {
                                    if (MafiaPlayer.IsCharacterData())
                                    {
                                        ...
                                        {
                                            SendTextInfoForPlayer(MafiaPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.SomebodyJail, mostCommon),
                                                10000);
                                        ...
                                        {
                                            SendTextInfoForPlayer(MafiaPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.VoteEquals), 10000);
                                        }
                                    }
                                }
                                catch (Exception e)
                                {
                                    Log.Write($"mafiaGameTimerFunction Foreach #28 Exception: {e}");
                                }
                            ...
                            if (playerToKick != null && playerToKick.IsCharacterData() && foreachSessionData != null)
                            {
                                ...
                                NAPI.Entity.SetEntityPosition(playerToKick,
                                ...
                                else if (LobbyList[index].totalMafia <= 0)
                                {
                                    mafiaGame_finish(index, 2);
                                }
                                else
                                {
                                    LobbyList[index].GameStatus = 3;
                    ...
                    else
                    {
                        foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                            ...
                            {
                                Log.Write($"mafiaGameTimerFunction Foreach #29 Exception: {e}");
                            }
            ...
            {
                Log.Write($"mafiaGameTimerFunction Exception: {e}");
            }
        }
        ...
        private static void mafiaGame_finish(int index, int state)
        {
            try
            {
                ...
                var current_reward = LobbyList[index].LobbyPrice * LobbyList[index].totalPlayers /
                                     LobbyList[index].MafiaPlayers.Count;
                ...
                foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                    try
                    {
                        if (MafiaPlayer.IsCharacterData())
                        {
                            ...
                            if (state == 1)
                            {
                                ...
                                SendTextInfoForPlayer(MafiaPlayer,
                                    LangFunc.GetText(LangType.Ru, DataName.MafiaWin, current_reward), 10000);
                            ...
                            else if (state == 2)
                            {
                                ...
                                SendTextInfoForPlayer(MafiaPlayer,
                                    LangFunc.GetText(LangType.Ru, DataName.MirWin, current_reward), 10000);
                    ...
                    {
                        Log.Write($"mafiaGame_finish Foreach Exception: {e}");
                    }
            ...
            {
                Log.Write($"mafiaGame_finish Exception: {e}");
            }
        }
        ...
        public static void LeaveMafiaGame(ExtPlayer player)
        {
            try
            {
                ...
                var index = sessionData.InMafiaLobby;
                if (index >= 0 && LobbyList.ContainsKey(index))
                    if (LobbyList[index].MafiaPlayers.Contains(player))
                    {
                        if (LobbyList[index].MafiaGamePlayersInfo.ContainsKey(player.Value) &&
                        ...
                        if (LobbyList[index].MafiaPlayers.Count < 6 && LobbyList[index].GameStatus > 1)
                        {
                            ...
                            foreach (var MafiaPlayer in LobbyList[index].MafiaPlayers)
                                try
                                {
                                    ...
                                    {
                                        Notify.Send(MafiaPlayer, NotifyType.Error, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.OstalosMaloPla), 3000);
                                ...
                                {
                                    Log.Write($"LeaveMafiaGame Foreach Exception: {e}");
                                }
                                ...
                                try
                                {
                                    if (MafiaPlayer.IsCharacterData())
                                        Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 3,
                                            JsonConvert.SerializeObject(new int[3]
                                            {
                                                LobbyList[index].MafiaPlayers.Count, 6, LobbyList[index].MaxPlayers
                                            }), "mafia", LobbyList[index].GameStartSeconds);
                                }
                                catch (Exception e)
                                {
                                    Log.Write($"LeaveMafiaGame Foreach Exception: {e}");
            ...
            {
                Log.Write($"LeaveMafiaGame Exception: {e}");
            }
        }
                    ...
                    foreach (var foreachPlayer in PlayersInLobbyMenu)
                        if (foreachPlayer.IsCharacterData())
                            Trigger.ClientEvent(foreachPlayer, "airsoft_updateLobbyList_client",
                                JsonConvert.SerializeObject(LobbyList), "mafia");
                }
            }
            catch (Exception e)
            {
        ...
        public void OnPlayerDeath(ExtPlayer player, Player killer, uint reason)
        {
            try
            {
                ...
                var index = sessionData.InMafiaLobby;
                if (index >= 0 && LobbyList.ContainsKey(index) && player.HasSharedData("mafiaGameRole") &&
                    player.GetSharedData<int>("mafiaGameRole") >= 1)
                    if (LobbyList[index].MafiaPlayers.Contains(player))
                        LeaveMafiaGame(player);
            }
            catch (Exception e)
