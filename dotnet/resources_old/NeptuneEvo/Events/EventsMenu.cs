...
namespace NeptuneEvo.Events
{
    public class EventsMenu : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                CustomColShape.CreateSphereColShape(new Vector3(-488.47617, -401.0276, 34.79613), 7f, 0,
                    ColShapeEnums.EventsMenu);
                Repository.CreateQuest("a_m_m_afriamer_01", new Vector3(-486.10715, -399.39944, 34.546597), -47.65183f,
                ...
                Main.CreateBlip(new Main.BlipData(491, LangFunc.GetText(LangType.Ru, DataName.Events),
                    new Vector3(-483.149, -400.09946, 34.546608), 6, true));
            }
            catch (Exception e)
            {
        ...
        public static void EventsMenuInteraction(ExtPlayer player)
        {
            ...
            {
                Log.Write($"EventsMenuInteraction Exception: {e}");
            }
        }
        ...
        public static void OutEventsMenuZone(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OutEventsMenuZone Exception: {e}");
            }
        }
        ...
        public static void SelectEventServer(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (index == 0 && sessionData.InAirsoftLobby == -1 && sessionData.InTanksLobby == -1)
                    MafiaGame.CheckMafiaLobby(player);
                else if (index == 1 && sessionData.InAirsoftLobby == -1 && sessionData.InMafiaLobby == -1)
                    TankRoyale.CheckLobby(player);
                else if (index == 2 && sessionData.InMafiaLobby == -1 && sessionData.InTanksLobby == -1)
                    Airsoft.LoadAirsoftLobbyList(player);
            }
            catch (Exception e)
            {
                Log.Write($"SelectEventServer Exception: {e}");
            }
        }
    }
}