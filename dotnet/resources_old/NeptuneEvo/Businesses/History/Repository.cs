...
namespace NeptuneEvo.Businesses.History
{
    ...
    {
        private static readonly List<Businesshistories> InsertHistoryList = new List<Businesshistories>();
        private static readonly nLog Log = new nLog("Businesses.History.Repository");

        private static readonly int MaxHistoryTime = 30;
        ...
        public static bool IsCountInsertHistory()
        {
            return InsertHistoryList.Count > 0;
        }
        ...
        public static async void GetHistory(ExtPlayer player, int bizId)
        {
            try
            {
                ...
                var historyList = await db.Businesshistory
                    .Where(bh =>
                        bh.Bizid == bizId && bh.Date >= DateTime.Now.Subtract(new TimeSpan(MaxHistoryTime, 0, 0, 0)))
                ...
                for (var i = 0; i < MaxHistoryTime; i++)
                {
                    ...
                    var historyProductCount = historyList
                        .Where(hl =>
                            hl.Date.Day == time.Day && hl.Date.Month == time.Month && hl.Date.Year == time.Year)
                        .Sum(hl => hl.Price);
                ...
                Trigger.ClientEvent(player, "client.phone.business.initStats",
                    JsonConvert.SerializeObject(historyChart), JsonConvert.SerializeObject(historyProductsList),
                    JsonConvert.SerializeObject(historyUsersList));
            }
            catch (Exception e)
            {
