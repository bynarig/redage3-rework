    ...
    public class Repository
    {
        ...
        public static async Task<int> Create(ExtPlayer player, string firstName, string lastName)
        {
            try
            {
                ...
                {
                    Trigger.ClientEvent(player, "client.characters.create.error",
                        LangFunc.GetText(LangType.Ru, DataName.ErrorLenghtName));
                    return -1;
                }
                ...
                {
                    Trigger.ClientEvent(player, "client.characters.create.error",
                        LangFunc.GetText(LangType.Ru, DataName.NameUsed), 3000);
                    return -1;
                }
                ...
                if (Character.Repository.LoginsBlck.Contains(accountData.Login))
                    GameLog.Connected(fullname, characterData.UUID, sessionData.RealSocialClub, sessionData.RealHWID,
                        sessionData.Value, "-", accountData.Login); // Пошли нахуй дети шлюх, которые сливают IP
                else
                    GameLog.Connected(fullname, characterData.UUID, sessionData.RealSocialClub, sessionData.RealHWID,
                        sessionData.Value, sessionData.Address, accountData.Login);
                ...
                var questsData = await qMain.Load(db, characterData.UUID);

                //

                var phoneData = await Players.Phone.Repository.Load(db, characterData.UUID, characterData.Sim,
                ...
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        GameLog.AccountLog(accountData.Login, accountData.HWID, accountData.IP, accountData.SocialClub,
                            $"Создан персонаж {characterData.FirstName} {characterData.LastName}");

                        player.SetSharedData("NewUser", true);

                        //

                        qMain.InitQuests(player, questsData, true);
