    ...
    public class Repository
    {
        ...
        public static void Spawn(ExtPlayer player)
        {
            try
            {
                ...
                EverydayAward.OnPlayerSpawn(player);
                ...
                if (characterData.Warns > 0 && DateTime.Now > characterData.Unwarn)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.OneWarnSnyat, characterData.Warns), 3000);
                }

                //

                Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", ItemId.BagWithMoney);
