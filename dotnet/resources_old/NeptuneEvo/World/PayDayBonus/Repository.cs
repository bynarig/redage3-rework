...
namespace NeptuneEvo.World.PayDayBonus
{
    ...
    {
        private static readonly int MinTime = 30;

        private static readonly List<ExtPlayer> AntiAfkPlayers = new List<ExtPlayer>();
        ...
        public static void Bonus()
        {
            ...
            if (players.Count > 0)
            {
                ...
                for (var i = 0; i < Main.ServerSettings.NumberWinners; i++)
                {
                ...
                foreach (var foreachPlayer in winnersPlayer)
                {
                    ...
                    UpdateData.RedBucks(foreachPlayer, Main.DonateSettings.HappyHoursRB, "СЧАСТЛИВЫЕ ЧАСЫ");
                    Trigger.ClientEvent(foreachPlayer, "hud.info", "Удача на твоей стороне!",
                        $"Поздравляем с победой в розыгрыше! {Main.DonateSettings.HappyHoursRB} RedBucks уже зачислены на твой аккаунт.",
                        "СЧАСТЛИВЫЕ ЧАСЫ", "https://cloud.redage.net/cloud/img/time.png");

                    winersName += $" {foreachSessionData.Name}";
                }

                NAPI.Chat.SendChatMessageToAll(
                    $"~o~[СЧАСТЛИВЫЕ ЧАСЫ] Розыгрыш {Main.DonateSettings.HappyHoursRB} RedBucks проводится каждый час только среди активных игроков.");
                NAPI.Chat.SendChatMessageToAll(
                    $"~o~[СЧАСТЛИВЫЕ ЧАСЫ] Победителями этого часа становятся: {winersName}. Они получают по {Main.DonateSettings.HappyHoursRB}, поздравляем!");
            }
        }
