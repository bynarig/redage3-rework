...
namespace NeptuneEvo.World.War
{
    public class Repository
    {
        ...
        private static readonly ushort ProtectionCounting = 60 * 1;
        private static readonly ushort WarCounting = 60 * 10;

        private static readonly IReadOnlyDictionary<uint, ushort> WeaponPoints = new Dictionary<uint, ushort>
        {
        ...
        {
            var min1 = (int)time1.Subtract(new DateTime(1970, 1, 1)).TotalMinutes;
            var min2 = (int)time2.Subtract(new DateTime(1970, 1, 1)).TotalMinutes;
            if (min1 > min2)
                return min1 - min2;
        ...
        {
            var min1 = (int)time1.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
            var min2 = (int)time2.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
            if (min1 > min2)
                return min1 - min2;
        ...
        public static void Open(ExtPlayer player, ushort objectId, ushort mapId, string mapName, Vector3 position,
            float range, WarType type, ushort attackingId, ushort protectingId)
        {
            try
            {
                ...
                {
                    var fractionDataAttack = Manager.GetFractionData(protectingId);
        ...
        public static ushort War(ExtPlayer player, Players.Models.WarData warData, sbyte typeBattle, sbyte composition,
            sbyte weaponsCategory, sbyte day = 0, sbyte hour = 0, sbyte min = 0, bool isCheckTime = true,
            bool isWarInterface = true)
        {
            ...
            try
            {
                ...
                if (isCheckTime)
                {
                    ...
                    if (Main.ServerNumber != 0 && DateTime.Now.AddHours(2) > time)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Стрелку можно забить минимум за два часа", 3000);
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Стрелку можно забить минимум за 5 минут", 3000);
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Эта зона уже находится в процессе захвата.", 8000);
                    if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                    return 0;
                }

                if (warData.Type == WarType.OrgWarZone &&
                    !Organizations.FamilyZones.Repository.IsAttack(player, time, day, warData.AttackingId,
                        warData.ProtectingId))
                    return 0;
        ...
        private static List<WarData> GetWars(WarType type, int mapId, int uuid, int fractionId, int organizationId,
            bool isProtection = true, bool isCreate = false)
        {
            ...
            foreach (var w in Wars.Values.ToList())
            {
                ...
                if (isProtection && w.Status == WarStatus.Protection && w.ProtectingId != fractionId &&
                    w.ProtectingId != organizationId)
                    continue;

                if ((w.Type == WarType.Gangs || w.Type == WarType.Mafia) && w.AttackingId != fractionId &&
                    w.ProtectingId != fractionId)
                    continue;

                if ((w.Type == WarType.OrgWarZone || w.Type == WarType.OrgHouse) && w.AttackingId != organizationId &&
                    w.ProtectingId != organizationId)
                    continue;

                if (w.Composition > 0 && w.AttackingPlayersInZone >= w.Composition &&
                    (w.AttackingId == fractionId || w.AttackingId == organizationId))
                    continue;

                if (w.Composition > 0 && w.ProtectingPlayersInZone >= w.Composition &&
                    (w.ProtectingId == fractionId || w.ProtectingId == organizationId))
                    continue;
        ...
        public static void InWarZone(ExtPlayer player, WarType type, int index)
        {
            try
            {
                ...
                var wars = GetWars(type, index, player.GetUUID(),
                    memberFractionData != null ? memberFractionData.Id : -1,
        ...
        public static void EntryZone(ExtPlayer player, ushort warId)
        {
            try
            {
                ...
                {
                    var fractionDataAttack = Manager.GetFractionData(war.AttackingId);
                    if (fractionDataAttack != null)
                        attackName = fractionDataAttack.Name;

                    var fractionDataProtecting = Manager.GetFractionData(war.ProtectingId);
                ...
                Trigger.ClientEvent(player, "inWarZone", war.Type, war.GripType, attackName, protectingName, time,
                    war.AttackingCount, war.ProtectingCount, war.WeaponsCategory, war.AttackingPlayersInZone,
                    war.ProtectingPlayersInZone);
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Trigger.ClientEvent(foreachPlayer, "playersWarZone", war.AttackingPlayersInZone,
                        war.ProtectingPlayersInZone);
                }
        ...
        private static void ExitZone(ExtPlayer player, WarData war, WarType type = WarType.None,
        ...
        public static void OnPlayerSpawn(ExtPlayer player)
        {
            try
            {
                if (!player.IsFractionAccess(RankToAccess.IsWar, false) &&
                    !player.IsOrganizationAccess(RankToAccess.IsWar, false))
                    return;
                ...
                var wars = GetWars(WarType.None, -1, player.GetUUID(),
                    memberFractionData != null ? memberFractionData.Id : -1,
                    memberOrganizationData != null ? memberOrganizationData.Id : -1);

                foreach (var war in wars)
                    Trigger.ClientEvent(player, "createWarZone", war.Id, war.Position.X, war.Position.Y, war.Position.Z,
                        war.Range);

                wars = GetWars(WarType.None, -1, player.GetUUID(),
                    memberFractionData != null ? memberFractionData.Id : -1,
                    memberOrganizationData != null ? memberOrganizationData.Id : -1, isCreate: true);
                ...
                foreach (var war in wars)
                {
                    ...
                    var isAttack = (memberFractionData != null && memberFractionData.Id == war.AttackingId) ||
                                   (memberOrganizationData != null && memberOrganizationData.Id == war.AttackingId);
        ...
        public static bool OnPlayerDeath(ExtPlayer player, ExtPlayer killer, uint weapon)
        {
            try
            {
                ...
                if (war.Type == WarType.Gangs || war.Type == WarType.Mafia)
                {
                    ...
                    NAPI.Player.SpawnPlayer(player, Manager.FractionSpawns[fractionMemberData.Id]);
                }
                else
                {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Trigger.ClientEvent(foreachPlayer, "countWarZone", war.AttackingCount, war.ProtectingCount,
                        killerId, playerId, weapon);
                }

                if (war.GripType == WarGripType.LastSurvivor && war.IsStartWar &&
                    (war.AttackingPlayersInZone <= 0 || war.ProtectingPlayersInZone <= 0))
                    EndWar(warId);
        ...
        private static void Notification(ushort warId, string text = null, bool isWarZone = false, int minute = 0,
            ...
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    else
                    {
                        ...
                        if (foreachOrganizationMemberData.Id != war.AttackingId &&
                            foreachOrganizationMemberData.Id != war.ProtectingId)
                            continue;
        ...
        private static void Notification(ExtPlayer player, WarData war, string text = null, int minute = 0,
            bool isAttack = false, Vector3 wayPoint = null)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                if (text == null)
                {
                    ...
                    if (wayPoint != null)
                        Trigger.ClientEvent(player, "infoWarZone", -1, isAttack, war.MapName, war.GripType, time,
                            war.Composition, war.WeaponsCategory, wayPoint.X, wayPoint.Y);
                    else
                        Trigger.ClientEvent(player, "infoWarZone", -1, isAttack, war.MapName, war.GripType, time,
                            war.Composition, war.WeaponsCategory);
                }
                else
                {
                    ...
                    if (war.Time <= time)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (var foreachPlayer in Character.Repository.GetPlayers())
                            {
                                ...
                                Trigger.ClientEvent(foreachPlayer, "createWarZone", war.Id, war.Position.X,
                                    war.Position.Y, war.Position.Z, war.Range);
                            }
                        }
                    }
                    else
            ...
            try
            {
                ...
                switch (war.Type)
                {
                    case WarType.OrgWarZone:
                        if (draw)
                        {
                            Manager.SendCoolOrganizationMsg(war.ProtectingId, "Война", "Ничья", "Вы сыграли в ничью.",
                                "", 10000);
                            Manager.SendCoolOrganizationMsg(war.AttackingId, "Война", "Ничья", "Вы сыграли в ничью.",
                                "", 10000);
                        ...
                        {
                            Manager.SendCoolOrganizationMsg(war.ProtectingId, "Война", "Поражение",
                                "Вы проиграли войну и потеряли территорию :(", "", 10000);
                            Manager.SendCoolOrganizationMsg(war.AttackingId, "Война", "Победа",
                                "Вы победили! Территория переходит под ваш контроль, поздравляем!", "", 10000);
                        ...
                        {
                            Manager.SendCoolOrganizationMsg(war.ProtectingId, "Война", "Победа",
                                "Вы удержали территорию! Поздравляем!", "", 10000);
                            Manager.SendCoolOrganizationMsg(war.AttackingId, "Война", "Поражение",
                                "Вы проиграли! Территория остаётся у стороны защиты.", "", 10000);
                    ...
                    case WarType.Gangs:
                        if (victoryAttackers)
                        {
                            Manager.SendCoolFractionMsg(war.ProtectingId, "Капт", "Поражение",
                                LangFunc.GetText(LangType.Ru, DataName.CaptureLoseByDef), "", 10000);
                            Manager.SendCoolFractionMsg(war.AttackingId, "Капт", "Победа",
                                LangFunc.GetText(LangType.Ru, DataName.CaptureWinByAttack), "", 10000);
                        ...
                        {
                            Manager.SendCoolFractionMsg(war.ProtectingId, "Капт", "Победа",
                                LangFunc.GetText(LangType.Ru, DataName.CaptureWinByDef), "", 10000);
                            Manager.SendCoolFractionMsg(war.AttackingId, "Капт", "Поражение",
                                LangFunc.GetText(LangType.Ru, DataName.CaptureLoseByAttack), "", 10000);
                        ...
                        foreach (var foreachPlayer in Character.Repository.GetPlayers())
                        {
                            ...
                            {
                                Wallet.Change(foreachPlayer, Main.CaptureWin);
                                GameLog.Money("server", $"player({foreachPlayer.GetUUID()})", Main.CaptureWin,
                                    "winCapture");
                            }
                    ...
                    case WarType.Mafia:
                        if (victoryAttackers)
                        {
                            Manager.sendFractionMessage(war.ProtectingId,
                                LangFunc.GetText(LangType.Ru, DataName.DefLoseBizWar));
                            Manager.sendFractionMessage(war.AttackingId,
                                LangFunc.GetText(LangType.Ru, DataName.AttackWinBizWar));
                        }
                        else
                        {
                            Manager.sendFractionMessage(war.ProtectingId,
                                LangFunc.GetText(LangType.Ru, DataName.DefWinBizWar));
                            Manager.sendFractionMessage(war.AttackingId,
                                LangFunc.GetText(LangType.Ru, DataName.AttackLoseBizWar));
                        }
                        ...
                        foreach (var foreachPlayer in Character.Repository.GetPlayers())
                        {
                            ...
                            if (foreachMemberFractionData.Id == victoryId)
                            {
                                Wallet.Change(foreachPlayer, Main.BizwarWin);
                ...
                switch (war.Type)
                {
                    case WarType.OrgWarZone:
                        ...
                        break;
                    case WarType.Gangs:
                        GangsCapture.Update((byte)war.ObjectId, victoryId, war.AttackingId, war.ProtectingId);
                        break;
                    case WarType.Mafia:
                        MafiaWars.Update((byte)war.ObjectId, victoryId, war.MapId, war.AttackingId, war.ProtectingId);
                        break;
                }
                //}
        ...
        public static void Unix()
        {
            try
            {
                foreach (var id in Wars.Keys.ToList()) UpdateWar(id);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
