...
namespace NeptuneEvo.Players.Phone.Mechanic.Orders
{
    public class Repository
    {
        public static void StartWork(ExtPlayer player)
        {
            ...
            foreach (var order in Mechanic.Repository.OrdersList)
            {
            ...
            if (sessionData.WorkData.OnWork)
            {
                ...
                if (sessionData.WorkData.Player != null)
                {
                    ...
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AutomechLeave), 3000);
                }
            }
        }
        ...
        public static void GetSelect(ExtPlayer player, bool isTake = false)
        {
            ...
            if (characterData.WorkID == (int)JobsId.CarMechanic &&
                (sessionData.RentData != null || sessionData.WorkData.OnWork))
            {
                var taxiData = Mechanic.Repository.OrdersList.FirstOrDefault(ol => ol.Driver == player);
            ...
            Trigger.ClientEvent(player, "client.phone.mechjob.initSelect", JsonConvert.SerializeObject(selectedOrders),
                isTake);
        }
        ...
        public static void OnTake(ExtPlayer player, int id)
        {
            ...
            if (vehicle != null && characterData.WorkID == (int)JobsId.CarMechanic &&
                (sessionData.RentData != null || sessionData.WorkData.OnWork))
            {
                var mechanicData = Mechanic.Repository.OrdersList.FirstOrDefault(ol => ol.PlayerUUID == id);
                ...
                if (mechanicData != null && mechanicData.Player.IsCharacterData())
                {
                    ...
                    Trigger.ClientEvent(mechanicData.Player, "client.phone.mech.updateOrder", sessionData.Name,
                        vehicle.NumberPlate, player.Position.X, player.Position.Y, player.Position.Z);

                    mechanicData.DriverTimer = Timers.Start(2500, () => OnUpdatePos(player, mechanicData.Player), true);

                    foreach (var foreachPlayer in Mechanic.Repository.GetListMechanic())
                    {
        ...
        public static void OnUpdatePos(ExtPlayer player, ExtPlayer target)
        {
            ...
            if (player.Position.DistanceTo2D(target.Position) < 10f)
            {
                ...
                Mechanic.Repository.OnCancel(target, isNotRemove: false);
                return;
            }
            ...
            if (vehicle != null && characterData.WorkID == (int)JobsId.CarMechanic &&
                (sessionData.RentData != null || sessionData.WorkData.OnWork))
                Trigger.ClientEvent(target, "client.phone.mech.updatePosOrder", player.Position.X, player.Position.Y,
                    player.Position.Z);
        }
    }
}