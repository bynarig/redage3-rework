...
namespace NeptuneEvo.Players.Phone.Cars
{
    public class Repository
    {
        public static void GetCarData(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "client.phone.cars.init", JsonConvert.SerializeObject(carsOwnerData), inGarage,
                ownerName == sessionData.Name);
        }

        public static List<List<object>> GetCarToHouse(ExtPlayer player, string ownerName, List<string> vehiclesNumber,
            Garage garage = null, bool inPark = false)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                foreach (var number in vehiclesNumber)
                {
                    ...
                    var carData = new List<object>
                    {
                        ...
                        AirAutoRoom.isAirCar(vehicleData.Model), //isAir
                    ...
                    if (vehicleData.Holder == sessionData.Name) //sell
                    {
                        ...
                        if (BusinessManager.BusProductsData.ContainsKey(vehicleData.Model))
                            price = Wallet.GetPriceToVip(player,
