...
namespace NeptuneEvo.Players.Session
{
    public class Repository
    {
        ...
        private static readonly int SessionDays = 30;
        ...
        public static async Task<SessionEnum> GetSession(ServerBD db, ExtPlayer player)
        {
                ...
                if (session != null)
                {
                    ...
                    if (account != null)
                    {
                        ...
                        var ban = await db.Banned
                            .Where(v => v.Account == account.Login ||
                        ...
                        if (session.OneTime)
                        {
                            ...
                            await db.Sessions
                                .Where(s => s.Hash ==
                                            Accounts.Repository.GetSha256(
                                                $"{sessionData.RealSocialClub}_{sessionData.Address}"))
                                .DeleteAsync();
                        }
                        else
                        {
            ...
            {
                Log.Write($"GetSession Exception: {e}");
            }
        ...
        public static async Task SaveSession(ServerBD db, ExtPlayer player)
        {
            ...
            {
                Log.Write($"DeleteSessions Exception: {e}");
            }
        }
                ...
                try
                {
                    ...
                    if (!isSession)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SessionSaved), 3000);
                    }
                    else
                    {
                        await db.Sessions
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SessionBroken), 3000);
                    }
                }
                catch (Exception e)
                {
