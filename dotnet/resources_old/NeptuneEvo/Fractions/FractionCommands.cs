...
{
    internal class FractionCommands : Script
    {
            ...
            try
            {
                ...
                if (sessionData.DeathData.InDeath)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantSitVehCritical), 3000);
                    return;
                }
                ...
                if (sessionData.Follower != null)
                {
                    VehicleManager.WarpPlayerOutOfVehicle(player);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.ReleaseBeforeSitveh), 3000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void respawnFractionCars(ExtPlayer player)
        {
            try
            {
                ...
                {
                    var ticks = Main.NextCarRespawn[fractionData.Id].Ticks - DateTime.Now.Ticks;
                    if (ticks <= 0) return;
                    var g = new DateTime(ticks);
                    if (g.Hour >= 1)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CanDoByTime1h, g.Hour, g.Minute, g.Second), 3000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CanDoByTime, g.Minute, g.Second), 3000);
                    return;
                }
                ...
                var vehiclesLocalData = Entities.Vehicles.All.Cast<ExtVehicle>()
                    ...
                    .Where(v => v.VehicleLocalData.Access == VehicleAccess.Fraction &&
                                v.VehicleLocalData.Fraction == fractionData.Id)
                    .ToList();
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.RespawnFractionVehs), 3000);

                if (fractionData.IsLeader(memberFractionData.Rank))
                    Manager.sendFractionMessage(fractionData.Id,
                        "!{#FF8C00}[F] " + LangFunc.GetText(LangType.Ru, DataName.WhoRespawnFractionVehs, player.Name,
                            player.Value), true);
            }
            catch (Exception e)
            {
                Log.Write($"respawnFractionCars Exception: {e}");
            }
        }
        ...
        public static void playerPressCuffBut(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouInCar), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PlayerInCar), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantCuffWhenCasino, targetSessionData.IsCasinoGame),
                        3000);
                    return;
                }

                if (sessionData.DeathData.InDeath) return;
                if (sessionData.Follower != null || sessionData.Following != null ||
                    targetCharacterData.ArrestTime >= 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantUseOnThisPlayer), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantUseOnThisPlayerWhenCuffed), 3000);
                    return;
                }
                ...
                var sound = ""; // message /me after uncuff
                var cdata = targetSessionData.CuffedData;
                if (characterData.AdminLVL != 0 || Manager.FractionTypes[fracId] == FractionsType.Gov ||
                    Manager.FractionTypes[fracId] == FractionsType.Nongov) // for gov factions or admins
                {
                ...
                else // for mafia
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoKeysFromCuffs), 3000);
                        return;
                    }

                    if (!targetSessionData.HandsUp && !cdata.Cuffed)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PlayerMustHandsUp), 3000);
                        return;
                    }

                    var cuffs = Repository.isItem(player, "inventory", ItemId.Cuffs);
                    var count = cuffs == null ? 0 : cuffs.Item.Count;
                    if (!cdata.Cuffed && count == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouHaveNoCuffs), 3000);
                        return;
                    }

                    if (!cdata.Cuffed) Repository.RemoveIndex(player, cuffs.Location, cuffs.Index);
                    sound = "zips";
                ...
                if (!cdata.Cuffed)
                {
                    var Bags = Repository.GetItemData(target, "accessories", 8).ItemId;
                    if (Bags == ItemId.BagWithMoney || Bags == ItemId.BagWithDrill)
                        Repository.ItemsDropToIndex(target, "accessories", 8);
                    ...
                    if (fracId == (int)Models.Fractions.CITY || Configs.IsFractionPolic(fracId) ||
                        fracId == (int)Models.Fractions.ARMY ||
                        fracId == (int)Models.Fractions.MERRYWEATHER) cdata.CuffedByCop = true;
                    else cdata.CuffedByMafia = true;

                    Repository.ItemsClose(target, true);
            ...
            {
                Log.Write($"playerPressCuffBut Exception: {e}");
            }
        }
        ...
        public static void giveParamedicLic(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MinorYearsToDoThis), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PriceCanBeFromTo, Main.MinPMLic, Main.MaxPMLic), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PlayerHavePMLic), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PlayerCantAcceptOffers), 3000);
                    return;
                }
                ...
                EventSys.SendCoolMsg(player, "Предложение", "Покупка",
                    $"{LangFunc.GetText(LangType.Ru, DataName.YouOfferedPMLicTo, target.Value, Wallet.Format(price))}",
                    "", 5000);
                Trigger.ClientEvent(target, "openDialog", "PM_LIC",
                    LangFunc.GetText(LangType.Ru, DataName.PlayerOfferedPMLic, player.Value, Wallet.Format(price)));
                targetSessionData.SellItemData.Seller = player;
                targetSessionData.SellItemData.Price = price;
            }
            catch (Exception e)
            {
                Log.Write($"giveParamedicLic Exception: {e}");
            }
        }
        ...
        public static void acceptPmLic(ExtPlayer player)
        {
            try
            {
                ...
                if (player.Position.DistanceTo(seller.Position) > 2)
                {
                    ...
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SellerTooFar), 3000);
                    return;
                }
                ...
                Wallet.Change(seller, price / 20);

                var fractionData = Manager.GetFractionData((int)Models.Fractions.CITY);
                if (fractionData != null)
                    fractionData.Money += Convert.ToInt32(price * 0.95);

                GameLog.Money($"player({characterData.UUID})", "frac(6)", price,
                    $"buyPmlic({sellerCharacterData.UUID})");
                GameLog.Money("frac(6)", $"player({sellerCharacterData.UUID})", price / 20,
                    $"sellPmlic({characterData.UUID})");
                characterData.Licenses[8] = true;
                Table.Logs.Repository.AddLogs(seller, FractionLogsType.GiveGunLic,
                    LangFunc.GetText(LangType.Ru, DataName.GivenPmLicTo, player.Value, characterData.UUID));
                //Chars.Repository.PlayerStats(player);
                GameLog.FracLog(seller.GetFractionId(), sellerCharacterData.UUID, characterData.UUID, seller.Name,
                    player.Name, "givePmLic");
                Notify.Send(seller, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouGivePmLic, player.Name), 3000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouBoughtPmLic, seller.Name), 3000);
                sellerSessionData.SellItemData = new SellItemData();
                sessionData.SellItemData = new SellItemData();
            }
            catch (Exception e)
            {
                Log.Write($"acceptPmLic Exception: {e}");
            }
        }
        ...
        public static void CMD_OpenFractionStock(ExtPlayer player)
        {
            try
            {
                ...
                if (fractionData != null && fractionData.Id >= (int)Models.Fractions.FAMILY)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы открыли склад фракции",
                        3000);
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.OpenStock, "Открыл склад");
                    Manager.sendFractionMessage(fractionData.Id,
            ...
            {
                Log.Write($"CMD_OpenFractionStock Exception: {e}");
            }
        }
        ...
        public static void CMD_fopenstock(ExtPlayer player)
        {
            ...
            {
                Log.Write($"CMD_fopenstock Exception: {e}");
            }
        }
        ...
        public static void CMD_CloseFractionStock(ExtPlayer player)
        {
            try
            {
                ...
                Table.Logs.Repository.AddLogs(player, FractionLogsType.CloseStock, "Закрыл склад");
                Manager.sendFractionMessage(fractionData.Id,
            ...
            {
                Log.Write($"CMD_CloseFractionStock Exception: {e}");
            }
        }
        ...
        public static void CMD_fclosestock(ExtPlayer player)
        {
            ...
            {
                Log.Write($"CMD_fclosestock Exception: {e}");
            }
        }
        ...
        public static void CMD_opengs(ExtPlayer player)
        {
            try
            {
                ...
                if (fractionData.Id == (int)Models.Fractions.CITY || fractionData.Id == (int)Models.Fractions.POLICE ||
                    fractionData.Id == (int)Models.Fractions.FIB || fractionData.Id == (int)Models.Fractions.ARMY ||
                    fractionData.Id == (int)Models.Fractions.SHERIFF)
                {
                    ...
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.OpenStock, "Открыл склад №2");
                    Manager.sendFractionMessage(fractionData.Id,
                        "!{#ADFF2F}[F] " + $"{player.Name} ({player.Value}) открыл склад №2.", true);
                    Trigger.ClientEvent(player, "client.frac.main.isGunStock", true);
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_opengs Exception: {e}");
            }
        }
        ...
        public static void CMD_closegs(ExtPlayer player)
        {
            try
            {
                    ...
                    fractionData.Id == (int)Models.Fractions.SHERIFF)
                {
                    ...
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.CloseStock, "Закрыл склад №2");
                    Manager.sendFractionMessage(fractionData.Id,
                        "!{#ADFF2F}[F] " + $"{player.Name} ({player.Value}) закрыл склад №2.", true);
                    Trigger.ClientEvent(player, "client.frac.main.isGunStock", false);
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_closegs Exception: {e}");
            }
        }
        ...
        public static void SetFracRankOffline(ExtPlayer player, int uuid, int rank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Нельзя установить отрицательный или нулевой ранг", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Игрока не найдено в Вашей семье", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете изменить ранг игроку, у которого ранг выше, чем ваш собственный..", 8000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете установить такой ранг", 3000);
                    return;
                }
                ...
                if (targetMemberFractionData.Rank > rank)
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.SetRank,
                        $"Понизил оффлайн {targetMemberFractionData.Name} ({memberFractionData.UUID}) в должности ({targetMemberFractionData.Rank} -> {rank})");
                else
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.SetRank,
                        $"Повысил оффлайн {targetMemberFractionData.Name} ({memberFractionData.UUID}) в должности ({targetMemberFractionData.Rank} -> {rank})");

                Player.Repository.SetRank(fractionData.Id, uuid, rank);

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы изменили ранг игрока игрока {targetMemberFractionData.Name} на {fractionData.Ranks[rank].Name}",
                    6000);
            }
            catch (Exception e)
            {
                Log.Write($"SetFracRankOffline Exception: {e}");
            }
        }
        ...
        public static void SetFracRank(ExtPlayer player, ExtPlayer target, int newRank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantUpToRank), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете повысить этого игрока", 3000);
                    return;
                }
                ...
                if (targetMemberFractionData.Rank > newRank)
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.SetRank,
                        $"Понизил {target.Name} ({targetMemberFractionData.UUID}) в должности ({targetMemberFractionData.Rank} -> {newRank})");
                else
                    Table.Logs.Repository.AddLogs(player, FractionLogsType.SetRank,
                        $"Повысил {target.Name} ({targetMemberFractionData.UUID}) в должности ({targetMemberFractionData.Rank} -> {newRank})");
                ...
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Теперь вы {Manager.GetFractionRankName(memberFractionData.Id, newRank)} во фракции", 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы изменили ранг игрока {target.Name} на {Manager.GetFractionRankName(memberFractionData.Id, newRank)}",
                    6000);
            }
            catch (Exception e)
            {
                Log.Write($"SetFracRank Exception: {e}");
            }
        }
        ...
        public static void InviteToFraction(ExtPlayer sender, ExtPlayer target)
        {
            try
            {
                ...
                if (memberFractionData == null)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не состоите во фракции",
                        3000);
                    return;
                }
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок слишком далеко от Вас",
                        3000);
                    return;
                }
                   ...
                   )
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Игрок уже состоит в организации", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Необходим как минимум 1 уровень для приглашения игрока во фракцию", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Невозможно принять этого игрока", 3000);
                    return;
                }

                if (Manager.FractionTypes[memberFractionData.Id] == FractionsType.Gov &&
                    !targetCharacterData.Licenses[7])
                {
                ...
                Trigger.ClientEvent(target, "openDialog", "INVITED_FRAC",
                    $"{sender.Name} пригласил Вас в {Manager.FractionNames[memberFractionData.Id]}");

                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы пригласили во фракцию {target.Name}", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"InviteToFraction Exception: {e}");
            }
        }
        ...
        public static void UnInviteFromFraction(ExtPlayer sender, ExtPlayer target, bool mayor = false)
        {
            try
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете выгнать этого игрока", 3000);
                    return;
                }

                Player.Repository.RemoveFractionMemberData(memberFractionData.Id, target.GetUUID());

                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                    $"Вас выгнали из семьи {fractionData.Name}", 3000);
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выгнали из семьи {target.Name}", 3000);
                Table.Logs.Repository.AddLogs(sender, FractionLogsType.UnInvite,
                    $"Выгнал {target.Name} ({target.GetUUID()})");
            }
            catch (Exception e)
            {
                Log.Write($"UnInviteFromFraction Exception: {e}");
            }
        }
        ...
        public static void ticketToTarget(ExtPlayer player, ExtPlayer target, int sum, string reason)
        {
            try
            {
                ...
                var targetCharacterData = target.GetCharacterData();
                if (targetCharacterData == null) return;
                if (memberFractionData.Id == (int)Models.Fractions.CITY &&
                    !SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.Court))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы должны находиться в здании суда", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        $"Ограничение по штрафу {Main.TicketLimit}", 3000);
                    return;
                }
                ...
                if (targetCharacterData.Money < sum && Bank.GetBalance(targetCharacterData.Bank) < sum)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PlayerNotEnoughMoney), 3000);
                    return;
                ...
                var ticketsData = targetSessionData.TicketsData;
                ticketsData.Price = sum;
                ticketsData.Target = player;
                ticketsData.Reason = reason;
                Trigger.ClientEvent(target, "openDialog", "TICKET",
                    $"{player.Name} выписал Вам штраф в размере {sum}$ за {reason}. Оплатить?");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выписали штраф для {target.Name} в размере {sum}$ за {reason}", 3000);
                Table.Logs.Repository.AddLogs(player, FractionLogsType.Ticket,
                    $"Выписал штраф {target.Name} ({targetCharacterData.UUID})");
            }
            catch (Exception e)
            {
                Log.Write($"ticketToTarget Exception: {e}");
            }
        }
        ...
        public static void ticketConfirm(ExtPlayer target, bool confirm)
        {
            try
            {
                ...
                if (targetCharacterData == null) return;
                var ticketsData = targetSessionData.TicketsData;
                var player = ticketsData.Target;
                var sum = ticketsData.Price;
                var reason = ticketsData.Reason;
                ...
                if (confirm)
                {
                    ...
                    if (!Wallet.Change(target, -sum) && !Bank.Change(targetCharacterData.Bank, -sum, false))
                    {
                    ...
                    Wallet.Change(player, Convert.ToInt32(sum * 0.4));

                    EventSys.SendCoolMsg(target, "Штраф", "Успешная оплата",
                        $"Вы оплатили штраф в размере {sum}$ за {reason}", "", 7000);
                    EventSys.SendCoolMsg(player, "Штраф", "Успешная оплата",
                        $"{target.Name} оплатил штраф в размере {sum}$ за {reason}", "", 7000);
                    ...
                    Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                        "!{#FF8C00}[F] " + $"{player.Name} оштрафовал {target.Name} на {sum}$. Причина: {reason}",
                        true);
                    Manager.sendFractionMessage((int)Models.Fractions.SHERIFF,
                        "!{#FF8C00}[F] " + $"{player.Name} оштрафовал {target.Name} на {sum}$ Причина: {reason}", true);
                    GameLog.Ticket(characterData.UUID, targetCharacterData.UUID, sum, reason, player.Name, target.Name);
                }
                else
                {
                    EventSys.SendCoolMsg(target, "Штраф", "Отказ!",
                        $"Вы отказались платить штраф в размере {sum}$ за {reason}", "", 7000);
                    EventSys.SendCoolMsg(player, "Штраф", "Отказ!",
                        $"{target.Name} отказался платить штраф в размере {sum}$ за {reason}", "", 7000);
            ...
            {
                Log.Write($"ticketConfirm Exception: {e}");
            }
        }
        ...
        public static void arrestTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (player == target)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Невозможно применить на себе",
                        3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы должны начать рабочий день",
                        3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы должны быть возле камеры",
                        3000);
                    return;
                }
                ...
                Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                    "!{#FF8C00}[F] " +
                    $"{player.Name} посадил в КПЗ {target.Name} ({targetCharacterData.WantedLVL.Reason})", true);
                Manager.sendFractionMessage((int)Models.Fractions.SHERIFF,
                    "!{#FF8C00}[F] " +
                    $"{player.Name} посадил в КПЗ {target.Name} ({targetCharacterData.WantedLVL.Reason})", true);
                Manager.sendFractionMessage((int)Models.Fractions.FIB,
                    "!{#FF8C00}[F] " +
                    $"{player.Name} посадил в КПЗ {target.Name} ({targetCharacterData.WantedLVL.Reason})", true);
                Table.Logs.Repository.AddLogs(player, FractionLogsType.Arrest,
                    $"Посадил в КПЗ {target.Name} ({targetCharacterData.UUID})");

                target.Eval(
                    "mp.game.audio.playSoundFrontend(-1, \"Mission_Pass_Notify\", \"DLC_HEISTS_GENERAL_FRONTEND_SOUNDS\", true);");

                targetCharacterData.ArrestTime = targetCharacterData.WantedLVL.Level >= 5
                    ? 3600
                    : targetCharacterData.WantedLVL.Level * 10 * 60;
                ...
                var minutes = Convert.ToInt32(targetCharacterData.ArrestTime / 60);
                ...
                EventSys.SendCoolMsg(target, "WASTED", "Арест",
                    $"Игрок ({player.Value}) посадил Вас на {minutes} минут", "", 15000);
                EventSys.SendCoolMsg(player, "WADTED", "Арест",
                    $"Вы посадили игрока ({target.Value}) на {minutes} минут", "", 10000);

                GameLog.Arrest(player.GetUUID(), targetCharacterData.UUID, targetCharacterData.WantedLVL.Reason,
                    targetCharacterData.WantedLVL.Level, player.Name, target.Name);
                GameLog.FracLog(memberFractionData.Id, player.GetUUID(), targetCharacterData.UUID, player.Name,
                    target.Name, $"arrest({targetCharacterData.WantedLVL.Reason})");
                arrestPlayer(target);
            }
            catch (Exception e)
            {
                Log.Write($"arrestTarget Exception: {e}");
            }
        }
        ...
        public static void releasePlayerFromPrison(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы освободили игрока ({target.Value}) из тюрьмы", 3000);
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) освободил Вас из тюрьмы", 3000);
                Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                    "!{#FF8C00}[F] " +
                    $"{player.Name.Replace('_', ' ')} выпустил из КПЗ {target.Name.Replace('_', ' ')}", true);
                Manager.sendFractionMessage((int)Models.Fractions.SHERIFF,
                    "!{#FF8C00}[F] " +
                    $"{player.Name.Replace('_', ' ')} выпустил из КПЗ {target.Name.Replace('_', ' ')}", true);
                Manager.sendFractionMessage((int)Models.Fractions.FIB,
                    "!{#FF8C00}[F] " +
                    $"{player.Name.Replace('_', ' ')} выпустил из КПЗ {target.Name.Replace('_', ' ')}", true);
                Table.Logs.Repository.AddLogs(player, FractionLogsType.Rfp,
                    $"Выпустил из КПЗ {target.Name} ({targetCharacterData.UUID})");
            ...
            {
                Log.Write($"releasePlayerFromPrison Exception: {e}");
            }
        }
        ...
        public static void arrestTimer(ExtPlayer player)
        {
            ...
            {
                Log.Write($"arrestTimer Exception: {e}");
            }
        }
        ...
        public static void freePlayer(ExtPlayer player, bool demorgan)
        {
                ...
                if (sessionData.TimersData.ArrestTimer != null)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            if (demorgan)
                            {
                                ...
                                else
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                        "Наказание было снято, но воспользоваться голосовым чатом Вы сможете после истечения срока Mute.",
                                        10000);
                                }
                            }
                            else
                            ...
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                                "Вы были освобождены из тюрьмы", 3000);
                        }
                        catch (Exception e)
                        {
                            Log.Write($"freePlayer Task Exception: {e}");
                        }
                    });
                }
            }
            ...
            {
                Log.Write($"freePlayer Exception: {e}");
            }
        }
        ...
        public static void arrestPlayer(ExtPlayer player)
        {
            try
            {
                ...
                Repository.RemoveAllIllegalStuff(player);
            }
            catch (Exception e)
            {
                Log.Write($"arrestPlayer Exception: {e}");
            }
        }
        ...
        public static void unCuffPlayer(ExtPlayer player, bool withanim = true)
        {
            ...
            {
                Log.Write($"unCuffPlayer Exception: {e}");
            }
        }
        ...
        public void ClientEvent_playerPressFollow(ExtPlayer player)
        {
            try
            {
                ...
                if (sessionData.Follower != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Вы отпустили игрока ({target.Value})", 3000);
                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                        $"Игрок ({player.Value}) отпустил Вас", 3000);
                    unFollow(player, target);
                }
                else
                {
            ...
            {
                Log.Write($"ClientEvent_playerPressFollow Exception: {e}");
            }
        }
        ...
        public static void unFollow(ExtPlayer player, ExtPlayer target)
        {
            ...
            {
                Log.Write($"unFollow Exception: {e}");
            }
        }
        ...
        public static void targetFollowPlayer(ExtPlayer player, ExtPlayer target, bool force = false)
        {
            try
            {
                ...
                if (Manager.FractionTypes[fracid] == FractionsType.Gov) // for gov factions
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Вы должны сначала начать рабочий день", 3000);
                        return;
                    }
                ...
                if (sessionData.Follower != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы уже тащите за собой игрока",
                        3000);
                    return;
                }
                ...
                Commands.RPChat("sme", player, "повел" + (characterData.Gender ? "" : "а") + " {name} за собой",
                    target);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы потащили за собой игрока ({target.Value})", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) потащил Вас за собой", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"targetFollowPlayer Exception: {e}");
            }
        }
        ...
        public static void targetUnFollowPlayer(ExtPlayer player)
        {
            try
            {
                ...
                if (sessionData.Follower != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы никого не тащите за собой",
                        3000);
                    return;
                }
            ...
            {
                Log.Write($"targetUnFollowPlayer Exception: {e}");
            }
        }
        ...
        public static void carSuPlayer(ExtPlayer player, string number, string reason)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Личной машины с таким номером не существует", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        $"Машина с этим номером уже в розыске, причина: {Police.WantedVehicles[number].Item1}", 3000);
                    return;
                }

                Police.WantedVehicles.Add(number, (reason, vehicleData.Model));
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы объявили транспортное средство [{number}] в розыск", 3000);
                ...
                Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                    "!{#FF8C00}[F] " +
                    $"{player.Name.Replace('_', ' ')} объявил транспортное средство [{number}] в розыск. Причина: {reason}",
                    true);
                Manager.sendFractionMessage((int)Models.Fractions.SHERIFF,
                    "!{#FF8C00}[F] " +
                    $"{player.Name.Replace('_', ' ')} объявил транспортное средство [{number}] в розыск. Причина: {reason}",
                ...
                Table.Logs.Repository.AddLogs(player, FractionLogsType.Su,
                    $"Объявил в розыск т/с {vehicleData.Model} ({number})");
            }
            catch (Exception e)
            {
                Log.Write($"carSuPlayer Exception: {e}");
            }
        }
        ...
        public static void suPlayer(ExtPlayer player, int pasport, int stars, string reason)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Паспорта с таким номером не существует", 3000);
                    return;
                }

                var target = (ExtPlayer)NAPI.Player.GetPlayerFromName(Main.PlayerNames[pasport]);
                var targetCharacterData = target.GetCharacterData();
                if (targetCharacterData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Владелец паспорта должен быть в сети", 3000);
                    return;
                }
                ...
                if (player != target)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Вы не можете выдать такое количество звёзд", 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            "Вы объявили игрока " + target.Name.Replace('_', ' ') + " в розыск", 3000);
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                            $"{player.Name.Replace('_', ' ')} объявил Вас в розыск. Причина: {reason}", 3000);
                        Table.Logs.Repository.AddLogs(player, FractionLogsType.Su,
                            $"Объявил в розыск {target.Name} ({targetCharacterData.UUID})");
                        var oldStars = targetCharacterData.WantedLVL == null ? 0 : targetCharacterData.WantedLVL.Level;
                        ...
                        target.Eval(
                            "mp.game.audio.playSoundFrontend(-1, \"LOOSE_MATCH\", \"HUD_MINI_GAME_SOUNDSET\", true);");
                        GameLog.FracLog(memberFractionData.Id, player.GetUUID(), targetCharacterData.UUID, player.Name,
                            target.Name, $"su({wantedLevel.Level})");
                        ...
                        foreach (var foreachPlayer in Character.Repository.GetPlayers())
                        {
                            ...
                            if (foreachMemberFractionData.Id == (int)Models.Fractions.POLICE ||
                                foreachMemberFractionData.Id == (int)Models.Fractions.FIB)
                                Trigger.SendChatMessage(foreachPlayer,
                                    "!{#FF8C00}[F] " +
                                    $"{player.Name} объявил в розыск {target.Name}. Причина: {reason}");
                        }
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Вы не можете выдать такое кол-во звёзд", 3000);
                    }
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете объявить в розыск самого себя", 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"suPlayer Exception: {e}");
            }
        }
        ...
        public static void playerInCar(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                if (!player.IsFractionAccess(RankToAccess.InCar, false) &&
                    !player.IsOrganizationAccess(RankToAccess.InCar)) return;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Невозможно использовать на себе", 3000);
                    return;
                }
                ...
                if (!targetSessionData.CuffedData.Cuffed)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок должен быть в наручниках",
                        3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы запихали игрока ({target.Value}) в машину", 3000);
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) запихал Вас в машину", 3000);
                Commands.RPChat("sme", player, " открыл дверь и усадил {name} в машину", target);
            }
            catch (Exception e)
            {
                Log.Write($"playerInCar Exception: {e}");
            }
        }
        ...
        public static void playerOutCar(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetSessionData == null) return;
                if (!target.IsCharacterData()) return;
                if (sessionData.StreetRaceTime > DateTime.Now ||
                    targetSessionData.StreetRaceTime > DateTime.Now) return;
                if (Main.IHaveDemorgan(player)) return;
                if (player != target)
                {
                    if (!player.IsFractionAccess(RankToAccess.Pull, false) &&
                        !player.IsOrganizationAccess(RankToAccess.Pull)) return;
                    ...
                    if (player.Position.DistanceTo(target.Position) <= 2f)
                    {
                        if (target.IsInVehicle)
                        {
                            if (player.IsInVehicle)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        "Нельзя выкинуть человека, сидя в другой машине.", 3000);
                                    return;
                                }
                            }

                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                $"Вы выкинули игрока ({target.Value}) из машины", 3000);
                            Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                                $"Игрок ({player.Value}) Выкинул Вас из машины", 3000);
                            VehicleManager.WarpPlayerOutOfVehicle(target);
                            Commands.RPChat("sme", player, " открыл дверь и вытащил {name} из машины", target);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок не в машине",
                                3000);
                        }
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Игрок слишком далеко от Вас", 3000);
                    }
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не можете Выкинуть сами себя из машины", 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"playerOutCar Exception: {e}");
            }
        }
        ...
        public static void setWargPoliceMode(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Общая тревога включается через кнопку, которая находится в вышке Fort Zancudo", 5000);
                    return;
                }
            ...
            {
                Log.Write($"setWargPoliceMode Exception: {e}");
            }
        }
        ...
        public static void takeGunLic(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    if ((!SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.lspd) ||
                         !SafeZones.IsSafeZone(targetSessionData.InsideSafeZone, SafeZones.ZoneName.lspd)) &&
                        (!SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.Sheriffs) ||
                         !SafeZones.IsSafeZone(targetSessionData.InsideSafeZone, SafeZones.ZoneName.Sheriffs)) &&
                        (!SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.govfib) ||
                         !SafeZones.IsSafeZone(targetSessionData.InsideSafeZone, SafeZones.ZoneName.govfib)) &&
                        (!SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.lesnik3) ||
                         !SafeZones.IsSafeZone(targetSessionData.InsideSafeZone, SafeZones.ZoneName.lesnik3)) &&
                        (!SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.Sheriffs2) ||
                         !SafeZones.IsSafeZone(targetSessionData.InsideSafeZone, SafeZones.ZoneName.Sheriffs2)))
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                            "Изымать лицензию на оружие можно только в участке.", 3000);
                        return;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У игрока нет лицензии на оружие", 3000);
                    return;
                }

                GameLog.FracLog(fracId, characterData.UUID, targetCharacterData.UUID, player.Name, target.Name,
                    "takeGunLic");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы отобрали лицензию на оружие у игрока ({target.Value})", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) отобрал у Вас лицензию на оружие", 3000);
                targetCharacterData.Licenses[6] = false;
                Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeGunLic,
                    $"Изъял лицензию на оружие у {target.Value} ({targetCharacterData.UUID})");
            }
            catch (Exception e)
            {
                Log.Write($"takeGunLic Exception: {e}");
            }
        }
        ...
        public static void takeHelLic(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У игрока нет лицензии на вертолёт", 3000);
                    return;
                }

                GameLog.FracLog(player.GetFractionId(), characterData.UUID, targetCharacterData.UUID, player.Name,
                    target.Name, "takeHelLic");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы отобрали лицензию на вертолёт у игрока ({target.Value})", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) отобрал у Вас лицензию на вертолёт", 3000);
                targetCharacterData.Licenses[4] = false;
                Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeHelLic,
                    $"Изъял лицензию на вертолёт у {target.Value} ({targetCharacterData.UUID})");
            }
            catch (Exception e)
            {
                Log.Write($"takeHelLic Exception: {e}");
            }
        }
        ...
        public static void takePlaneLic(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У игрока нет лицензии на самолёт", 3000);
                    return;
                }

                GameLog.FracLog(player.GetFractionId(), characterData.UUID, targetCharacterData.UUID, player.Name,
                    target.Name, "takePlaneLic");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы отобрали лицензию на самолёт у игрока ({target.Value})", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) отобрал у Вас лицензию на самолёт", 3000);
                targetCharacterData.Licenses[5] = false;
                Table.Logs.Repository.AddLogs(player, FractionLogsType.TakePlaneLic,
                    $"Изъял лицензию на самолёт у {target.Value} ({targetCharacterData.UUID})");
            }
            catch (Exception e)
            {
        ...
        public static void giveGunLic(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GunLicLimit, Main.MinGunLic, Main.MaxGunLic), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У игрока уже есть лицензия на оружие", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "В данный момент невозможно предложить этому человеку что-либо", 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(target, "openDialog", "GUN_LIC",
                    $"Игрок ({player.Value}) предложил Вам купить лицензию на оружие за ${Wallet.Format(price)}");
                EventSys.SendCoolMsg(player, "Предложение", "Покупка",
                    $"Вы предложили купить лицензию на оружие игроку ({target.Value}) за ${Wallet.Format(price)}", "",
                    5000);
                targetSessionData.SellItemData.Seller = player;
                targetSessionData.SellItemData.Price = price;
            }
            ...
            {
                Log.Write($"giveGunLic Exception: {e}");
            }
        }
        ...
        public static void acceptGunLic(ExtPlayer player)
        {
            try
            {
                ...
                GameLog.Money($"player({characterData.UUID})", "frac(6)", price,
                    $"buyGunlic({sellerCharacterData.UUID})");
                GameLog.Money("frac(6)", $"player({sellerCharacterData.UUID})", price / 20,
                    $"sellGunlic({characterData.UUID})");
                characterData.Licenses[6] = true;
                qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage33, 2, true);
                qMain.UpdateQuestsComplete(player, Zdobich.QuestName, (int)zdobich_quests.Stage33, true);
                Table.Logs.Repository.AddLogs(seller, FractionLogsType.GivePmLic,
                    $"Выдал лицензию на оружие {player.Value} ({characterData.UUID})");
                //Chars.Repository.PlayerStats(player);
                GameLog.FracLog(seller.GetFractionId(), sellerCharacterData.UUID, characterData.UUID, seller.Name,
                    player.Name, "giveGunLic");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы купили лицензию на оружие у игрока ({seller.Value}) за {Wallet.Format(price)}$", 3000);
                Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) купил у Вас лицензию на оружие", 3000);
            ...
            {
                Log.Write($"acceptGunLic Exception: {e}");
            }
        }
        ...
        public static void playerTakeGuns(ExtPlayer player, ExtPlayer target, bool force = false)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Игрок должен быть связан или с поднятыми руками", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Ограбление возможно только в маске", 3000);
                    return;
                }

                player.AddTableScore(TableTaskId.Item26);

                if (DateTime.Now < sessionData.TimingsData.NextGunRob ||
                    DateTime.Now < targetSessionData.TimingsData.NextGunRob)
                {
                    Commands.RPChat("sme", player, "хорошенько обшарив {name}, ничего не нашёл", target);
                    return;
                }

                var _ItemsToRemove = new List<ItemStruct>();
                ...
                if (Repository.ItemsData.ContainsKey(locationName))
                    foreach (var Location in Repository.ItemsData[locationName].Keys)
                        //if (Location.Key == "accessories") continue;
                    foreach (var itemData in Repository.ItemsData[locationName][Location]) //Todo
                    {
                        var sItem = itemData.Value;
                        var ItemsInfo = Repository.ItemsInfo[sItem.ItemId];
                        ...
                        if (ItemsInfo.functionType == newItemType.Weapons ||
                            ...
                            if (!Repository.HeavyWeapons.Contains(sItem.ItemId) &&
                                !Repository.StrongWeapons.Contains(sItem.ItemId) && sItem.ItemId != ItemId.Flashlight &&
                                sItem.ItemId != ItemId.Crowbar && sItem.ItemId != ItemId.Wrench &&
                                sItem.ItemId != ItemId.Hammer)
                                _ItemsToRemove.Add(new ItemStruct(Location, itemData.Key, sItem));
                    }

                if (Bags != null && Repository.ItemsData.ContainsKey($"backpack_{Bags.SqlId}") &&
                    Repository.ItemsData[$"backpack_{Bags.SqlId}"].ContainsKey("backpack"))
                    foreach (var itemData in Repository.ItemsData[$"backpack_{Bags.SqlId}"]["backpack"]) //Todo
                    {
                        var sItem = itemData.Value;
                        var ItemsInfo = Repository.ItemsInfo[sItem.ItemId];
                        ...
                        if (ItemsInfo.functionType == newItemType.Weapons ||
                            ...
                            if (!Repository.HeavyWeapons.Contains(sItem.ItemId) &&
                                ...
                                sItem.ItemId != ItemId.Crowbar && sItem.ItemId != ItemId.Wrench &&
                                sItem.ItemId != ItemId.Hammer)
                                _ItemsToRemove.Add(new ItemStruct("backpack", itemData.Key, sItem));
                    }
                ...
                var item = _ItemsToRemove[Main.rnd.Next(0, _ItemsToRemove.Count)];
                if (item.Location != "backpack")
                {
                    Repository.ItemsDropToIndex(target, item.Location, item.Index);
                }
                else
                {
                    Repository.ItemsDrop(target,
                        new InventoryItemData(0, item.Item.ItemId, item.Item.Count, item.Item.Data));
                    if (Repository.ItemsData.ContainsKey($"backpack_{Bags.SqlId}") &&
                        Repository.ItemsData[$"backpack_{Bags.SqlId}"].ContainsKey("backpack") &&
                        Repository.ItemsData[$"backpack_{Bags.SqlId}"]["backpack"].ContainsKey(item.Index))
                    {
                        var ItemIdDell = Repository.ItemsData[$"backpack_{Bags.SqlId}"]["backpack"][item.Index].ItemId;
                        Repository.ItemsData[$"backpack_{Bags.SqlId}"]["backpack"][item.Index].ItemId = ItemId.Debug;
                        Repository.UpdateSqlItemData(locationName, "backpack", item.Index,
                            Repository.ItemsData[$"backpack_{Bags.SqlId}"]["backpack"][item.Index], ItemIdDell);
                        Repository.ItemsData[$"backpack_{Bags.SqlId}"]["backpack"].TryRemove(item.Index, out _);

                        Repository.isBackpackItemsData(target, true);
                    }
                }


                var itemName = Repository.ItemsInfo[item.Item.ItemId].Name;
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы украли {itemName} у игрока ({target.Value})", 3000);
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                    $"Игрок ({player.Value}) украл у Вас {itemName}", 3000);
        ...
        public static void playerTakeoffMask(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                var maskItem = Repository.isItem(target, "accessories", ItemId.Mask);
                if (targetSessionData.HeadPocket)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Вы сняли мешок с игрока ({target.Value})", 3000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Игрок ({player.Value}) снял с Вас мешок", 3000);
                    Commands.RPChat("sme", player, "снял" + (characterData.Gender ? "" : "а") + " мешок с {name}",
                        target);
                }
                else if (target.HasSharedData("IS_MASK") && target.GetSharedData<bool>("IS_MASK") && maskItem != null &&
                         Repository.IsBeard(characterData.Gender, maskItem.Item))
                {
                    Repository.RemoveIndex(target, maskItem.Location, maskItem.Index);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        $"Вы сорвали маску с игрока ({target.Value})", 3000);
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                        $"Игрок ({player.Value}) сорвал с Вас маску", 3000);
                    Commands.RPChat("sme", player, " сорвал маску с {name}", target);
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У игрока нет маски/мешка",
                        3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"playerTakeoffMask Exception: {e}");
            }
        }
        ...
        public static void playerHandsupOffer(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !sessionData.HandsUp)
                {
                    if (!targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath &&
                        !targetSessionData.HandsUp)
                    {
                        if (targetSessionData.RequestData.IsRequested)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Вы не можете заставить игрока поднять руки в данный момент", 3000);
                            return;
                        }
                        ...
                        EventSys.SendCoolMsg(target, "Предложение", "Поднять руки",
                            $"Игрок ({player.Value}) заставляет вас поднять руки. Вы желаете подчиниться требованиям и поднять руки? Y/N - принять/отклонить",
                            "", 10000);
                        //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы заставляете игрока ({target.Value}) поднять руки.", 3000);
                        EventSys.SendCoolMsg(player, "Предложение", "Поднять руки",
                            $"Вы заставляете игрока ({target.Value}) поднять руки.", "", 7000);
            ...
            {
                Log.Write($"playerHandsupOffer Exception: {e}");
            }
        }
        ...
        public static void playerHandsupOfferAgree(ExtPlayer player)
        {
            try
            {
                ...
                if (!targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath &&
                    !targetSessionData.HandsUp && !target.IsInVehicle)
                {
                    if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !sessionData.HandsUp &&
                        !player.IsInVehicle)
                    {
            ...
            {
                Log.Write($"playerHandsupOfferAgree Exception: {e}");
            }
        }
        ...
        public static void robberyTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL != 0 || targetCharacterData.LVL < 2 ||
                    targetCharacterData.Money <= 100 || DateTime.Now < targetSessionData.TimingsData.NextRob)
                {
                ...
                var max = targetCharacterData.Money >= Main.PricesSettings.MaxOgrableine
                    ? Main.PricesSettings.MaxOgrableine
                    : Convert.ToInt32(targetCharacterData.Money) - 20;
                var min = max - 20 < 0 ? max : max - 20;
                var found = Main.rnd.Next(min, max + 1);
                if (targetCharacterData.Money >= 10000)
                    Commands.RPChat("sdo", target, "При себе большое количество денег");
                else Commands.RPChat("sdo", target, $"При себе ${Wallet.Format(targetCharacterData.Money)}");
                Wallet.Change(target, -found);
                Wallet.Change(player, found);
            ...
            {
                Log.Write($"robberyTarget Exception: {e}");
            }
        }
        ...
        public static void playerChangePocket(ExtPlayer player, ExtPlayer target, bool force = false)
        {
            try
            {
                ...
                {
                    var aItem = Repository.isItem(player, "inventory", ItemId.Pocket);
                    if (aItem == null)
                    {
                    ...
                    Repository.RemoveIndex(player, aItem.Location, aItem.Index);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Вы надели мешок на игрока ({target.Value})", 3000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Игрок ({player.Value}) надел на Вас мешок", 3000);
                    Commands.RPChat("sme", player, "надел" + (characterData.Gender ? "" : "а") + " мешок на {name}",
                        target);
                }
            }
            catch (Exception e)
            {
                Log.Write($"playerChangePocket Exception: {e}");
            }
        }
        ...
        public static void giveMedicalLic(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У игрока уже есть мед. карта",
                        3000);
                    return;
                }

                targetCharacterData.Licenses[7] = true;
                qMain.UpdateQuestsStage(target, Zdobich.QuestName, (int)zdobich_quests.Stage31, 2, true);
                qMain.UpdateQuestsComplete(target, Zdobich.QuestName, (int)zdobich_quests.Stage31, true);
                Table.Logs.Repository.AddLogs(player, FractionLogsType.GiveMedLic,
                    $"Выдал медицинскую карту {target.Value} ({targetCharacterData.UUID})");
                //Chars.Repository.PlayerStats(target);
                GameLog.FracLog(player.GetFractionId(), characterData.UUID, targetCharacterData.UUID, player.Name,
                    target.Name, "giveMedLic");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выдали игроку {target.Name} медицинскую карту", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"{player.Name} выдал Вам медицинскую карту", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"giveMedicalLic Exception: {e}");
            }
        }
        ...
        public static void giveQr(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (Repository.isItem(target, "inventory", ItemId.Qr) != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouAlreadyHaveQR), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantQrSebe), 3000);
                    return;
                }

                if (Repository.AddNewItem(target, $"char_{targetCharacterData.UUID}", "inventory", ItemId.Qr) == -1)
                {
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter,
                ...
                Table.Logs.Repository.AddLogs(player, FractionLogsType.GiveMedLic,
                    $"Выдал QR код {target.Name}({targetCharacterData.UUID})");
                player.AddTableScore(TableTaskId.Item23);
                GameLog.FracLog(fracId, characterData.UUID, targetCharacterData.UUID, player.Name, target.Name,
                    "giveQr");

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы выдали игроку {target.Name} QR-код", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"{player.Name} выдал Вам QR-код",
                    3000);
        ...
        public static void sellMedKitToTarget(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                if (player.IsFractionAccess(RankToAccess.Medkit))
                {
                    ...
                    if (Repository.isItem(player, "inventory", ItemId.HealthKit) == null || Repository.TradeGet(target))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У Вас нет аптечек с собой",
                            3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Вы должны установить цену от 500$ до 1500$", 3000);
                        return;
                    }
                    ...
                    if (targetCharacterData.Money < price)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У игрока нет столько денег",
                            3000);
                        return;
                    }

                    if ((targetSessionData.SellItemData.Seller != null ||
                         targetSessionData.SellItemData.Buyer != null) && Repository.TradeGet(target))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    ...
                    if (Main.IHaveDemorgan(player, true)) return;
                    Trigger.ClientEvent(target, "openDialog", "PAY_MEDKIT",
                        $"Медик ({player.Value}) предложил купить Вам аптечку за ${Wallet.Format(price)}.");
                    targetSessionData.SellItemData.Seller = player;
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        $"Вы предложили купить игроку ({target.Value}) аптечку за {Wallet.Format(price)}$", 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"sellMedKitToTarget Exception: {e}");
            }
        }
        ...
        public static void healTarget(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                if (player.IsFractionAccess(RankToAccess.Heal))
                {
                    ...
                    if (player.Position.DistanceTo(target.Position) > 2)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок слишком далеко.",
                            3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            $"Вы должны установить цену от {Main.MinHealLimit}$ до {Main.MaxHealLimit}$", 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "У данного человека уже есть предложение.", 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "В данный момент вы не предложить лечение.", 3000);
                        return;
                    }
                    ...
                    if (NAPI.Player.IsPlayerInAnyVehicle(player) && NAPI.Player.IsPlayerInAnyVehicle(target))
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Игрок сидит в другой машине.", 3000);
                            return;
                        }
                        ...
                        if (vehicleLocalData != null)
                        {
                            if (vehicleLocalData.Access == VehicleAccess.Fraction &&
                                vehicleLocalData.Fraction == (int)Models.Fractions.EMS &&
                                (veh.Model == NAPI.Util.GetHashKey("emsnspeedo") ||
                                 veh.Model == NAPI.Util.GetHashKey("emsroamer") ||
                                 veh.Model == NAPI.Util.GetHashKey("vapidse") ||
                                 veh.Model == (uint)VehicleHash.Ambulance || veh.Model == (uint)VehicleHash.Frogger ||
                                 veh.Model == (uint)VehicleHash.Supervolito ||
                                 veh.Model == (uint)VehicleHash.Maverick || veh.Model == (uint)VehicleHash.Lguard))
                            {
                                targetSessionData.SellItemData.Seller = player;
                                targetSessionData.SellItemData.Price = price;
                                targetSessionData.SellItemData.Buyer = target;
                                ...
                                sessionData.SellItemData.Price = price;
                                sessionData.SellItemData.Buyer = target;
                                Trigger.ClientEvent(target, "openDialog", "PAY_HEAL",
                                    $"Медик ({player.Value}) предложил лечение за ${Wallet.Format(price)}");
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                    $"Вы предложили лечение игроку ({target.Value}) за {Wallet.Format(price)}$", 3000);
                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    "Вы сидите не в карете/вертолёте EMS.", 3000);
                            }
                        }
                    }
                    else if (SafeZones.IsSafeZone(sessionData.InsideSafeZone, SafeZones.ZoneName.EMS) &&
                             SafeZones.IsSafeZone(targetSessionData.InsideSafeZone, SafeZones.ZoneName.EMS))
                    {
                        targetSessionData.SellItemData.Seller = player;
                        targetSessionData.SellItemData.Price = price;
                        targetSessionData.SellItemData.Buyer = target;
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Вы должны быть в больнице или карете скорой помощи.", 3000);
                        ;
                    }
                }
            }
