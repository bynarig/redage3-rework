...
namespace NeptuneEvo.Fractions.Player
{
    public static class Repository
    {
                ...
                if (Configs.IsFractionPolic(memberFractionData.Id))
                    lock (Police.WantedVehicles)
                    {
                        if (Police.WantedVehicles.Count >= 1)
                            foreach (var numb in Police.WantedVehicles.Keys)
        ...
        public static int GetFractionId(this ExtPlayer player)
        {
            ...
            return (int)Models.Fractions.None;
        }
        ...
        {
            return player.GetFractionMemberData() != null;
        }

        public static bool IsFractionLeader(this ExtPlayer player, int fracId = (int)Models.Fractions.None)
        {
            var memberFractionData = player.GetFractionMemberData();
            if (memberFractionData != null)
            {
                ...
                if (fractionData != null)
                {
                    if (fracId != (int)Models.Fractions.None && fracId != memberFractionData.Id)
                        return false;
        ...
        public static void UpdateAccess(int fracId, int uuid, List<RankToAccess> access, List<RankToAccess> locks,
            bool isSave = true)
        {
            var memberFractionData = Manager.GetFractionMemberData(uuid, fracId);
            if (memberFractionData == null)
