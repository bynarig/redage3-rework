...
{
    internal class AlcoFabrication : Script
    {
        ...
        private static readonly int[] DrinksPrices = new int[3] { 75, 115, 150 };
        private static readonly int[] DrinksMats = new int[3] { 5, 7, 10 };

        private static readonly Dictionary<int, List<ItemId>> DrinksInClubs = new Dictionary<int, List<ItemId>>
        {
            ...
            {
                (int)Models.Fractions.ARMENIAN,
                new List<ItemId> { ItemId.ArmDrink1, ItemId.ArmDrink2, ItemId.ArmDrink3 }
            }
        };
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(121, "Клуб Bahama Mamas West",
                    new Vector3(-1388.761, -586.3921, 29.09945), 0, true));
                Main.CreateBlip(new Main.BlipData(121, "Клуб Vanilla", new Vector3(141.3792, -1281.576, 28.2172), 0,
                    true));
                Main.CreateBlip(new Main.BlipData(121, "Клуб Tequilla", new Vector3(-564.5512, 275.6993, 81.98249), 0,
                    true));
                //Main.CreateBlip(new Main.BlipData(121, "Клуб Diamond", new Vector3(945.4526, 16.871, 115.0442), 0, true));

                Main.CreateBlip(
                    new Main.BlipData(136, "Кафе Stand Up", new Vector3(-439.601, 271.704, 82.015), 0, true));
                Main.CreateBlip(new Main.BlipData(136, "Китайский ресторан", new Vector3(-158.242, 303.829, 98.822), 0,
                    true));

                Main.CreateBlip(new Main.BlipData(674, "White Company", new Vector3(-858.5887, -424.25665, 84.235405),
                    73, true));
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    ClubsStocks.Add(id,
                        new Stock(Convert.ToInt32(Row["mats"]), Convert.ToInt32(Row["alco1"]),
                            Convert.ToInt32(Row["alco2"]), Convert.ToInt32(Row["alco3"]),
                            Convert.ToSingle(Convert.ToInt32(Row["pricemod"]) / 100),
                            UnloadPoints[id] + new Vector3(0, 0, 0.8)));
                }
                ...
                {
                    CustomColShape.CreateCylinderColShape(pair.Value, 1f, 2, NAPI.GlobalDimension,
                        ColShapeEnums.EnterAlcoShop, pair.Key);

                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1,
                        new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~w~Club\n\"{ClubsNames[pair.Key]}\"",
                        pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true,
                        NAPI.GlobalDimension);
                }
                ...
                {
                    CustomColShape.CreateCylinderColShape(pair.Value, 1f, 2, NAPI.GlobalDimension,
                        ColShapeEnums.ExitAlcoShop, pair.Key);


                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1,
                        new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel("~w~Выход", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0,
                ...
                {
                    CustomColShape.CreateCylinderColShape(pair.Value, 5, 5, NAPI.GlobalDimension,
                        ColShapeEnums.UnloadPoints, pair.Key);

                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 4.5), new Vector3(), new Vector3(), 5,
                        new Color(255, 0, 0, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel("~w~Разгрузить материалы", pair.Value + new Vector3(0, 0, 0.5), 5f,
                        0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                ...
                {
                    CustomColShape.CreateCylinderColShape(pair.Value, 1.5f, 2, NAPI.GlobalDimension,
                        ColShapeEnums.BuyPoints, pair.Key);

                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1,
                        new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel("~w~Купить алкоголь", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f,
        ...
        public static void OnEnterAlcoShop(ExtPlayer player, int index)
        {
                        ...
                        case 10:
                            if (!Main.DoorsControl.ContainsKey("lcn_0") || Main.DoorsControl["lcn_0"])
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.ClubDoorClosed), 3000);
                                return;
                            }
            ...
            {
                Log.Write($"OnEnterAlcoShop Exception: {e}");
            }
        }
        ...
        public static void OnExitAlcoShop(ExtPlayer player, int index)
        {
            ...
            {
                Log.Write($"OnExitAlcoShop Exception: {e}");
            }
        }
        ...
        public static void OnUnloadPoints(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.ClubNotInFraction, Manager.GetName(index)), 3000);
                    return;
                }
                ...
                var club = index;
                var matCount = Repository.getCountItem(VehicleManager.GetVehicleToInventory(player.Vehicle.NumberPlate),
                    ItemId.Material);
                if (matCount == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CarNoMats), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.WarehouseFilled), 3000);
                    return;
                }

                Repository.Remove(null, VehicleManager.GetVehicleToInventory(player.Vehicle.NumberPlate), "vehicle",
                    ItemId.Material, matCount);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.FromCarToClubWarehouse), 3000);
                Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeMedkits,
                    LangFunc.GetText(LangType.Ru, DataName.FtableLoadedFromCarToClubWarehouse, matCount));
            }
            catch (Exception e)
            {
                Log.Write($"OnUnloadPoints Exception: {e}");
            }
        }
        ...
        public static void ResistTimer(ExtPlayer player)
        {
            try
            {
                ...
                var resistData = sessionData.ResistData;
            ...
            {
                Log.Write($"ResistTimer Exception: {e}");
            }
        }
        ...
        public static void SaveAlco()
        {
            try
            {
                foreach (var club in ClubsStocks)
                    try
                    {
                        ...
                        {
                            CommandText =
                                "UPDATE alcoclubs SET alco1=@val0,alco2=@val1,alco3=@val2,pricemod=@val3,mats=@val4 WHERE id=@val5"
                        };
                    ...
                    {
                        Log.Write($"SaveAlco Foreach Exception: {e}");
                    }
            ...
            {
                Log.Write($"SaveAlco Exception: {e}");
            }
        }
        ...
        internal class Stock
        {
            ...
            {
                Label = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(
        ...
        public static void OnBuyPoints(ExtPlayer player, int club)
        {
            ...
            {
                Log.Write($"OpenBuyAlcoholMenu Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_alcoMenu(ExtPlayer player, int action, int index)
        {
            try
            {
                ...
                var alcoCounts = new List<int>
                    { ClubsStocks[fractionId].Alco1, ClubsStocks[fractionId].Alco2, ClubsStocks[fractionId].Alco3 };
                var invItem = DrinksInClubs[fractionId][index];
                ...
                switch (action)
                {
                    case 0: // buy
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Недостаточно товара",
                                3000);
                            return;
                        }

                        if (Repository.isFreeSlots(player, invItem) != 0) return;
                        var price = Convert.ToInt32(DrinksPrices[index] * ClubsStocks[fractionId].PriceModifier);
                        ...
                        Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", invItem);
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"Вы купили {Repository.ItemsInfo[invItem].Name}", 3000);
                        return;
                    case 1: // take
                        if (alcoCounts[index] <= 0)
                        {
                        ...
                        if (Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", invItem) == -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        ...
                        //OpenBuyAlcoholMenu(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"Вы взяли {Repository.ItemsInfo[invItem].Name}. На складе {alcoCounts[index] - 1}шт",
                            3000);
                        return;
                    case 2: // craft
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                $"На складе максимум {Repository.ItemsInfo[invItem].Name}", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "На складе недостаточно материалов", 3000);
                            return;
                        }
                        ...
                        //OpenBuyAlcoholMenu(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"Вы скрафтили {Repository.ItemsInfo[invItem].Name}. На складе {alcoCounts[index] + 1}шт",
                            3000);
                        return;
                    case 3: // setprice
                        Trigger.ClientEvent(player, "openInput", "Установить цену",
                            "Введите цену для алкоголя в процентах", 3, "club_setprice");
                        return;
                }
            }
            catch (Exception e)
            {
                Log.Write($"RemoteEvent_alcoMenu Exception: {e}");
            }
        }
        ...
        public static void SetAlcoholPrice(ExtPlayer player, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Установите цену от 50% до 150%",
                        3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы изменили цену алкогольной продукции до {price}%", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"SetAlcoholPrice Exception: {e}");
            }
        }
