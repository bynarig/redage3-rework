    ...
    public class Repository
    {
        ...
        public static void UpdateRankScore(ExtPlayer player, int id, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        $"Минимальное значение {NeptuneEvo.Table.Repository.MinScore} максимальное {NeptuneEvo.Table.Repository.MaxScore}",
                        4500);
                    return;
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Logs.Repository.AddLogs(player, FractionLogsType.UnInvite,
                        $"Выгнал {targetMemberFractionData.Name} ({targetMemberFractionData.UUID})");
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.TiUvolil, targetMemberFractionData.Name), 3000);
                }
