...
{
    internal class Sheriff : Script
    {
        private static readonly nLog Log = new nLog("Fractions.Sheriff");

        private static int PedSecretaryId;
        private static int PedSecondSecretaryId;

        private static readonly Vector3 FirstArrestPosition = new Vector3(1860.3392, 3692.5254, 30.25941);
        private static readonly Vector3 SecondArrestPosition = new Vector3(-444.3758, 6011.2217, 27.985634);
        ...
        private static readonly Vector3 CloakroomSpecialPosition = new Vector3(1848.5563, 3695.9812, 34.26695);
        private static readonly Vector3 VehicleBoostPosition = new Vector3(1866.0756, 3698.5723, 33.59682);
        ...
        private static readonly Vector3 SecondCloakroomSpecialPosition = new Vector3(-456.57333, 6013.906, 31.7);
        private static readonly Vector3 SecondStockPosition = new Vector3(-437.08975, 5997.4136, 31.7);
        private static readonly Vector3 VehicleRepairPosition = new Vector3(-448.36835, 5994.5537, 31.32247);
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                CustomColShape.CreateCylinderColShape(FirstArrestPosition, 6, 3, 0, ColShapeEnums.FractionSheriffArrest,
                    1);
                CustomColShape.CreateCylinderColShape(SecondArrestPosition, 7, 3, 0,
                    ColShapeEnums.FractionSheriffArrest, 2);
                ...
                CustomColShape.CreateCylinderColShape(CloakroomSpecialPosition, 1, 2, 0, ColShapeEnums.FractionSheriff,
                    3);
                ...
                CustomColShape.CreateCylinderColShape(SecondCloakroomPosition, 1, 2, 0, ColShapeEnums.FractionSheriff,
                    2);
                CustomColShape.CreateCylinderColShape(SecondCloakroomSpecialPosition, 1, 2, 0,
                    ColShapeEnums.FractionSheriff, 3);
                CustomColShape.CreateCylinderColShape(SecondStockPosition, 1, 2, 0, ColShapeEnums.FractionSheriff, 5);

                NAPI.Marker.CreateMarker(30, SecondCloakroomPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));
                NAPI.Marker.CreateMarker(30, SecondCloakroomSpecialPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));
                NAPI.Marker.CreateMarker(20, SecondStockPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));

                CustomColShape.CreateCylinderColShape(VehicleRepairPosition, 3, 5, 0, ColShapeEnums.FractionSheriff,
                    7); // repair 
                ...
                Ped ped = Repository.CreateQuest("s_f_y_sheriff_01", new Vector3(1852.9309, 3688.9907, 34.266937),
                    -150.65309f, title: "~y~NPC~w~ Виктория Ньютон\nСекретарь",
                    colShapeEnums: ColShapeEnums.FracSheriff);
                PedSecretaryId = ped.Value;

                Ped secondPed = Repository.CreateQuest("s_f_y_sheriff_01", new Vector3(-448.6393, 6012.3833, 31.7),
                    -45.60703f, title: "~y~NPC~w~ Виктория Элиз\nСекретарь", colShapeEnums: ColShapeEnums.FracSheriff);
                PedSecondSecretaryId = secondPed.Value;

                Repository.CreateQuest("s_m_m_fibsec_01", FirstGunsPosition, -87.25532f,
                    title: "~y~NPC~w~ Агент Александр\nСкладской", colShapeEnums: ColShapeEnums.FracSheriff);
                Repository.CreateQuest("s_m_m_fibsec_01", SecondGunsPosition, 40.875225f,
                    title: "~y~NPC~w~ Агент Роуз\nСкладской", colShapeEnums: ColShapeEnums.FracSheriff);
                Repository.CreateQuest("s_m_m_armoured_02", new Vector3(-444.31284, 6010.7974, 31.716427), 47.103275f,
                    title: "~y~NPC~w~ Сержант Соболь\nВызвать сотрудника",
                    colShapeEnums: ColShapeEnums.CallSheriffMember);
            }
            catch (Exception e)
            {
        ...
        public static void OpenCallSheriffMemberDialog(ExtPlayer player, int _)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "CallSheriffMemberDialog",
                    LangFunc.GetText(LangType.Ru, DataName.AreYouWantToCallGov));
            }
            catch (Exception e)
            {
                Log.Write($"OpenCallSheriffMemberDialog Exception: {e}");
            }
        }
        ...
        public static void OnFractionSheriff(ExtPlayer player, int interact)
        {
            try
            {
                ...
                switch (interact)
                {
                    ...
                    case 5:
                        ...
                        Chars.Repository.LoadOtherItemsData(player, "Fraction", "18", 5,
                    ...
                    case 7:
                        ...
                        {
                            var ticks = Main.NextFixcarSheriffVeh.Ticks - DateTime.Now.Ticks;
                            if (ticks <= 0) return;
                        ...
                        if (vehicleLocalData != null)
                        {
                            if (vehicleLocalData.Access == VehicleAccess.Fraction &&
                                vehicleLocalData.Fraction == (int)Models.Fractions.SHERIFF)
                            {
            ...
            {
                Log.Write($"OnFractionSheriff Exception: {e}");
            }
        }
        ...
        [Interaction(ColShapeEnums.FractionSheriffArrest, true)]
        public static void InFractionSheriffArrest(ExtPlayer player, int index)
        {
            ...
            {
                Log.Write($"InFractionSheriffArrest Exception: {e}");
            }
        }
        ...
        public static void OutFractionSheriffArrest(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OutFractionSheriffArrest Exception: {e}");
            }
        }
        ...
        public static void OpenSpecialPoliceMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (player.Position.DistanceTo(CloakroomSpecialPosition) >= 5 &&
                    player.Position.DistanceTo(SecondCloakroomSpecialPosition) >= 5)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
