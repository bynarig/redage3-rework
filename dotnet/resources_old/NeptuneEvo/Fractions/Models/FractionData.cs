...
namespace NeptuneEvo.Fractions.Models
{
    public class FractionData : TableData
    {
        ...
        public Dictionary<int, List<RankToAccess>> RanksDefaultAccess { get; set; } =
            new Dictionary<int, List<RankToAccess>>();
        ...
        public void UpdateLabel()
        {
            ...
            if (Drugs == 0 && Materials == 0 && MedKits == 0)
            {
                text += "Склад пуст";
            }
            else
        ...
        public bool IsLeader(int rank)
        {
            return rank == LeaderRank();
        }
                ...
                try
                {
                    ...
                    foreach (var rankData in Ranks)
                        await db.Fractionranks
                            .Where(v => v.Fraction == Id && v.Rank == rankData.Key)
        ...
        public void SaveRank(int id)
        {
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    var rankData = Ranks[id];

                    await db.Fractionranks
                        .Where(v => v.Fraction == Id && v.Rank == id)
        ...
        public void SaveTasksData()
        {
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    await db.Fractions
                        .Where(v => v.Id == Id)
                        .Set(v => v.TasksData, JsonConvert.SerializeObject(TasksData))
                        .UpdateAsync();
                }
                catch (Exception e)
                {
