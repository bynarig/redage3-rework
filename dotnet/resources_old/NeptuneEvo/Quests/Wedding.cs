...
namespace NeptuneEvo.Quests
{
    internal class Wedding : Script
    {
        ...
        public void onResourceStart()
        {
            ...
            Repository.CreateQuest("cs_priest", new Vector3(-786.86f, -708.91, 30.322592), -94f,
                title: "~y~NPC~w~ Отец Михаил\nСвященник", colShapeEnums: ColShapeEnums.QuestWedding);
        }
        ...
        public static void Perform(ExtPlayer player)
        {
            ...
            {
                type = 1;
                var target = Main.GetPlayerByUUID(characterData.WeddingUUID);
        ...
        public void OpenDialog(ExtPlayer player, string name, int typeSurname)
        {
            ...
            if (characterData.WeddingUUID == 0)
            {
                ...
                if (typeSurname != 0 && typeSurname != 2 &&
                    Main.PlayerNames.Values.Contains($"{characterData.FirstName}_{targetCharacterData.LastName}"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
            ...
            switch (type)
            {
                case 0:
                    if (typeSurname == 0 || typeSurname == 2)
                        Trigger.ClientEvent(player, "openDialog", "Wedding",
                            LangFunc.GetText(LangType.Ru, DataName.YouWantWedding, target.Name, Wallet.Format(price)));
                    else
                        Trigger.ClientEvent(player, "openDialog", "Wedding",
                            LangFunc.GetText(LangType.Ru, DataName.YouWantWeddingFamily, target.Name,
                                Wallet.Format(price), target.Name.Split("_")[1]));
                    break;
                case 1:
                    Trigger.ClientEvent(player, "openDialog", "Wedding",
                        LangFunc.GetText(LangType.Ru, DataName.YouDivorce, target.Name, Wallet.Format(price)));
                    break;
                case 2:
                    Trigger.ClientEvent(player, "openDialog", "Wedding",
                        LangFunc.GetText(LangType.Ru, DataName.YouDivorceSolo, Wallet.Format(price)));
                    break;
            }
        }
        ...
        public static void OnBoda(ExtPlayer player, ExtPlayer target, int typeSurname)
        {
            ...
            if (player.Position.DistanceTo(target.Position) > 5f)
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouTooFar), 3000);
                return;
            }

            if (typeSurname == 0 &&
                Main.PlayerNames.Values.Contains($"{targetCharacterData.FirstName}_{characterData.LastName}"))
            {
            ...
            var targetUUID = targetCharacterData.UUID;
            ...
            if (characterData.LastName != targetCharacterData.LastName)
            {
                if (typeSurname == 0)
                {
                    var newName = $"{targetCharacterData.FirstName}_{characterData.LastName}";
                    Character.Change.Repository.ChangeName(target, newName);
                }
                else if (typeSurname == 1)
                {
                    var newName = $"{characterData.FirstName}_{targetCharacterData.LastName}";
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.WeddingSuc, target.Name), 10000);
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.WeddingSuc, player.Name), 10000);
            NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.SucWedding, ChatColors.AMP,
        ...
        public static void OnDivorcio(ExtPlayer player, bool isOne = false)
        {
            ...
            if (characterData == null || characterData.WeddingUUID == 0)
                return;
            var price = isOne ? DivorcioOnePrice : DivorcioPrice;
            ...
            var uuid = characterData.WeddingUUID;
            ...
            if (targetCharacterData != null)
            {
                ...
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YourDivorced, player.Name), 10000);
            }
            else
            {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.YouDivorced, target.Name), 10000);
        }
    }
}