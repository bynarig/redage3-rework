...
namespace NeptuneEvo.Quests
{
    ...
    internal class Granny : Script
    {
        ...
        public static granny_quests Get(ExtPlayer player, int Line, bool Reward = false)
        {
            ...
            switch ((granny_quests)Line)
            {
                ...
                case granny_quests.Beer:
                    var wItemStruct = Repository.isItem(player, "inventory", ItemId.Beer);
                    if (wItemStruct != null)
                    {
                        Repository.RemoveIndex(player, wItemStruct.Location, wItemStruct.Index);
                        return granny_quests.Introduction;
                    }

                    break;
                case granny_quests.Bear:
                    if (Repository.getCountItem($"char_{characterData.UUID}", ItemId.Bear, false) >= 2)
                    {
                        Repository.Remove(player, $"char_{characterData.UUID}", "inventory", ItemId.Bear, 2);
        ...
        public static void Take(ExtPlayer player, int Line)
        {
            ...
            switch ((granny_quests)Line)
            {
                ...
                case granny_quests.Introduction:
                    qMain.UpdateQuestsLine(player, "npc_granny", (int)granny_quests.Introduction,
                        (int)granny_quests.Bear);
        ...
        public static void Open(ExtPlayer player, int index)
        {
            ...
            var isBool = qMain.SetQuests(player, "npc_granny");
            if (!isBool) return;
            var questData = player.GetQuest();
            if (questData == null)
                return;
            ...
            {
                var isItem = Repository.isItem(player, "inventory", ItemId.Beer) != null ? true : false;
                if (isItem)
            ...
            {
                var isItem = Repository.getCountItem($"char_{characterData.UUID}", ItemId.Bear, false) >= 5
                    ? true
                    : false;
                if (isItem)
                {
            ...
            Trigger.ClientEvent(player, "client.quest.open", index, "npc_granny", questData.Line, questData.Status,
                questData.Complete);
        }
        ...
        public void take_quest_item(ExtPlayer player, int index)
        {
            ...
            if (Repository.isFreeSlots(player, ItemId.Bear) != 0) return;
            ...
            Timers.StartOnce(5000, () =>
            {
                try
                {
                    ...
                    if (Repository.isFreeSlots(player, ItemId.Bear) != 0) return;
                    Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Bear);
                    ...
                    if (index >= 2)
                        ...
                        Trigger.SendChatMessage(player,
                            "Вы собрали 1/2 Мишек. Остался последний Мишка. На карте отмечено следующее примерное расположение последнего Мишки!");
                }
                catch (Exception e)
                {
