...
namespace NeptuneEvo.Houses
{
    internal class Hotel : Script
    {
        ...
        private static readonly Vector3[] CarsGet = new Vector3[4]
        {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (var pos in HotelEnters)
                {
                    ...
                    Main.CreateBlip(
                        new Main.BlipData(475, LangFunc.GetText(LangType.Ru, DataName.Hotel), pos, 16, true));

                    CustomColShape.CreateCylinderColShape(pos, 1.5f, 5f, 0, ColShapeEnums.EnterHotel, HotelID);

                    NAPI.Marker.CreateMarker(1, pos - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f,
                        new Color(255, 255, 255, 220));
                    NAPI.TextLabel.CreateTextLabel("~w~Отель", pos + new Vector3(0, 0, 0.5), 5f, 0.4f, 0,
                        new Color(255, 255, 255));

                    CustomColShape.CreateCylinderColShape(InteriorDoor, 1.5f, 3, (uint)HotelID, ColShapeEnums.ExitHotel,
                        HotelID);
                    NAPI.Marker.CreateMarker(1, InteriorDoor, new Vector3(), new Vector3(), 1, new Color(255, 255, 255),
                        false, (uint)HotelID);
                ...
                foreach (var pos in CarsGet)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pos - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f,
                        new Color(255, 255, 255, 220));
                    NAPI.TextLabel.CreateTextLabel("~w~Скутер Отеля", pos + new Vector3(0, 0, 0.5), 5f, 0.4f, 0,
                        new Color(255, 255, 255));
                    HotelID++;
                }
            }
        ...
        public static void OnCarRentHotel(ExtPlayer player, int Index)
        {
            ...
            if (player.IsInVehicle)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    "Невозможно использовать, сидя в машине.", 3000);
                return;
            }
            ...
            if (Index == characterData.HotelID)
            {
                ...
                {
                    var hotveh = sessionData.HotelData.Car;
                    VehicleStreaming.DeleteVehicle(hotveh);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы успешно отдали скутер.",
                        3000);
                    sessionData.HotelData.Car = null;
                    return;
                }

                var vehicle = VehicleStreaming.CreateVehicle((uint)VehicleHash.Faggio2, player.Position, player.Heading,
                    0, 0, "HOTEL", engine: true, locked: true, acc: VehicleAccess.Hotel, petrol: 100);
        ...
        public static void SendToRoom(ExtPlayer player, int Index)
        {
            ...
            {
                Log.Write($"SendToRoom Exception: {e}");
            }
        }
        ...
        public static void ExtendHotelRent(ExtPlayer player, int hours)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не поселены ни в один отель",
                        3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Аренда может быть оплачена только на 10 часов", 3000);
                    return;
                }

                var price = Main.HotelRent * hours;
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы продлили аренду на {hours} часов, Вас выселят через {characterData.HotelLeft} часов", 10000);
            }
            catch (Exception e)
            {
                Log.Write($"ExtendHotelRent Exception: {e}");
            }
        }
        ...
        public static void MoveOutPlayer(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.InsideHotelID != -1)
                    NAPI.Entity.SetEntityPosition(player,
                        HotelEnters[characterData.InsideHotelID] + new Vector3(0, 0, 1.12));
            ...
            {
                Log.Write($"MoveOutPlayer Exception: {e}");
            }
        }
        ...
        public static void OpenHotelBuyMenu(ExtPlayer player)
        {
            try
            {
                ...
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.HotelRent, Main.HotelRent),
                    "rent"));
            ...
            {
                Log.Write($"OpenHotelBuyMenu Exception: {e}");
            }
        }
        ...
        private static void callback_hotelbuy(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                switch (listItem)
                {
                    case "rent":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Вы проживаете в доме и не можете арендовать комнату в отеле", 3000);
                            return;
                        }

                        if (UpdateData.CanIChange(player, Main.HotelRent, true) != 255) return;
                        Wallet.Change(player, -Main.HotelRent);
                        var HotelId = CustomColShape.GetDataToEnum(player, ColShapeEnums.EnterHotel);
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            "Вы арендовали комнату в отеле на 1ч. Продлить аренду можно в телефоне (M)", 3000);
            ...
            {
                Log.Write($"callback_hotelbuy Exception: {e}");
            }
        }
        ...
        public static void OpenHotelManageMenu(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OpenHotelManageMenu Exception: {e}");
            }
        }
        ...
        private static void callback_hotelmanage(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                {
                    case "extend":
                        Trigger.ClientEvent(player, "openInput", $"Продлить аренду ({Main.HotelRent}$/ч)",
                            "Введите количество часов", 1, "extend_hotel_rent");
                        return;
                    case "moveout":
                        MoveOutPlayer(player);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Вы выселились из отеля",
                            3000);
                        return;
                }
            }
            ...
            {
                Log.Write($"callback_hotelmanage Exception: {e}");
            }
        }
    }
}