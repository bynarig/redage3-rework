...
namespace NeptuneEvo.Houses
{
    ...
    internal class GarageType
    {
        public GarageType(Vector3 position, List<Vector3> vehiclesPositions, List<Vector3> vehiclesRotations,
            int maxCars, int price = -1, bool isDonate = false)
        {
            Position = position;
        ...
        [JsonIgnore] public List<Vector3> VehiclesPositions { get; }

        [JsonIgnore] public List<Vector3> VehiclesRotations { get; }
    ...
    public class Garage
    {
        [JsonIgnore] public ConcurrentDictionary<int, int> CarSlots = new ConcurrentDictionary<int, int>();

        [JsonIgnore] private ExtMarker IntMarker;

        [JsonIgnore] private ExtColShape IntShape;

        [JsonIgnore] public bool IsSave;

        [JsonIgnore] private bool Locked;
        ...
        [JsonIgnore] public int BDType { get; set; }

        [JsonIgnore] public bool Upgraded { get; set; }
        ...
        public int UpdateCarSlot(int autoId)
        {
            try
            {
                ...
                {
                    key = -1;
                    for (var i = 0; i < garageType.MaxCars; i++)
            ...
            {
                GarageManager.Log.Write($"UpdateCarSlot Exception: {e}");
                return -1;
            }
        }
        ...
        public bool IsGarageToNumber(int autoId)
        {
            ...
            var vehicle = Repository.GetVehicleToNumber(VehicleAccess.Personal, vehicleData.Number);
            if (vehicle == null)
                return false;
        ...
        public void DeleteCar(string number, bool resetPosition = true, bool isRevive = false)
        {
            try
            {
                ...
                var vehiclePlayer = Repository.GetVehicleToNumber(VehicleAccess.Personal, number);
            ...
            {
                GarageManager.Log.Write($"DeleteCar Exception: {e}");
            }
        }
        ...
        public void CreateShape()
        {
            try
            {
                ...
                Shape = CustomColShape.CreateCylinderColShape(Position - new Vector3(0, 0, 1), 2f, 4f, 0,
                    ColShapeEnums.EnterGarage, Id);
            }
            catch (Exception e)
            {
                GarageManager.Log.Write($"CreateShape Exception: {e}");
            }
        }
        ...
        public void Create()
        {
            Trigger.SetTask(async () =>
            {
                ...
                {
                    GarageManager.Log.Write($"Create Exception: {e}");
                }
            });
        }
        ...
        public async Task Save(ServerBD db)
        {
            ...
            {
                GarageManager.Log.Write($"Save Exception: {e}");
            }
        }
        ...
        public void Delete()
        {
            Trigger.SetTask(async () =>
            {
                ...
                {
                    GarageManager.Log.Write($"Delete Exception: {e}");
                }
            });
        }
        ...
        public void Destroy(bool withShape = true)
        {
            ...
            {
                GarageManager.Log.Write($"Destroy Exception: {e}");
            }
        }
        ...
        public void SpawnCar(string number, bool check = false)
        {
            try
            {
                ...
                var vehicle = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vehicleData.Model),
                    ...
                    number, dimension: (uint)Dimension, locked: true, engine: false, acc: VehicleAccess.Garage,
                    petrol: vehicleData.Fuel, dirt: vehicleData.Dirt);
            ...
            {
                GarageManager.Log.Write($"SpawnCar Exception: {e}");
            }
        }
        ...
        public void SpawnCars(List<string> vehiclesNumber, House house = null)
        {
            try
            {
                ...
                if (house != null)
                {
                    ...
                    {
                        var vehicleData = VehicleManager.GetVehicleToAutoId(carSlot.Value);
                        if (vehicleData == null || !vehiclesNumber.Contains(vehicleData.Number) ||
                            (vehicleData.Holder != house.Owner && !house.Roommates.ContainsKey(vehicleData.Holder)))
                        {
                            if (vehicleData != null)
                                DestroyCar(vehicleData.Number);
            ...
            {
                GarageManager.Log.Write($"SpawnCars Exception: {e}");
            }
        }
        ...
        public void SpawnCarToPos(string vehicleNumber, House house = null)
        {
            try
            {
                ...
                if (Repository.IsVehicleToNumber(VehicleAccess.Personal, vehicleData.Number))
                    return;

                if (house != null && house.Roommates.ContainsKey(vehicleData.Holder) &&
                    !CarSlots.Values.Contains(vehicleData.SqlId))
                    return;

                if (!string.IsNullOrEmpty(vehicleData.Position))
                {
                    SpawnCarAtPosition(vehicleData.Number, JsonConvert.DeserializeObject<Vector3>(vehicleData.Position),
                        JsonConvert.DeserializeObject<Vector3>(vehicleData.Rotation));
                }
                else
                {
                    if (Type != -1 && Type != 6)
            ...
            {
                GarageManager.Log.Write($"SpawnCarToPos Exception: {e}");
            }
        }
        ...
        public void SpawnCarAtPosition(string number, Vector3 position, Vector3 rotation)
        {
            try
            {
                ...
                var vehicle = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vehicleData.Model), position,
                    rotation.Z, 0, 0, number, petrol: vehicleData.Fuel, acc: VehicleAccess.Personal, locked: true,
                    dirt: vehicleData.Dirt);
                VehicleManager.ApplyCustomization(vehicle);
            }
            catch (Exception e)
            {
                GarageManager.Log.Write($"SpawnCarAtPosition Exception: {e}");
            }
        }
        ...
        public void GetVehicleFromGarage(string number, ExtPlayer sender = null)
        {
            try
            {
                ...
                if (Type != -1 && Type != 6) // Other Garages
                {
                    ...
                    var vehicle = Repository.GetVehicleToNumber(VehicleAccess.Personal, number);
                    var vehicleLocalData = vehicle.GetVehicleLocalData();
                ...
                {
                    var vehicle = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vehicleData.Model),
                        Position + new Vector3(0, 0, 0.3), Rotation.Z, 0, 0, number, petrol: vehicleData.Fuel,
                        acc: VehicleAccess.Personal, dirt: vehicleData.Dirt);
            ...
            {
                GarageManager.Log.Write($"GetVehicleFromGarage Exception: {e}");
            }
        }
        ...
        public void SendVehicleIntoGarage(string number)
        {
            ...
            {
                GarageManager.Log.Write($"SendVehicleIntoGarage Exception: {e}");
            }
        }
        ...
        public void SendPlayer(ExtPlayer player)
        {
            ...
            {
                GarageManager.Log.Write($"SendPlayer Exception: {e}");
            }
        }
        ...
        public void RemovePlayer(ExtPlayer player)
        {
            ...
            {
                GarageManager.Log.Write($"RemovePlayer Exception: {e}");
            }
        }
        ...
        public void SendVehiclesInsteadNearest(List<string> vehiclesNumber)
        {
            ...
            {
                GarageManager.Log.Write($"SendVehiclesInsteadNearest Exception: {e}");
            }
        }
            ...
            try
            {
                if (Type != 7)
                    IntMarker = (ExtMarker)NAPI.Marker.CreateMarker(1,
                        GarageManager.GarageTypes[Type].Position - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(),
                        2f, new Color(255, 255, 255, 220), false, (uint)Dimension);

                IntShape = CustomColShape.CreateCylinderColShape(
                    GarageManager.GarageTypes[Type].Position - new Vector3(0, 0, 1.12), 2f, 4f, (uint)Dimension,
                    ColShapeEnums.ExitGarage);
            }
            catch (Exception e)
            {
                GarageManager.Log.Write($"CreateInterior Exception: {e}");
            }
        }
    }
    ...
    internal class GarageManager : Script
    {
            ...
            { -1, new GarageType(new Vector3(), new List<Vector3>(), new List<Vector3>(), 1) },
            {
                0, new GarageType(new Vector3(178.9925, -1005.661, -99.5995),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(-0.1147747, 0.02747092, 183.3471)
                    }, 2, Main.PricesSettings.GaragesPrice[0])
            },
            {
                1, new GarageType(new Vector3(206.9094, -999.0917, -100),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(-0.1146501, -0.03047129, 165.095),
                        new Vector3(-0.1146501, -0.03047129, 165.095)
                    }, 3, Main.PricesSettings.GaragesPrice[1])
            },
            {
                2, new GarageType(new Vector3(206.9094, -999.0917, -100),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(-0.1146501, -0.03047129, 165.095),
                        new Vector3(-0.1146501, -0.03047129, 165.095)
                    }, 4, Main.PricesSettings.GaragesPrice[2])
            },
            {
                3, new GarageType(new Vector3(206.9094, -999.0917, -100),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(-0.1146501, -0.03047129, 165.095),
                        new Vector3(-0.1146501, -0.03047129, 165.095)
                    }, 5, Main.PricesSettings.GaragesPrice[3])
            },
            {
                4, new GarageType(new Vector3(206.9094, -999.0917, -100),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(-0.1146501, -0.03047129, 165.095),
                        new Vector3(-0.1146501, -0.03047129, 165.095)
                    }, 6, Main.PricesSettings.GaragesPrice[4])
            },
            {
                5, new GarageType(new Vector3(240.411, -1004.753, -100),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(0, 0, 91.82264),
                        new Vector3(0, 0, 91.82264)
                    }, 10, Main.PricesSettings.GaragesPrice[5])
            },
            { 6, new GarageType(new Vector3(), new List<Vector3>(), new List<Vector3>(), 1) },
            {
                7, new GarageType(new Vector3(-1396.274, -480.7186, 57.100), // Херня в эдиторе
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(0.0, 0.0, 122.8532),
                        new Vector3(0.0, 0.0, 74.71387)
                    }, 15, Main.PricesSettings.GaragesPrice[6], true)
            },
            {
                8, new GarageType(new Vector3(1295.379, 264.5836, -49.875),
                    ...
                    new List<Vector3>
                    {
                        ...
                        //
                        new Vector3(0, 0, 90.8486) //7
                    }, 23, Main.PricesSettings.GaragesPrice[7], true)
            },
            {
                9, new GarageType(new Vector3(1380.1096, 259.4516, -48.99376),
                    ...
                    new List<Vector3>
                    {
                        ...
                        new Vector3(0, 0, -88.9513), //35
                        new Vector3(0, 0, -88.9513) //36
                    }, 38, Main.PricesSettings.GaragesPrice[8], true)
            }
        };
        ...
        public static async void Init()
        {
            try
            {
                ...
                foreach (var garageData in garages)
                {
                    ...
                    garage.CarSlots =
                        JsonConvert.DeserializeObject<ConcurrentDictionary<int, int>>(garageData.CarSlots);
        ...
        public static void OnEnterGarage(ExtPlayer player, int index)
        {
            ...
            if (player.IsInVehicle)
            {
                ...
                if (!garage.IsCarNumber(vehicleData.SqlId))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantEnterGarageVeh), 3000);
                    return;
                }
            ...
            if (garage.Type == -1 || garage.Type == 6)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveNoCars), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CarInState), 3000);
                    return;
                }
        ...
        private void UpdateCarSlots(ExtPlayer player, int autoId, int index)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.GarageError), 3000);
                return;
            }
            ...
            if (garage.CarSlots.ContainsKey(index))
            {
                ...
                var vehicle1 = Repository.GetVehicleToNumber(VehicleAccess.Personal, vehicleData1.Number);
                ...
                var vehicle2 = Repository.GetVehicleToNumber(VehicleAccess.Personal, vehicleData2.Number);
                ...
                if ((vehicle1 != null && vehicleLocalData1.Access != VehicleAccess.Garage) ||
                    (vehicle2 != null && vehicleLocalData2.Access != VehicleAccess.Garage))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.TwoCarsInGarage), 3000);
                    Trigger.ClientEvent(player, "client.phone.cars.error");
                    return;
                }
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CarMustInGarage), 3000);
                    Trigger.ClientEvent(player, "client.phone.cars.error");
                    return;
                }
            ...
            var carsOwnerData =
                Players.Phone.Cars.Repository.GetCarToHouse(player, house?.Owner, house?.GetVehiclesCarNumber(),
                    garage);
            Trigger.ClientEvent(player, "client.phone.cars.init", JsonConvert.SerializeObject(carsOwnerData), true,
                house.Owner == sessionData.Name);
        ...
        public static void CMD_SetGarage(ExtPlayer player, int ID)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы должны стоять на маркере недвижимости", 3000);
                    return;
                }
            ...
            {
                Log.Write($"CMD_SetGarage Exception: {e}");
            }
        }
        ...
        public static void CMD_CreateGarage(ExtPlayer player, int type)
        {
            ...
            {
                Log.Write($"CMD_CreateGarage Exception: {e}");
            }
        }
        ...
        public static void CMD_TpHouse(ExtPlayer player, int id)
        {
            ...
            {
                Log.Write($"CMD_TpHouse Exception: {e}");
            }
        }
        ...
        public static void CMD_tpBiz(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL,
                    $"{player.Name} ({player.Value}) телепортировался к бизнесу {id}");
                GameLog.Admin($"{player.Name}", $"tpBiz({id})", "null");
            }
            catch (Exception e)
            {
                Log.Write($"CMD_tpBiz Exception: {e}");
            }
        }
        ...
        public static void CMD_RemoveGarage(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы должны стоять на маркере гаража", 3000);
                    return;
                }
            ...
            {
                Log.Write($"CMD_RemoveGarage Exception: {e}");
            }
        }
