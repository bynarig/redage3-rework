...
namespace NeptuneEvo.Jobs
{
    internal class Bus : Script
    {
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "createCheckpoint", 3, 1, BusWays[way][0].Pos - new Vector3(0, 0, 1.12), 4, 0,
        ...
        public static void OnPlayerEnterVehicle(ExtPlayer player, ExtVehicle vehicle, sbyte seatId)
        {
            ...
            if (characterData.Money >= Main.BusPay)
            {
                ...
                Wallet.Change(player, -Main.BusPay);
        ...
        [Interaction(ColShapeEnums.BusWays, true)]
        public static void InBusWays(ExtPlayer player, int Index, int ListId)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    if (characterData.WorkID != (int)JobsId.Bus || !sessionData.WorkData.OnWork ||
                        sessionData.WorkData.WorkWay != Index) return;
                    var way = sessionData.WorkData.WorkWay;
                    if (ListId != sessionData.WorkData.WorkCheck) return;
                    var check = sessionData.WorkData.WorkCheck;
                    ...
                    if (!BusWays[way][check].IsStop)
                    {
                        ...
                        var direction = check + 1 != BusWays[way].Count
                        ...
                        Trigger.ClientEvent(player, "createCheckpoint", 3, 1,
                            BusWays[way][check].Pos - new Vector3(0, 0, 1.12), 4, 0, color.Red, color.Green, color.Blue,
                            direction);
                        Trigger.ClientEvent(player, "createWaypoint", BusWays[way][check].Pos.X,
                            BusWays[way][check].Pos.Y);
                        Trigger.ClientEvent(player, "createWorkBlip", BusWays[way][check].Pos);
                        sessionData.WorkData.WorkCheck = check;
                        var payment = Convert.ToInt32(Main.BuswaysPayments[way] *
                                                      Group.GroupPayAdd[accountData.VipLvl] *
                                                      Main.ServerSettings.MoneyMultiplier);

                        var jobLevelInfo = characterData.JobSkills.ContainsKey((int)JobsId.Bus)
                        ...
                        if (qMain.GetQuestsLine(player, Zdobich.QuestName) == (int)zdobich_quests.Stage11)
                        {
                            ...
                            if (sessionData.WorkData.PointsCount < qMain.GetQuestsData(player, Zdobich.QuestName,
                                    (int)zdobich_quests.Stage11))
                            ...
                            {
                                qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage11, 1,
                                    true);
                                qMain.UpdateQuestsComplete(player, Zdobich.QuestName, (int)zdobich_quests.Stage11,
                                    true);
                            ...
                            else
                            {
                                ...
                                Trigger.SendChatMessage(player,
                                    LangFunc.GetText(LangType.Ru, DataName.YouEarnedJob,
                                        sessionData.WorkData.PointsCount, 500 - sessionData.WorkData.PointsCount));
                            }
                        }
                    }
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.BusStop), 6000);
                        var mypos = player.Position;
                        sessionData.TimersData.BusTimer =
                            Timers.StartOnce(5000, () => timer_busStop(player, way, check, mypos), true);
                        foreach (var foreachPlayer in Main.GetPlayersInRadiusOfPosition(mypos, 30))
                        {
                            if (!foreachPlayer.IsCharacterData()) continue;
                            Trigger.SendChatMessage(foreachPlayer,
                                LangFunc.GetText(LangType.Ru, DataName.NextBus, BusWaysNames[way]));
                        }
                    }
                }
            }
            ...
            {
                Log.Write($"busCheckpointEnterWay Exception: {e}");
            }
        }
        ...
        private static void timer_busStop(ExtPlayer player, int way, int check, Vector3 mypos)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouCanGoNext), 3000);
                var payment = Convert.ToInt32(Main.BuswaysPayments[way] * Group.GroupPayAdd[accountData.VipLvl] *
                                              Main.ServerSettings.MoneyMultiplier);
                ...
                var direction = check + 1 < BusWays[way].Count
                    ...
                    : BusWays[way][0].Pos - new Vector3(0, 0, 1.12);
                var color = BusWays[way][check].IsStop ? new Color(255, 255, 255) : new Color(255, 0, 0);
                Trigger.ClientEvent(player, "createCheckpoint", 3, 1, BusWays[way][check].Pos - new Vector3(0, 0, 1.12),
                    4, 0, color.Red, color.Green, color.Blue, direction);
                ...
                foreach (var foreachPlayer in Main.GetPlayersInRadiusOfPosition(mypos, 30))
                {
                    ...
                    Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BusStartsWay, BusWaysNames[way]), 1000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"timer_busStop Exception: {e}");
            }
        }
