...
{
    internal class TreesData
    {
        public string RebirthTimer;
        ...
        public ExtPlayer StartedTreeHitPlayerInfo { get; set; }
        public ExtPlayer LastTreeHitPlayer { get; set; }
        public DateTime LastTreeHitTime { get; set; } = DateTime.MinValue;
    }

    internal class Lumberjack : Script
    {
        private static readonly nLog Log = new nLog("Jobs.Lumberjack");

        private static readonly Dictionary<int, TreesData> TreesData = new Dictionary<int, TreesData>();

        private static readonly List<(string, Vector3)> TreesPositions = new List<(string, Vector3)>
        {
        ...
        private static readonly List<int> TreeStock = new List<int>
        {
            0, // Дуб
            0, // Клен
            0 // Сосна
        ...
        public static ExtTextLabel TreeStockLabel { get; set; }
        ...
        public void onResourceStartHandler()
        {
            try
            {
                for (var i = 0; i < TreesPositions.Count; i++)
                {
                    TreesData.Add(i,
                        new TreesData(i,
                            new Vector3(TreesPositions[i].Item2.X, TreesPositions[i].Item2.Y,
                                TreesPositions[i].Item2.Z + 1.25)));
                    TreesData[i].TreeShape = CustomColShape.CreateSphereColShape(
                        new Vector3(TreesPositions[i].Item2.X, TreesPositions[i].Item2.Y,
                            TreesPositions[i].Item2.Z + 1.25), 1.75f, 0, ColShapeEnums.LumberjackTree, i);
                    TreesData[i].TreeObject = NAPI.Object.CreateObject(NAPI.Util.GetHashKey(TreesPositions[i].Item1),
                        new Vector3(TreesPositions[i].Item2.X, TreesPositions[i].Item2.Y,
                            TreesPositions[i].Item2.Z - 0.45), new Vector3(0, 0, 0), 255, 0);
                }

                Main.CreateBlip(new Main.BlipData(468, LangFunc.GetText(LangType.Ru, DataName.LesniyeResi),
                    new Vector3(-490.60272, -1124.4965, 25.138412), 2, true));
 ...
 */
                Main.CreateBlip(new Main.BlipData(119, LangFunc.GetText(LangType.Ru, DataName.OhotnMagaz),
                    new Vector3(-758.8592, -618.1002, 30.2762), 21, true));

                TreeStockLabel = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(
                    Main.StringToU16(LangFunc.GetText(LangType.Ru, DataName.Hranilishe, TreeStock[0], TreeStock[1],
                        TreeStock[2])), new Vector3(-540.13434, 5380.038, 70.48429), 5F, 0.5F, 0,
                    new Color(255, 255, 255), true, 0);

                PedSystem.Repository.CreateQuest("s_m_y_construct_01", new Vector3(-758.8592, -618.1002, 30.2762),
                    -90.1f, title: LangFunc.GetText(LangType.Ru, DataName.BearGrylls),
                    colShapeEnums: ColShapeEnums.HuntingShop);
                PedSystem.Repository.CreateQuest("s_m_y_construct_01", new Vector3(-1640.2056, -802.55475, 10.231294),
                    58.71f, title: LangFunc.GetText(LangType.Ru, DataName.DmitrySkupshik),
                    colShapeEnums: ColShapeEnums.TreesSell);
            }
            catch (Exception e)
            {
                Log.Write($"onResourceStartHandler Exception: {e}");
            }
        }

        [Interaction(ColShapeEnums.HuntingShop, true)]
        ...
        public static void callback_HuntingShop(ExtPlayer player, int index)
        {
                ...
                switch (index)
                {
                    case 74:
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server",
                            Main.PricesSettings.InstrumentPrices[0], "buyHuntingShop(74)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.BoughtAxe), 3000);
                    ...
                    case 75:
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server",
                            Main.PricesSettings.InstrumentPrices[1], "buyHuntingShop(75)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.BoughtPickaxe1), 3000);
                        BattlePass.Repository.UpdateReward(player, 57);
                        return;
                    case 76:
                        if (UpdateData.CanIChange(player, Main.PricesSettings.InstrumentPrices[2], true) != 255) return;
                        if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                ItemId.Pickaxe2, 1, "1248") == -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoSpaceInventory), 3000);
                            return;
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server",
                            Main.PricesSettings.InstrumentPrices[2], "buyHuntingShop(76)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.BoughtPickaxe2), 3000);
                        BattlePass.Repository.UpdateReward(player, 57);
                        return;
                    case 77:
                        if (UpdateData.CanIChange(player, Main.PricesSettings.InstrumentPrices[3], true) != 255) return;
                        if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                ItemId.Pickaxe3, 1, "2250") == -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoSpaceInventory), 3000);
                            return;
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server",
                            Main.PricesSettings.InstrumentPrices[3], "buyHuntingShop(77)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.BoughtPickaxe3), 3000);
                        BattlePass.Repository.UpdateReward(player, 57);
                        return;
                }
            }
            ...
            {
                Log.Write($"callback_HuntingShop Exception: {e}");
            }
        }

        [Interaction(ColShapeEnums.LumberjackTree, true)]
        public static void OnEnterTreeShape(ExtPlayer player, int TreeIndex)
        {
            try
            {
                ...
                if (TreesData.ContainsKey(TreeIndex))
                {
                    ...
                    Trigger.ClientEvent(player, "lumberjackJob_setTreeInfo", TreeIndex, treesData.TreeStage,
                        treesData.TreePosition, treesData.TreeHP);
                }
            }
            catch (Exception e)
            {
                Log.Write($"OnEnterTreeShape Exception: {e}");
            }
        }
        ...
        public static void OnExitTreeShape(ExtPlayer player, int TreeIndex)
        {
            ...
            {
                Log.Write($"OnExitTreeShape Exception: {e}");
            }
        }
        ...
        public static void Lumberjack_HitTree(ExtPlayer player, int TreeIndex)
        {
            try
            {
                ...
                if (TreesData.ContainsKey(TreeIndex))
                {
                    ...
                    var WorkAxe = Chars.Repository.isItem(player, "inventory", ItemId.WorkAxe);
                    if (WorkAxe == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouHaveNoAxe), 3000);
                        return;
                    }

                    if (treesData.StartedTreeHitPlayerInfo != null)
                    {
                        if (!treesData.StartedTreeHitPlayerInfo.IsCharacterData())
                        {
                            treesData.StartedTreeHitPlayerInfo = null;
                        }
                        else if (treesData.StartedTreeHitPlayerInfo.Position.DistanceTo(treesData.TreePosition) >= 3)
                        {
                            treesData.StartedTreeHitPlayerInfo = null;
                        }
                        else if (treesData.StartedTreeHitPlayerInfo == player)
                        {
                            return;
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.SomebodyCutting), 3000);
                            return;
                        }
                    }
                    ...
                    {
                        if (!treesData.LastTreeHitPlayer.IsCharacterData())
                        {
                            treesData.LastTreeHitPlayer = null;
                        }
                        else if (treesData.LastTreeHitPlayer.Position.DistanceTo(treesData.TreePosition) >= 3)
                        {
                            treesData.LastTreeHitPlayer = null;
                        }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AxeBroken), 3000);
                        Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", WorkAxe.Item.ItemId);
                    }
            ...
            {
                Log.Write($"Lumberjack_HitTree Exception: {e}");
            }
        }
        ...
        public static void Lumberjack_CutTree(ExtPlayer player, int TreeIndex)
        {
            try
            {
                ...
                if (TreesData.ContainsKey(TreeIndex))
                {
                    ...
                    treesData.TreeObject = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("prop_tree_stump_01"),
                        new Vector3(TreesPositions[TreeIndex].Item2.X, TreesPositions[TreeIndex].Item2.Y,
                            TreesPositions[TreeIndex].Item2.Z - 0.3), new Vector3(0, 0, 0), 255, 0);
                    ...
                    if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", item_type) == -1)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoMoreLogs), 3000);
                    else
                        treesData.TreeStage = 2;

                    NAPI.ClientEvent.TriggerClientEventInRange(treesData.TreePosition, 2.5f,
                        "lumberjackJob_updateTreeInfo", TreeIndex, treesData.TreeStage, treesData.TreeHP);

                    if (treesData.RebirthTimer != null) Timers.Stop(treesData.RebirthTimer);

                    treesData.RebirthTimer = Timers.StartOnce($"TreeRebirthTimer{TreeIndex}", 15000,
                        () => TreeRebirthTimerFunction(TreeIndex), true);
            ...
            {
                Log.Write($"Lumberjack_CutTree Exception: {e}");
            }
        }
        ...
        public static void Lumberjack_StartAnimCutTree(ExtPlayer player)
        {
            ...
            {
                Log.Write($"Lumberjack_StartAnimCutTree Exception: {e}");
            }
        }
        ...
        public static void Lumberjack_StopCutTree(ExtPlayer player, int TreeIndex)
        {
            ...
            {
                Log.Write($"Lumberjack_StopCutTree Exception: {e}");
            }
        }
        ...
        public static void Lumberjack_TakeTimber(ExtPlayer player, int TreeIndex)
        {
            ...
            {
                Log.Write($"Lumberjack_TakeTimber Exception: {e}");
            }
        }
        ...
        private static void TreeRebirthTimerFunction(int TreeIndex)
        {
            try
            {
                if (TreesData.ContainsKey(TreeIndex))
                {
                    ...
                    treesData.TreeObject = NAPI.Object.CreateObject(
                        NAPI.Util.GetHashKey(TreesPositions[TreeIndex].Item1),
                        new Vector3(TreesPositions[TreeIndex].Item2.X, TreesPositions[TreeIndex].Item2.Y,
                            TreesPositions[TreeIndex].Item2.Z - 0.45), new Vector3(0, 0, 0), 255, 0);
            ...
            {
                Log.Write($"TreeRebirthTimerFunction Exception: {e}");
            }
        }
        ...
        public static void PlayerSellTrees(ExtPlayer player, int stock_item_index, int amount)
        {
            try
            {
                ...
                if (TreeStock.Count < stock_item_index) return;

                var item_type = ItemId.WoodOak;
                var price = Main.LumberjackPrice.OakPrice;
                ...
                var woodamount = Chars.Repository.isItem(player, "inventory", item_type);
                var count = woodamount == null ? 0 : woodamount.Item.Count;
                var success = false;

                if (count < amount)
                {
                    count = Chars.Repository.getCountToLacationItem($"char_{characterData.UUID}", "inventory",
                        item_type);
                    if (count >= amount)
                    {
                        success = true;
                        for (var i = 0; i < amount; i++)
                            Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", item_type);
                    }
                }
                else
                {
                ...
                if (success)
                {
                    var totalmoney = Convert.ToInt32(price * amount);
                    Wallet.Change(player, totalmoney);
                    GameLog.Money("server", $"player({characterData.UUID})", totalmoney,
                        $"sellTrees({stock_item_index},{amount})");

                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SellResources, Wallet.Format(totalmoney)), 3000);
                    //BattlePass.Repository.UpdateReward(player, 59);

                    TreeStock[stock_item_index] += amount;
                    TreeStockLabel.Text = Main.StringToU16(LangFunc.GetText(LangType.Ru, DataName.Hranilishe,
                        TreeStock[0], TreeStock[1], TreeStock[2]));
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NotEnoughRes), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"PlayerSellOres Exception: {e}");
            }
        }
    }
}