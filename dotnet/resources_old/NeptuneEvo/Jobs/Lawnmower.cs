...
{
    internal class Lawnmower : Script
    {
        ...
        public void onResourceStartHandler()
        {
            try
            {
                for (var i = 0; i < MowerWays.Count; i++)
                for (var d = 0; d < MowerWays[i].Count; d++)
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            var way = MowerWays[0];
            Trigger.ClientEvent(player, "createCheckpoint", 4, 1, way[0] - new Vector3(0, 0, 1.12), 2, 0, 255, 0, 0,
                way[1] - new Vector3(0, 0, 1.12));
            Trigger.ClientEvent(player, "createWaypoint", way[0].X, way[0].Y);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.StartLawnmower), 3000);
        }
        ...
        [Interaction(ColShapeEnums.MowerWays, true)]
        public void InMowerWays(ExtPlayer player, int index, int listId)
        {
            try
            {
                ...
                {
                    if (vehicleLocalData.WorkId != JobsId.Lawnmower || characterData.WorkID != (int)JobsId.Lawnmower ||
                        !sessionData.WorkData.OnWork || sessionData.WorkData.WorkWay != index ||
                        sessionData.WorkData.WorkCheck != listId) return;
                    ...
                    var payment = Convert.ToInt32(Main.LawnmowerPayment * Group.GroupPayAdd[accountData.VipLvl] *
                                                  Main.ServerSettings.MoneyMultiplier);

                    var jobLevelInfo = characterData.JobSkills.ContainsKey(1)
                        ? Main.GetPlayerJobLevelBonus(1, characterData.JobSkills[1])
                        : (0, 1);
                    ...
                    {
                        characterData.JobSkills.Add(1, 1);
                    }
                    ...
                    {
                        var direction = check + 2 != MowerWays[way].Count
                            ? MowerWays[way][check + 2]
                            : MowerWays[way][0] - new Vector3(0, 0, 1.12);
                        Trigger.ClientEvent(player, "createCheckpoint", 4, 1,
                            MowerWays[way][check + 1] - new Vector3(0, 0, 1.12), 2, 0, 255, 0, 0, direction);
                        Trigger.ClientEvent(player, "createWaypoint", MowerWays[way][check + 1].X,
                            MowerWays[way][check + 1].Y);
                    ...
                    else
                    {
                        var next_way = 0;
                        Trigger.ClientEvent(player, "createCheckpoint", 4, 1,
                            MowerWays[next_way][0] - new Vector3(0, 0, 1.12), 2, 0, 255, 0, 0,
                            MowerWays[next_way][1] - new Vector3(0, 0, 1.12));
                        Trigger.ClientEvent(player, "createWaypoint", MowerWays[next_way][0].X,
                            MowerWays[next_way][0].Y);
                        sessionData.WorkData.WorkCheck = 0;
                        ...
                        if (bonusPay > 0)
                        {
                            Wallet.Change(player, bonusPay);
                            GameLog.Money("server", $"player({characterData.UUID})", payment, "lawnBonusWay");
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.BonusPay, bonusPay), 3000);
            ...
            {
                Log.Write($"mowerCheckpointEnterWay Exception: {e}");
            }
        }
    }
}