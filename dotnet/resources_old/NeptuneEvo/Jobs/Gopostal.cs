...
{
    internal class Gopostal : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(LangFunc.GetText(LangType.Ru, DataName.PostHouse),
                    Coords[0] + new Vector3(0, 0, 0.3), 10F, 0.6F, 0, new Color(0, 180, 0));
                ...
                NAPI.Marker.CreateMarker(1, Coords[0] - new Vector3(0, 0, 1.5), new Vector3(), new Vector3(), 1f,
        ...
        public static void OnJobGoPosta(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не работаете курьером.",
                        3000);
                    return;
                }
                    ...
                    case "get":
                        if (characterData.WorkID != (int)JobsId.Postman)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoPostman), 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouNotRazdaliPosilki,
                                    sessionData.WorkData.Packages), 3000);
                            return;
                        }

                        byte count = 10;
                        var jobLevelInfo = characterData.JobSkills.ContainsKey((int)JobsId.Postman)
                            ? Main.GetPlayerJobLevelBonus((int)JobsId.Postman,
                                characterData.JobSkills[(int)JobsId.Postman])
                            : (0, 1);
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            byte count = 10;
            var jobLevelInfo = characterData.JobSkills.ContainsKey((int)JobsId.Postman)
                ? Main.GetPlayerJobLevelBonus((int)JobsId.Postman, characterData.JobSkills[(int)JobsId.Postman])
                : (0, 1);
            if (jobLevelInfo.Item1 == 3) count = 15;
            else if (jobLevelInfo.Item1 >= 4) count = 20;
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.PostmanStartJob, count), 3000);
        ...
        public static void GoPostal_onEntityEnterColShape(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.LeaveJobVeh), 3000);
                    return;
                }

                if (sessionData.WorkData.Packages == 0 ||
                    !Attachments.HasAttachment(player, Attachments.AttachmentsName.Postal))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У вас нет посылки в руке",
                        3000);
                    return;
                }
                ...
                if (jobLevelInfo.Item1 <= 4)
                {
                    ...
                    if (vehiclePost == null || !vehiclePost.Exists ||
                        player.Position.DistanceTo(vehiclePost.Position) >= 50)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoHozyain), 3000);
                    return;
                }

                var payment = Convert.ToInt32(coef * Main.PostalPayment * Group.GroupPayAdd[accountData.VipLvl] *
                                              Main.ServerSettings.MoneyMultiplier);

                var maxpayment = 3500 * Main.ServerSettings.MoneyMultiplier;
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MailsLeft, sessionData.WorkData.Packages), 8000);
                ...
                else
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoMails), 3000);
                }
                ...
                {
                    characterData.JobSkills.Add((int)JobsId.Postman, 1);
                }
            }
            catch (Exception e)
            {
