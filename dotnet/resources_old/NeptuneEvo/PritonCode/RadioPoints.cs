...
namespace NeptuneEvo.PritonCode
{
    ...
    {
        private static readonly Dictionary<Vector3, (string, float)> _radioPoints =
            new Dictionary<Vector3, (string, float)>
            {
        ...
        public RadioPoints()
        {
            foreach (var (position, (radioUrl, range)) in _radioPoints)
            {
                var colShape = NAPI.ColShape.CreateSphereColShape(position, range, 0);

                colShape.OnEntityEnterColShape += (shape, player) => { Sounds.Play2d((ExtPlayer)player, radioUrl); };
                colShape.OnEntityExitColShape += (shape, player) => { Sounds.Stop2d((ExtPlayer)player); };
            }
        }
    }
}