...
namespace NeptuneEvo.Functions
{
    ...
    internal class CommandsAccess : Script
    {
        ...
        private static readonly string DefaultAdminAccess = JsonConvert.SerializeObject(new Dictionary<string, sbyte>
        {
            { AdminCommands.Id, 1 },
        ...
        private static readonly string DefaultVipAccess = JsonConvert.SerializeObject(new Dictionary<string, sbyte>
        {
            { VipCommands.Cam, 5 },
        ...
        private static readonly string[] LoginAccess =
        {
        ...
        public async void LoadCommands()
        {
            ...
            if (account.Count == 0)
            {
                foreach (var access in AdminAccess)
                ...
                foreach (var access in VipAccess)
                    db.Insert(new Adminaccesses
                    {
                        Command = access.Key,
                        Isadmin = false,
            ...
            {
                foreach (var command in account)
        ...
        public void CMD_updateadminaccess(ExtPlayer player)
        {
            ...
            if (!CanUseCmd(player, AdminCommands.Updateadminaccess)) return;
            LoadCommands();
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                "Вы успешно обновили доступы к командам!", 10000);
        }
        ...
        {
            if (!player.IsCharacterData()) return;
            if (!CanUseCmd(player, AdminCommands.Defaultadminaccess)) return;
            AdminAccess = JsonConvert.DeserializeObject<Dictionary<string, sbyte>>(DefaultAdminAccess);
            VipAccess = JsonConvert.DeserializeObject<Dictionary<string, sbyte>>(DefaultVipAccess);
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                "Вы успешно сделали стандартные доступы к командам!", 10000);
        }
        ...
        public static bool CanUseCmd(ExtPlayer player, string cmd, string args = "")
        {
            ...
            if (LoginAccess.Contains(cmd) &&
                (LoginsDirector.Contains(accountData.Login) || Main.ServerNumber == 0)) return true;
            ...
            Task.Run(() =>
            {
                ...
                foreach (var clData in shoesData)
                {
                    ...
                    foreach (var text in clData.Value.Textures)
                    {
                        if (variation.ContainsKey(dictionary))
                        {
                            Trigger.ClientEvent(player, "startVariation", clData.Key, variation[dictionary],
                                clData.Value.Variation, text);
                            Thread.Sleep(250);
                        }

                        if (prop.ContainsKey(dictionary))
                        {
                            Trigger.ClientEvent(player, "startProp", clData.Key, prop[dictionary],
                                clData.Value.Variation, text);
                            Thread.Sleep(250);
                        }
                    }
        ...
        public void camtypeid(ExtPlayer player, string name, int startid, int count)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    Trigger.ClientEvent(player, "startVariationCam", variation[dictionary]);
                    Trigger.ClientEvent(player, "startVariation", -1, variation[dictionary],
                        shoesData[startid].Variation, 0);
                }
                ...
                for (var i = startid; i < startid + count; i++)
                    if (shoesData.ContainsKey(i))
                    {
                        ...
                        foreach (var text in clData.Textures)
                        {
                            if (variation.ContainsKey(dictionary))
                            {
                                Trigger.ClientEvent(player, "startVariation", i, variation[dictionary],
                                    clData.Variation, text);
                                Thread.Sleep(250);
                            }
