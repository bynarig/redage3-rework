    ...
    public class Repository
    {
        public static void GetVehicles(ExtPlayer player)
        {
            ...
            foreach (var v in organizationData.Vehicles)
            {
                ...
                var vehicle =
                    VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.OrganizationGarage, v.Key);
            ...
            Trigger.ClientEvent(player, "client.org.main.vehicles", isVehicleUpdateRank,
                JsonConvert.SerializeObject(vehiclesList), isSellVehicle);
        }
        ...
        public static void UpdateRank(ExtPlayer player, string number, int rank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantUpVehicleRank), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantSetNullRank), 3000);
                    return;
                }

                if (vehicleLocalData.Access == VehicleAccess.Organization &&
                    vehicleLocalData.Fraction == memberOrganizationData.Id)
                {
                    vehicleLocalData.MinRank = rank;
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SuccessEditRankVehWithPrikol, vData.model, number),
                        3000);
                }
        ...
        public static void Evacuation(ExtPlayer player, string number)
        {
            try
            {
                ...
                if (vehicleLocalData.Fraction == memberOrganizationData.Id &&
                    memberOrganizationData.Rank >= vehicleLocalData.MinRank)
                {
                    if (characterData.Money < 20)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoMoneyNeed, 20 - characterData.Money), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CarUsed), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CarNoNeedEvac), 3000);
                        return;
                    }
                    ...
                    vehicle = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(organizationVehicle.model),
                        Manager.GaragePositions[organizationVehicle.garageId],
                        Manager.GarageRotations[organizationVehicle.garageId], 0, 0, number,
                        dimension: organizationData.GetDimension(), locked: true, acc: VehicleAccess.OrganizationGarage,
                        fr: organizationData.Id, minrank: organizationVehicle.rank, petrol: petrol, dirt: dirt);
                    VehicleManager.OrgApplyCustomization(vehicle, organizationVehicle.customization);
                    Wallet.Change(player, -20);
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CarOtognana), 3000);
                    GetVehicles(player);
                }
            }
            catch (Exception e)
        ...
        public static void Gps(ExtPlayer player, string number)
        {
            try
            {
                    ...
                    memberOrganizationData.Rank >= vehicleLocalData.MinRank)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GPSOtmechenoCarFraction), 3000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void DefaultRanks(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.SucResetDefaultVehRanks), 4500);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
