...
namespace NeptuneEvo.Organizations.Table.Player
{
    public class Repository
    {
        public static void MainLoad(ExtPlayer player, bool isLoadLogs = true)
        {
            ...
            var warCount = FamilyZones.Repository.FamilyZones.Values
        ...
        public static void GetMembers(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "client.org.main.members", JsonConvert.SerializeObject(playerData),
        ...
        public static void Reprimand(ExtPlayer player, int uuid, string name, string text)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MaxVigovorLength), 4500);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantFindPlayerFraction, name), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantVigovor, name), 3000);
                    return;
                }

                ;
                Logs.Repository.AddLogs(player, OrganizationLogsType.Reprimand,
                    LangFunc.GetText(LangType.Ru, DataName.GivenVigovor, name, uuid, text));
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
        ...
        public static void Disolver(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Вы не являетесь владельцем семьи.", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "Чтобы распустить семью Вы должны продать все транспортные средства семьи", 3000);
                    return;
                }
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Organizations.Player.Repository.RemoveOrganizationMemberData(orgId, foreachPlayer.GetUUID());
                ...
                EventSys.SendCoolMsg(player, "Семья", "Семья распущена!",
                    "Вы распустили семью, деньги были выданы на руки.", "", 10000);
