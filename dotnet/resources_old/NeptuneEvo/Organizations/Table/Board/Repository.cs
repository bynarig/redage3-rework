    ...
    public class Repository
    {
        ...
        public static void Add(ExtPlayer player, string title, string text)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MaxTableNewsText), 4500);
                    return;
                }

                var testmsg = $"{title.ToLower()} {text.ToLower()}";
                ...
                Fractions.Manager.sendOrganizationMessage(memberOrganizationData.Id,
                    "[F] " + LangFunc.GetText(LangType.Ru, DataName.TableNewNews), true);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouAddedTableNews), 3000);
        ...
        public static void Update(ExtPlayer player, int index, string title, string text)
        {
            try
            {
                ...
                if (boardData.UUId != player.GetUUID() &&
                    !player.IsOrganizationAccess(RankToAccess.EditAllTabletWall)) return;
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouEditedTableNews), 3000);
                Get(player, index + 1);
            }
            catch (Exception e)
            {
        ...
        public static void Delete(ExtPlayer player, int index)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouDeletedTableNews), 3000);
                Get(player, 1);
            }
            catch (Exception e)
            {
