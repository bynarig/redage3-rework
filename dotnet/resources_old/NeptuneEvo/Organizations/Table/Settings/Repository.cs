...
namespace NeptuneEvo.Organizations.Table.Settings
{
    public class Repository
    {
            ...
            try
            {
                ...
                if (organizationData.IsOpenStock)
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы открыли склад семьи",
                        3000);
                    Logs.Repository.AddLogs(player, OrganizationLogsType.OpenStock, "Открыл склад");
                }
                else
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы закрыли склад семьи",
                        3000);
                    Logs.Repository.AddLogs(player, OrganizationLogsType.CloseStock, "Закрыл склад");
                }
        ...
        public static void SaveSetting(ExtPlayer player, string slogan, byte salary, string discord, int colorR,
            int colorG, int colorB)
        {
            try
            {
                ...
                Player.Repository.MainLoad(player);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
