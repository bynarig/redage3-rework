...
namespace NeptuneEvo.Character
{
    public static class Repository
    {
        ...
        public static void SavePlayerPosition(this ExtPlayer player)
        {
            ...
            {
                var position = characterData.ExteriorPos;
                characterData.CurPosition = position + new Vector3(0, 0, 1.12);
            }
            else if (characterData.InsideHotelID != -1)
            {
                var position = Hotel.HotelEnters[characterData.InsideHotelID];
                characterData.CurPosition = position + new Vector3(0, 0, 1.12);
            }
            else if (characterData.TuningShop != -1)
            {
                var position = BusinessManager.BizList[characterData.TuningShop].EnterPoint;
                characterData.CurPosition = position + new Vector3(0, 0, 1.12);
            }
            ...
            else if (characterData.IsInPostalStock)
            {
            ...
            {
                characterData.CurPosition = Ems.emsCheckpoints[2];
