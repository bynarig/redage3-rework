...
namespace NeptuneEvo.Database.Models
{
    public static class AccountModels
    {
        ...
        private static readonly List<IUpdatable<Characters>> DBListUpdate = new List<IUpdatable<Characters>>();
        ...
        private static async void Worker()
        {
            while (true)
            {
                try
                {
                    ...
                    foreach (var update in updateList) await update.UpdateAsync();
                }
                catch (Exception e)
                {
