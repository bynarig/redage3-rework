...
namespace NeptuneEvo.Players.Phone.Auction
{
    ...
    {
        private static readonly List<AuctionData> AuctionsData = new List<AuctionData>();

        private static readonly List<ExtPlayer> Players = new List<ExtPlayer>();
        ...
        {
            return AuctionsData.Any(a => a.Type == type && a.ElementId == id);
        }
        ...
        public static void Init(ExtPlayer player)
        {
            ...
            foreach (var auctionData in listAuctions) auctions.Add(GetData(auctionData));

            Trigger.ClientEvent(player, "client.phone.auction.init", JsonConvert.SerializeObject(myAuctions),
        ...
        public static bool IsBet(int uuid, AuctionType type)
        {
            return AuctionsData.Any(a =>
                a.Type == type && (a.CreateUUID == uuid || a.BetsData.Any(b => b.UUID == uuid)));
        }

        public static void OnCreate(ExtPlayer player, AuctionType type, int elementId, string text, string image,
            int price)
        {
            var characterData = player.GetCharacterData();
            if (characterData == null)
        ...
        private static void Insert(ExtPlayer player, AuctionType type, int elementId, string title, string text,
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    GameLog.Money($"player({characterData.UUID})", "server", price,
                        $"auction [#{autoId}]: Создание лота ({type.ToString()})");
                    ...
                    Trigger.SetMainTask(() =>
                    {
                        Trigger.ClientEvent(player, "client.phone.auction.addMyItem",
                            JsonConvert.SerializeObject(GetData(auctionData)));
        ...
        public static bool IsSavingAuctions()
        {
            return AuctionsData.Any(a => a.IsSave);
        }
        ...
        private static void End(AuctionData auctionData)
        {
            ...
            Messages.Repository.AddSystemMessageToUuid(auctionData.CreateUUID, (int)DefaultNumber.Auction,
                $"Аукцион на {auctionData.Title} закончен.", DateTime.Now);
        ...
        public static void OnBet(ExtPlayer player, int id, int price)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    "Вы не можете сделать ставку на свой лот.", 3000);
                return;
            }

            if (auctionData.LastBetUUID == characterData.UUID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Последняя ставка и так ваша.",
                    3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    "Ставка не может быть меньше, чем текущая.", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoBankMoney), 3000);
                return;
            }

            GameLog.Money($"player({characterData.UUID})", "auction(Bet)", priceAndPercent,
                $"auction [#{auctionData.Id}]: Ставка ({auctionData.Type.ToString()})");
            ReturnMoney(auctionData.LastBetUUID, auctionData.LastPrice, auctionData.Id, auctionData.Type,
            ...
            {
                SendPlayer(JsonConvert.SerializeObject(GetData(auctionData)));
            }
        }
