...
namespace NeptuneEvo.Players.Phone.Tinder
{
    ...
    {
        private static readonly Dictionary<int, TinderData> List = new Dictionary<int, TinderData>();
        ...
        public static void Init(ExtPlayer player)
        {
            ...
            else
            {
                ...
                Trigger.ClientEvent(player, "client.phone.tinder.init", JsonConvert.SerializeObject(data),
                    JsonConvert.SerializeObject(likesData), JsonConvert.SerializeObject(listData));
            }
        }
        ...
        private static List<List<object>> GetListRandom(List<int> likes, TinderType type, int count = 5)
        {
            ...
            var idList = Horses.Shuffle(tinders);
        ...
        public static void OnAction(ExtPlayer player, int uuid, bool isLove)
        {
            ...
            if (isLove)
            {
                ...
                {
                    var sim = Main.SimCards.FirstOrDefault(u => u.Value == uuid).Key;
                    Trigger.ClientEvent(player, "client.phone.tinder.addLikes", Main.PlayerNames[uuid],
                        List[uuid].Avatar, Main.SimCards.ContainsKey(sim) ? sim : -1);
                    Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tinder,
                        LangFunc.GetText(LangType.Ru, DataName.TinderMatch), DateTime.Now);
                    var target = Main.GetPlayerByUUID(uuid);
                    if (target != null)
                        Messages.Repository.AddSystemMessage(target, (int)DefaultNumber.Tinder,
                            LangFunc.GetText(LangType.Ru, DataName.TinderMatch), DateTime.Now);
                }
            }
            else
        ...
        public static void OnSave(ExtPlayer player, string avatar, string text, TinderType type, bool isVisible)
        {
            ...
            if (text.Length < 5 || text.Length > 150)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    "Минимальное количество символов - 5, а максимальное - 150", 7000);
                return;
            }
