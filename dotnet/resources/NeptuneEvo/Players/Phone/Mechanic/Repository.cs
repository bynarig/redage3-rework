...
namespace NeptuneEvo.Players.Phone.Mechanic
{
    public class Repository
    {
        ...
        public static void OnOrder(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.AlreadyCallMechanik), 3000);
                return;
            }
            ...
            foreach (var foreachPlayer in GetListMechanic())
            {
                ...
                mechanicCount++;
                Trigger.ClientEvent(foreachPlayer, "client.phone.mechjob.add", characterData.UUID, player.Name,
            ...
            if (mechanicCount > 0)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.WaitMechanikCall, mechanicCount), 3000);
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoMechanikNear), 3000);
            }
        }

        public static void
            OnCancel(ExtPlayer player, bool isDisconnect = false,
                bool isNotRemove = true) //Отмена для таксиста или вызывающего
        {
            var characterData = player.GetCharacterData();
            if (characterData == null)
                return;
            ...
            {
                if (!isDisconnect)
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не вызывали автомеханика",
                        3000);
            ...
            if (isDriver)
            {
                if (isNotRemove)
                {
                    ...
                    if (!isDisconnect)
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.TripCanceled), 3000);
                }
            ...
            else
            {
                if (isNotRemove)
                {
                    ...
                    if (!isDisconnect)
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCancelTripMech), 3000);
                }
            ...
            if (target != null)
            {
                ...
                if (isDriver)
                {
                    if (isNotRemove)
                    {
                        foreach (var foreachPlayer in GetListMechanic())
                        {
                            ...
                            Trigger.ClientEvent(foreachPlayer, "client.phone.mechjob.add", targetCharacterData.UUID,
                ...
                else
                {
                    if (isNotRemove)
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PassengerCancel), 3000);
