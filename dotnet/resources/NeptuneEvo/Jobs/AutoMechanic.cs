...
{
    internal class AutoMechanic : Script
    {
        ...
        public static void mechanicRepair(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YourNotAutoMech), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantRepairYourVehicle), 3000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PlayerMustBeInVeh), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.Price5to30), 3000);
                        return;
                    }
                    ...
                    var sellItemData = targetSessionData.SellItemData;
                    if ((sellItemData.Buyer != null || sellItemData.Seller != null) &&
                    ...
                    Trigger.ClientEvent(target, "openDialog", "REPAIR_CAR",
                        LangFunc.GetText(LangType.Ru, DataName.MechanikRepairTo, player.Value, Wallet.Format(price)));
                    ...
                    EventSys.SendCoolMsg(player, "Предложение", "Механик",
                        LangFunc.GetText(LangType.Ru, DataName.MechanikRepairFrom, target.Value, Wallet.Format(price)),
                        "", 5000);
        ...
        public static void mechanicPay(ExtPlayer player)
        {
            try
            {
                ...
                var driver = sessionData.SellItemData.Seller;
                ...
                Wallet.Change(driver, price);
                GameLog.Money($"player({characterData.UUID})", $"player({driverCharacterData.UUID})", price,
                    "mechanicRepair");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.RepairPayed), 3000);
                Notify.Send(driver, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.PlayerPayedRepair, player.Value), 3000);
                ...
                if (qMain.GetQuestsLine(driver, Zdobich.QuestName) == (int)zdobich_quests.Stage11)
                {
                    ...
                    if (driverSessionData.WorkData.PointsCount <
                        qMain.GetQuestsData(player, Zdobich.QuestName, (int)zdobich_quests.Stage11))
                        driverSessionData.WorkData.PointsCount =
                            qMain.GetQuestsData(player, Zdobich.QuestName, (int)zdobich_quests.Stage11) + price;
                    ...
                    if (driverSessionData.WorkData.PointsCount >= 500)
                    {
                        ...
                        Trigger.SendChatMessage(driver,
                            "!{#fc0}" + LangFunc.GetText(LangType.Ru, DataName.QuestPartComplete));
                    }
                    else
                    {
                        qMain.UpdateQuestsData(driver, Zdobich.QuestName, (int)zdobich_quests.Stage11,
                            driverSessionData.WorkData.PointsCount.ToString());
                        //todo translate (было DataName.PointsQuestGot)
                        Trigger.SendChatMessage(driver,
                            $"Вы заработали ${driverSessionData.WorkData.PointsCount}. Осталось ещё ${500 - driverSessionData.WorkData.PointsCount}.");
                    }
                }
                ...
                if (driverSessionData.WorkData.LastClientTime < DateTime.Now)
                {
                    ...
                    {
                        driverCharacterData.JobSkills.Add(5, 1);
                    }
        ...
        public static void buyFuel(ExtPlayer player, int fuel)
        {
            try
            {
                ...
                if (rentData == null || characterData.WorkID != (int)JobsId.CarMechanic ||
                    !sessionData.WorkData.OnWork || !player.IsInVehicle || rentData.Vehicle != player.Vehicle)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MustBeMechanikAndWorkCar), 3000);
                    return;
                }

                var jobLevelInfo = characterData.JobSkills.ContainsKey(5)
                    ? Main.GetPlayerJobLevelBonus(5, characterData.JobSkills[5])
                    : (0, 1);
                if (jobLevelInfo.Item1 < 3)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.Command3lvl), 3000);
                    return;
                }

                var BizID = CustomColShape.GetDataToEnum(player, ColShapeEnums.BusinessAction);
                if (BizID == (int)ColShapeData.Error || !BusinessManager.BizList.ContainsKey(BizID) ||
                    BusinessManager.BizList[BizID].Type != 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MustBeAtGas), 3000);
                    return;
                }
                ...
                var amount = biz.Products[0].Price * fuel;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.UnavaibleForThisVeh), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.FillCanFuel), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoFuelAtGasStation), 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouFillBack, vehicleLocalData.VehLoadedFuel), 5000);
            }
            catch (Exception e)
            {
        ...
        public static void mechanicFuel(ExtPlayer player, ExtPlayer target, int fuel, int pricePerLitr)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.FillErrorLiter), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.FillPriceError), 3000);
                        return;
                    }
                    ...
                    Trigger.ClientEvent(target, "openDialog", "FUEL_CAR",
                        LangFunc.GetText(LangType.Ru, DataName.PlayerOfferFill, player.Value, fuel,
                            fuel * pricePerLitr));
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouOfferFill, target.Value, fuel, fuel * pricePerLitr),
                        7000);
                }
            }
            catch (Exception e)
        ...
        public static void mechanicPayFuel(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData == null) return;
                var price = sessionData.SellItemData.Price;
                var fuel = sessionData.SellItemData.Count;
                var driver = sessionData.SellItemData.Seller;
                var driverSessionData = driver.GetSessionData();
                if (driverSessionData == null) return;
                var driverCharacterData = driver.GetCharacterData();
                ...
                if (!driver.IsInVehicle)
                {
                    ...
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MechanikMustBeInJobCar), 3000);
                    return;
                }
                ...
                if (vehicleLocalData1 != null && vehicleLocalData2 != null)
                {
                    ...
                    if (vehicleLocalData1.VehLoadedFuel < fuel)
                    {
                        ...
                        sessionData.SellItemData = new SellItemData();
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.MechanikNoFuel), 3000);
                        return;
                    }
                    ...
                    GameLog.Money($"player({characterData.UUID})", $"player({driverCharacterData.UUID})", price,
                        "mechanicFuel");
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouPayFillRepairVeh), 3000);
                    Notify.Send(driver, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PlayerPayedFillVeh, player.Value), 3000);
                    Commands.RPChat("sme", driver, LangFunc.GetText(LangType.Ru, DataName.FilledVeh));
                    var carFuel = vehicleLocalData2.Petrol + fuel > vehicleLocalData2.MaxPetrol
                        ? vehicleLocalData2.MaxPetrol
                        : vehicleLocalData2.Petrol + fuel;
