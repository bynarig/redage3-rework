...
{
    internal class Truckers : Script
    {
        private static readonly nLog Log = new nLog("Jobs.Truckers");

        public static List<Vector3> GetProduct = new List<Vector3>
        {
            new Vector3(777.0558, -2974.624,
                5.80066) // 24/7 products && burger-shot && clothesshop && tattoo-salon && barber-shop && masks shop && ls customs
        };

        [ServerEvent(Event.ResourceStart)]
        public void OnResourceStart()
        {
            for (var i = 0; i < GetProduct.Count; i++)
            {
                Main.CreateBlip(new Main.BlipData(557, LangFunc.GetText(LangType.Ru, DataName.ZagruzkaTovara),
                    GetProduct[i], 84, true));
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.GoToZagruz), 10000);
        ...
        [Interaction(ColShapeEnums.GetProductTrucker, true)]
        public static void OnGetProductTrucker(ExtPlayer player)
        {
            ...
            if (characterData.WorkID == (int)JobsId.Trucker)
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    "Откройте телефон для взятия заказа", 10000);
        }
        ...
        public static void playerGotProducts(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DoveziteDo, BusinessManager.BusinessTypeNames[biz.Type]),
                        3000);
                    sessionData.OrderData.GotProduct = true;
                    sessionData.OrderData.WayPoint = biz;
                    Trigger.ClientEvent(player, "createWaypoint", biz.UnloadPoint.X, biz.UnloadPoint.Y);
                    Trigger.ClientEvent(player, "createCheckpoint", 10, 1, biz.UnloadPoint - new Vector3(0.0, 0.0, 1.5),
                        7, 0, 255, 0, 0);

                    if (Chars.Repository.isFreeSlots(player, ItemId.Wrench) == 0)
                        Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Wrench);
                }
            }
            catch (Exception e)
            {
        ...
        [Interaction(ColShapeEnums.Trucker, true)]
        public static void OnTrucker(ExtPlayer player, int bizId)
        {
            try
            {
                ...
                if (characterData == null) return;
                var rentData = sessionData.RentData;
                if (rentData == null || characterData.WorkID != (int)JobsId.Trucker ||
                    (characterData.WorkID == (int)JobsId.Trucker && !sessionData.WorkData.OnWork)) return;
                if (rentData.Vehicle != player.Vehicle)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MustBeInTruck), 3000);
                    return;
                }

                var uid = sessionData.OrderData.Order;
                if (uid == -1 || (BusinessManager.Orders.ContainsKey(uid) && BusinessManager.Orders[uid] != bizId) ||
                    !sessionData.OrderData.GotProduct) return;
                if (!BusinessManager.Orders.ContainsKey(uid))
                {
                    CancelOrder(player);
                    return;
                ...
                var payment =
                    Players.Phone.Property.Businesses.Orders.Repository.GetPrice(bizOrder.Amount, bizOrder.Name, coef);
                ...
                sessionData.OrderData.WayPoint = null;
                if (ow != null)
                    Notify.Send(ow, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouOrderDeliveried, bizOrder.Name), 3000);
                ...
                sessionData.OrderData.Order = -1;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.TrucSuccDelivery), 3000);

                if (sessionData.LastPointTime > DateTime.Now)
                    Trigger.SendToAdmins(1,
                        LangFunc.GetText(LangType.Ru, DataName.AnticheatDostavka, player.Name, player.Value));
                ...
                {
                    characterData.JobSkills.Add((int)JobsId.Trucker, 1);
                }

                var wrench = Chars.Repository.isItem(player, "inventory", ItemId.Wrench);
                if (wrench != null)
                    Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", wrench.Item.ItemId);
            }
            catch (Exception e)
            {
