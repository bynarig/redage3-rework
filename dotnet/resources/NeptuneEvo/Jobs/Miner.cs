...
{
    internal class Miner : Script
    {
        private static readonly nLog Log = new nLog("Jobs.Miner");

        private static readonly Dictionary<int, int> MinerJobPlayersOreTypeInfo = new Dictionary<int, int>();

        private static readonly List<int> MineStockOres = new List<int>
        {
            0, // Ископаемый уголь
            0, // Железная руда
            0, // Золотая руда
        ...
        private static readonly List<int> PlantStockOres = new List<int>
        {
            0, // Ископаемый уголь
            0, // Железная руда
            0, // Золотая руда
        ...
        public static ExtTextLabel MineStockLabel { get; set; }
        public static ExtTextLabel PlantStockLabel { get; set; }
        ...
        public void Event_ResourceStart()
        {
            try
            {
                Main.CreateBlip(new Main.BlipData(527, "Гражданская шахта",
                    new Vector3(124.831116, -397.94046, 41.263977), 4, true));
                ...
                MineStockLabel = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(
                    Main.StringToU16(
                    ...
                    new Vector3(-595.855, 2094.609, 131.4356), 5F, 0.5F, 0, new Color(255, 255, 255), true, 0);
                PlantStockLabel = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(
                    Main.StringToU16(
                    ...
                    new Vector3(1066.399, -1980.138, 31.01464), 5F, 0.5F, 0, new Color(255, 255, 255), true, 0);

                PedSystem.Repository.CreateQuest("s_m_y_construct_01", new Vector3(-1637.993, -800.12006, 10.246064),
                    57.71f, title: "~y~NPC~w~ Марк\nСкупщик руды", colShapeEnums: ColShapeEnums.OresSell);

                CustomColShape.CreateSphereColShape(new Vector3(-595.855, 2094.609, 131.4356), 5f, 0,
                    ColShapeEnums.GovMineStock);
            }
            catch (Exception e)
            {
        ...
        public static void GovMineStockLoad(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PickaxeBroken), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouMustBeInVeh), 3000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.VehNoResources), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantGruzitVehResc), 3000);
                        return;
                    }

                    var count = Chars.Repository.getCountItem(VehicleManager.GetVehicleToInventory(vehicle.NumberPlate),
                        GetItems());
                    if (count >= 3200)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.VehMaxResc), 3000);
                        return;
                    }

                    if (MineStockOres[0] == 0 && MineStockOres[1] == 0 && MineStockOres[2] == 0 &&
                        MineStockOres[3] == 0 && MineStockOres[4] == 0 && MineStockOres[5] == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.WarehouseEmpty), 3000);
                        return;
                    }
                    ...
                    for (var i = 0; i < 6; i++)
                        if (MineStockOres[i] >= 1)
                        {
                            ...
                            {
                                Chars.Repository.AddNewItem(null,
                                    VehicleManager.GetVehicleToInventory(vehicle.NumberPlate), "vehicle", item_type,
                                    MineStockOres[i]);

                                if (item_type == ItemId.Coal)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Puglya, MineStockOres[i]);
                                else if (item_type == ItemId.Iron)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Piron, MineStockOres[i]);
                                else if (item_type == ItemId.Sulfur)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Psera, MineStockOres[i]);
                                else if (item_type == ItemId.Emerald)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Pizumrud, MineStockOres[i]);
                            ...
                            else
                            {
                                ...
                                if (item_type == ItemId.Coal)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Puglya, 3200 - count);
                                else if (item_type == ItemId.Iron)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Piron, 3200 - count);
                                else if (item_type == ItemId.Sulfur)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Psera, 3200 - count);
                                else if (item_type == ItemId.Emerald)
                                    LoadInfo += LangFunc.GetText(LangType.Ru, DataName.Pizumrud, 3200 - count);
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SucLoadResToVeh), 3000);
                    SaveMineStocks(1);

                    LoadInfo = LoadInfo.Remove(LoadInfo.Length - 2);
                    Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeOre,
                        LangFunc.GetText(LangType.Ru, DataName.LoadedFromWarehouse, LoadInfo));
                }
            }
            catch (Exception e)
            {
                Log.Write($"GovMineStockLoad Exception: {e}");
            }
        }
        ...
        public static void PlayerEnteredMineShape(ExtPlayer player, int zone)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.OnlyGosMine), 3000);
                    return;
                }
            ...
            {
                Log.Write($"PlayerEnteredMineShape Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_PlayerStartedMining(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantStartJobNow), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"PlayerStartedMining Exception: {e}");
            }
        }
        ...
        public static void PlayerStopCarryOre(ExtPlayer player)
        {
            ...
            {
                Log.Write($"PlayerStopCarryOre Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_PlayerMinedGovResources(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveNoPickaxe), 3000);
                    return;
                }
                ...
                if (MainPickaxe.Item.Data == "" || MainPickaxe.Item.Data.Length < 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PickaxeBroken), 3000);
                    Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", MainPickaxe.Item.ItemId);
                }
                else if
                    ((MainPickaxe.Item.ItemId == ItemId.Pickaxe1 && Convert.ToInt32(MainPickaxe.Item.Data) - 1 <= 0) ||
                     (MainPickaxe.Item.ItemId == ItemId.Pickaxe2 && Convert.ToInt32(MainPickaxe.Item.Data) - 1 <= 0) ||
                     (MainPickaxe.Item.ItemId == ItemId.Pickaxe3 && Convert.ToInt32(MainPickaxe.Item.Data) - 1 <= 0))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PickaxeBroken), 3000);
                    Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", MainPickaxe.Item.ItemId);
                ...
                {
                    if (MainPickaxe.Item.ItemId == ItemId.Pickaxe1)
                        MainPickaxe.Item.Data = $"{Convert.ToInt32(MainPickaxe.Item.Data) - 1}";
                    else if (MainPickaxe.Item.ItemId == ItemId.Pickaxe2)
                        MainPickaxe.Item.Data = $"{Convert.ToInt32(MainPickaxe.Item.Data) - 1}";
                    else if (MainPickaxe.Item.ItemId == ItemId.Pickaxe3)
                        MainPickaxe.Item.Data = $"{Convert.ToInt32(MainPickaxe.Item.Data) - 1}";

                    Chars.Repository.SetItemData(player, MainPickaxe.Location, MainPickaxe.Index, MainPickaxe.Item,
                        true);
                }
            ...
            {
                Log.Write($"PlayerMinedGovResources Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_PlayerMinedResources(ExtPlayer player)
        {
            ...
            {
                Log.Write($"PlayerMinedResources Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_PlayerPutGovResources(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.SklafFulled), 3000);
                Trigger.ClientEvent(player, "mineJob_updateOreCarryStatus", false);
            ...
            {
                Log.Write($"PlayerPutGovResources Exception: {e}");
            }
        }
        ...
        public static void UpdateOres()
        {
            ...
            {
                var _OresData = new Dictionary<int, OreData>();
                var OresList = db.Ores.ToList();
                foreach (var ore in OresList) _OresData.Add(ore.Index, new OreData(ore.Price, (ItemId)ore.ItemId));
                OresData = _OresData;
            }
        }
        ...
        public static void RemoteEvent_PlayerSellOres(ExtPlayer player, int stock_item_index, int amount)
        {
            try
            {
                ...
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (!OresData.ContainsKey(stock_item_index)) return;

                var item_type = OresData[stock_item_index].ItemId;
                var price = OresData[stock_item_index].Price;

                var oreamount = Chars.Repository.isItem(player, "inventory", item_type);
                var count = oreamount == null ? 0 : oreamount.Item.Count;
                ...
                else
                {
                    ...
                    GameLog.Money("server", $"player({characterData.UUID})", totalmoney,
                        $"sellOre({stock_item_index},{amount})");
        ...
        public static void SaveMineStocks(int mine_index)
        {
            try
            {
                ...
                if (mine_index == 1 && MineStockLabel != null)
                    MineStockLabel.Text = Main.StringToU16(
                        $"~y~Состояние хранилища:\n\nИскопаемого угля {MineStockOres[0]} ед.\nЖелезной руды {MineStockOres[1]} ед.\nСерной руды {MineStockOres[3]} ед.\nДрагоценных камней {MineStockOres[2] + MineStockOres[4] + MineStockOres[5]} ед.");
            ...
            {
                Log.Write($"SaveMineStocks Exception: {e}");
            }
        }
