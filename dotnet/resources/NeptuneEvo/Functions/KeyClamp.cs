...
namespace NeptuneEvo.Functions
{
    public class KeyClamp : Script
    {
        ...
        public static void ClearKeyClamp(ColShapeEnums colShapeId, int index = (int)ColShapeData.Error,
            int listId = (int)ColShapeData.Error)
        {
            try
            {
                foreach (var foreachPlayer in Entities.Players.All.Cast<ExtPlayer>())
                {
                    ...
                    if (foreachKeyClampData.ColShapesData.ColShapeId != colShapeId ||
                        foreachKeyClampData.ColShapesData.Index != index ||
                        foreachKeyClampData.ColShapesData.ListId != listId)
                        continue;
