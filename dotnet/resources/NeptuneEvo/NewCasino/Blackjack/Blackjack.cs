...
{
    internal class Blackjack : Script
    {
        ...
        public static Dictionary<ExtPlayer, BlackjackPlayerInfo> PlayerData =
            new Dictionary<ExtPlayer, BlackjackPlayerInfo>();
        ...
        public static void Init()
        {
            try
            {
                ...
                var ranks = new string[13]
                    { "02", "03", "04", "05", "06", "07", "08", "09", "10", "jack", "queen", "king", "ace" };
                var suits = new string[4] { "spd", "hrt", "dia", "club" };
                ...
                DefaultDeck =
                    _List; // Дабы не пересоздавать Дэку каждую новую игру, так как она всё равно каждый раз одна
            }
            catch (Exception e)
            {
        ...
        public static void GetDeck(int index)
        {
            try
            {
                ...
                if (DefaultDeck.Count == 0)
                {
                    ...
                    DefaultDeck =
                        _List; // Если вдруг что-то пойдёт не так, то мы перестрахуемся и тут еще раз пропишем DefaultDeck
                }
        ...
        private static string takeCard(int index)
        {
            try
            {
                ...
                var List = DealerData[index].Cards;
            ...
            {
                Log.Write($"takeCard Exception: {e}");
                return null;
            }
        }
        ...
        private static int CardValue(string card)
        {
            ...
            {
                Log.Write($"CardValue Exception: {e}");
                return 10;
            }
        }
        ...
        private static int handValue(List<string> List)
        {
            try
            {
                ...
                foreach (var v in List) tmpValue += CardValue(v);
            ...
            {
                Log.Write($"handValue Exception: {e}");
                return 0;
            }
        }
        ...
        private static int AddDealerHand(int index, string card)
        {
            ...
            {
                Log.Write($"AddDealerHand Exception: {e}");
                return 0;
            }
        }
        ...
        private static int DealerHandValue(int index)
        {
            ...
            {
                Log.Write($"DealerHandValue Exception: {e}");
                return 0;
            }
        }
        ...
        private static int AddPlayerHand(ExtPlayer player, string card)
        {
            ...
            {
                Log.Write($"AddPlayerHand Exception: {e}");
                return 0;
            }
        }
        ...
        private static int AddPlayerSplitHand(ExtPlayer player, string card)
        {
            ...
            {
                Log.Write($"AddPlayerSplitHand Exception: {e}");
                return 0;
            }
        }
        ...
        private static int PlayerHandValue(ExtPlayer player)
        {
            ...
            {
                Log.Write($"PlayerHandValue Exception: {e}");
                return 0;
            }
        }
        ...
        private static int PlayerSplitHandValue(ExtPlayer player)
        {
            ...
            {
                Log.Write($"PlayerSplitHandValue Exception: {e}");
                return 0;
            }
        }

        [RemoteEvent("server.blackjack.character_occupy_place")]
        public static void BlackjackOpen(ExtPlayer player, int Index, int SlotId, float PosX, float PosY, float PosZ,
            float Rotation)
        {
            try
            {
                ...
                if (!characterData.InCasino || !characterData.IsAlive || sessionData.CuffedData.Cuffed ||
                    DealerData[Index].Players.Contains(player)) return;
                if (characterData.Money < 500)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.Table500Bucks), 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(player, "client.blackjack.character_occupy_place", Index, SlotId,
                    !DealerData[Index].GameRunning ? 1 : 0, 0);
            ...
            {
                Log.Write($"BlackjackOpen Exception: {e}");
            }
        }
        ...
        private static void PlayDealerAnim(int Index, Vector3 Position, string animDict, string anim)
        {
            ...
            {
                Log.Write($"PlayDealerAnim Exception: {e}");
            }
        }
        ...
        private static void PlayDealerSpeech(int Index, Vector3 Position, string speech)
        {
            ...
            {
                Log.Write($"PlayDealerSpeech Exception: {e}");
            }
        }
        ...
        public static void SetBet(ExtPlayer player, int money)
        {
            try
            {
                ...
                var bInfo = PlayerData[player];
                if (DealerData[bInfo.Index].GameRunning)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantBetByGame), 5000);
                    return;
                }

                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.BetSuc), 1000);
                ...
                bInfo.Join = true;
                bInfo.Rate = money;
                Trigger.ClientEventInRange(player.Position, 250f, "client.blackjack.PlaceBetChip", bInfo.Index,
                    bInfo.SlotId, money, false, false);
                ...
                {
                    if (player.IsCharacterData())
                        Trigger.PlayAnimation(player, "anim_casino_b@amb@casino@games@shared@player@", "idle_var_01",
                            3);
                ...
                if (BlackjackData[bInfo.Index].Timer == null)
                {
                    ...
                    BlackjackData[bInfo.Index].Timer = Timers.Start(1000, () => { BlackJackGame(bInfo.Index, mypos); });
                }
            }
            catch (Exception e)
            {
                Log.Write($"SetBet Exception: {e}");
            }
        }
        ...
        private static void BlackJackGame(int Index, Vector3 Position)
        {
                ...
                switch (Table.State)
                {
                    case 0: // Время после первой ставки и до начала раздачи карт
                        ...
                        if (--Table.Time > 0)
                        {
                            ...
                            {
                                foreach (var foreachPlayer in DealerData[Index].Players)
                                {
                                    ...
                                    if (PlayerData.ContainsKey(foreachPlayer) && PlayerData[foreachPlayer].Join)
                                    {
                                        ...
                                        if (Table.Time == 1)
                                        {
                                            ...
                                            Trigger.ClientEvent(foreachPlayer, "client.blackjack.RetrieveCards", Index,
                                                0);
                                        }
                                    }
                                }
                            ...
                            lock (DealerData[Index].Players)
                            {
                                foreach (var foreachPlayer in DealerData[Index].Players)
                                {
                                    ...
                                    if (PlayerData.ContainsKey(foreachPlayer))
                                    {
                                        ...
                                        if (PlayerData[foreachPlayer].Join &&
                                            !DealerData[Index].GamePlayers.Contains(foreachPlayer))
                                            DealerData[Index].GamePlayers.Add(foreachPlayer);
                                    }
                                }
                            }

                            if (DealerData[Index].GamePlayers.Count ==
                                0) // Если вдруг игроков не оказалось - отключаем таймер стола
                            {
                        ...
                        if (Table.Time > 0) Table.Time--;
                            ...
                            switch (Table.AdditionalState)
                            {
                                case 0: // Дилер получает первую карту
                                    ...
                                    Trigger.ClientEventInRange(Position, 250f, "client.blackjack.GiveCard", Index, 0,
                                        Table.Count - 1, Table.CardInHand, Table.Count == 1);
                                    PlayDealerAnim(Index, Position, "anim_casino_b@amb@casino@games@blackjack@dealer",
                                        "deal_card_self");
                                    Table.Time = 1;
                                ...
                                default: // Дилер раздаёт игрокам первую карту
                                    ...
                                    {
                                        foreach (var foreachPlayer in DealerData[Index].GamePlayers)
                                            if (foreachPlayer.IsCharacterData() &&
                                                PlayerData.ContainsKey(foreachPlayer) &&
                                                !Table.CardsGiven.Contains(foreachPlayer))
                                            {
                                                givecard = true;
                                                Table.CardsGiven.Add(foreachPlayer);
                                                var bInfo = PlayerData[foreachPlayer];
                                                Table.CardInHand = takeCard(Index);
                                                Table.Count = AddPlayerHand(foreachPlayer, Table.CardInHand);
                                                Trigger.ClientEventInRange(Position, 250f, "client.blackjack.GiveCard",
                                                    Index, bInfo.SlotId, Table.Count - 1, Table.CardInHand);
                                                PlayDealerAnim(Index, Position,
                                                    "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                    $"deal_card_player_0{bInfo.SlotId}");
                                                PlayDealerSpeech(Index, Position,
                                                    $"MINIGAME_BJACK_DEALER_{PlayerHandValue(foreachPlayer)}");
                                                Table.Time = 1;
                                                break;
                                            }
                                    }
                        ...
                        if (Table.Time > 0) Table.Time--;
                            ...
                            switch (Table.AdditionalState)
                            {
                                case 0: // Дилер получает вторую карту
                                    ...
                                    if (Table.Count > 1)
                                        PlayDealerSpeech(Index, Position,
                                            $"MINIGAME_BJACK_DEALER_{CardValue(Table.CardInHand)}");
                                    Table.Time = 1;
                                    Table.AdditionalState = 1;
                                    break;
                    ...
                    case 4: // Начинаем взаимодействие с игроками, фокусируемся на одном из них
                        ...
                        else if (Table.Time == 0)
                        {
                            ...
                            lock (DealerData[Index].GamePlayers)
                            {
                                foreach (var foreachPlayer in DealerData[Index].GamePlayers)
                                    if (foreachPlayer.IsCharacterData() && PlayerData.ContainsKey(foreachPlayer) &&
                                        !Table.DealerFocused
                                            .Contains(
                                                foreachPlayer)) // Не опрашиваем игроков, которые уже были опрошены
                                    {
                                        if (PlayerHandValue(foreachPlayer) <
                                            21) // Опрашиваем всех игроков, которые не имеют 21 на столе
                                        {
                                            target = foreachPlayer;
                                            break;
                                        }

                                        if (!Table.DealerFocused.Contains(foreachPlayer))
                                            Table.DealerFocused
                                                .Add(
                                                    foreachPlayer); // Добавляем в лист опрошенных, потому что у игрока уже 21, его не нужно опрашивать повторно
                                    }
                            }
                            ...
                            PlayDealerAnim(Index, Position, "anim_casino_b@amb@casino@games@blackjack@dealer",
                                $"dealer_focus_player_0{PlayerData[target].SlotId}_idle_intro");
                            Table.Time = 1;
                            Table.State = 5;
                        }
                    ...
                    case 5: // Получаем решение игрока, на котором мы фокусируемся
                        ...
                        switch (Table.AdditionalState)
                        {
                            case 0: // Начинаем обработку первого решения игрока
                                if (Table.DealerFocus == null || !Table.DealerFocus.IsCharacterData() ||
                                    !PlayerData.ContainsKey(Table
                                        .DealerFocus)) // Если игрок вышел со стола (вылет или еще что-то), то берём нового игрока из списка
                                {
                                    Table.State = 4;
                                    Table.Time = 0;
                                    Table.StateInWork = false;
                            ...
                            case 1: // Обрабатываем первое решение игрока
                                if (Table.DealerFocus != null)
                                {
                                    ...
                                    if (targetCharacterData != null && PlayerData.ContainsKey(Table.DealerFocus) &&
                                        Table.DealerFocused.Contains(Table.DealerFocus))
                                    {
                                        var bInfo = PlayerData[Table.DealerFocus];
                                        if (bInfo.Join && bInfo.Hand.Count < 5)
                                        {
                                            if (bInfo.Move == null)
                                            {
                                                ...
                                                else if (Table.Time == 0)
                                                {
                                                    ...
                                                    Table.AdditionalState = 0;
                                                    Table.Time = 0;
                                                    Trigger.ClientEvent(Table.DealerFocus, "client.blackjack.isBtn", 0,
                                                        0);
                                                }
                                                ...
                                                Trigger.ClientEvent(Table.DealerFocus, "client.blackjack.SyncTimer",
                                                    Table.Time);
                                            }
                                            ...
                                            else if (bInfo.Move == "hit")
                                            {
                                                ...
                                                PlayDealerAnim(Index, Position,
                                                    "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                    $"hit_card_player_0{bInfo.SlotId}");
                                                PlayDealerSpeech(Index, Position,
                                                    $"MINIGAME_BJACK_DEALER_{PlayerHandValue(Table.DealerFocus)}");
                                                Table.Time = 1;
                                                if (bInfo.Hand.Count < 5 && PlayerHandValue(Table.DealerFocus) < 21 &&
                                                    !bInfo.Doubled)
                                                {
                                                    Table.AdditionalState =
                                                        2; // Если меньше 5 карт и общее число меньше 21 и это не дабл
                                                }
                                                else
                                                {
                                                    Table.State = 4;
                                            ...
                                            {
                                                bInfo.Move = null;
                                                if (Wallet.Change(Table.DealerFocus, -bInfo.Rate))
                                                {
                                                    GameLog.CasinoBJLog(-bInfo.Rate);
                                                    GameLog.Money($"player({targetCharacterData.UUID})", "system",
                                                        bInfo.Rate, "BJDouble");
                                                    bInfo.Rate = bInfo.Rate * 2;
                                                    Trigger.ClientEventInRange(Position, 250f,
                                                        "client.blackjack.PlaceBetChip", bInfo.Index, bInfo.SlotId,
                                                        bInfo.Rate, true, false);
                                                    bInfo.Doubled = true;
                                                    Table.CardInHand = takeCard(Index);
                                                    Table.Count = AddPlayerHand(Table.DealerFocus, Table.CardInHand);
                                                    Trigger.ClientEventInRange(Position, 250f,
                                                        "client.blackjack.GiveCard", Index, bInfo.SlotId,
                                                        Table.Count - 1, Table.CardInHand);
                                                    PlayDealerAnim(Index, Position,
                                                        "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                        $"hit_card_player_0{bInfo.SlotId}");
                                                    PlayDealerSpeech(Index, Position,
                                            ...
                                            else if (bInfo.Move == "split")
                                            {
                                                ...
                                                if (Wallet.Change(Table.DealerFocus, -bInfo.Rate))
                                                {
                                                    ...
                                                    Trigger.ClientEventInRange(Position, 250f,
                                                        "client.blackjack.PlaceBetChip", bInfo.Index, bInfo.SlotId,
                                                        bInfo.Rate, false, true);
                                                    PlayDealerAnim(Index, Position,
                                                        "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                        $"split_card_player_0{bInfo.SlotId}");
                            ...
                            case 3: // Выдаем к
                                if (Table.DealerFocus != null)
                                {
                                    if (Table.DealerFocus.IsCharacterData() &&
                                        PlayerData.ContainsKey(Table.DealerFocus) &&
                                        Table.DealerFocused.Contains(Table.DealerFocus))
                                    {
                                        var bInfo = PlayerData[Table.DealerFocus];
                                        Trigger.ClientEventInRange(Position, 250f, "client.blackjack.SplitHand", Index,
                                            bInfo.SlotId, bInfo.SplitHand.Count - 1);
                                        Table.Time = 0;
                            ...
                            case 4: // Пережидаем анимацию дилера
                                if (Table.DealerFocus != null)
                                {
                                        ...
                                        Table.DealerFocused.Contains(Table.DealerFocus))
                                    {
                                        ...
                                        Table.CardInHand = takeCard(Index);
                                        Table.Count = AddPlayerHand(Table.DealerFocus, Table.CardInHand);
                                        Trigger.ClientEventInRange(Position, 250f, "client.blackjack.GiveCard", Index,
                                            bInfo.SlotId, Table.Count - 1, Table.CardInHand);
                                        PlayDealerAnim(Index, Position,
                                            "anim_casino_b@amb@casino@games@blackjack@dealer",
                                            $"hit_card_player_0{bInfo.SlotId}");
                                        PlayDealerSpeech(Index, Position,
                            ...
                            case 6: // Пережидаем анимацию дилера
                                if (Table.DealerFocus != null)
                                {
                                        ...
                                        Table.DealerFocused.Contains(Table.DealerFocus))
                                    {
                                        ...
                                        Trigger.ClientEventInRange(Position, 250f, "client.blackjack.GiveCard", Index,
                                            bInfo.SlotId, Table.CountSplit - 1, Table.CardSplitInHand, false, true);
                                        PlayDealerAnim(Index, Position,
                                            "anim_casino_b@amb@casino@games@blackjack@dealer",
                                            $"hit_second_card_player_0{bInfo.SlotId}");
                                        PlayDealerSpeech(Index, Position,
                                            $"MINIGAME_BJACK_DEALER_{PlayerSplitHandValue(Table.DealerFocus)}");
                                ...
                                if (Table.DealerFocus != null)
                                {
                                        ...
                                        Table.DealerFocused.Contains(Table.DealerFocus))
                                    {
                                        ...
                                        if (bInfo.Hand.Count < 5 && PlayerHandValue(Table.DealerFocus) < 21)
                                        {
                                            ...
                                            PlayDealerAnim(Index, Position,
                                                "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                $"dealer_focus_player_0{bInfo.SlotId}_idle");
                                        }
                                        else if (bInfo.SplitHand.Count < 5 &&
                                                 PlayerSplitHandValue(Table.DealerFocus) < 21)
                                        {
                                            PlayDealerSpeech(Index, Position, "MINIGAME_BJACK_DEALER_ANOTHER_CARD");
                                            Table.State = 10; // Если меньше 5 карт и общее число меньше 21
                                            PlayDealerAnim(Index, Position,
                                                "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                $"dealer_focus_player_0{bInfo.SlotId}_idle_split");
                                        }
                                        else
                                        {
                                            Table.State = 4;
                    ...
                    case 6: // Дилер работает со своими картами
                        ...
                        switch (Table.AdditionalState)
                        {
                            case 0:
                                PlayDealerAnim(Index, Position, "anim_casino_b@amb@casino@games@blackjack@dealer",
                                    "turn_card");
                                Trigger.ClientEventInRange(Position, 250f, "client.blackjack.DealerTurnOverCard",
                                    Index);
                                Table.Time = 1;
                                Table.AdditionalState = 1;
                                break;
                            case 1: // Дилер набирает карты до тех пор, пока не получит число 17 или выше
                                ...
                                else if (Table.Time == 0)
                                {
                                    ...
                                    if (Table.Value < 17)
                                    {
                                        ...
                                        Table.Count = AddDealerHand(Index, Table.CardInHand);
                                        Trigger.ClientEventInRange(Position, 250f, "client.blackjack.GiveCard", Index,
                                            0, Table.Count - 1, Table.CardInHand, Table.Count == 1);
                                        PlayDealerAnim(Index, Position,
                                            "anim_casino_b@amb@casino@games@blackjack@dealer",
                                            "deal_card_self_second_card");
                                        Table.Time = 1;
                    ...
                    case 7: // Работаем по каждому игроку, который есть за столом отдельно
                        ...
                        lock (DealerData[Index].GamePlayers)
                        {
                            foreach (var foreachPlayer in DealerData[Index].GamePlayers)
                                if (foreachPlayer.IsCharacterData() && PlayerData.ContainsKey(foreachPlayer) &&
                                    PlayerData[foreachPlayer].Join && Table.DealerFocused.Contains(foreachPlayer))
                                {
                        ...
                        if (winner == null) // Все игроки за столом получили призы, очищаем стол дилера и игру
                        {
                            ...
                            var newPlayers = new List<ExtPlayer>();
                            foreach (var foreachPlayer in DealerData[Index].Players)
                                if (foreachPlayer.IsCharacterData() && PlayerData.ContainsKey(foreachPlayer))
                                {
                        ...
                        switch (Table.AdditionalState)
                        {
                            case 0:
                                if (Table.DealerFocus != null)
                                {
                                    var targetCharacterData = Table.DealerFocus.GetCharacterData();
                                    if (targetCharacterData != null && PlayerData.ContainsKey(Table.DealerFocus) &&
                                        PlayerData[Table.DealerFocus].Join)
                                    {
                                        var bInfo = PlayerData[Table.DealerFocus];
                                            ...
                                            if (pHandValue <= 21)
                                            {
                                                if (pHandValue == 21)
                                                {
                                                    if (bInfo.SplitHand.Count == 2) // BlackJack
                                                    {
                                                        ...
                                                        wincount += rounded;
                                                        GameLog.Money("system", $"player({targetCharacterData.UUID})",
                                                            rounded, "BJWinSplit");
                                                        Wallet.Change(Table.DealerFocus, rounded);
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        GameLog.Money("system", $"player({targetCharacterData.UUID})",
                                                            bInfo.Rate * 2, "BJWinSplit");
                                                        Wallet.Change(Table.DealerFocus, bInfo.Rate * 2);
                                                ...
                                                else
                                                {
                                                    if (pHandValue == Table.Value)
                                                    {
                                                        wincount += bInfo.Rate;
                                                        GameLog.Money("system", $"player({targetCharacterData.UUID})",
                                                            bInfo.Rate, "BJDrawSplit");
                                                        Wallet.Change(Table.DealerFocus, bInfo.Rate);
                                                        GameLog.CasinoBJLog(bInfo.Rate);
                                                    }
                                        ...
                                        if (pHandValue <= 21) // У игрока 21 или меньше
                                        {
                                            ...
                                            {
                                                if (Table.Value == 21 ||
                                                    (Table.Value < 21 && Table.Value > pHandValue)) // Проигрышь
                                                {
                                        ...
                                        bInfo.Join = false;
                                        if (wincount > 0)
                                            Trigger.ClientEvent(Table.DealerFocus, "client.blackjack.betWin", wincount);
                                        Table.SlotIDReceive = bInfo.SlotId;
                                        PlayDealerAnim(Index, Position,
                                            "anim_casino_b@amb@casino@games@blackjack@dealer",
                                            $"retrieve_cards_player_0{Table.SlotIDReceive}");
                                    }
                                    else
                                    {
                                        Table.State = 7;
                            ...
                            case 1: // Забираем карты со стола игрока и переходим к следующему игроку
                                Trigger.ClientEventInRange(Position, 250f, "client.blackjack.RetrieveCards", Index,
                                    Table.SlotIDReceive);
                                Table.State = 7;
                                Table.AdditionalState = 1;
                                break;
                    ...
                    case 10: // Работаем со второй рукой сплит-системы
                        ...
                        switch (Table.AdditionalState)
                        {
                            case 0: // Начинаем обработку первого решения игрока
                                ...
                                PlayDealerSpeech(Index, Position, "MINIGAME_BJACK_DEALER_ANOTHER_CARD");
                                PlayDealerAnim(Index, Position, "anim_casino_b@amb@casino@games@blackjack@dealer",
                                    $"dealer_focus_player_0{PlayerData[Table.DealerFocus].SlotId}_idle_split");
                                Table.AdditionalState = 1;
                                Table.Time = 15;
                                break;
                            case 1: // Обрабатываем первое решение игрока
                                        ...
                                        Table.DealerFocused.Contains(Table.DealerFocus))
                                    {
                                        ...
                                        if (bInfo.Join && bInfo.SplitHand.Count < 5)
                                        {
                                            ...
                                            else if (bInfo.Move == "hit")
                                            {
                                                bInfo.Move = null;
                                                Table.CardSplitInHand = takeCard(Index);
                                                Table.CountSplit = AddPlayerSplitHand(Table.DealerFocus,
                                                    Table.CardSplitInHand);
                                                Trigger.ClientEventInRange(Position, 250f, "client.blackjack.GiveCard",
                                                    Index, bInfo.SlotId, Table.CountSplit - 1, Table.CardSplitInHand,
                                                    false, true);
                                                PlayDealerAnim(Index, Position,
                                                    "anim_casino_b@amb@casino@games@blackjack@dealer",
                                                    $"hit_second_card_player_0{bInfo.SlotId}");
                                                ...
                                                {
                                                    Table.AdditionalState =
                                                        2; // Если меньше 5 карт и общее число меньше 21
                                                }
                                                else
                                                {
                                                    Table.State = 4;
            ...
            {
                Log.Write($"BlackJackGame Exception: {e}");
            }
        }
        ...
        private static void AnimationGameEnd(ExtPlayer player, string anim)
        {
            try
            {
                ...
                {
                    var rand = new Random();
                    Trigger.PlayAnimation(player, "anim_casino_b@amb@casino@games@shared@player@",
                        $"reaction_{anim}_var_0{rand.Next(1, 5)}", 3);
            ...
            {
                Log.Write($"AnimationGameEnd Exception: {e}");
            }
        }
        ...
        public static void BlackjackMove(ExtPlayer player, string move)
        {
            ...
            {
                Log.Write($"BlackjackMove Exception: {e}");
            }
        }
        ...
        public static void BlackjackLeave(ExtPlayer player)
        {
            ...
            {
                Log.Write($"BlackjackLeave Exception: {e}");
            }
        }
        ...
        public static void BlackjackLeave(ExtPlayer player, bool type)
        {
            try
            {
                ...
                if (DealerData.ContainsKey(bInfo.Index))
                {
                    ...
                    {
                        Wallet.Change(player, bInfo.Rate);
                        GameLog.CasinoBJLog(bInfo.Rate);
                        GameLog.Money("system", $"player({characterData.UUID})", bInfo.Rate, "BJBetReturn");
                    }

                    if (DealerData[bInfo.Index].Chairs.Contains(bInfo.SlotId))
                        DealerData[bInfo.Index].Chairs.Remove(bInfo.SlotId);
                    if (DealerData[bInfo.Index].Players.Contains(player))
                        DealerData[bInfo.Index].Players.Remove(player);

                    Trigger.ClientEventInRange(player.Position, 250f, "client.blackjack.RetrieveCards", bInfo.Index,
                        bInfo.SlotId);
                    if (DealerData[bInfo.Index].GamePlayers.Contains(player))
                        DealerData[bInfo.Index].GamePlayers.Remove(player);

                    if ((DealerData[bInfo.Index].GameRunning && DealerData[bInfo.Index].GamePlayers.Count == 0) ||
                        (!DealerData[bInfo.Index].GameRunning && DealerData[bInfo.Index].Players.Count == 0))
                    {
                ...
                if (!type)
                {
                    ...
                    Trigger.ClientEvent(player, "client.blackjack.PlayDealerSpeech", bInfo.Index,
        ...
        public class BJTableData
        {
            ...
            public byte AdditionalState { get; set; }
            public int Value { get; set; }
            public int SlotIDReceive { get; set; }
            public ExtPlayer DealerFocus { get; set; }
