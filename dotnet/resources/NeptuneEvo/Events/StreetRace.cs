...
namespace NeptuneEvo.Events
{
    internal class StreetRaceData
    {
        ...
        public ExtPlayer created { get; set; }
        public ExtPlayer joined { get; set; }
        public Vector3 Position { get; set; } = new Vector3();
        public bool Status { get; set; }
    }

    internal class StreetRace : Script
    {
        ...
        public static void OpenStreetRaceMenu(ExtPlayer player)
        {
            try
            {
                ...
                var playersInRange =
                    Main.GetPlayersInRadiusOfPosition(player.Position, 10f, UpdateData.GetPlayerDimension(player));

                foreach (var foreachPlayer in playersInRange)
                    if (!PlayerToStreetRaceId.ContainsKey(foreachPlayer.Value) && foreachPlayer.Value != player.Value &&
                        foreachPlayer.IsInVehicle && foreachPlayer.VehicleSeat == (int)VehicleSeat.Driver)
                    {
                        if (characterData.Friends.ContainsKey(foreachPlayer.Name))
                            frameList.List.Add(new ListData($"{foreachPlayer.Name} - {foreachPlayer.Value} ID",
                                foreachPlayer.Value));
                        else
            ...
            {
                Log.Write($"OpenStreetRaceMenu Exception: {e}");
            }
        }
        ...
        private static void callback_selectChar(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                var target = Main.GetPlayerByID(Convert.ToInt32(listItem));
                ...
                Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.StreetRace),
                    LangFunc.GetText(LangType.Ru, DataName.EnterStavka0to100k), 6, "streetrace");
            }
            catch (Exception e)
            {
                Log.Write($"callback_selectChar Exception: {e}");
            }
        }
        ...
        public static void SelectRate(ExtPlayer player, int rate)
        {
            try
            {
                ...
                if (!PlayerToStreetRaceId.ContainsKey(player.Value)) return;
                if (!StreetRaceList.ContainsKey(player.Value)) return;
                var streetRaceData = StreetRaceList[PlayerToStreetRaceId[player.Value]];
            ...
            {
                Log.Write($"SelectRate Exception: {e}");
            }
        }
        ...
        public static void ClearData(ExtPlayer player, int messageId)
        {
            ...
            {
                if (!PlayerToStreetRaceId.ContainsKey(player.Value)) return;
                var streetRaceId = PlayerToStreetRaceId[player.Value];
                PlayerToStreetRaceId.Remove(player.Value);
                if (!StreetRaceList.ContainsKey(streetRaceId)) return;
                var streetRaceData = StreetRaceList[streetRaceId];
                var WinPlayer = streetRaceData.created.Value == player.Value
                    ? streetRaceData.joined
                    : streetRaceData.created;
                ...
                switch (messageId)
                {
                    ...
                    case 1:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouDiedLose), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VragDiedWin), 3000);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VragDiedCancel), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VragDkSWin), 3000);
                        }

                        break;
                    case 2:
                        Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.VragDkWin), 3000);
                        break;
                    case 3:
                        if (streetRaceData.Status)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouLeftTsLose), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VragLeftTsWin), 3000);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouleftTsCancel), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VragLeftTsCancel), 3000);
                        }
                    ...
                    case 4:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouMustTsLose), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouMustTsLose), 3000);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.VragLeftTsWinS), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouLeftTsCancel), 3000);
                        }
                    ...
                    case 5:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerFarWin), 3000);
                            Notify.Send(WinPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.YouFarCancel), 3000);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
        ...
        public static void StreetRacePosition(ExtPlayer player, float sr_X, float sr_Y, float sr_Z)
        {
            try
            {
                ...
                if (!PlayerToStreetRaceId.ContainsKey(player.Value)) return;
                if (!StreetRaceList.ContainsKey(player.Value)) return;
                var streetRaceData = StreetRaceList[player.Value];
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath)
                {
                    ...
                    if (targetSessionData != null && !targetSessionData.CuffedData.Cuffed &&
                        !targetSessionData.DeathData.InDeath)
                    {
                        ...
                        EventSys.SendCoolMsg(streetRaceData.joined, "Гонка", "Вызов на гонку",
                            $"{LangFunc.GetText(LangType.Ru, DataName.PlayerOFferGonka, player.Value, streetRaceData.rate)}",
                            "", 10000);
                        EventSys.SendCoolMsg(player, "Гонка", "Вызов на гонку",
                            $"{LangFunc.GetText(LangType.Ru, DataName.YouOfferGonka, streetRaceData.joined.Value)}", "",
                            10000);
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantGonkaNow), 3000);
                    }
                }
                else
                {
            ...
            {
                Log.Write($"StreetRacePosition Exception: {e}");
            }
        }
        ...
        public static void Accept(ExtPlayer player)
        {
            try
            {
                ...
                if (!streetRaceData.created.IsInVehicle ||
                    streetRaceData.created.VehicleSeat != (int)VehicleSeat.Driver)
                {
                    ClearData(streetRaceData.created, 4);
                    return;
                }

                if (!streetRaceData.joined.IsInVehicle || streetRaceData.joined.VehicleSeat != (int)VehicleSeat.Driver)
                {
                    ClearData(streetRaceData.joined, 4);
                    return;
                }

                if (streetRaceData.created.Position.DistanceTo(streetRaceData.joined.Position) > 10)
                {
                    ClearData(streetRaceData.created, 5);
                    return;
                }

                if (UpdateData.CanIChange(streetRaceData.created, streetRaceData.rate, true) != 255)
                {
                    ClearData(streetRaceData.created, 0);
                    return;
                }

                if (UpdateData.CanIChange(streetRaceData.joined, streetRaceData.rate, true) != 255)
                {
                    ClearData(streetRaceData.joined, 0);
                    return;
                }
                ...
                Trigger.ClientEvent(streetRaceData.created, "client.streetrace.start", streetRaceData.Position.X,
                    streetRaceData.Position.Y, streetRaceData.Position.Z, true);
                Trigger.ClientEvent(streetRaceData.joined, "client.streetrace.start", streetRaceData.Position.X,
                    streetRaceData.Position.Y, streetRaceData.Position.Z, false);
            }
            catch (Exception e)
            {
                Log.Write($"Accept Exception: {e}");
            }
        }
        ...
        public static void StreetRaceFinished(ExtPlayer player)
        {
            try
            {
                ...
                if (!StreetRaceList.ContainsKey(streetRaceId)) return;
                var streetRaceData = StreetRaceList[streetRaceId];
                var cost = Convert.ToInt32(streetRaceData.rate * 2 * 0.9);
                ...
                EventSys.SendCoolMsg(player, "Гонка", "Победа!",
                    $"{LangFunc.GetText(LangType.Ru, DataName.YouWinGonka, cost)}", "", 10000);
                StreetRaceList[streetRaceId].Status = false;
                var LosePlayer = streetRaceData.created.Value == player.Value
                    ? streetRaceData.joined
                    : streetRaceData.created;
                ...
                EventSys.SendCoolMsg(LosePlayer, "Гонка", "Поражение :(",
                    $"{LangFunc.GetText(LangType.Ru, DataName.YouLoseGonka, cost)}", "", 10000);
            ...
            {
                Log.Write($"StreetRaceFinished Exception: {e}");
            }
        }
    }
}