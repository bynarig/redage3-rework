...
{
    internal class Island : Script
    {
        private static readonly nLog Log = new nLog("Events.Island");

        private static readonly Vector3 MiddleOfIsland = new Vector3(4945, -4687, 10);
        ...
        [Interaction(ColShapeEnums.Island, true)]
        public static void InIsland(ExtPlayer player)
        {
            if (!player.IsCharacterData()) return;
            player.Eval(
                "mp.game.invoke(\"0x9A9D1BA639675CF1\", \"HeistIsland\", true);mp.game.invoke(\"0x5E1460624D194A38\", true);");
        }
        ...
        public static void OutIsland(ExtPlayer player)
        {
            if (!player.IsCharacterData()) return;
            player.Eval(
                "mp.game.invoke(\"0x9A9D1BA639675CF1\", \"HeistIsland\", false);mp.game.invoke(\"0x5E1460624D194A38\", false);");
        }
    }
}