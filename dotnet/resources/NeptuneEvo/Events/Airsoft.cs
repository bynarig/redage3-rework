...
namespace NeptuneEvo.Events
{
    ...
    {
        public LobbyData(int index, string name, int price, string password, int mode, int weapon, int lobby_map,
            int max_players)
        {
            LobbyIndex = index;
            LobbyName = name;
        ...
        public int LobbyPrice { get; set; } //
        public string LobbyPassword { get; set; } //
        public int LobbyMode { get; set; } //

        [JsonIgnore] public int LobbyWeapon { get; set; }

        [JsonIgnore] public int LobbyMap { get; set; }

        [JsonIgnore] public int MaxPlayers { get; set; } = 4;

        [JsonIgnore] public int GameStatus { get; set; }

        [JsonIgnore] public int GameStartSeconds { get; set; } = 120;
        ...
        [JsonIgnore]
        public Dictionary<string, int> GunGameTop { get; set; } =
            new Dictionary<string, int>(); // ТОП игроков на GunGame
            ...
            GameFinalReward
        {
            get;
            set;
        } // Выплата на каждого члена победившей команды (1x1, 2x2, 3x3, 5x5) / выплата победителю GunGame

        [JsonIgnore] public string LobbyGameTimer { get; set; }

        [JsonIgnore] public List<ExtPlayer> LobbyPlayers { get; set; } = new List<ExtPlayer>();
    }
    ...
    internal class PlayerData
    {
        ...
        public int CorrectHealth { get; set; }
        public bool IsGunGamePlayer { get; set; }
    }

    internal class AirsoftWeaponData
    {
    ...
    internal class Airsoft : Script
    {
        private static readonly nLog Log = new nLog("Events.Airsoft");

        private static readonly int DefaultDimension = 2000;

        private static readonly Dictionary<int, LobbyData> LobbyList = new Dictionary<int, LobbyData>();
        public static Dictionary<ExtPlayer, PlayerData> AirsoftPlayerData = new Dictionary<ExtPlayer, PlayerData>();
        private static readonly List<ExtPlayer> PlayersInLobbyMenu = new List<ExtPlayer>();

        private static readonly List<(Vector3, byte, byte)> MapSpawnPoints = new List<(Vector3, byte, byte)>
        {
            // Дамба:
        ...
        private static readonly List<Vector3> GunGameMapSpawnPoints = new List<Vector3>
        {
            // Дамба:
        ...
        private static readonly List<(Vector3, byte)> MainMapsInfo = new List<(Vector3, byte)>
        {
        ...
        private static readonly AirsoftWeaponData[] WeaponModels = new AirsoftWeaponData[31]
        {
        ...
        private static readonly AirsoftWeaponData[] GunGameWeaponModels = new AirsoftWeaponData[17]
        {
        ...
        public static void AirsoftLobbiesStartInterval()
        {
            try
            {
                ...
                foreach (var lobby in LobbyList)
                    ...
                    {
                        var lobbyMinPlayers = lobby.Value.LobbyMode == 5 ? 2 : lobby.Value.MaxPlayers;
                        if (lobbyMinPlayers == lobby.Value.MaxPlayers ||
                            lobby.Value.LobbyPlayers.Count < lobbyMinPlayers) continue;
            ...
            {
                Log.Write($"AirsoftLobbiesStartInterval Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_UpdateServerPlayersInLobbyMenuList(ExtPlayer player, int state)
        {
            try
            {
                ...
                else if (state == 2 && PlayersInLobbyMenu.Contains(player)) PlayersInLobbyMenu.Remove(player);
            }
            catch (Exception e)
            {
                Log.Write($"UpdateServerPlayersInLobbyMenuList Exception: {e}");
            }
        }
        ...
        public static void LoadAirsoftLobbyList(ExtPlayer player)
        {
            try
            {
                ...
                if (!FunctionsAccess.IsWorking("LoadAirsoftLobbyList"))
                {
                ...
                qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage30, 1, true);
                Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 1,
                    JsonConvert.SerializeObject(LobbyList.Values), "airsoft");
            }
            catch (Exception e)
            {
                Log.Write($"LoadAirsoftLobbyList Exception: {e}");
            }
        }

        [RemoteEvent("airsoft_createLobby_server")]
        public static void RemoteEvent_airsoft_createLobby_server(ExtPlayer player, string lobby_name, int lobby_price,
            string lobby_password, int lobby_mode, int lobby_weapon, int lobby_map)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                if (!FunctionsAccess.IsWorking("airsoft_createLobby_server"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.FunctionOffByAdmins), 3000);
                    return;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.Map1x1only), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.LobbyAlreadyCreated), 3000);
                    return;
                }

                if (sessionData.InAirsoftLobby >= 0) LeaveLobbyFunction(player);

                if (characterData.Money < lobby_price)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.LobbyNeedMoney, lobby_price), 3000);
                    return;
                }

                var MaxLobbyPlayers = 2;
                ...
                LobbyList.Add(player.Value,
                    new LobbyData(player.Value, lobby_name.Length < 1 ? player.Name.Split("_")[0] : lobby_name,
                        lobby_price, lobby_password, lobby_mode, lobby_weapon, lobby_map, MaxLobbyPlayers));
                LobbyList[player.Value].LobbyPlayers.Add(player);
                UpdateLobbyList();

                if (lobby_mode != 5) LobbyList[player.Value].GameFinalReward = lobby_price * 2;
                ...
                GameLog.Money($"player({characterData.UUID})", "server", lobby_price,
                    $"createLobby({MaxLobbyPlayers})");

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.LobbyCreated), 3000);

                Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 3,
                    JsonConvert.SerializeObject(new int[3]
                    {
                        LobbyList[player.Value].LobbyPlayers.Count, lobby_mode == 5 ? 2 : MaxLobbyPlayers,
                        LobbyList[player.Value].MaxPlayers
                    }), "airsoft", LobbyList[player.Value].GameStartSeconds);
            }
            catch (Exception e)
            {
                Log.Write($"airsoft_createLobby_server Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_airsoft_joinLobby_server(ExtPlayer player, int index, string password)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.LobbyNotFound), 3000);
                    return;
                }
                ...
                if (sessionData.InAirsoftLobby >= 0 && sessionData.InAirsoftLobby == index)
                {
                    Trigger.ClientEvent(player, "airsoft_lobbyMenuHandler", 3,
                        JsonConvert.SerializeObject(new int[3]
                        {
                            LobbyList[index].LobbyPlayers.Count,
                            LobbyList[index].LobbyMode == 5 ? 2 : LobbyList[index].MaxPlayers,
                            LobbyList[index].MaxPlayers
                        }), "airsoft", LobbyList[index].GameStartSeconds);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MatchStarted), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.LobbyIncorrectPassowrd), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.LobbyFilled), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.ToEnterNeedMoney, LobbyList[index].LobbyPrice), 1500);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.LobbyConnected), 3000);
                ...
                if (LobbyList[index].LobbyMode == 5) // GunGame
                {
                    ...
                    if (LobbyList[index].LobbyPlayers.Count == 2)
                    {
                        ...
                        LobbyList[index].LobbyGameTimer = Timers.StartOnce($"LobbyGameTimer_{index}", 120000,
                            () => LobbyGameTimerFunction(index), true);

                        foreach (var foreachPlayer in LobbyList[index].LobbyPlayers)
                            if (foreachPlayer.IsCharacterData())
                            {
                                Trigger.ClientEvent(foreachPlayer, "airsoft_lobbyMenuHandler", 5,
                                    JsonConvert.SerializeObject(new int[3]
                                    {
                                        LobbyList[index].LobbyPlayers.Count,
                                        LobbyList[index].LobbyMode == 5 ? 2 : LobbyList[index].MaxPlayers,
                                ...
                                Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.MinLobbyOk), 10000);
                            }
                    }
                ...
                else if (LobbyList.ContainsKey(index) && LobbyList[index].LobbyMode != 5 &&
                         LobbyList[index].LobbyPlayers.Count >= LobbyList[index].MaxPlayers) // 1x1, 2x2, 3x3, 5x5
                {
                    LobbyList[index].GameStatus = 1;

                    var NextPlayerTeam = 0;

                    if (LobbyList[index].LobbyGameTimer != null) Timers.Stop(LobbyList[index].LobbyGameTimer);

                    var MatchTime = 300000;

                    if (LobbyList[index].LobbyMode >= 2) MatchTime = 600000;

                    LobbyList[index].LobbyGameTimer = Timers.StartOnce($"LobbyGameTimer_{index}", MatchTime,
                        () => LobbyGameTimerFunction(index), true);
                    ...
                    foreach (var foreachPlayer in LobbyList[index].LobbyPlayers)
                        if (foreachPlayer.IsCharacterData())
                        {
                            ...
                            Trigger.ClientEvent(foreachPlayer, "airsoft_updateAreaLimit", 1,
                                new Vector3(MainMapsInfo[LobbyList[index].LobbyMap].Item1.X,
                                    MainMapsInfo[LobbyList[index].LobbyMap].Item1.Y,
                                    MainMapsInfo[LobbyList[index].LobbyMap].Item1.Z),
                                MainMapsInfo[LobbyList[index].LobbyMap].Item2);

                            var random_point = new Random().Next(1, MapSpawnPoints.Count);
                            ...
                            NAPI.Entity.SetEntityPosition(foreachPlayer,
                                new Vector3(MapSpawnPoints[random_point].Item1.X, MapSpawnPoints[random_point].Item1.Y,
                                    MapSpawnPoints[random_point].Item1.Z));
                            Trigger.Dimension(foreachPlayer, (uint)(index + DefaultDimension));

                            if (!AirsoftPlayerData.ContainsKey(foreachPlayer))
                                AirsoftPlayerData.Add(foreachPlayer, new PlayerData(foreachPlayer.Health, false));
                            else AirsoftPlayerData[foreachPlayer].CorrectHealth = foreachPlayer.Health;

                            Repository.ItemsUse(player, "accessories", 7);
                            ...
                            if (LobbyList[index].LobbyMode >= 2)
                                SendTextInfoForPlayer(foreachPlayer,
                                    LangFunc.GetText(LangType.Ru, DataName.MatchOk, NextPlayerTeam + 1));
            ...
            {
                Log.Write($"airsoft_joinLobby_server Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_airsoft_respawnPlayer(ExtPlayer player)
        {
            try
            {
                ...
                var lobbyId = sessionData.InAirsoftLobby;
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId) && LobbyList[lobbyId].LobbyPlayers.Contains(player))
                {
                    if (LobbyList[lobbyId].LobbyMode != 5) // 1x1, 2x2, 3x3, 5x5
                    {
                        ...
                        if (LobbyList[lobbyId].LobbyMap == 0)
                        {
                            if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0)
                                random_point = new Random().Next(0, 5);
                            else if (player.GetSharedData<int>("PlayerAirsoftTeam") == 1)
                                random_point = new Random().Next(5, 10);
                        }
                        else if (LobbyList[lobbyId].LobbyMap == 1)
                        {
                            if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0)
                                random_point = new Random().Next(10, 15);
                            else if (player.GetSharedData<int>("PlayerAirsoftTeam") == 1)
                                random_point = new Random().Next(15, 20);
                        }
                        else if (LobbyList[lobbyId].LobbyMap == 2)
                        {
                            if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0)
                                random_point = new Random().Next(20, 25);
                            else if (player.GetSharedData<int>("PlayerAirsoftTeam") == 1)
                                random_point = new Random().Next(25, 30);
                        }
                        else if (LobbyList[lobbyId].LobbyMap == 3)
                        {
                            if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0)
                                random_point = new Random().Next(30, 35);
                            else if (player.GetSharedData<int>("PlayerAirsoftTeam") == 1)
                                random_point = new Random().Next(35, 40);
                        }
                    ...
                    {
                        var random_point = new Random().Next(1, GunGameMapSpawnPoints.Count);
                        ...
                        NAPI.Entity.SetEntityPosition(player,
                            new Vector3(GunGameMapSpawnPoints[random_point].X, GunGameMapSpawnPoints[random_point].Y,
                                GunGameMapSpawnPoints[random_point].Z));
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"airsoft_respawnPlayer Exception: {e}");
            }
        }
        ...
        public static void DeathCheck(ExtPlayer player, ExtPlayer killer, uint weapon)
        {
            try
            {
                ...
                {
                    foreach (var foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                    {
                    ...
                    {
                        if (killer.IsCharacterData() && AirsoftPlayerData.ContainsKey(killer) &&
                            AirsoftPlayerData[killer].IsGunGamePlayer)
                        {
                            killer.SetSharedData("killsWeapon",
                                Convert.ToInt32(killer.GetSharedData<int>("killsWeapon")) + 1);

                            if (Convert.ToInt32(killer.GetSharedData<int>("weaponLevel")) + 1 <
                                GunGameWeaponModels.Length)
                            {
                                killer.SetSharedData("weaponLevel",
                                    Convert.ToInt32(killer.GetSharedData<int>("weaponLevel")) + 1);

                                GiveGun(killer,
                                    GunGameWeaponModels[Convert.ToInt32(killer.GetSharedData<int>("weaponLevel"))]);
                            ...
                            else
                            {
                                ...
                                foreach (var foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                {
                                    ...
                                    if (foreachCharacterData != null)
                                    {
                                        ...
                                        Trigger.ClientEvent(foreachPlayer, "airsoft_updateAirsoftLobbyValue",
                                            foreachSessionData.InAirsoftLobby);

                                        if (player != foreachPlayer)
                                            NAPI.Entity.SetEntityPosition(foreachPlayer,
                                                new Vector3(-478.86032, -395.27307, 34.027653));
                                        else
                                        ...
                                        if (AirsoftPlayerData.ContainsKey(foreachPlayer))
                                        {
                                            foreachPlayer.Health =
                                                Convert.ToInt32(AirsoftPlayerData[foreachPlayer].CorrectHealth);
                                        ...
                                        if (qMain.UpdateQuestsStage(foreachPlayer, Zdobich.QuestName,
                                                (int)zdobich_quests.Stage30, 1, 2, true))
                                            qMain.UpdateQuestsComplete(foreachPlayer, Zdobich.QuestName,
                                                (int)zdobich_quests.Stage30, true);
                                        ...
                                        {
                                            var amount = Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9);
                                            Wallet.Change(foreachPlayer, amount);
                                            GameLog.Money("server", $"player({foreachCharacterData.UUID})", amount,
                                                "lobbyWin(GG)");
                                            SendTextInfoForPlayer(foreachPlayer,
                                                $"Вы выиграли этот матч и получили {amount}$");
                                        }
                                        else
                                        {
                                            if (GunGameWinner == null)
                                            {
                                                ...
                                                GameLog.Money("server", $"player({foreachCharacterData.UUID})",
                                                    LobbyList[lobbyId].LobbyPrice, "lobbyDraw(GG)");
                                                SendTextInfoForPlayer(foreachPlayer,
                                                    LangFunc.GetText(LangType.Ru, DataName.MatchNoWinner,
                                                        LobbyList[lobbyId].LobbyPrice));
                                            }
                                            else
                                            {
                                                SendTextInfoForPlayer(foreachPlayer,
                                                    LangFunc.GetText(LangType.Ru, DataName.GungameWinner,
                                                        GunGameWinner));
                                            }
                                        }
                                    }
                        ...
                        if (LobbyList.ContainsKey(lobbyId))
                        {
                            ...
                            NAPI.Player.SpawnPlayer(player,
                                new Vector3(GunGameMapSpawnPoints[random_point].X,
                                    GunGameMapSpawnPoints[random_point].Y, GunGameMapSpawnPoints[random_point].Z));
                            ...
                            GiveGun(player,
                                GunGameWeaponModels[Convert.ToInt32(player.GetSharedData<int>("weaponLevel"))]);
                        }
                    }
                    else if (LobbyList[lobbyId].LobbyMode != 5 &&
                             LobbyList[lobbyId].GameStatus == 1) // 1x1, 2x2, 3x3, 5x5
                    {
                        if (LobbyList[lobbyId].LobbyMode == 0)
                        {
                            ...
                            if (killer != null && killer.IsCharacterData())
                            {
                                ...
                                if (killerSessionData != null)
                                    if (killerSessionData.InAirsoftLobby == lobbyId &&
                                        Convert.ToInt32(killer.GetSharedData<int>("PlayerAirsoftTeam")) !=
                                        Convert.ToInt32(player.GetSharedData<int>("PlayerAirsoftTeam")))
                                    {
                                        ...
                                        var winner_team = -1; // -1 если ничья
                                        if (Convert.ToInt32(killer.GetSharedData<int>("PlayerAirsoftTeam")) == 0 ||
                                            Convert.ToInt32(killer.GetSharedData<int>("PlayerAirsoftTeam")) == 1)
                                            winner_team =
                                                Convert.ToInt32(killer.GetSharedData<int>("PlayerAirsoftTeam"));
                                        ...
                                        foreach (var foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                        {
                                            ...
                                            if (foreachCharacterData != null)
                                            {
                                                if (winner_team ==
                                                    Convert.ToInt32(
                                                        foreachPlayer.GetSharedData<int>("PlayerAirsoftTeam")))
                                                {
                                                    Wallet.Change(foreachPlayer,
                                                        Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9));
                                                    GameLog.Money("server", $"player({foreachCharacterData.UUID})",
                                                        Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9),
                                                        "lobbyWin(DUEL_PVP)");
                                                    SendTextInfoForPlayer(foreachPlayer,
                                                        LangFunc.GetText(LangType.Ru, DataName.MatchWinner,
                                                            Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9)));
                                                }
                                                else if (winner_team == -1)
                                                {
                                                    Wallet.Change(foreachPlayer, LobbyList[lobbyId].LobbyPrice);
                                                    GameLog.Money("server", $"player({foreachCharacterData.UUID})",
                                                        LobbyList[lobbyId].LobbyPrice, "lobbyDraw(DUEL_PVP)");
                                                    SendTextInfoForPlayer(foreachPlayer,
                                                        LangFunc.GetText(LangType.Ru, DataName.MatchNichya,
                                                            LobbyList[lobbyId].LobbyPrice));
                                                }
                                                else
                                                {
                                                    SendTextInfoForPlayer(foreachPlayer,
                                                        LangFunc.GetText(LangType.Ru, DataName.YouLose));
                                                }
                                                ...
                                                NAPI.Player.SpawnPlayer(foreachPlayer,
                                                    new Vector3(-478.86032, -395.27307, 34.027653));
                                                Trigger.Dimension(foreachPlayer);
                            ...
                            else
                            {
                                ...
                                if (LobbyList[lobbyId].LobbyMap == 0)
                                {
                                    if (Convert.ToInt32(player.GetSharedData<int>("PlayerAirsoftTeam")) == 0)
                                        random_point = new Random().Next(0, 5);
                                    else if (Convert.ToInt32(player.GetSharedData<int>("PlayerAirsoftTeam")) == 1)
                                        random_point = new Random().Next(5, 10);
                                }
                                else if (LobbyList[lobbyId].LobbyMap == 1)
                                {
                                ...
                                NAPI.Player.SpawnPlayer(player,
                                    new Vector3(MapSpawnPoints[random_point].Item1.X,
                                        MapSpawnPoints[random_point].Item1.Y, MapSpawnPoints[random_point].Item1.Z));
                                Trigger.Dimension(player, (uint)(lobbyId + DefaultDimension));
                            ...
                            if (killer != null && killer.IsCharacterData())
                            {
                                ...
                                if (killerSessionData != null)
                                    if (killerSessionData.InAirsoftLobby == lobbyId &&
                                        ...
                                        Convert.ToInt32(player.GetSharedData<int>("PlayerAirsoftTeam")))
                                    {
                                        LobbyList[killerSessionData.InAirsoftLobby]
                                            .GameScore[
                                                Convert.ToInt32(killer.GetSharedData<int>("PlayerAirsoftTeam"))] += 1;

                                        foreach (var foreachPlayer in LobbyList[killerSessionData.InAirsoftLobby]
                                                     .LobbyPlayers)
                                            if (foreachPlayer.IsCharacterData())
                                                Trigger.ClientEvent(foreachPlayer, "airsoft_updateStats_client", 1,
                                                    LobbyList[killerSessionData.InAirsoftLobby].GameScore[0],
                                                    LobbyList[killerSessionData.InAirsoftLobby].GameScore[1]);
                                    }
                            }
            ...
            {
                Log.Write($"DeathCheck Exception: {e}");
            }
        }
        ...
        public static void OnPlayerDisconnected(ExtPlayer player)
        {
            try
            {
                ...
                {
                    if (AirsoftPlayerData.ContainsKey(player) && AirsoftPlayerData[player].IsGunGamePlayer)
                    {
                ...
                if (LobbyList.ContainsKey(lobbyId) && LobbyList[lobbyId].LobbyPlayers.Contains(player))
                {
                    ...
                    {
                        if (LobbyList[lobbyId].LobbyGameTimer != null) Timers.Stop(LobbyList[lobbyId].LobbyGameTimer);
                ...
                if (PlayersInLobbyMenu.Contains(player)) PlayersInLobbyMenu.Remove(player);
            }
            catch (Exception e)
            {
                Log.Write($"Event_OnPlayerDisconnected (Airsoft) Exception: {e}");
            }
        }
        ...
        private static void LobbyGameTimerFunction(int index)
        {
            try
            {
                ...
                if (LobbyList[index].LobbyMode == 5) // GunGame
                {
                    if (LobbyList[index].GameStatus == 0)
                    {
                                ...
                                try
                                {
                                    if (foreachPlayer.IsCharacterData())
                                    {
                                        ...
                                        Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.NoPlayersMatch), 3000);
                                    }
                                }
                                catch (Exception e)
                                {
                                    Log.Write($"LobbyGameTimerFunction Foreach #1 Exception: {e}");
                                }
                        ...
                        {
                            LobbyList[index].GameStatus = 1;
                            LobbyList[index].LobbyGameTimer = Timers.StartOnce($"LobbyGameTimer_{index}", 900000,
                                () => LobbyGameTimerFunction(index), true);
                                ...
                                try
                                {
                                    if (foreachPlayer.IsCharacterData())
                                    {
                                        ...
                                        if (LobbyList[index].LobbyMap == 0) random_point = new Random().Next(0, 10);
                                            ...
                                            random_point = new Random().Next(10, 20);
                                        else if (LobbyList[index].LobbyMap == 2)
                                            random_point = new Random().Next(20, 30);
                                        else if (LobbyList[index].LobbyMap == 3)
                                            random_point = new Random().Next(30, 40);
                                        else if (LobbyList[index].LobbyMap == 5)
                                            random_point = new Random().Next(40, 50);
                                        else if (LobbyList[index].LobbyMap == 6)
                                            random_point = new Random().Next(50, 60);

                                        NAPI.Entity.SetEntityPosition(foreachPlayer,
                                            new Vector3(GunGameMapSpawnPoints[random_point].X,
                                                GunGameMapSpawnPoints[random_point].Y,
                                                GunGameMapSpawnPoints[random_point].Z));
                                        Trigger.Dimension(foreachPlayer, (uint)(index + DefaultDimension));

                                        if (!AirsoftPlayerData.ContainsKey(foreachPlayer))
                                        {
                                            AirsoftPlayerData.Add(foreachPlayer,
                                                new PlayerData(foreachPlayer.Health, true));
                                        }
                                        else
                                        {
                                        ...
                                        Repository.ItemsUse(foreachPlayer, "accessories", 7);

                                        foreachPlayer.Health = 100;
                                        foreachPlayer.Armor = 0;
                                        GiveGun(foreachPlayer,
                                            GunGameWeaponModels[foreachPlayer.GetSharedData<int>("weaponLevel")]);
                                        ...
                                        SendTextInfoForPlayer(foreachPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.MexitHelp));
                                    }
                                }
                                catch (Exception e)
                                {
                                    Log.Write($"LobbyGameTimerFunction Foreach #2 Exception: {e}");
                                }
                        }
                    }
                    else
                    {
                        var GunGameWinner = LobbyList[index].GunGameTop
                            .FirstOrDefault(x => x.Value == LobbyList[index].GunGameTop.Values.Max()).Key;
                        ...
                        foreach (var foreachPlayer in LobbyList[index].LobbyPlayers)
                            try
                            {
                                ...
                                if (foreachCharacterData != null)
                                {
                                    ...
                                    if (foreachPlayer.Name == GunGameWinner)
                                    {
                                        var amount = Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9);
                                        Wallet.Change(foreachPlayer, amount);
                                        GameLog.Money("server", $"player({foreachCharacterData.UUID})", amount,
                                            "lobbyWin(GG)");
                                        SendTextInfoForPlayer(foreachPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.MatchWinner, amount));
                                    }
                                    else
                                    {
                                        if (GunGameWinner == null)
                                        {
                                            ...
                                            GameLog.Money("server", $"player({foreachCharacterData.UUID})",
                                                LobbyList[index].LobbyPrice, "lobbyDraw(GG)");
                                            SendTextInfoForPlayer(foreachPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.MatchNoWinner,
                                                    LobbyList[index].LobbyPrice));
                                        }
                                        else
                                        {
                                            SendTextInfoForPlayer(foreachPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.GungameWinner, GunGameWinner));
                                        }
                                    }
                                }
                            }
                            ...
                            {
                                Log.Write($"LobbyGameTimerFunction Foreach #3 Exception: {e}");
                            }
                ...
                else // 1x1, 2x2, 3x3, 5x5
                {
                    if (LobbyList[index].GameStatus != 0)
                    {
                        ...
                        if (LobbyList[index].GameScore[0] > LobbyList[index].GameScore[1])
                            winner_team = 0;
                        else if (LobbyList[index].GameScore[0] < LobbyList[index].GameScore[1]) winner_team = 1;
                        ...
                        foreach (var foreachPlayer in LobbyList[index].LobbyPlayers)
                            try
                            {
                                ...
                                if (foreachCharacterData != null)
                                {
                                    ...
                                    {
                                        Wallet.Change(foreachPlayer,
                                            Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9));
                                        GameLog.Money("server", $"player({foreachCharacterData.UUID})",
                                            Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9), "lobbyWin(TPVP)");

                                        if (LobbyList[index].LobbyMode == 1)
                                            SendTextInfoForPlayer(foreachPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.MatchWinner,
                                                    Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9)));
                                        else
                                            SendTextInfoForPlayer(foreachPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.YouTeamWin,
                                                    Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9)));
                                    }
                                    else if (winner_team == -1)
                                    {
                                        Wallet.Change(foreachPlayer, LobbyList[index].LobbyPrice);
                                        GameLog.Money("server", $"player({foreachCharacterData.UUID})",
                                            LobbyList[index].LobbyPrice, "lobbyDraw(TPVP)");
                                        SendTextInfoForPlayer(foreachPlayer,
                                            LangFunc.GetText(LangType.Ru, DataName.MatchNichya,
                                                LobbyList[index].LobbyPrice));
                                    }
                                    else
                                    {
                                        if (LobbyList[index].LobbyMode == 1)
                                            ...
                                            SendTextInfoForPlayer(foreachPlayer,
                                                LangFunc.GetText(LangType.Ru, DataName.YouTeamLose));
                                    }
                            ...
                            {
                                Log.Write($"LobbyGameTimerFunction Foreach #4 Exception: {e}");
                            }
            ...
            {
                Log.Write($"LobbyGameTimerFunction Exception: {e}");
            }
        }
        ...
        public static void CMD_mexit(ExtPlayer player)
        {
            try
            {
                ...
                {
                    if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0 ||
                        player.GetSharedData<int>("PlayerAirsoftTeam") == 1 || (AirsoftPlayerData.ContainsKey(player) &&
                            AirsoftPlayerData[player].IsGunGamePlayer))
                    {
                        ...
                        if (AirsoftPlayerData.ContainsKey(player))
                            player.Health = Convert.ToInt32(AirsoftPlayerData[player].CorrectHealth);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.ErrorMatch), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_mexit Exception: {e}");
            }
        }
        ...
        public static void OutLobbyZone(ExtPlayer player)
        {
            try
            {
                ...
                var lobbyId = sessionData.InAirsoftLobby;
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId) &&
                    Convert.ToInt32(player.GetSharedData<int>("PlayerAirsoftTeam")) != 0 &&
                    Convert.ToInt32(player.GetSharedData<int>("PlayerAirsoftTeam")) != 1)
                    if (!AirsoftPlayerData.ContainsKey(player) || (AirsoftPlayerData.ContainsKey(player) &&
                                                                   !AirsoftPlayerData[player].IsGunGamePlayer))
                        LeaveLobbyFunction(player);
            }
            catch (Exception e)
            {
                Log.Write($"OutLobbyZone Exception: {e}");
            }
        }
        ...
        public static void LeaveLobbyFunction(ExtPlayer player)
        {
                ...
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId))
                {
                    if (LobbyList[lobbyId].LobbyPlayers.Contains(player))
                    {
                        if (LobbyList[lobbyId].GameStatus == 0 || LobbyList[lobbyId].LobbyMode == 5)
                        {
                            ...
                            GameLog.Money("server", $"player({characterData.UUID})", LobbyList[lobbyId].LobbyPrice,
                                "lobbyDraw(TPVP)");
                        }
                                ...
                                foreach (var foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                    if (foreachPlayer.IsCharacterData())
                                        Trigger.ClientEvent(foreachPlayer, "airsoft_lobbyMenuHandler", 5,
                                            JsonConvert.SerializeObject(new int[3]
                                            {
                                                LobbyList[lobbyId].LobbyPlayers.Count,
                                                LobbyList[lobbyId].LobbyMode == 5 ? 2 : LobbyList[lobbyId].MaxPlayers,
                                                LobbyList[lobbyId].MaxPlayers
                                            }));
                            }
                            else if (LobbyList[lobbyId].LobbyMode != 5 &&
                                     LobbyList[lobbyId].GameStatus != 0) // 1x1, 2x2, 3x3, 5x5
                            {
                                var mates_count = 0;

                                foreach (var foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                    if (foreachPlayer.IsCharacterData())
                                        if (foreachPlayer.GetSharedData<int>("PlayerAirsoftTeam") ==
                                            player.GetSharedData<int>("PlayerAirsoftTeam") && foreachPlayer != player)
                                            mates_count += 1;

                                if (mates_count == 0 || LobbyList[lobbyId].LobbyMode == 0 ||
                                    LobbyList[lobbyId].LobbyMode == 1)
                                {
                                    if (LobbyList[lobbyId].LobbyGameTimer != null)
                                        Timers.Stop(LobbyList[lobbyId].LobbyGameTimer);
                    ...
                    if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0 ||
                        ...
                        (AirsoftPlayerData.ContainsKey(player) && AirsoftPlayerData[player].IsGunGamePlayer))
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.LeftMatch), 3000);
                    else
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.LeftLobby), 3000);
            ...
            {
                Log.Write($"LeaveLobbyFunction Exception: {e}");
            }
        }
        ...
        public static void UpdateLobbyList()
        {
            try
            {
                ...
                {
                    foreach (var foreachPlayer in PlayersInLobbyMenu)
                        if (foreachPlayer.IsCharacterData())
                            Trigger.ClientEvent(foreachPlayer, "airsoft_updateLobbyList_client",
                                JsonConvert.SerializeObject(LobbyList), "airsoft");
                }
            }
            catch (Exception e)
            {
                Log.Write($"UpdateLobbyList Exception: {e}");
            }
        }
        ...
        public static void GiveGun(ExtPlayer player, AirsoftWeaponData weaponData)
        {
            ...
            {
                if (!player.IsCharacterData()) return;
                Trigger.ClientEvent(player, "client.weapon.give",
                    (int)WeaponRepository.GetHash(weaponData.weapon.ToString()), weaponData.ammo);
            }
            catch (Exception e)
            {
                Log.Write($"GiveGun (Airsoft) Exception: {e}");
            }
        }
        ...
        public static void ClearGun(ExtPlayer player)
        {
            ...
            {
                Log.Write($"ClearGun (Airsoft) Exception: {e}");
            }
        }
        ...
        public static void SendTextInfoForPlayer(ExtPlayer player, string msg)
        {
            ...
            {
                Log.Write($"SendTextInfoForPlayer Exception: {e}");
            }
        }
    }
}