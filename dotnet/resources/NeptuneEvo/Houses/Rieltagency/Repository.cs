...
namespace NeptuneEvo.Houses.Rieltagency
{
    ...
    {
        private static readonly List<ExtPlayer> OpenInterfacePlayer = new List<ExtPlayer>();
        private static readonly int BuyPrice = 500;


        public static void OnResourceStart()
        {
            Main.CreateBlip(new Main.BlipData(374, LangFunc.GetText(LangType.Ru, DataName.Rieltorskoye),
                new Vector3(-710.49, 267.8656, 83.14731), 2, true));
            PedSystem.Repository.CreateQuest("a_m_y_business_02", new Vector3(-710.49, 267.8656, 83.14731), -92.04525f,
                title: "~y~NPC~w~ Илон Таск\nРиэлтор", colShapeEnums: ColShapeEnums.Rieltagency);
        }
        ...
        public static void OnBuy(ExtPlayer player, int id, int type)
        {
            ...
            if (type == 0)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NotFindHome), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.HomeAlreadyBought), 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(player, "client.rieltagency.addBlip", 374, 82, id, house.Position.X,
                    house.Position.Y, LangFunc.GetText(LangType.Ru, DataName.Rieltorskoye));
            }
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NotFindBiz), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizAlreadyBought), 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(player, "client.rieltagency.addBlip", 464, 82, id, business.EnterPoint.X,
                    business.EnterPoint.Y, LangFunc.GetText(LangType.Ru, DataName.Rieltorskoye));
            }

            EventSys.SendCoolMsg(player, "Риэлтор", "Покупка информации",
                $"{LangFunc.GetText(LangType.Ru, DataName.YouBoughtInfo)}", "", 15000);
            ...
            GameLog.Money($"player({characterData.UUID})", $"buyRieltagency({type})", price,
                $"buyRieltagency({id},{type}))");
