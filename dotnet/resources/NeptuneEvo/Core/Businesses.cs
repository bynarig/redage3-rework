...
namespace NeptuneEvo.Core
{
    internal class BusinessManager : Script
    {
        ...
        private static readonly LsCustoms[] LsCustomsPlaces = new LsCustoms[5]
        {
        ...
        public static ConcurrentDictionary<int, int>
            Orders = new ConcurrentDictionary<int, int>(); // key - ID заказа, value - ID бизнеса
        ...
        public static string[] ClothesShopToType =
        {
        ...
        private static readonly Dictionary<ClothesComponent, int> ClothesComponentToSlot =
            new Dictionary<ClothesComponent, int>
            {
        ...
        private static readonly Dictionary<ClothesComponents.BarberComponent, int> BarberComponentToSlot =
            new Dictionary<ClothesComponents.BarberComponent, int>
            {
        ...
        private static readonly Dictionary<ClothesComponents.TattooComponent, int> TattooComponentToSlot =
            new Dictionary<ClothesComponents.TattooComponent, int>
            {
        ...
        private static int GetLsCustomsPlaces(Vector3 pos)
        {
            var i = 0;
            var index = 0;
            var lastDist = LsCustomsPlaces[0].Position.DistanceTo(pos);
            foreach (var p in LsCustomsPlaces)
            {
        ...
        public static void InitBusProducts()
        {
            using (var db = new ConfigBD("ConfigDB"))
            {
                ...
                foreach (var busProduct in busProducts)
                    busProductsData.Add(busProduct.Name,
                        new BusProductData(busProduct.Price, busProduct.OtherPrice, busProduct.Percent,
                            busProduct.MaxCount, busProduct.Type, busProduct.ItemId, busProduct.Toggled));
        ...
        public static void UpdateBusProd(Business data)
        {
            try
            {
                ...
                foreach (var prod in data.Products)
                {
                    ...
                    else
                    {
                        ...
                        else if (data.Type == 9 || data.Type == 10 || data.Type == 11 || data.Type == 12)
                ...
                {
                    case 0:
                        foreach (var busProductData in BusProductsData)
                        {
                            ...
                            if (product == null)
                            {
                                ...
                                if (busProductData.Key == "Лотерейный билет")
                                    data.Products.Add(new Product(500, 0, 0, busProductData.Key, false));
                                else
                                    data.Products.Add(new Product(busProductData.Value.Price, 0, 1, busProductData.Key,
                                        false));
                                changed = true;
                            }
                        ...
                        foreach (var product in data.Products.ToList())
                        {
                            ...
                            if (BusProductsData.ContainsKey(product.Name)) continue;
                            data.Products.Remove(product);
                            changed = true;
                        }
                    ...
                    case 6:
                        foreach (var busProductData in BusProductsData)
                        {
                            ...
                            {
                                data.Products.Add(new Product(busProductData.Value.Price, 0, 0, busProductData.Key,
                                    false));
                                changed = true;
                            }
                        }
            ...
            {
                Log.Write($"UpdateBusProd Exception: {e}");
            }
        }
        ...
        public static void Init()
        {
            try
            {
                Main.CreateBlip(new Main.BlipData(73, "Premium Clothes Shop",
                    new Vector3(-1126.9141, -1440.1637, 4.108331), 35, true));
                Repository.CreateQuest("a_m_y_hipster_01", new Vector3(-1126.9141, -1440.1637, 4.108331 + 1.12),
                    -63.85f, title: "~y~NPC~w~ Вовчик", colShapeEnums: ColShapeEnums.PremiumShop);
                ...
                foreach (DataRow Row in result.Rows)
                    try
                    {
                        var enterpoint = JsonConvert.DeserializeObject<Vector3>(Row["enterpoint"].ToString());
                        var unloadpoint = JsonConvert.DeserializeObject<Vector3>(Row["unloadpoint"].ToString());
                        var bankmoney = Convert.ToInt32(Row["money"]);
                        ...
                        */
                        var prodlist = JsonConvert.DeserializeObject<List<Product>>(Row["products"].ToString());

                        var id = Convert.ToInt32(Row["id"]);
                        var data = new Business(id, Row["owner"].ToString(), Convert.ToInt32(Row["sellprice"]),
                            Convert.ToInt32(Row["type"]), prodlist, enterpoint, unloadpoint, bankmoney,
                            Convert.ToInt32(Row["mafia"]),
                            JsonConvert.DeserializeObject<List<Order>>(Row["orders"].ToString()),
                        ...
                        if (data.Type == 12)
                        {
                            var index = GetLsCustomsPlaces(data.EnterPoint);
                            if (!LsCustomsPlacesToBusnessId.ContainsKey(index))
                                LsCustomsPlacesToBusnessId.Add(index, id);
                    ...
                    {
                        Log.Write("Cant load Business: " + x);
                    }
        ...
        public static void SavingBusiness()
        {
            try
            {
                foreach (var b in BizList.Keys)
                    BizList[b].IsSave = true;
            }
            catch (Exception e)
            {
                Log.Write($"SavingBusiness Exception: {e}");
            }
        }
        ...
        public void OnResourceStop()
        {
            ...
            {
                Log.Write($"OnResourceStop Exception: {e}");
            }
        }
        ...
        public static List<Product> fillProductList(int type)
        {
            try
            {
                var _ProductsList = new List<Product>();
                switch (type)
                {
                    case 0:
                        foreach (var busProductData in BusProductsData)
                        {
                            ...
                            Product product;
                            if (busProductData.Key == "Лотерейный билет")
                                product = new Product(500, 0, 0, busProductData.Key, false);
                    ...
                    case 2:
                        foreach (var name in CarsNames[0])
                        {
                            ...
                            var product = new Product(productsData.Price, 0, 0, name, false);
                            _ProductsList.Add(product);
                        }
                    ...
                    case 6:
                        ...
                        {
                            if (busProductData.Value.Type != BusProductToType.Gun) continue;
                            var product = new Product(busProductData.Value.Price, 0, 5, busProductData.Key, false);
                            _ProductsList.Add(product);
                        }

                        _ProductsList.Add(new Product(BusProductsData["Патроны"].Price, 0, 5, "Патроны", false));
                        _ProductsList.Add(new Product(BusProductsData["Модификации"].Price, 0, 5, "Модификации",
                            false));
                        break;
                    case 7:
                    ...
                    case 8:
                        foreach (var busProductData in BusProductsData)
                        {
                            ...
                            var product = new Product(busProductData.Value.Price, 10, 1, busProductData.Key, false);
                            _ProductsList.Add(product);
                        }
            ...
            {
                Log.Write($"fillProductList Exception: {e}");
        ...
        public static void OnBusinessAction(ExtPlayer player, int ID)
        {
            try
            {
                ...
                var bizlistid = sessionData.BizID;
                ...
                var biz = BizList[bizlistid];
                if (biz.IsOwner() && !Main.PlayerNames.Values.Contains(biz.Owner))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizNotWork, BusinessTypeNames[biz.Type]), 3000);
                    return;
                }
                ...
                switch (biz.Type)
                {
                    ...
                    case 7:
                        ...
                        if ((sessionData.WorkData.OnDuty &&
                            ...
                            sessionData.WorkData.OnWork)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.MustEndWorkDay), 3000);
                            return;
                        }
                    ...
                    case 9:
                        ...
                        Trigger.ClientEvent(player, "client.shop.open", "tattoo", characterData.Gender,
                            JsonConvert.SerializeObject(realClothes), JsonConvert.SerializeObject(dataJson), 0,
                            JsonConvert.SerializeObject(custom.Tattoos));
                        return;
                    case 10:
                        if ((sessionData.WorkData.OnDuty &&
                             Manager.FractionTypes[player.GetFractionId()] == FractionsType.Gov) ||
                        ...
                        TryUpdateArmor(player);
                        Trigger.ClientEvent(player, "client.shop.open", "barber", characterData.Gender,
                            JsonConvert.SerializeObject(realClothes), JsonConvert.SerializeObject(dataJson), 0);
                        return;
                    case 12: // ERORR 21
                        StartTuning(player);
                    ...
                    case 13:
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CARWASH_PAY",
                            LangFunc.GetText(LangType.Ru, DataName.CarWashWant, biz.Products[0].Price));
                        return;
                    case 14:
                        if (sessionData.Follower != null)
                        {
            ...
            {
                Log.Write($"OnBusinessAction Exception: {e}");
            }
        }
        ...
        public static void enterPetShop(ExtPlayer player, string prodname)
        {
            try
            {
                ...
                var mydim = (uint)(player.Value + 500);
                ...
                var _Prices = new List<int>();
                var biz = BizList[sessionData.TempBizID];
                for (byte i = 0; i != 9; i++) _Prices.Add(biz.Products[0].Price);
                Trigger.ClientEvent(player, "openPetshop", JsonConvert.SerializeObject(PetNames),
                    JsonConvert.SerializeObject(PetHashes), JsonConvert.SerializeObject(_Prices), mydim);
                BattlePass.Repository.UpdateReward(player, 152);
            }
            catch (Exception e)
            {
                Log.Write($"enterPetShop Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_petshopBuy(ExtPlayer player, string petName)
        {
            try
            {
                ...
                if (HouseManager.HouseTypeList[house.Type].PetPosition == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.HomeNotForPets), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoPets), 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NewPet, petName), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"RemoteEvent_petshopBuy Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_petshopCancel(ExtPlayer player)
        {
            try
            {
                ...
                if (sessionData.TempBizID == -1 || !BizList.ContainsKey(sessionData.TempBizID)) return;
                Trigger.StopAnimation(player);
                var enterPoint = BizList[sessionData.TempBizID].EnterPoint;
                Trigger.Dimension(player);
            ...
            {
                Log.Write($"RemoteEvent_petshopCancel Exception: {e}");
            }
        }
                ...
                if (player.IsInVehicle)
                {
                    if (player.VehicleSeat == (int)VehicleSeat.Driver)
                    {
                        ...
                        if (VehicleStreaming.GetVehicleDirt(vehicle) >= 0.01f)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.NetTovaraNaSkladeBiza), 3000);
                                return;
                            }
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.WashedCar), 5000);
                        ...
                        else
                        {
                            Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NotDirtyCar), 5000);
                        }
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.OnlyDriverWashCar), 5000);
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"Carwash_Pay Exception: {e}");
            }
        }
        ...
        public static void StartTuning(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData.Access == VehicleAccess.Personal)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.VeloCantBeTuned), 3000);
                        return;
                    }

                    if (AirAutoRoom.isAirCar(vehicle.Model))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Воздушный транспорт не может быть затюнингован", 3000);
                        return;
                    }
                    ...
                    var modelPrice = BusProductsData[model].Price;
                    modelPrice = CarsNames[2].Contains(model) ? modelPrice * 10 : modelPrice;

                    Trigger.ClientEvent(player, "client.custom.open", biz.Products[0].Price, modelPrice,
                        JsonConvert.SerializeObject(vehicleData.Components), 0);
                }
                else if (vehicleLocalData.Access == VehicleAccess.Organization &&
                         player.IsOrganizationAccess(RankToAccess.OrgTuning))
                {
                    var bizId = sessionData.BizID;
                    if (!BizList.ContainsKey(bizId)) return;
                    if (sessionData.Following != null)
                    ...
                    Trigger.ClientEvent(player, "client.custom.open", 100, modelPrice,
                        JsonConvert.SerializeObject(organizationVehicle.customization), 1);
                }
            }
            catch (Exception e)
            {
                Log.Write($"StartTuning Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_exitTuning(ExtPlayer player, int eventType)
        {
            try
            {
                ...
                switch (eventType)
                {
                    ...
                    case 2:
                        ...
                        {
                            if (bizId == -1) return;
                            vehicle.Position = new Vector3(BizList[bizId].EnterPoint.X, BizList[bizId].EnterPoint.Y,
                                BizList[bizId].EnterPoint.Z + 1.0);
            ...
            {
                Log.Write($"RemoteEvent_exitTuning Exception: {e}");
            }
        }
        ...
        public static int GetPrice(string category, int index, int pricePercent, int priceVehicle)
        {
            try
            {
                if (category == "WindowTint" && WindowTintModToIndex.ContainsKey(index))
                    index = WindowTintModToIndex[index];
                var pricePercentf = pricePercent / 100f;
                var priceVehiclef = priceVehicle / 100f;
                if (index == -1)
                    return Convert.ToInt32(pricePercentf * priceVehiclef * ComponentsPrice[category] * (0.5f / 100f));
                if (index == 0) index = 1;
                var indexf = index / 100f;
                return Convert.ToInt32(pricePercentf * priceVehiclef * ComponentsPrice[category] * indexf);
            }
            catch (Exception e)
            {
                Log.Write($"GetPrice Exception: {e}");
                return 0;
            }
        }

        [RemoteEvent("server.custom.buy")]
        public static void RemoteEvent_buyTuning(ExtPlayer player, string category, int index, int otherIndex, int r,
            int g, int b)
        {
            try
            {
                ...
                if (vehicleData == null)
                    return;
                var vehModel = vehicleData.Model;
                var modelPrice = BusProductsData[vehModel].Price;
                modelPrice = CarsNames[2].Contains(vehModel) ? modelPrice * 10 : modelPrice;
                var price = GetPrice(category, index, biz.Products[0].Price, modelPrice);
                if (price == 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoDetailsError), 3000);
                        return;
                    }
                }

                var amount = Convert.ToInt32(price * 0.45 / 2000);
                if (amount <= 0) amount = 1;
                if (!takeProd(biz.ID, amount, "Запчасти", price))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoDetails), 3000);
                    return;
                }
            ...
            {
                Log.Write($"RemoteEvent_buyTuning Exception: {e}");
            }
        }

        [RemoteEvent("server.custom.gosBuy")]
        public static void RemoteEvent_buyGosTuning(ExtPlayer player, string category, int index, int otherIndex, int r,
            int g, int b)
        {
            try
            {
                ...
                var modelPrice = BusProductsData[vehModel].Price;
                modelPrice = CarsNames[2].Contains(vehModel) ? modelPrice * 10 : modelPrice;
                var price = GetPrice(category, index, 100, modelPrice);
                if (price == 0)
                {
                    if (modelPrice < 100000)
                    {
            ...
            {
                Log.Write($"RemoteEvent_buyGosTuning Exception: {e}");
            }
        }

        private static bool SetCustomization(ExtPlayer player, string category, int index, int otherIndex, int r, int g,
            int b, int price, string textLogs = "ExoticAutoRoom")
        {
            var characterData = player.GetCharacterData();
            ...
            GameLog.Money($"player({characterData.UUID})", textLogs, price,
                $"buyTuning({player.Vehicle.NumberPlate},{category},{index} ({r}, {g}, {b}))");
            Wallet.Change(player, -price);

            vehicleData.Components = GetCustomization(vehicleData.Components, category, index, otherIndex, r, g, b);

            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.BuyTunNotify, Wallet.Format(price)), 3000);
            BattlePass.Repository.UpdateReward(player, 44);
            Trigger.ClientEvent(player, "client.custom.updatecomponents",
                JsonConvert.SerializeObject(vehicleData.Components));
            ...
            qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage28, 1, true);
            qMain.UpdateQuestsComplete(player, Zdobich.QuestName, (int)zdobich_quests.Stage28, true);
            return true;
        }

        [RemoteEvent("server.custom.orgBuy")]
        public static void RemoteEvent_orgBuyTuning(ExtPlayer player, string category, int index, int otherIndex, int r,
            int g, int b)
        {
            try
            {
                ...
                GameLog.Money($"player({player.GetUUID()})", "server", price,
                    $"buyOrgTuning({number},{category},{index} ({r}, {g}, {b}))");
                Wallet.Change(player, -price);

                var customization = organizationVehicle.customization;
                organizationVehicle.customization =
                    GetCustomization(customization, category, index, otherIndex, r, g, b);
                organizationVehicle.SaveCustomization(number);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.BuyTunNotify, Wallet.Format(price)), 3000);
                Trigger.ClientEvent(player, "client.custom.updatecomponents",
                    JsonConvert.SerializeObject(customization));
            }
            catch (Exception e)
            {
                Log.Write($"RemoteEvent_orgBuyTuning Exception: {e}");
            }
        }

        private static VehicleCustomization GetCustomization(VehicleCustomization myvehcustom, string category,
            int index, int otherIndex, int r, int g, int b)
        {
            switch (category)
            {
                case "Spoiler":
        ...
        public static bool takeProd(int bizid, int amount, string prodname, int addMoney)
        {
            try
            {
                ...
                foreach (var p in biz.Products)
                {
                    ...
                    var bData = Bank.Get(Main.PlayerBankAccs[biz.Owner]);
            ...
            {
                Log.Write($"takeProd Exception: {e}");
                return false;
            }
        }
        ...
        public static Vector3 getNearestBiz(ExtPlayer player, int type)
        {
            ...
            {
                Log.Write($"getNearestBiz Exception: {e}");
                return null;
            }
        }
        ...
        public static void OpenClothes(ExtPlayer player, Business biz, bool isDonate = false, bool isGloves = false,
            List<string> dellClothes = null, List<string> addClothes = null)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                if ((sessionData.WorkData.OnDuty &&
                     Manager.FractionTypes[player.GetFractionId()] == FractionsType.Gov) || sessionData.WorkData.OnWork)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MustEndWorkDay), 3000);
                    return;
                ...
                var clothesPriceList = !isDonate
                    ? ClothesComponents.ClothesComponentPriceData[characterData.Gender]
                    : ClothesComponents.ClothesComponentDonateData[characterData.Gender];
                var dataJson = new Dictionary<string, List<List<object>>>();
                ...
                TryUpdateArmor(player);
                Trigger.ClientEvent(player, "client.shop.open", "clothes", characterData.Gender,
                    JsonConvert.SerializeObject(realClothes), JsonConvert.SerializeObject(dataJson),
                    Convert.ToByte(isDonate));
            ...
            {
                Log.Write($"OpenClothes Exception: {e}");
            }
        }

        private static void TryUpdateArmor(ExtPlayer player)
        {
            var item = Chars.Repository.GetItemData(player, "accessories", 7);
            if (item.ItemId == ItemId.BodyArmor)
            {
                item.Data = player.Armor.ToString();
                Chars.Repository.SetItemData(player, "accessories", 7, item, true);
            }
        }
        ...
        public static void ClothesBuy(ExtPlayer player, string name, int id, int texture, bool isDonate = false)
        {
            try
            {
                ...
                var biz = !isDonate ? BizList[sessionData.TempBizID] : null;
                var prod = !isDonate ? biz.Products[0] : null;

                var gender = characterData.Gender;

                var dictionary = (ClothesComponent)Enum.Parse(typeof(ClothesComponent), name);
                var slotId = ClothesComponentToSlot[dictionary];
                ...
                var tempPrice = !isDonate ? shoesData.Price : shoesData.Donate;

                var price = !isDonate ? Convert.ToInt32(tempPrice / 100f * prod.Price) : tempPrice;

                if (price <= 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantBuyThis), 3000);
                    return;
                }
                ...
                var amount = Convert.ToInt32(price * 10 * 0.75 / 100); // * 10, чтобы больше матов снимать
                if (amount <= 0) amount = 1;

                if (slotId == -1 ||
                    Chars.Repository.isFreeSlots(player, Chars.Repository.AccessoriesInfo[slotId]) != 0) return;
                if (!isDonate)
                {
                    if (dictionary == ClothesComponent.Masks &&
                        !takeProd(biz.ID, price >= 15000 ? 6 : 1, "Маски", price))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NetTovaraNaSkladeBiza), 3000);
                        return;
                ...
                var Data = $"{id}_{texture}_{gender}";
                Chars.Repository.ChangeAccessoriesItem(player, slotId, Data);
                if (!isDonate)
                {
                    GameLog.Money($"player({characterData.UUID})", $"biz({biz.ID})", price,
                        $"buyClothes({name}{Data})");
                    Wallet.Change(player, -price);
                ...
                else
                {
                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge,
                        LangFunc.GetText(LangType.Ru, DataName.PremClothB, price), DateTime.Now);
                    UpdateData.RedBucks(player, -price, LangFunc.GetText(LangType.Ru, DataName.PremClothB, price));
                }

                EventSys.SendCoolMsg(player, "Покупка", "Покупка одежды",
                    $"{LangFunc.GetText(LangType.Ru, DataName.BuyNewClothes)}", "", 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.BuyNewClothesView, price), 2000);
                ...
                if (qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage20, 2, 3, true))
                    qMain.UpdateQuestsComplete(player, Zdobich.QuestName, (int)zdobich_quests.Stage20, true);
            }
            catch (Exception e)
            {
                Log.Write($"RemoteEvent_buyClothes Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_cancelClothes(ExtPlayer player)
        {
            ...
            {
                Log.Write($"RemoteEvent_cancelClothes Exception: {e}");
            }
        }

        [RemoteEvent("server.barber.buy")]
        public static void BarberBuy(ExtPlayer player, string name, int id, int color, int colorHighlight,
            float opacity, bool isDonate = false)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                var dictionary =
                    (ClothesComponents.BarberComponent)Enum.Parse(typeof(ClothesComponents.BarberComponent), name);
                ...
                var tempPrice = !isDonate ? barberData.Price : barberData.Donate;
                var price = !isDonate ? Convert.ToInt32(tempPrice / 100f * prod.Price) : tempPrice;
                if (!isDonate && UpdateData.CanIChange(player, price, true) != 255) return;
                if (isDonate && accountData.RedBucks < price)
                {
                ...
                {
                    UpdateData.RedBucks(player, -price, LangFunc.GetText(LangType.Ru, DataName.PremiumClothesBuy));
                }
                ...
                switch (dictionary)
                {
                    ...
                    case ClothesComponents.BarberComponent.Eyes:
                        ...
                        break;
                    default:
                        var slotId = BarberComponentToSlot[dictionary];
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.BarberBuy, price), 5000);
            ...
            {
                Log.Write($"RemoteEvent_buyBarber Exception: {e}");
            }
        }
        ...
        public static void TattooBuy(ExtPlayer player, string name, int id, bool isDonate = false)
        {
            try
            {
                ...
                var dictionary =
                    (ClothesComponents.TattooComponent)Enum.Parse(typeof(ClothesComponents.TattooComponent), name);
                ...
                foreach (var t in customTattoos.Values)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.ThisTattooIs), 3000);
                        return;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MaxTattoo), 3000);
                    return;
                }
                ...
                var tempPrice = !isDonate ? tattooData.Price : tattooData.Donate;
                var price = !isDonate ? Convert.ToInt32(tempPrice / 100f * prod.Price) : tempPrice;
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.TattooBuy, tattooData.Name, price), 8000);
            ...
            {
                Log.Write($"RemoteEvent_buyTattoo Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_cancelTattoo(ExtPlayer player) // BARBER EXIT
        {
            ...
            {
                Log.Write($"RemoteEvent_cancelTattoo Exception: {e}");
            }
        }
        ...
        public static void fillCar(ExtPlayer player, int lvl)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantZapravit), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.Zaglushite), 3000);
                        return;
                    }
                    ...
                    var biz = BizList[sessionData.BizID];
                    var price = lvl * biz.Products[0].Price;
                    if (isGov)
                    {
                        if (fractionData == null || Manager.FractionTypes[fractionData.Id] != FractionsType.Gov)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.GosZapravit), 6000);
                            return;
                        }

                        if (vehicleLocalData.Access != VehicleAccess.Fraction ||
                            vehicleLocalData.Fraction != fractionData.Id)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.GosGosZapravit), 6000);
                            return;
                        }
                    ...
                    else if (UpdateData.CanIChange(player, price, true) != 255)
                    {
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.ZapravkaIs, biz.Products[0].Lefts), 7000);
                        return;
                    }
                    ...
                    if (isGov)
                    {
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                            LangFunc.GetText(LangType.Ru, DataName.ZapravkaGos), DateTime.Now);
                    }
                    else
                    {
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                            LangFunc.GetText(LangType.Ru, DataName.ZapravkaSuccess, price), DateTime.Now);
                    ...
                    Commands.RPChat("sme", player,
                        "заправил" + (characterData.Gender ? "" : "а") + LangFunc.GetText(LangType.Ru, DataName.veh));
                }
            }
            catch (Exception e)
            {
                Log.Write($"fillCar Exception: {e}");
            }
        }
        ...
        public static void buyBusinessCommand(ExtPlayer player)
        {
            try
            {
                ...
                if (player.IsInVehicle) return;
                if (characterData.DemorganTime >= 1) return;
                var id = CustomColShape.GetDataToEnum(player, ColShapeEnums.BusinessAction);
                if (id == (int)ColShapeData.Error)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NeedBeNearBiz), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantGetMoreThanBiz,
                            Group.GroupMaxBusinesses[accountData.VipLvl]), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizAuc), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouBizAuc), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizYours), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizOtherPlayer), 3000);
                    return;
                }
                ...
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                    LangFunc.GetText(LangType.Ru, DataName.BizBuy, BusinessTypeNames[biz.Type], biz.SellPrice),
                    DateTime.Now);
            ...
            {
                Log.Write($"buyBusinessCommand Exception: {e}");
            }
        }
        ...
        public static async void createBusinessCommand(ExtPlayer player, int govPrice, int type, double taxes)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NalogLimit), 3000);
                    return;
                }

                var pos = player.Position;
                pos.Z -= 1.12F;
                var products_list = fillProductList(type);
                var productlist = JsonConvert.SerializeObject(products_list);
                lastBizID++;

                var bankID = await Bank.Create("", 3, 1000);

                using var cmd = new MySqlCommand
                {
                    CommandText =
                        "INSERT INTO businesses (id, owner, sellprice, type, products, enterpoint, unloadpoint, money, mafia, orders, tax) VALUES (@val0,@val1,@val2,@val3,@val4,@val5,@val6,@val7,@val8,@val9,@val10)"
                };
                ...
                NAPI.Task.Run(() =>
                {
                    var biz = new Business(lastBizID, "Государство", govPrice, type, products_list, pos, new Vector3(),
                        bankID, -1, new List<Order>(), taxes);
                    biz.UpdateLabel();
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizCreated, biz.ID), 3000);
                });
            }
            catch (Exception e)
            {
                Log.Write($"createBusinessCommand Exception: {e}");
            }
        }
        ...
        public static void createBusinessUnloadpoint(ExtPlayer player, int bizid)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.PointUnloadBuy, bizid), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"createBusinessUnloadpoint Exception: {e}");
            }
        }
        ...
        public static void changeBizTax(ExtPlayer player, int id, double taxes)
        {
            try
            {
                ...
                var biz = BizList.FirstOrDefault(b => b.Value.ID == id).Value;
                ...
                if (biz != null)
                {
                    ...
                    var owner = (ExtPlayer)NAPI.Player.GetPlayerFromName(biz.Owner);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NalogChasIzmenen, id, taxes * 100), 3000);
                    if (owner != null)
                    {
                        Notify.Send(owner, NotifyType.Alert, NotifyPosition.Center,
                            LangFunc.GetText(LangType.Ru, DataName.NalogIzmenen, biz.Tax * 100, taxes * 100), 10000);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.VladeletNotify, owner.Name, owner.Value), 5000);
                    }
            ...
            {
                Log.Write($"changeBizTax Exception: {e}");
            }
        }
        ...
        public static void deleteBusinessCommand(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizIdNotFound), 3000);
                    return;
                }
                ...
                MySQL.Query(cmd);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.BizDeleted), 3000);
                if (biz.Type == 6)
                {
                    using var cmd1 = new MySqlCommand
                    {
                ...
                if (ownerCharacterData == null)
                {
                    ...
                    using var data = MySQL.QueryRead(cmd1);
            ...
            {
                Log.Write($"deleteBusinessCommand Exception: {e}");
            }
        }
        ...
        public static void sellBusinessCommand(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                var targetSellItemData = targetSessionData.SellItemData;
                if ((targetSellItemData.Buyer != null || targetSellItemData.Seller != null) &&
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizMaxPlayer), 3000);
                    return;
                }

                var biz = BizList[characterData.BizIDs[0]];
                if (price < biz.SellPrice)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizCantSellPrice, biz.SellPrice), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizNotYour), 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(target, "openDialog", "BUSINESS_BUY",
                    LangFunc.GetText(LangType.Ru, DataName.YouWantSellBiz, player.Name, BusinessTypeNames[biz.Type],
                        Wallet.Format(price)));
                EventSys.SendCoolMsg(player, "Предложение", "Покупка",
                    $"{LangFunc.GetText(LangType.Ru, DataName.BizWantSellToYou, target.Value, Wallet.Format(price))}",
                    "", 10000);
            ...
            {
                Log.Write($"sellBusinessCommand Exception: {e}");
            }
        }
        ...
        public static void acceptBuyBusiness(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData == null) return;
                var sellItemData = sessionData.SellItemData;
                var target = sellItemData.Seller;
                var targetSessionData = target.GetSessionData();
                var targetCharacterData = target.GetCharacterData();
                ...
                var price = sellItemData.Price;
                var biz = BizList[sellItemData.Count];
                if (player.Position.DistanceTo(target.Position) > 2)
                {
                    sessionData.SellItemData = new SellItemData();
                ...
                if (!targetCharacterData.BizIDs.Contains(biz.ID))
                {
                    ...
                    targetSessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizNoBelongsTo), 3000);
                    return;
                }
                ...
                if (characterData.BizIDs.Count >= Group.GroupMaxBusinesses[accountData.VipLvl])
                {
                    ...
                    targetSessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MaxBizAmount), 3000);
                    return;
                }
                ...
                GameLog.Money($"player({characterData.UUID})", $"player({targetCharacterData.UUID})", price,
                    $"buyBiz({biz.ID})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы купили у {target.Name.Replace('_', ' ')} {BusinessTypeNames[biz.Type]} за {Wallet.Format(price)}$",
                    3000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"{player.Name.Replace('_', ' ')} купил у Вас {BusinessTypeNames[biz.Type]} за {Wallet.Format(price)}$",
                    3000);
            ...
            {
                Log.Write($"acceptBuyBusiness Exception: {e}");
            }
        }
        ...
        public static void changeOwner(string oldName, string newName)
        {
            ...
            {
                Log.Write($"changeOwner NAPI.Task Exception: {e}");
            }
        }

        public class BusProductData
        {
            public BusProductData(int Price, int OtherPrice, int Percent, int MaxCount, sbyte Type, int ItemId,
                bool Toggled)
            {
                this.Price = Price;
        ...
        public static void OpenBizShopMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (sessionData.BizID == -1 || !BizList.ContainsKey(sessionData.BizID)) return;
                var biz = BizList[sessionData.BizID];
                var jsonData = new List<Manager.FracMatsData>();
                var index = 0;
                foreach (var p in biz.Products)
                {
                    var busProductData = BusProductsData[p.Name];
                    if (busProductData.ItemId != ItemId.Debug)
                        jsonData.Add(new Manager.FracMatsData(index,
                            Chars.Repository.ItemsInfo[busProductData.ItemId].Name,
                            Chars.Repository.ItemsInfo[busProductData.ItemId].Icon, $"{p.Price}$",
                            (int)busProductData.ItemId));
                    else if (p.Name == "Сим-карта")
                        jsonData.Add(new Manager.FracMatsData(index, "Сим-карта", "sm-icon-sim", $"{p.Price}$"));
                    else if (p.Name == "Лотерейный билет")
                        jsonData.Add(new Manager.FracMatsData(index, "Лотерейный билет", "sm-icon-lotary",
                            $"{p.Price}$"));
                    index++;
                }
            ...
            {
                Log.Write($"OpenBizShopMenu Exception: {e}");
            }
        }
        ...
        public static void Event_ShopCallback(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (prod.Name == "Сим-карта")
                {
                    ...
                    Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.SimCard, 1,
                        number.ToString());

                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        $"Вы купили сим-карту с номером {number}. Она добавлена вам в инвентарь", 3000);
                    //Chars.Repository.PlayerStats(player);
                    biz.BuyItemBusiness(characterData.UUID, "Сим-карта", prod.Price);
                    Wallet.Change(player, -prod.Price);
                    GameLog.Money($"player({characterData.UUID})", $"biz({biz.ID})", prod.Price,
                        $"buyShopSim({number})");
                }
                else if (prod.Name == "Лотерейный билет")
                {
                    var mynumb = (uint)Lottery.LotteryBought.Count() + 1;
                    if (!Lottery.LotteryBought.ContainsKey(mynumb))
                    {
                        ...
                        switch (mynumb)
                        {
                            ...
                            case 5001:
                                NAPI.Chat.SendChatMessageToAll(
                                    $"~g~[ЛОТЕРЕЯ] Продано более {mynumb - 1} лотерейных билетов. Сумма джекпота: {Wallet.Format(Lottery.Price)}$!");
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"Вы купили лотерейный билет #{mynumb}. Дополнительная информация: /lottery", 7000);
                        qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage29, 1, true);
                ...
                else
                {
                    ...
                    if (busProductData.ItemId != ItemId.Debug)
                    {
                        ...
                        if (ItemId.Pickaxe1 == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "300");
                        else if (ItemId.Pickaxe2 == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "1248");
                        else if (ItemId.Pickaxe3 == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "2250");
                        else if (ItemId.WorkAxe == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "1250");
                        else if (ItemId.Bong == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "420");
                        else if (ItemId.Vape == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "100");
                        else if (ItemId.Hookah == busProductData.ItemId)
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, "1000");
                        else
                            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory",
                                busProductData.ItemId, 1, busProductData.ItemId == ItemId.Bag ? "40_0_true" : "");

                        var itemInfo = Chars.Repository.ItemsInfo[busProductData.ItemId];

                        EventSys.SendCoolMsg(player, "Покупка", "Покупка предмета",
                            $"{LangFunc.GetText(LangType.Ru, DataName.YouBuy, itemInfo.Name, prod.Price)}", "", 6000);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            $"{LangFunc.GetText(LangType.Ru, DataName.YouBuy, itemInfo.Name, prod.Price)}", 1000);

                        if (itemInfo.functionType == newItemType.Eat /* && biz.Type == 8*/)
                        {
                            qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage21, 1, true);
                        ...
                        if (ItemId.GasCan == busProductData.ItemId)
                            qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage8, 0, 1, true);

                        if (ItemId.Wrench == busProductData.ItemId)
                            qMain.UpdateQuestsStage(player, Zdobich.QuestName, (int)zdobich_quests.Stage8, 1, 2, true);
            ...
            {
                Log.Write($"Event_ShopCallback Exception: {e}");
            }
        }
        ...
        public static void OpenPetrolMenu(ExtPlayer player)
        {
            try
            {
                ...
                EventSys.SendCoolMsg(player, "Заправка", "Добро пожаловать!", $"Цена за литр: {prod.Price}$", "",
                    10000);
                // Notify.Send(player, NotifyType.Info, NotifyPosition.Top, $"Цена за литр: {prod.Price}$", 7000);
            }
            catch (Exception e)
            {
                Log.Write($"OpenPetrolMenu Exception: {e}");
            }
        }
        ...
        public static void OpenGunShopMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (!player.IsCharacterData()) return;
                if (sessionData.TempBizID == -1 || !BizList.ContainsKey(sessionData.TempBizID)) return;
                var _WeaponsData = new List<List<Manager.CraftData>>();
                var biz = BizList[sessionData.TempBizID];
                for (var i = 0; i < gunsCat.Count; i++)
                {
                    var _CraftsData = new List<Manager.CraftData>();

                    foreach (var g in biz.Products)
                        if (gunsCat[i].Contains(g.Name))
                        {
                            var wType = (ItemId)Enum.Parse(typeof(ItemId), g.Name);
                            if (Chars.Repository.ItemsInfo.ContainsKey(wType))
                                _CraftsData.Add(new Manager.CraftData(Chars.Repository.ItemsInfo[wType].Name,
                                    Chars.Repository.ItemsInfo[wType].Icon, g.Price));
                        }
                ...
                foreach (var ammo in AmmoPrices) _AmmoData.Add(Convert.ToInt32(ammo / 100f * ammoPrice));

                Trigger.ClientEvent(player, "client.gunshop.open", JsonConvert.SerializeObject(_WeaponsData),
                    JsonConvert.SerializeObject(_AmmoData),
                    biz.Products.FirstOrDefault(p => p.Name == "Модификации").Price);
                BattlePass.Repository.UpdateReward(player, 129);
            }
            catch (Exception e)
            {
                Log.Write($"OpenGunShopMenu Exception: {e}");
            }
        }
        ...
        public static void Event_WShopComponent(ExtPlayer player, int cat, int index, string componentId)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (sessionData.TempBizID == -1 || !BizList.ContainsKey(sessionData.TempBizID)) return;
                var bizid = sessionData.TempBizID;
                if (!characterData.Licenses[6])
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У Вас нет лицензии на оружие. Получить её можно в полицейском департаменте.", 10000);
                    return;
                }
                ...
                if (!WeaponComponents.WeaponsComponents.ContainsKey((uint)(Hash)WeaponRepository.GetHash(prodName)))
                    return;
                var totalPrice = 0;
                uint componentHash = 0;
                var wType = ItemId.Debug;
                foreach (var w in WeaponComponents.WeaponsComponents[(uint)(Hash)WeaponRepository.GetHash(prodName)]
                             .Components)
                ...
                if (UpdateData.CanIChange(player, totalPrice, true) != 255) return;
                var amount = Convert.ToInt32(totalPrice * 0.75 / 100);
                if (amount <= 0) amount = 1;
                ...
                GameLog.Money($"player({characterData.UUID})", $"biz({biz.ID})", totalPrice,
                    $"buyWShop(component({1}))");
                Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", wType, 1,
                    $"{(uint)(Hash)WeaponRepository.GetHash(prodName)}_{componentHash}");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы купили {Chars.Repository.ItemsInfo[wType].Name} за {totalPrice}$", 3000);
                BattlePass.Repository.UpdateReward(player, 1);
            }
            catch (Exception e)
            {
                Log.Write($"Event_WShopComponent Exception: {e}");
            }
        }
        ...
        public static void Event_WShopAmmo(ExtPlayer player, int category, int ammo)
        {
            try
            {
                ...
                var totalPrice = ammo * Convert.ToInt32(AmmoPrices[category] / 100f * prod.Price);
                if (UpdateData.CanIChange(player, totalPrice, true) != 255) return;
                var prodamount = Convert.ToInt32(AmmoPrices[category] * ammo / 10);
                ...
                Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", AmmoTypes[category],
                    ammo);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы купили {Chars.Repository.ItemsInfo[AmmoTypes[category]].Name} x{ammo} за {totalPrice}$", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"Event_WShopAmmo Exception: {e}");
            }
        }
        ...
        private static readonly List<ItemId> AmmoTypes = new List<ItemId>
        {
        ...
        public static void Event_WShop(ExtPlayer player, int cat, int index)
        {
            try
            {
                ...
                if (UpdateData.CanIChange(player, prod.Price, true) != 255) return;
                var wType = (ItemId)Enum.Parse(typeof(ItemId), prod.Name);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы купили {prod.Name} за {prod.Price}$", 3000);
                BattlePass.Repository.UpdateReward(player, 130);
            }
            catch (Exception e)
            {
                Log.Write($"Event_WShop Exception: {e}");
            }
        }
    ...
    public class Order
    {
        ...
        [JsonIgnore] public bool Taked { get; set; }

        [JsonIgnore] public int UID { get; set; }
    }
    ...
    {
        [JsonIgnore] private readonly ExtTextLabel mafiaLabel;
        ...
        [JsonIgnore] private ExtMarker marker;

        [JsonIgnore] public int Pribil;

        [JsonIgnore] private ExtColShape shape;

        [JsonIgnore] private ExtColShape truckerShape;

        [JsonIgnore] public int Zatratq;

        public Business(int id, string owner, int sellPrice, int type, List<Product> products, Vector3 enterPoint,
            Vector3 unloadPoint, int bankID, int mafia, List<Order> orders, double tax = 0.026)
        {
            ID = id;
            Owner = owner;
            SellPrice = sellPrice;
            ...
            truckerShape = CustomColShape.CreateCylinderColShape(UnloadPoint - new Vector3(0, 0, 1), 8, 10,
                NAPI.GlobalDimension, ColShapeEnums.Trucker, ID);
            ...
            Main.CreateBlip(new Main.BlipData(BusinessManager.BlipByType[Type], BusinessManager.BusinessTypeNames[Type],
                EnterPoint, BusinessManager.BlipColorByType[Type], true));
            ...
            label = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(Main.StringToU16("Business"),
                new Vector3(EnterPoint.X, EnterPoint.Y, EnterPoint.Z + 1.5), textrange, 0.5F, 0,
                new Color(255, 255, 255), true, 0);
            mafiaLabel = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(Main.StringToU16("Mafia: none"),
                new Vector3(EnterPoint.X, EnterPoint.Y, EnterPoint.Z + 2), 5F, 0.5F, 0, new Color(255, 255, 255), true,
                0);
            UpdateLabel();
            if (Type != 1)
                marker = (ExtMarker)NAPI.Marker.CreateMarker(1, EnterPoint - new Vector3(0, 0, range - 0.3f),
                    new Vector3(), new Vector3(), range, new Color(255, 255, 255, 220), false, 0);
        }
        ...
        public void UpdateLabel()
        {
            Trigger.SetMainTask(() =>
            {
                try
                {
                    var text = $"~w~{BusinessManager.BusinessTypeNames[Type]}\n";
                    ...
                    text += $"~c~ID{ID}";
                    label.Text = text;
                    if (Mafia != -1) mafiaLabel.Text = $"~w~Мафия: ~r~{Manager.GetName(Mafia)}";
                    else mafiaLabel.Text = "~w~Мафия: ~w~пусто";
                }
                catch (Exception e)
                {
                    BusinessManager.Log.Write($"UpdateLabel Exception: {e}");
                }
            });
        }
        ...
        public void Destroy()
        {
            ...
            {
                BusinessManager.Log.Write($"Destroy Exception: {e}");
            }
        }
        ...
        {
            Businesses.History.Repository.AddHistory(uuid, ID, itemName, cost);
            Pribil += cost;
        }
