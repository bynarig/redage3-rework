...
{
    internal class EventSys : Script
    {
        public static CustomEvent
            AdminEvent = new CustomEvent(); // Одновременно можно будет создать только одно мероприятие.
                    ...
                    if (AdminEvent.EventMembers.ContainsKey(player))
                    {
                        ...
                        if (AdminEvent.EventState == 2)
                            if (AdminEvent.EventMembers.Count == 0)
                                CloseAdminEvent(AdminEvent.Admin, 0, -1, 0, -1, 0);
                    }
            }
            catch (Exception e)
            {
        ...
        public static bool ExitFromMP(ExtPlayer player, bool check = true)
        {
            try
            {
                ...
                if (AdminEvent.EventMembers.ContainsKey(player))
                {
                    ...
                    var sessionData = AdminEvent.EventMembers[player];
                    DeleteClientFromEvent(player);
                    if (check && AdminEvent.EventState == 2)
                        if (AdminEvent.EventMembers.Count == 0)
                            CloseAdminEvent(AdminEvent.Admin, 0, -1, 0, -1, 0);
                    NAPI.Player.SpawnPlayer(player,
                        new Vector3(sessionData.Position.X, sessionData.Position.Y, sessionData.Position.Z));
            ...
            {
                Log.Write($"ExitFromMP Exception: {e}");
            }

            return false;
        }

        [Command(AdminCommands.Createmp,
            "Используйте: /createmp [Лимит участников] [Радиус зоны] [Название мероприятия]", GreedyArg = true)]
        public static void CreateEvent(ExtPlayer player, ushort members, float radius, string eventname)
        {
            try
            {
                ...
                if (AdminEvent.EventState == 0)
                {
                    ...
                    {
                        if (radius >= 10)
                            AdminEvent.Zone = CustomColShape.CreateSphereColShape(player.Position, radius,
                                UpdateData.GetPlayerDimension(player), ColShapeEnums.AdminMP);
                        ...
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.EventStart,
                            ChatColors.AMP, eventname));
                        if (members != 0)
                            NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.EventMemberLimit,
                                ChatColors.AMP, members));
                        else
                            NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.LimitMemberNoStart,
                                ChatColors.AMP));
                        if (AdminEvent.Zone != null)
                            NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpRadiusTp,
                                ChatColors.AMP, radius));
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.TpMpTp, ChatColors.AMP));
                    }
                    else
                    {
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.TooLongMpName));
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.OneMpExists));
                }
            }
            catch (Exception e)
            {
                Log.Write($"CreateEvent Exception: {e}");
            }
        }
        ...
        public static void SendCoolMsg(ExtPlayer player, string subTitle, string title, string desc, string image,
            ushort timeWait)
        {
            foreach (var foreachPlayer in Repository.GetPlayers())
            {
            ...
            try
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    if (AdminEvent.EventMembers.Count >= 1)
                    {
                        ...
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpNameStarted,
                            ChatColors.AMP, AdminEvent.Name));
                        ...
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpPlayers, ChatColors.AMP,
                            AdminEvent.EventMembers.Count));
                    }
                    else
                    {
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.CantStartMpWithout));
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.MpNotStarted));
                }
            }
            catch (Exception e)
            {
                Log.Write($"StartEvent Exception: {e}");
            }
        }

        [Command(AdminCommands.Stopmp,
            "Используйте: /stopmp [ID Победителя] [Награда #1] [ID Победителя2?] [Награда #2?] [ID Победителя3?] [Награда #3?]")] // ne vishlo
        public static void MPReward(ExtPlayer player, int pid, uint reward, int pid2 = -1, uint reward2 = 0,
            int pid3 = -1, uint reward3 = 0)
        {
            try
            {
                ...
                if (AdminEvent.EventState == 2)
                {
                    if (reward <= AdminEvent.RewardLimit)
                    {
                        if (AdminEvent.Winner == null)
                        {
                            ...
                            if (target != null)
                            {
                                if (AdminEvent.EventMembers.ContainsKey(target) || AdminEvent.Admin == target)
                                    CloseAdminEvent(target, reward, pid2, reward2, pid3, reward3);
                                else
                                    Trigger.SendChatMessage(player,
                                        LangFunc.GetText(LangType.Ru, DataName.PlayerNotFoundMp));
                            }
                            else
                            {
                                Trigger.SendChatMessage(player,
                                    LangFunc.GetText(LangType.Ru, DataName.CantFindPlayerWithId));
                            }
                        }
                        else
                        {
                            Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.WinnerBilNaznachen));
                        }
                    }
                    else
                    {
                        Trigger.SendChatMessage(player,
                            "Награда не может превышать выставленный лимит: " + AdminEvent.RewardLimit + ".");
                    }
                }
                else
                {
            ...
            {
                Log.Write($"MPReward Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (UpdateData.GetPlayerDimension(player) == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantUse0Dim), 3000);
                    return;
                }
                ...
                if (AdminEvent.EventState >= 1)
                {
                    if (count >= 1 && count <= 10)
                    {
                        ...
                        if (vehHash != 0)
                        {
                            ...
                            {
                                var vehicle = VehicleStreaming.CreateVehicle(vehHash,
                                    new Vector3(player.Position.X + 4 * i, player.Position.Y, player.Position.Z),
                                    player.Rotation.Z, c1, c2, "EVENTCAR", acc: VehicleAccess.Event, numb: -2,
                                    dimension: UpdateData.GetPlayerDimension(player), petrol: 9999, engine: true);
                        ...
                        else
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.NoVehThatName), 3000);
                            }
                            else
                            {
                                var model1 = NAPI.Util.GetHashKey(model);
                                for (byte i = 0; i != count; i++)
                                {
                                    var vehicle = VehicleStreaming.CreateVehicle(model1,
                                        new Vector3(player.Position.X + 4 * i, player.Position.Y, player.Position.Z),
                                        player.Rotation.Z, c1, c2, "EVENTCAR", acc: VehicleAccess.Event, numb: -2,
                                        dimension: UpdateData.GetPlayerDimension(player), petrol: 9999, engine: true);
                                    vehicle.PrimaryColor = c1;
                    ...
                    {
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.OneTime10vehs));
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.VehOnlyMp));
                }
            }
            catch (Exception e)
            {
                Log.Write($"CreateMPVehs Exception: {e}");
            }
        }
        ...
        public static void GiveMPSkin(ExtPlayer player, int pid, string pedModel)
        {
                ...
                if (target != null)
                {
                    ...
                    if (AdminEvent.EventMembers.ContainsKey(target))
                    {
                        ...
                        Trigger.SendToAdmins(1,
                            LangFunc.GetText(LangType.Ru, DataName.AdminMpChangedSkin, ChatColors.AMP, AdminEvent.Name,
                                player.Name, target.Name, pedModel));
                        ...
                        if (pedModel.Equals("-1"))
                        {
                            if (targetSessionData.AdminSkin)
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.VisualPlayerReturned), 3000);
                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.VisualPlayerNotChanged), 3000);
                            }
                        }
                        else
                        {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PlayerIdFoundButNotMp), 3000);
                    }
                }
                else
                {
            ...
            {
                Log.Write($"GiveMPSkin Exception: {e}");
            }
        }
        ...
        public static void GiveMPSkins(ExtPlayer player, string pedModel, int radius)
        {
            try
            {
                ...
                Trigger.SendToAdmins(1,
                    LangFunc.GetText(LangType.Ru, DataName.AdminMpChangedSkinRadius, ChatColors.AMP, AdminEvent.Name,
                        player.Name, pedModel, radius));
            }
            catch (Exception e)
            {
                Log.Write($"GiveMPSkins Exception: {e}");
            }
        }
        ...
        public static void MPAdminPos(ExtPlayer player)
        {
            try
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    foreach (var foreachPlayer in AdminEvent.EventMembers.Keys)
                    {
                        if (foreachPlayer == null) continue;
                        Trigger.ClientEvent(foreachPlayer, "createMPWaypoint", player.Value, player.Position.X,
                            player.Position.Y, UpdateData.GetPlayerDimension(player));
                    }

                    Trigger.ClientEvent(player, "createMPWaypoint", player.Value, player.Position.X, player.Position.Y,
                        UpdateData.GetPlayerDimension(player));
                    Trigger.SendToAdmins(1,
                        LangFunc.GetText(LangType.Ru, DataName.PostavilMetkuVsem, ChatColors.AMP, AdminEvent.Name,
                            player.Name));
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MetkuCanPostaviTposleMp), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"MPAdminPos Exception: {e}");
            }
        }
                ...
                if (newreward <= 999999)
                {
                    ...
                    try
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.MpReward, newreward), 3000);
                    }
                    catch (Exception e)
                    {
                        Log.Write($"SetMPReward #2 Exception: {e}");
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.MpRewardLimit));
                }
            }
            catch (Exception e)
            {
                Log.Write($"SetMPReward Exception: {e}");
            }
        }
        ...
        public static void CMD_MPGlobal(ExtPlayer player, string text)
        {
            try
            {
                ...
                if (AdminEvent.EventState >= 1)
                {
                    ...
                    Trigger.SendChatMessage(player, $"{ChatColors.AMP}[{AdminEvent.Name}] {player.Name}: {text}");
                    lock (AdminEvent.EventMembers)
                    {
                        foreach (var foreachPlayer in AdminEvent.EventMembers.Keys)
                        {
                            ...
                            Trigger.SendChatMessage(foreachPlayer,
                                $"{ChatColors.AMP}[{AdminEvent.Name}] {player.Name}: {text}");
                ...
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.MpGlobalrError));
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_MPGlobal Exception: {e}");
            }
        }
        ...
        public static void TpToMp(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.IsBannedMP)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BlacklistedMp, characterData.BanMPReason), 5000);
                    return;
                }

                if (characterData.InsideHouseID == -1 && player.Dimension == 0 && characterData.DemorganTime <= 0 &&
                    characterData.ArrestTime <= 0 && !sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath &&
                    sessionData.SitPos == -1 && sessionData.InTanksLobby == -1)
                {
                    if (AdminEvent.EventState == 1)
                    {
                        if (!AdminEvent.EventMembers.ContainsKey(player))
                        {
                            if (AdminEvent.MembersLimit == 0 || AdminEvent.EventMembers.Count < AdminEvent.MembersLimit)
                            {
                                ...
                                Trigger.SendChatMessage(player,
                                    LangFunc.GetText(LangType.Ru, DataName.YouTpedMp, AdminEvent.Name));
                            }
                            else
                            {
                                Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.MpListFull));
                            }
                        }
                        else
                        {
                            Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.YouAlreadyMped));
                        }
                    }
                    else
                    {
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.TpClosed));
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.TpUnavaible));
                }
            }
            catch (Exception e)
            {
                Log.Write($"TpToMp Exception: {e}");
            }
        }
        ...
        public static void MPKick(ExtPlayer player, int pid, string reason)
        {
            try
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    if (target != null)
                    {
                        if (AdminEvent.EventMembers.ContainsKey(target))
                        {
                            AdminEvent.Admin = player;
                            Trigger.SendToAdmins(1,
                                LangFunc.GetText(LangType.Ru, DataName.AdminKickedMpS, ChatColors.AMP, AdminEvent.Name,
                                    player.Name, target.Name, reason));
                            lock (AdminEvent.EventMembers)
                            {
                                foreach (var foreachPlayer in AdminEvent.EventMembers.Keys)
                                {
                                    if (foreachPlayer == null) continue;
                                    Trigger.SendChatMessage(foreachPlayer,
                                        LangFunc.GetText(LangType.Ru, DataName.AdminKickedMpS, ChatColors.AMP,
                                            AdminEvent.Name, player.Name, target.Name, reason));
                                }
                            }
                        ...
                        {
                            Trigger.SendChatMessage(player,
                                LangFunc.GetText(LangType.Ru, DataName.PlayerIdFoundButNotMp));
                        }
                    }
                    else
                    {
                ...
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.VignatMozhno));
                }
            }
            catch (Exception e)
            {
                Log.Write($"MPKick Exception: {e}");
            }
        }
        ...
        public static void BanMP(ExtPlayer player, int pid, string reason)
        {
            try
            {
                ...
                {
                    if (targetCharacterData.IsBannedMP)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PlayerBlocked), 5000);
                        return;
                    }

                    Trigger.SendPunishment(
                        LangFunc.GetText(LangType.Ru, DataName.AdminBlockedMpS, CommandsAccess.AdminPrefix, player.Name,
                            target.Name, target.Value, reason), target);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantFindPlayerWithId), 5000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"BanMP Exception: {e}");
            }
        }
        ...
        public static void UnbanMP(ExtPlayer player, int pid)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PlayerHasntBlocked), 5000);
                        return;
                    }

                    Trigger.SendToAdmins(1,
                        LangFunc.GetText(LangType.Ru, DataName.AdminUnblockedMp, player.Name, player.Value, target.Name,
                            target.Value));
                    Trigger.SendChatMessage(target,
                        LangFunc.GetText(LangType.Ru, DataName.AdminBlockedMp, player.Name));
            ...
            try
            {
                ...
                if (AdminEvent.EventState != 0)
                    if (AdminEvent.EventMembers.ContainsKey(player))
                    {
                        ExitFromMP(player);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.SucLeaveMp), 5000);
                    }
            }
            catch (Exception e)
            {
                Log.Write($"MPLeave Exception: {e}");
            }
        }
        ...
        public static void MPHeal(ExtPlayer player, byte health)
        {
            try
            {
                ...
                if (AdminEvent.EventState >= 1)
                {
                    ...
                    {
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.HpOneTo100));
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.HpOnlyUnderStart));
                }
            }
            catch (Exception e)
            {
                Log.Write($"MPHeal Exception: {e}");
            }
        }
        ...
        public static void MpPlayerList(ExtPlayer player)
        {
            try
            {
                ...
                if (AdminEvent.EventState != 0)
                {
                    var memcount = Convert.ToInt16(AdminEvent.EventMembers.Count);
                    if (memcount > 0)
                    {
                        if (memcount <= 15)
                        {
                            foreach (var foreachPlayer in AdminEvent.EventMembers.Keys)
                            {
                                if (foreachPlayer == null) continue;
                                Trigger.SendChatMessage(player,
                                    "ID: " + foreachPlayer.Value + " | Имя: " + foreachPlayer.Name);
                            }

                            Trigger.SendChatMessage(player,
                                LangFunc.GetText(LangType.Ru, DataName.PlayersOnMp, memcount));
                        }
                        else
                        {
                            Trigger.SendChatMessage(player,
                    ...
                    {
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.NoMpPlayers));
                    }
                }
                else
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.MpNotSozdano));
                }
            }
            catch (Exception e)
            {
                Log.Write($"MpPlayerList Exception: {e}");
            }
        }
        ...
        {
            GameLog.EventLogAdd(AdminEvent.Admin.Name, AdminEvent.Name, AdminEvent.MembersLimit,
                MySQL.ConvertTime(AdminEvent.Started));
        }

        private static void UpdateLastAdminEventLog()
        {
            GameLog.EventLogUpdate(AdminEvent.Admin.Name, AdminEvent.EventMembers.Count, AdminEvent.Winner.Name,
                AdminEvent.Reward, MySQL.ConvertTime(DateTime.Now), AdminEvent.RewardLimit, AdminEvent.MembersLimit,
                AdminEvent.Name);
        }

        public static void CloseAdminEvent(ExtPlayer winner, uint reward, int winnerid2, uint reward2, int winnerid3,
            uint reward3)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpEnded, ChatColors.AMP,
                    AdminEvent.Name));
                ...
                if (winner != AdminEvent.Admin)
                {
                    if (reward != 0)
                    {
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpWinner1, ChatColors.AMP,
                            winner.Name, Wallet.Format(reward)));
                        Wallet.Change(winner, (int)reward);
                    }
                    else
                    {
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.WinnerMp1, ChatColors.AMP,
                            winner.Name));
                    }
                    ...
                    if (winnerid2 != -1)
                    {
                        var winner2 = Main.GetPlayerByID(winnerid2);
                        if (winner2 != null && winner2 != AdminEvent.Admin)
                        {
                            if (reward2 != 0 && reward2 <= AdminEvent.RewardLimit)
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpWinner,
                                    ChatColors.AMP, winnernum, winner2.Name, Wallet.Format(reward2)));
                                Wallet.Change(winner2, (int)reward2);
                            }
                            else
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.WinnerMp,
                                    ChatColors.AMP, winnernum, winner2.Name));
                            }
                    ...
                    if (winnerid3 != -1)
                    {
                        var winner3 = Main.GetPlayerByID(winnerid3);
                        if (winner3 != null && winner3 != AdminEvent.Admin)
                        {
                            if (reward3 != 0 && reward3 <= AdminEvent.RewardLimit)
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.MpWinner,
                                    ChatColors.AMP, winnernum, winner3.Name, Wallet.Format(reward3)));
                                Wallet.Change(winner3, (int)reward3);
                            }
                            else
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.WinnerMp,
                                    ChatColors.AMP, winnernum, winner3.Name));
                            }
                        }
                    }
                }
                ...
                if (AdminEvent.EventMembers.Count != 0)
                    lock (AdminEvent.EventMembers)
                    {
                        foreach (var foreachPlayer in AdminEvent.EventMembers.Keys)
                        {
                            ...
                            SendCoolMsg(foreachPlayer, "Мероприятие", AdminEvent.Name,
                                $"{LangFunc.GetText(LangType.Ru, DataName.SpsMp)}", "", 10000);
                        }
                    }
            ...
            {
                Log.Write($"CloseAdminEvent Exception: {e}");
            }
        }
        ...
        public class CustomEvent
        {
            ...
            public ExtColShape Zone { get; set; }

            public byte
                EventState { get; set; } // 0 - МП не создано, 1 - Создано, но не началось, 2 - Создано и началось.
