...
{
    internal class Doormanager : Script
    {
        ...
        public static int RegisterDoor(int model, Vector3 Position)
        {
            ...
            {
                Log.Write($"RegisterDoor Exception: {e}");
                return allDoors.Count - 1;
            }
        }

        [Interaction(ColShapeEnums.Door, true)]
        public static void InDoor(ExtPlayer player, int index)
        {
            try
            {
                if (!player.IsCharacterData()) return;
                var door = allDoors[index];
                Trigger.ClientEvent(player, "setDoorLocked", door.Model, door.Position.X, door.Position.Y,
                    door.Position.Z, door.Locked, door.Angle);
            }
            catch (Exception e)
            {
                Log.Write($"InDoor Exception: {e}");
            }
        }
        ...
        public static void SetDoorLocked(int id, bool locked, float angle)
        {
            try
            {
                ...
                Trigger.ClientEventForAll("setDoorLocked", allDoors[id].Model, allDoors[id].Position.X,
                    allDoors[id].Position.Y, allDoors[id].Position.Z, allDoors[id].Locked, allDoors[id].Angle);
            }
            catch (Exception e)
            {
                Log.Write($"SetDoorLocked Exception: {e}");
            }
        }
