...
namespace NeptuneEvo.Core
{
    internal class CarRoom : Script
    {
        ...
        private static readonly Vector3[] TestDrivePositions = new Vector3[7]
        {
        ...
        public static void enterCarroom(ExtPlayer player)
        {
            ...
            {
                Log.Write($"enterCarroom Exception: {e}");
            }
        }

        #region Menu

        private static readonly IReadOnlyDictionary<string, Color> carColors = new Dictionary<string, Color>
        {
        ...
        public static void OpenCarromMenu(ExtPlayer player, Business biz)
        {
            try
            {
                ...
                foreach (var p in biz.Products)
                {
                    ...
                    names.Add(p.Name);
                    bagageSlots.Add(vMain.GetMaxSlots(NAPI.Util.GetHashKey(p.Name)));
                }

                Trigger.ClientEvent(player, "openAuto", JsonConvert.SerializeObject(names),
                    JsonConvert.SerializeObject(prices), JsonConvert.SerializeObject(gosPrices),
                    JsonConvert.SerializeObject(bagageSlots), "carroomBuy");
            }
            catch (Exception e)
            {
                Log.Write($"OpenCarromMenu Exception: {e}");
            }
        }
        ...
        public static void OpenCarromMenuGos(ExtPlayer player, List<string> Cars, bool isDonate = false)
        {
            try
            {
                ...
                foreach (var name in Cars)
                {
                    ...
                    gosPrices.Add(busProductData.Price);
                    names.Add(name);
                    bagageSlots.Add(vMain.GetMaxSlots(NAPI.Util.GetHashKey(name)));
                }

                Trigger.ClientEvent(player, "openAuto", JsonConvert.SerializeObject(names),
                    JsonConvert.SerializeObject(prices), JsonConvert.SerializeObject(gosPrices),
                    JsonConvert.SerializeObject(bagageSlots), isDonate ? "AutoroomGosDonateBuy" : "AutoroomGosBuy",
                    isDonate);
            }
            catch (Exception e)
            {
        ...
        public static void RemoteEvent_carroomBuy(ExtPlayer player, string vName, string color, int id)
        {
            try
            {
                ...
                if (id == 1) // Для себя
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Произошла ошибка, транспорта такой модели нет на складе.", 3000);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }
                    ...
                    if (house != null)
                        if (vehiclesCount >= GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].Type]
                                .MaxCars)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    ...
                    if (biz.IsOwner())
                    {
                        if (biz.Type == 15)
                        {
                            if (prod.Lefts != 0)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        "Машин этой модели больше нет на складе", 3000);
                                    RemoteEvent_carroomCancel(player);
                                    return;
                                }
                    ...
                    Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                        LangFunc.GetText(LangType.Ru, DataName.YouBuyCarV2, vName, prod.Price), DateTime.Now);
                }
                else if (id == 2) // В организацию
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "У Вашей семьи уже максимальное количество транспортных средств.", 6000);
                        if (upgraded <= 1)
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Вы можете увеличить количество транспортных средств купив улучшение для семьи!", 6000);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }

                    var price = BusinessManager.BusProductsData[vName].Price;
                    ...
                    var vNumber = Manager.CreateVehicle(organizationData.Id, vName, carColors[color]);
                    //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы купили {vName} с номером {vNumber} за {price}$.", 3000);
                    Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                        LangFunc.GetText(LangType.Ru, DataName.YouBuyCarV, vName, vNumber, price), DateTime.Now);
                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.BuyCar,
                        $"Купил {vName} ({vNumber})");
                }
            ...
            {
                Log.Write($"RemoteEvent_carroomBuy Exception: {e}");
            }
        }
        ...
        {
            OnAutoroomBuy(player, vName, color, id, false);
        }
        ...
        public static void DonateAutoroomBuy(ExtPlayer player, string vName, string color, int id)
        {
            ...
            OnAutoroomBuy(player, vName, color, id, true);
        }
        ...
        private static void OnAutoroomBuy(ExtPlayer player, string vName, string color, int id, bool isDonate)
        {
            try
            {
                ...
                var vehiclePrice = !isDonate ? busProductData.Price : busProductData.OtherPrice;
                ...
                if (id == 1) // Для себя
                {
                    ...
                    VehicleManager.Create(player, vName, carColors[color], carColors[color],
                        Logs: $"buyCar_donate({vName}");
                    Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                        LangFunc.GetText(LangType.Ru, DataName.YouBuyCarV3, vName), DateTime.Now);
                }
                else if (id == 2) // В организацию
                {
                    if (!player.IsOrganizationAccess(RankToAccess.OrgBuyCars))
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Вы не можете купить в организацию данное т/с.", 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "В Вашей семье уже максимальное количество транспортных средств.", 6000);
                        if (upgraded <= 1)
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Вы можете увеличить количество транспортных средств купив улучшение для семьи!", 6000);
                        RemoteEvent_carroomCancel(player);
                    ...
                    Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                        LangFunc.GetText(LangType.Ru, DataName.YouBuyCarV, vName, vNumber, vehiclePrice), DateTime.Now);
                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.BuyCar,
                        $"Купил {vName} ({vNumber})");
                }
                ...
                if (!isDonate)
                    Wallet.Change(player, -vehiclePrice);
                else
                    UpdateData.RedBucks(player, -vehiclePrice,
                        LangFunc.GetText(LangType.Ru, DataName.PremCarBuy, vName, vehiclePrice));
            ...
            {
                Log.Write($"OnAutoroomBuy Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_carroomCancel(ExtPlayer player)
        {
            ...
            {
                Log.Write($"RemoteEvent_carroomCancel Exception: {e}");
            }
        }
        ...
        public static void RemoteEvent_testDrive(ExtPlayer player, string vName, string color, byte type)
        {
            try
            {
                ...
                var correctPrice = type == 2 ? 300 : 100;
                ...
                Wallet.Change(player, -correctPrice);
                ...
                var veh = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vName), positionData.Item1,
                    positionData.Item2.Z, 1, 1, number, acc: VehicleAccess.AutoRoom, workdriv: characterData.UUID,
                    petrol: 9999, dimension: 555, engine: true);
                ...
                sessionData.TimersData.TestDriveTimer = Timers.StartOnce(120000, () => timer_exitVehicle(player), true);
                GameLog.Money($"player({characterData.UUID})", "server", correctPrice,
                    $"testDrive({vName}, {sessionData.TempBizID})");
            ...
            {
                Log.Write($"RemoteEvent_testDrive Exception: {e}");
            }
        }
        ...
        public static void timer_exitVehicle(ExtPlayer player, bool quit = false)
        {
            if (OnExitTestDrive(player) && !quit)
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    "Время, выделенное на тест-драйв, закончилось.", 3000);
        }
        ...
        public void onPlayerExitVehicleHandler(ExtPlayer player, ExtVehicle vehicle)
        {
            if (OnExitTestDrive(player))
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    "Тест-драйв был окончен, так как Вы покинули транспорт.", 3000);
        }
