...
namespace NeptuneEvo.Fractions
{
    ...
    public class FractionClothingSets : Script
    {
        ...
        public static Dictionary<int, Vector3> FractionMainCloakrooms = new Dictionary<int, Vector3>
        {
            ...
            { (int)Models.Fractions.LSNEWS, LsNewsSystem.LSNewsCoords[0] },
        ...
        public static Dictionary<Models.Fractions, Dictionary<bool, List<FractionSetData>>> FractionSets =
            new Dictionary<Models.Fractions, Dictionary<bool, List<FractionSetData>>>();

        public static
            Dictionary<bool, Dictionary<Models.Fractions, Dictionary<ClothesComponent, List<FractionClothesData>>>>
            FractionAvailableSets =
                new Dictionary<bool,
                    Dictionary<Models.Fractions, Dictionary<ClothesComponent, List<FractionClothesData>>>>();
        ...
        public void OnResourceStart()
        {
            try
            {
                FractionAvailableSets.Add(true,
                    new Dictionary<Models.Fractions, Dictionary<ClothesComponent, List<FractionClothesData>>>());
                FractionAvailableSets.Add(false,
                    new Dictionary<Models.Fractions, Dictionary<ClothesComponent, List<FractionClothesData>>>());
                ...
                foreach (var availableSet in FractionClothingSetsData.AvailableSets)
                {
                    ...
                    if (!FractionAvailableSets[gender].ContainsKey(availableSet.Fraction))
                        FractionAvailableSets[gender][availableSet.Fraction] =
                    ...
                    if (!FractionAvailableSets[gender][availableSet.Fraction].ContainsKey(availableSet.Component))
                        FractionAvailableSets[gender][availableSet.Fraction][availableSet.Component] =
                            new List<FractionClothesData>();
                    //
                    ...
                    if (index != -1)
                        FractionAvailableSets[gender][availableSet.Fraction][availableSet.Component][index].Textures
                            .Add(availableSet.Color);
                    else
                        FractionAvailableSets[gender][availableSet.Fraction][availableSet.Component].Add(
                            new FractionClothesData
                            {
        ...
        public static void OpenFractionClothingSetsMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (!FractionMainCloakrooms.ContainsKey(memberFractionData.Id) &&
                    !FractionSecondCloakrooms.ContainsKey(memberFractionData.Id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantGetForm), 3000);
                    return;
                }
                ...
                if (FractionSets.ContainsKey((Models.Fractions)memberFractionData.Id) &&
                    FractionSets[(Models.Fractions)memberFractionData.Id].ContainsKey(characterData.Gender))
                {
            ...
            {
                Log.Write($"OpenFractionClothingSetsMenu Exception: {e}");
            }
        }
        ...
        private static void callback_fraction_clothing_sets_menu(ExtPlayer player, object listItem) /// Никитос Чини
        {
            try
            {
                ...
                if ((FractionMainCloakrooms.ContainsKey(memberFractionData.Id) &&
                     player.Position.DistanceTo(FractionMainCloakrooms[memberFractionData.Id]) < 5) ||
                    (FractionSecondCloakrooms.ContainsKey(memberFractionData.Id) &&
                     player.Position.DistanceTo(FractionSecondCloakrooms[memberFractionData.Id]) < 5))
                    switch (type)
                    {
                        case "takeoff":
                        ...
                        default:
                            ...
                            {
                                var clothingList =
                                    FractionSets[(Models.Fractions)memberFractionData.Id][characterData.Gender]
                                        .Where(f => f.Rank <= memberFractionData.Rank)
                            ...
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.CantUseThisForm), 3000);
                            return;
                    }
            ...
            {
                Log.Write($"callback_fraction_clothing_sets_menu Exception: {e}");
            }
        }

        public static bool SetPlayerFactionClothingSet(ExtPlayer player, int fracid, string setname, bool gender,
            bool isDutySet = false)
        {
            try
            {
                var sessionData = player.GetSessionData();
                    ...
                    if (clothingList.Count > 0)
                    {
                        ...
                        foreach (var cItem in clothingList)
                        {
                            ...
                            if (Chars.Repository.ClothesComponentToComponentId.ContainsKey(availableSet.Component))
                                ClothesComponents.SetSpecialClothes(player,
                                    Chars.Repository.ClothesComponentToComponentId[availableSet.Component].SlotId,
                                    availableSet.Variation, availableSet.Color);

                            if (Chars.Repository.ClothesComponentToPropId.ContainsKey(availableSet.Component))
                                ClothesComponents.SetSpecialAccessories(player,
                                    Chars.Repository.ClothesComponentToPropId[availableSet.Component].SlotId,
                                    availableSet.Variation, availableSet.Color);
            ...
            {
                Log.Write($"SetPlayerFactionClothingSet Exception: {e}");
            }
