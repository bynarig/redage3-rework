...
{
    internal class MafiaWars : Script
    {
        ...
        private static readonly Vector3[] warPoints = new Vector3[5]
        {
        ...
        private static readonly Vector3[] warCenterPoints = new Vector3[5]
        {
            new Vector3(1713.4241, -1646.3751, 112.477806),
            new Vector3(33, -2678, 6),
            new Vector3(503.4392, -3163.064, 6.0652146),
        ...
        private static readonly Vector3[] warNearCenterPoints = new Vector3[5]
        {
        ...
        private static readonly Dictionary<int, ExtBlip> warBlips = new Dictionary<int, ExtBlip>();
        ...
        public void OnResourceStart()
        {
            try
            {
                ...
                {
                    CustomColShape.CreateSphereColShape(position, Range, NAPI.GlobalDimension, ColShapeEnums.WarPoint,
                        i);
                    warBlips.Add(i,
                        (ExtBlip)NAPI.Blip.CreateBlip(543, position, 1, 40,
                            Main.StringToU16(LangFunc.GetText(LangType.Ru, DataName.BizWar)), 255, 0, true, 0, 0));
                    i++;
                }
            }
            catch (Exception e)
        ...
        public static void CMD_startBizwar(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizWarBan, characterData.BanCrimeReason), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantCaptYourBiz), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BizNotBelongsToMaf), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MafiaTimeWar), 3000);
                    return;
                }
                ...
                if (Main.ServerNumber != 0 && DateTime.Now < nextCaptDate[fracId])
                {
                    ...
                    if (g.Minute >= 5)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.WarTimeMafHours, g.Hour, g.Minute, g.Second), 6000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.WarTimeMaf, g.Minute, g.Second), 6000);
                ...
                {
                    var ticks = protectDate[biz.Mafia].Ticks - DateTime.Now.Ticks;
                    if (ticks <= 0) return;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoOnline), 3000);
                    return;
                }

                if (Repository.Wars.Values.Any(w => w.Type == WarType.Mafia))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.WarBizGoing), 3000);
                    return;
                }

                var mapId = WorkManager.rnd.Next(0, 5);
                ...
                Trigger.SendToAdmins(1,
                    LangFunc.GetText(LangType.Ru, DataName.AdminBizWar, ChatColors.Red, player.Name, player.Value,
                        Manager.GetName(fracId), Manager.GetName(biz.Mafia)));
                GameLog.FracLog(fracId, characterData.UUID, -1, player.Name, "-1",
                    $"bizwar({biz.ID}, {BusinessManager.BusinessTypeNames[biz.Type]}, {Manager.FractionNames[biz.Mafia]})");

                Manager.sendFractionMessage(biz.Mafia,
                    LangFunc.GetText(LangType.Ru, DataName.BizWarDefendStart, Manager.GetName(fracId)));
            ...
            {
                Log.Write($"CMD_startBizwar Exception: {e}");
            }
        }
        ...
        [Interaction(ColShapeEnums.WarPoint, true)]
        public static void InWarPoint(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (fracId >= (int)Models.Fractions.LCN && fracId <= (int)Models.Fractions.ARMENIAN)
                    Repository.InWarZone(player, WarType.Mafia, index);
            }
            catch (Exception e)
            {
                Log.Write($"InWarPoint Exception: {e}");
            }
        }
        ...
        {
            Repository.OutWarZone(player, WarType.Mafia, index);
        }
        ...
        public static void CMD_tpbizwar(ExtPlayer player)
        {
            try
            {
                ...
                if (!player.IsFractionAccess(RankToAccess.BizwarJoin)) return;
                ...
                if (war != null && (fracId == war.ProtectingId || fracId == war.AttackingId))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AlreadyGoToWarZone), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AlreadyInWarZone), 3000);
                        return;
                    }
                    ...
                    if (fracId == war.ProtectingId)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.WastpCenterWar), 5000);
                    }
                    else
                    {
                        sessionData.PositionCaptureOrBizwar = player.GetPosition();
                        ...
                        sessionData.UsedTPOnCaptureOrBizwar = 1;
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.WasTPNearWar), 7000);
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_tpbizwar Exception: {e}");
            }
        }
    }
}