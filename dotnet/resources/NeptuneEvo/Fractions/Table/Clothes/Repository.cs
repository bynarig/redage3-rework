...
namespace NeptuneEvo.Fractions.Table.Clothes
{
    public class Repository
    {
        public static void ClothesLoad(ExtPlayer player)
        {
            ...
            {
                if (clothesList.Any(f => Convert.ToBoolean(f[0]) && f[1].ToString() == fractionSet.SetName))
                    continue;
        ...
        public static void ClothesUpdate(ExtPlayer player, string oldName, string newName, int rank, bool gender)
        {
            try
            {
                ...
                if (!FractionClothingSets.FractionMainCloakrooms.ContainsKey(fractionData.Id) &&
                    !FractionClothingSets.FractionSecondCloakrooms.ContainsKey(fractionData.Id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantEditForm), 3000);
                    return;
                }
                ...
                if ((FractionClothingSets.FractionMainCloakrooms.ContainsKey(fracId) &&
                     player.Position.DistanceTo(FractionClothingSets.FractionMainCloakrooms[fracId]) < 10) ||
                    (FractionClothingSets.FractionSecondCloakrooms.ContainsKey(fracId) &&
                     player.Position.DistanceTo(FractionClothingSets.FractionSecondCloakrooms[fracId]) < 10))
                        ...
                        FractionClothingSets.FractionSets[(Models.Fractions)fractionData.Id].ContainsKey(gender))
                    {
                        if (!oldName.Equals(newName))
                        {
                            ...
                            if (fractionSets.Any(f => f.SetName.Equals(newName)))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    gender
                                        ? LangFunc.GetText(LangType.Ru, DataName.ThisNameMaleFormUsed)
                                        : LangFunc.GetText(LangType.Ru, DataName.ThisNameFemaleFormUsed), 3000);
                                return;
                            }
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.SucEditFormName), 5000);
                        }
                        ...
                        FractionClothingSets.SetPlayerFactionClothingSet(player, fracId, newName, gender, false);
                        ...
                        var clothesPriceList =
                            FractionClothingSets.FractionAvailableSets[gender][(Models.Fractions)fracId].ToList();
                        ...
                        Trigger.ClientEvent(player, "client.shop.open", "clothes", characterData.Gender,
                            JsonConvert.SerializeObject(realClothes), JsonConvert.SerializeObject(dataJson), 2);
                        return;
                    }
        ...
        public static void EditClothingSet(ExtPlayer player, string name, int id, int texture)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.FormEditError), 3000);
                    return;
                }

                var fraction = fractionData.Id;
                if (fraction == 0) return;

                if ((FractionClothingSets.FractionMainCloakrooms.ContainsKey(fraction) &&
                     player.Position.DistanceTo(FractionClothingSets.FractionMainCloakrooms[fraction]) < 10) ||
                    (FractionClothingSets.FractionSecondCloakrooms.ContainsKey(fraction) &&
                     player.Position.DistanceTo(FractionClothingSets.FractionSecondCloakrooms[fraction]) < 10))
                {
                    var dictionary = (ClothesComponent)Enum.Parse(typeof(ClothesComponent), name);
                    int rank = memberFractionData.Rank;
                    var gender = sessionData.TempClothingData.Gender;
                    var setName = sessionData.TempClothingData.NewName;
                        ...
                        FractionClothingSets.FractionSets[(Models.Fractions)fractionData.Id].ContainsKey(gender))
                    {
                        ...
                        if (id == -1 && texture == 0)
                        {
                            ...
                            foreach (var item in toDeleteClothingList)
                            {
                                ...
                                if (aSet.Component == dictionary)
                                {
                                    ...
                                    FractionClothingSets.SetPlayerFactionClothingSet(player, fractionData.Id, setName,
                                        gender);
                                    //player.ClearAccessories();
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.SucDeleteFormComponent), 3000);
                                    return;
                                }
                            }

                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.FormError), 3000);
                            return;
                        }
                        ...
                        var availableSet = FractionClothingSetsData.AvailableSets
                            ...
                            .FirstOrDefault(f =>
                                f.value.Gender == Convert.ToByte(gender) &&
                                f.value.Fraction == (Models.Fractions)fractionData.Id &&
                                f.value.Component == dictionary && f.value.Variation == id && f.value.Color == texture);
                        ...
                        if (availableSet.value != null)
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.SucEditForm), 8000);
                            FractionClothingSets.SetPlayerFactionClothingSet(player, fractionData.Id, setName, gender);
                            //player.ClearAccessories();
                        ...
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantEditThisForm), 3000);
                        return;
                    }
                }
