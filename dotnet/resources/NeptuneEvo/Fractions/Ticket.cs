...
namespace NeptuneEvo.Fractions
{
    internal class Ticket : Script
    {
        ...
        //Координаты
        private static readonly Vector3 ImpoundLotPos = new Vector3(463.67065, -1070.8586, 29.211897);

        //
        public static Vector3 PedPos = new Vector3(470.6959, -1059.139, 29.211658);

        private static readonly float PedRot = 175.875656f;

        //
        private static readonly int Cost = 10;

        public static List<VehicleTicket> VehicleTickets = new List<VehicleTicket>();

        private static readonly IReadOnlyList<uint> CarsException = new List<uint>
        {
        ...
        private readonly IReadOnlyList<(Vector3, Vector3)> VehicleSpawnPos = new List<(Vector3, Vector3)>
        {
        ...
        public static async void OnResourceStart()
        {
            ...
            //
            Repository.CreateQuest("s_m_m_ciasec_01", PedPos, PedRot, questName: QuestName,
                title: "~y~NPC~w~ Роберт\nСотрудник полиции", colShapeEnums: ColShapeEnums.ImpoundLotPed,
                pedBlipData: new PedBlipData
                {
                    BlipId = 783,
        ...
        [Interaction(ColShapeEnums.ImpoundLot, true)]
            ...
            try
            {
                ...
                if (vehicle.Model != NAPI.Util.GetHashKey("flatbed"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MustEvac), 3000);
                    return;
                }
                ...
                var payment = Convert.ToInt32(sessionData.TicketCost * Cost * Main.ServerSettings.MoneyMultiplier);

                var maxpayment = 4000 * Main.ServerSettings.MoneyMultiplier;
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.SucEvac, payment), 3000);
            }
            catch (Exception e)
            {
        ...
        public static bool CarException(uint model)
        {
            return CarsException.Contains(model);
        }
        ...
        public static void OpenTicket(ExtPlayer player, ExtVehicle vehicle)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoEvacEvac), 5000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.CantEvac), 5000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoEvac), 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoEvacPlace), 3000);
                return;
            }
            ...
            switch (vehicleLocalData.Access)
            {
                ...
                case VehicleAccess.Fraction:
                    ...
                    switch ((SafeZones.ZoneName)sessionData.InsideSafeZone)
                    {
                        ...
                        case SafeZones.ZoneName.EMSpark:
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.CantEvac), 3000);
                                return;
                            }
                        ...
                        case SafeZones.ZoneName.govfib:
                            if ((Models.Fractions)fractionData.Id == Models.Fractions.FIB ||
                                (Models.Fractions)fractionData.Id == Models.Fractions.CITY)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.CantEvac), 3000);
                                return;
                ...
                case VehicleAccess.Organization:
                    var orgId = vehicleLocalData.Fraction;
        ...
        public void OnTicketEnd(ExtPlayer player, string ticketText, int ticketPrice, string cameraLink, bool isEvac)
        {
            ...
            {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.EvacSum), 5000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.EvacSymbols), 3000);
                return;
            }
            ...
            if (isEvac)
            {
                ...
                {
                    var ticks = vehicleLocalData.ExitTime.Ticks - DateTime.Now.Ticks;
                    if (ticks <= 0) return;
                    var g = new DateTime(ticks);
                    if (g.Hour >= 1)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoEvac1Hours, g.Hour, g.Minute, g.Second), 10000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NoEvac0Hours, g.Minute, g.Second), 10000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoEvacSidit), 7000);
                    return;
                }
            ...
            switch (vehicleLocalData.Access)
            {
                ...
                case VehicleAccess.Personal:
                    ...
                    var uuid = Main.PlayerUUIDs.GetValueOrDefault(vehicleData.Holder);
                    if (uuid > 0)
                    {
                    ...
                    Insert(player, ticketText, ticketPrice, cameraLink, isEvac, VehicleTicketType.Player, uuid,
                        vehicleData.Holder,
                ...
                case VehicleAccess.Work:
                    var name = Main.PlayerNames.ContainsKey(vehicleLocalData.WorkDriver)
                        ? Main.PlayerNames[vehicleLocalData.WorkDriver]
                        : "";

                    Insert(player, ticketText, ticketPrice, cameraLink, isEvac, VehicleTicketType.Player,
                        vehicleLocalData.WorkDriver, name,
                ...
                case VehicleAccess.Hotel:
                    ...
                    if (hotelSessionData != null && hotelCharacterData != null && hotelSessionData.HotelData != null &&
                        hotelSessionData.HotelData.Car == vehicleTicket)
                    {
                        hotelSessionData.HotelData.Car = null;
                        Insert(player, ticketText, ticketPrice, cameraLink, isEvac, VehicleTicketType.Player,
                            hotelCharacterData.UUID, $"{hotelCharacterData.FirstName}_{hotelCharacterData.LastName}",
                            -1, vehicleLocalData.NumberPlate, vehicleTicket.Model.ToString());
                    }
                ...
                case VehicleAccess.Fraction:
                    ...
                    if (isEvac)
                    {
                        ...
                        Manager.sendFractionMessage(fractionData.Id,
                            LangFunc.GetText(LangType.Ru, DataName.EvacShtrafS, player.Name.Replace("_", " "),
                                vehicleTicket.NumberPlate));
                    }
                    else
                    {
                        Manager.sendFractionMessage(fractionData.Id,
                            LangFunc.GetText(LangType.Ru, DataName.EvacShtraf, ticketPrice));
                    }
                ...
                case VehicleAccess.Organization:
                    ...
                    Insert(player, ticketText, ticketPrice, cameraLink, isEvac, VehicleTicketType.Organization, orgId,
                    ...
                    if (isEvac)
                    {
                        ...
                        fbAttach.Hash = vehicleTicket.Model;
                        VehicleStreaming.DeleteVehicle(vehicleTicket);
                        Manager.sendOrganizationMessage(orgId,
                            LangFunc.GetText(LangType.Ru, DataName.EvacShtrafS, player.Name.Replace("_", " "),
                                vehicleTicket.NumberPlate));
                    }
                    else
            ...
            if (target.IsCharacterData())
                Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.EvacShtraf, ticketPrice), 6000);
            player.AddTableScore(TableTaskId.Item9);
            player.AddTableScore(TableTaskId.Item15);
            ...
            if (isEvac)
            {
                if (target.IsCharacterData())
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.EvacShtrafS, player.Name.Replace("_", " "),
                            vehicleTicket.NumberPlate), 10000);
                ...
                Trigger.ClientEvent(player, "createWaypoint", PedPos.X, PedPos.Y);
            }
        }

        private static void Insert(ExtPlayer player, string ticketText, int ticketPrice, string cameraLink, bool isEvac,
            VehicleTicketType type, int uuid, string holder, int vehSqlId, string vehNumber, string model)
        {
            var sessionData = player.GetSessionData();
            if (sessionData == null)
                return;
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    var sqlId = await db.InsertWithInt32IdentityAsync(new Vehicletickets
                    {
                        VehAutoId = vehSqlId,
                        VehNumber = vehNumber,
                        Model = model,
        ...
        public static void OpenTickets(ExtPlayer player, VehicleTicketType type = VehicleTicketType.Player)
        {
            ...
            var vehicleTickets = VehicleTickets
                .Where(vt => (vt.Type == VehicleTicketType.Player && vehiclesNumber.Contains(vt.VehNumber)) ||
                             (vt.Type == VehicleTicketType.Organization && vt.HolderAutoId == orgid &&
                              vehiclesOrgData.ContainsKey(vt.VehNumber) &&
                              orgrank >= vehiclesOrgData[vt.VehNumber].rank))
                .ToList();
        ...
        public void OnTicketPayment(ExtPlayer player, int autoId)
        {
            ...
            if (vehicleTicket != null)
            {
                if (vehicleTicket.Type == VehicleTicketType.Player)
                {
                    ...
                    if (vehiclesNumber.Contains(vehicleTicket.VehNumber))
                    {
                        ...
                        Wallet.Change(player, -vehicleTicket.Price);
            ...
            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.SomethingWrong), 5000);
        }
    }
}