...
using Repository = NeptuneEvo.World.War.Repository;
using WarData = NeptuneEvo.Players.Models.WarData;

namespace NeptuneEvo.Fractions
{
    internal class GangsCapture : Script
    {
        ...
        public void OnResourceStart()
        {
            try
            {
                ...
                {
                    CustomColShape.Create2DColShape(gangZones[region.ID].X, gangZones[region.ID].Y, 100, 100, 0,
                        ColShapeEnums.GangZone, region.ID);
                    CustomColShape.CreateSphereColShape(gangZones[region.ID], Range, NAPI.GlobalDimension,
                        ColShapeEnums.WarGangZone, region.ID);
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (characterData.IsBannedCrime)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        $"Вам ограничили доступ к системам capture и bizwar. Причина: {characterData.BanCrimeReason}.",
                        5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouAreNotInRegion), 3000);
                    return;
                }
                ...
                switch (shapeData.Index)
                {
                    ...
                    case 89:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.DontClaimTitulRegion), 3000);
                        return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantClaimYourRegion), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.TimeYouCanClaimRegion), 3000);
                    return;
                }

                if (Main.ServerNumber != 0 && DateTime.Now < nextCaptDate[fracId])
                {
                    var ticks = nextCaptDate[fracId].Ticks - DateTime.Now.Ticks;
                    if (ticks <= 0) return;
                    ...
                    if (g.Minute >= 5)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCanClaim1h, g.Hour, g.Minute, g.Second), 3000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCanClaim, g.Minute, g.Second), 3000);
                ...
                {
                    var ticks = protectDate[region.GangOwner].Ticks - DateTime.Now.Ticks;
                    if (ticks <= 0) return;
                    ...
                    if (g.Minute >= 5)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCanClaimGangRegion1h, g.Hour, g.Minute, g.Second),
                            3000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCanClaimGangRegion, g.Minute, g.Second), 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DontHaveOnlineEnemyGang), 3000);
                    return;
                }

                if (Repository.Wars.Values.Any(w => w.Type == WarType.Gangs))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CaptIsGoingSomeGang), 3000);
                    return;
                }
                ...
                var warId = Repository.War(player, warData, (sbyte)WarGripType.LastSurvivor, 0, 0, isCheckTime: false,
                    isWarInterface: false);
                if (warId == 0) return;

                Trigger.ClientEventForAll("setZoneFlash", warData.ObjectId, true,
                    gangPointsColor[warData.ProtectingId]);

                Trigger.SendToAdmins(1,
                    $"{ChatColors.Red}[A] {player.Name} ({player.Value}) из {Manager.GetName(warData.AttackingId)} спровоцировал capture за территорию {Manager.GetName(warData.ProtectingId)}.");
                GameLog.FracLog(fracId, characterData.UUID, -1, player.Name, "-1",
                    $"capture({region.ID}, {Manager.FractionNames[region.GangOwner]})");

                Manager.SendCoolFractionMsg(warData.ProtectingId, "Капт", "На вас напали!",
                    LangFunc.GetText(LangType.Ru, DataName.CaptureAlertDefend, Manager.GetName(warData.AttackingId)),
                    "", 10000);
                Manager.SendCoolFractionMsg(warData.AttackingId, "Капт", "Нападение",
                    LangFunc.GetText(LangType.Ru, DataName.CaptureAlertAttack), "", 10000);
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Repository.EntryZone(foreachPlayer, warId);
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_startCapture Exception: {e}");
            }
        }
        ...
        [Interaction(ColShapeEnums.WarGangZone, true)]
        public void InWarGangZone(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (fracId >= (int)Models.Fractions.FAMILY && fracId <= (int)Models.Fractions.BLOOD)
                    Repository.InWarZone(player, WarType.Gangs, index);
            }
            catch (Exception e)
            {
                Log.Write($"InWarGangZone Exception: {e}");
            }
        }
        ...
        {
            Repository.OutWarZone(player, WarType.Gangs, index);
        }
        ...
        public static void CMD_tpcapture(ExtPlayer player)
        {
            try
            {
                ...
                var war = Repository.Wars.Values
                ...
                if (war != null && (fracId == war.ProtectingId || fracId == war.AttackingId))
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AlreadyTpToClaimRegion), 3000);
                        return;
                    }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouWasInClaimRegion), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouAlreadyInClaimRegion), 3000);
                    return;
                }
                ...
                if (fracId != war.ProtectingId)
                {
                    ...
                    EventSys.SendCoolMsg(player, "Капт", "Телепорт",
                        LangFunc.GetText(LangType.Ru, DataName.YouWasTpCenterClaimRegion), "", 5000);
                ...
                else
                {
                    ...
                    if (nearestCaptureCoord != null)
                    {
                        ...
                        sessionData.UsedTPOnCaptureOrBizwar = 1;
                        EventSys.SendCoolMsg(player, "Капт", "Телепорт",
                            LangFunc.GetText(LangType.Ru, DataName.YouWasTpNearClaimRegion), "", 5000);
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantFindNearClaimRegion), 3000);
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_tpcapture Exception: {e}");
            }
        }
        ...
        public static void LoadBlips(ExtPlayer player)
        {
            try
            {
                ...
                var wars = Repository.Wars.Values
                    .FirstOrDefault(w => w.Type == WarType.Gangs);
            ...
            {
                Log.Write($"LoadBlips Exception: {e}");
            }
        }
        ...
        public class GangPoint
        {
            ...
            public void Save()
            {
                Trigger.SetTask(async () =>
                {
                    ...
                    {
                        Log.Write($"Gangspoints Save Exception: {e}");
                    }
                });
            }
        }
