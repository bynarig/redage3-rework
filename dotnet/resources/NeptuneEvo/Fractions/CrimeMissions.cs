...
{
    internal class CarDelivery : Script
    {
        private static readonly nLog Log = new nLog("Fractions.CrimeMissions");

        private static readonly uint[] GangsVehiclesHashes = new uint[15]
        {
        ...
        private static readonly Color[] GangsVehiclesColors = new Color[15]
        {
            new Color(0, 0, 0), new Color(0, 0, 0), new Color(250, 250, 250), new Color(200, 0, 0),
            new Color(250, 250, 250), new Color(250, 250, 250), new Color(0, 0, 200), new Color(80, 80, 80),
            new Color(0, 0, 0), new Color(230, 90, 0), new Color(250, 250, 250), new Color(0, 0, 250),
            new Color(80, 80, 80), new Color(250, 250, 250), new Color(0, 200, 0)
        };

        private static readonly uint[] MafiaVehiclesHashes = new uint[3]
        {
        ...
        private static int LastGangSpawn;
        ...
        public void Event_ResourceStart()
        {
            try
            {
                CustomColShape.CreateCylinderColShape(GangStartDelivery, 2, 3, NAPI.GlobalDimension,
                    ColShapeEnums.CrimeGang);

                NAPI.TextLabel.CreateTextLabel("~w~Carter Scott", GangStartDelivery + new Vector3(0, 0, 2.5), 5f, 0.4f,
                ...
                {
                    CustomColShape.CreateCylinderColShape(pos.Value, 3, 3, NAPI.GlobalDimension,
                        ColShapeEnums.CrimeMafia, pos.Key);
                    NAPI.Marker.CreateMarker(1, pos.Value - new Vector3(0, 0, 2.7), new Vector3(), new Vector3(), 3,
                        new Color(255, 0, 0, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel("~w~Взять миссию по доставке", pos.Value, 5f, 0.4f, 0,
                ...
                CustomColShape.CreateCylinderColShape(PoliceEndDelivery, 3, 4, NAPI.GlobalDimension,
                    ColShapeEnums.PoliceDropDelivery);
                NAPI.Marker.CreateMarker(1, PoliceEndDelivery - new Vector3(0, 0, 2.5), new Vector3(), new Vector3(), 3,
                    new Color(255, 0, 0, 220), false, NAPI.GlobalDimension);
                ...
                {
                    CustomColShape.CreateCylinderColShape(pos, 5, 3, NAPI.GlobalDimension,
                        ColShapeEnums.GangEndDelivery, i);

                    NAPI.Marker.CreateMarker(1, pos - new Vector3(0, 0, 3.5), new Vector3(), new Vector3(), 4,
                        new Color(255, 0, 0, 220), false, NAPI.GlobalDimension);
                    i++;
                }
                ...
                {
                    CustomColShape.CreateCylinderColShape(pos, 5, 3, NAPI.GlobalDimension,
                        ColShapeEnums.MafiaEndDelivery, i);
        ...
        [Interaction(ColShapeEnums.PoliceDropDelivery, true)]
        public static void InPoliceDropDelivery(ExtPlayer player)
        {
            try
            {
                ...
                {
                    if (vehicleLocalData.Access != VehicleAccess.DeliveryGang &&
                        vehicleLocalData.Access != VehicleAccess.DeliveryMafia &&
                        vehicleLocalData.Access != VehicleAccess.DeliveryBike) return;
                    var target = vehicleLocalData.DeliveryData.WhosVeh;
            ...
            {
                Log.Write($"InPoliceDropDelivery Exception: {e}");
            }
        }


        [Interaction(ColShapeEnums.GangEndDelivery, true)]
        public static void InGangEndDelivery(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Попробуйте чуть позже",
                            3000);
                        return;
                    }
                    ...
                    var halfof = Convert.ToInt32(Main.GangCarDelivery / 2);
                    Wallet.Change(player, halfof);
                    fractionData.Money += halfof;
                    GameLog.Money("server", $"frac({fractionData.Id})", halfof, "dropCar");
                    Trigger.SendChatMessage(player,
                        "Сдача машины: !{#00FF00}" + halfof +
                        "$ ~w~были отправлены в общак банды и еще столько же тебе в карман. (" +
            ...
            {
                Log.Write($"InGangEndDelivery Exception: {e}");
            }
        }

        [Interaction(ColShapeEnums.MafiaEndDelivery, true)]
        public static void InMafiaEndDelivery(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access != VehicleAccess.DeliveryMafia &&
                        vehicleLocalData.Access != VehicleAccess.DeliveryBike) return;
                    if (vehicleLocalData.DeliveryData.End != index) return;
                    if (DateTime.Now < vehicleLocalData.DeliveryData.DataEnd)
                    {
                    ...
                    sessionData.DeliveryData.Point = -1;
                    sessionData.DeliveryData.Vehicle = null;
                    var halfof = Convert.ToInt32(Main.MafiaCarDelivery / 2);
                    Wallet.Change(player, halfof);
                    fractionData.Money += halfof;
                    GameLog.Money("server", $"frac({fractionData.Id})", halfof, "dropCar");
                    Trigger.SendChatMessage(player,
                        "Сдача фургона: !{#00FF00}" + halfof +
                        "$ ~w~были отправлены в общак группировки и еще столько же тебе в карман. (" +
                        $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()})");
                    player.AddTableScore(TableTaskId.Item27);
                }
            }
        ...
        public void Event_PlayerEnterVehicle(ExtPlayer player, ExtVehicle vehicle, sbyte seat)
        {
                    ...
                    switch (vehicleLocalData.Access)
                    {
                        case VehicleAccess.DeliveryGang:
                            if (Configs.IsFractionPolic(fracId))
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                    "Отвезите машину в полицейский участок", 3000);
                            }
                            ...
                            {
                                var end = vehicleLocalData.DeliveryData.End;
                                vehicleLocalData.DeliveryData.JStage = true;
                                Trigger.ClientEvent(player, "createWaypoint", GangEndDelivery[end].X,
                                    GangEndDelivery[end].Y);
                                sessionData.DeliveryData.Point = end;
                                Trigger.SendChatMessage(player,
                                    "Если вдруг Вы сбили точку доставки, то напишите в чат /mypoint");
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                                    "Доставьте машину в точку, отмеченную на карте", 3000);
                            }
                        ...
                        case VehicleAccess.DeliveryBike:
                            ...
                            else
                            {
                                var end = vehicleLocalData.DeliveryData.End;
                                vehicleLocalData.DeliveryData.JStage = true;
                                Trigger.ClientEvent(player, "createWaypoint", MafiaEndDelivery[end].X,
                                    MafiaEndDelivery[end].Y);
                                sessionData.DeliveryData.Point = end;
                                Trigger.SendChatMessage(player,
                                    "Если вдруг Вы сбили точку доставки, то напишите в чат /mypoint");
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
            ...
            {
                Log.Write($"Event_PlayerEnterVehicle Exception: {e}");
            }
        }
        ...
        public static void OnCrimeGang(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OnCrimeGang Exception: {e}");
            }
        }
        ...
        public static void OnCrimeMafia(ExtPlayer player, int id)
        {
            ...
            {
                Log.Write($"OnCrimeMafia Exception: {e}");
            }
        }
        ...
        public static void OnCrimeBiker(ExtPlayer player, int id)
        {
            ...
            {
                Log.Write($"OnCrimeBiker Exception: {e}");
            }
        }
        ...
        public static void Event_gangMission(ExtPlayer player, int id)
        {
            try
            {
                ...
                switch (id)
                {
                    case 0:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            "Недоступно на данный момент", 3000);
                        return;
                    case 1:
                        if (DateTime.Now < NextDelivery[fracId])
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Попробуйте позже",
                                3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                "Вы должны сначала доставить предыдущую машину", 3000);
                            return;
                        }

                        var rnd = new Random();
                        var vehicleNum = rnd.Next(0, 15);
                        var vehicle = VehicleStreaming.CreateVehicle(GangsVehiclesHashes[vehicleNum],
                            GangSpawnAutos[LastGangSpawn], GangSpawnAutosRot[LastGangSpawn].Z, 0, 0, "xxxxx",
                            acc: VehicleAccess.DeliveryGang, petrol: 100);
                        Trigger.ClientEvent(player, "createWaypoint", GangSpawnAutos[LastGangSpawn].X,
                            GangSpawnAutos[LastGangSpawn].Y);
                        ...
                        data.DataEnd =
                            DateTime.Now.AddSeconds(vehicle.Position.DistanceTo(GangEndDelivery[end]) / 100 * 2);
                        data.JStage = false;
                        data.WhosVeh = player;
                        ...
                        Trigger.SendChatMessage(player,
                            "Если вдруг Вы сбили координаты машины, то напишите в чат /mypoint");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            "Вы получили координаты машины. Сядьте в неё и отвезите в место, которое отмечено в GPS автомобиля.",
                            3000);
                        player.AddTableScore(TableTaskId.Item27);
                        return;
                }
            ...
            {
                Log.Write($"Event_gangMission Exception: {e}");
            }
        }
        ...
        public static void Event_mafiaMission(ExtPlayer player, int id)
        {
            try
            {
                ...
                var vehicle = VehicleStreaming.CreateVehicle(vehicleHash, MafiaStartDelivery[i] + new Vector3(0, 0, 1),
                    MafiaStartDeliveryRot[i].Z, 0, 0, "xxxxx", acc: VehicleAccess.DeliveryMafia, petrol: 100);
                vehicle.CustomPrimaryColor = new Color(0, 0, 0);
                vehicle.CustomSecondaryColor = new Color(0, 0, 0);
                var end = rnd.Next(0, 12);
                var vehicleLocalData = vehicle.GetVehicleLocalData();
                var data = vehicleLocalData.DeliveryData;
                data.End = end;
                data.Fraction = fracId;
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Вы получили фургон с {text} для транспортировки. Отвезите его в указанное на карте место", 3000);
                player.AddTableScore(TableTaskId.Item27);
            ...
            {
                Log.Write($"Event_mafiaMission Exception: {e}");
            }
        }
