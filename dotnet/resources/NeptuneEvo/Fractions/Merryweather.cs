...
{
    internal class Merryweather : Script
    {
        private static readonly nLog Log = new nLog("Fractions.Merryweather");

        public static Vector3[] Coords = new Vector3[3]
        {
            new Vector3(2036.511, 2941.482,
                -63.021739), // Колшэйп входа на другой этаж // 2036.511, 2941.482, -61.90174
            new Vector3(2157.496, 2921.067,
                -82.19534), // Колшэйп изнутри этажа, чтобы вернуться назад // 2157.496, 2921.067, -81.07534
        ...
        public static void OpenLiftMenu(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OpenLiftMenu Exception: {e}");
            }
        }
        ...
        public static void callback_lift(ExtPlayer player, int floor)
        {
            try
            {
                ...
                if (sessionData.Following != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.IsFollowing), 5000);
                    return;
                }

                var data = characterData.Gender ? "128_0_true" : "98_0_false";

                if (player.GetFractionId() != (int)Models.Fractions.MERRYWEATHER &&
                    Repository.isItem(player, "inventory", ItemId.Jewelry, data) == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
            ...
            {
                Log.Write($"callback_lift Exception: {e}");
            }
        }
    }
}