...
{
    internal class Cityhall : Script
    {
        ...
        private static readonly Vector3 StockPosition = new Vector3(-1299.6835, -557.7507, 30f);
        ...
        private static readonly Vector3 JobPosition4 = new Vector3(-116.56201, -604.71436, 36.28072);
        ...
        public void OnResourceStartHandler()
        {
            try
            {
                ...
                CustomColShape.CreateCylinderColShape(CloakroomPosition, 1, 2, 0,
                    ColShapeEnums.FractionCityhallBeginWorkDay, 2);
                NAPI.Marker.CreateMarker(30, CloakroomPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));

                CustomColShape.CreateCylinderColShape(SecondCloakroomPosition, 1, 2, 0,
                    ColShapeEnums.FractionCityhallBeginWorkDay, 2);
                ...
                Repository.CreateQuest("s_m_y_devinsec_01", GiveGunPosition, -139.39449f,
                    title: "~y~NPC~w~ Энтони Янг\nВыдача оружия", colShapeEnums: ColShapeEnums.FractionCityhallGunMenu);
                Repository.CreateQuest("s_m_y_devinsec_01", SecondGiveGunPosition, -129.7344f,
                    title: "~y~NPC~w~ Джонни Ганз\nВыдача оружия",
                    colShapeEnums: ColShapeEnums.FractionCityhallGunMenu);
                Repository.CreateQuest("mp_f_execpa_02", JobPosition, -112.454f,
                    title: "~y~NPC~w~ Эмма Смит\nУстройство на работу", colShapeEnums: ColShapeEnums.JobSelect);
                ...
                Repository.CreateQuest("mp_f_execpa_02", JobPosition4, -111.755f,
                    title: "~y~NPC~w~ Эмма Смит\nУстройство на работу", colShapeEnums: ColShapeEnums.JobSelect);
                ...
                for (var i = 0; i < CityhallLiftCheckpoints.Length; i++)
                {
                    CustomColShape.CreateCylinderColShape(CityhallLiftCheckpoints[i], 1, 2, 0,
                        ColShapeEnums.FractionCityhall, i < 4 ? 10 : 11);
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Лифт"),
                        new Vector3(CityhallLiftCheckpoints[i].X, CityhallLiftCheckpoints[i].Y,
                            CityhallLiftCheckpoints[i].Z), 5F, 0.3F, 0, new Color(255, 255, 255));
                    NAPI.Marker.CreateMarker(1, CityhallLiftCheckpoints[i] - new Vector3(0, 0, 1.25), new Vector3(),
                        new Vector3(), 1f, new Color(255, 255, 255, 220));
                }

                for (var i = 0; i < CourtCheckpoints.Length; i++)
                {
                    CustomColShape.CreateCylinderColShape(CourtCheckpoints[i], 1, 2, 0, ColShapeEnums.FractionCityhall,
                        12);
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Лифт"),
                        new Vector3(CourtCheckpoints[i].X, CourtCheckpoints[i].Y, CourtCheckpoints[i].Z), 5F, 0.3F, 0,
                        new Color(255, 255, 255));
                    NAPI.Marker.CreateMarker(1, CourtCheckpoints[i] - new Vector3(0, 0, 1.25), new Vector3(),
                        new Vector3(), 1f, new Color(255, 255, 255, 220));
                }

                for (var i = 0; i < SecondGovLiftCoords.Length; i++)
                {
                    CustomColShape.CreateCylinderColShape(SecondGovLiftCoords[i], 1, 2, 0,
                        ColShapeEnums.FractionCityhall, 13);
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Лифт"),
                        new Vector3(SecondGovLiftCoords[i].X, SecondGovLiftCoords[i].Y, SecondGovLiftCoords[i].Z), 5F,
                        0.3F, 0, new Color(255, 255, 255));
                    NAPI.Marker.CreateMarker(1, SecondGovLiftCoords[i] - new Vector3(0, 0, 1.25), new Vector3(),
                        new Vector3(), 1f, new Color(255, 255, 255, 220));
                }
            }
            catch (Exception e)
        ...
        public static void OpenCallGovMemberDialog(ExtPlayer player, int _)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "CallGovMemberDialog",
                    LangFunc.GetText(LangType.Ru, DataName.AreYouWantToCallGov));
            }
            catch (Exception e)
            {
                Log.Write($"OpenCallGovMemberDialog Exception: {e}");
            }
        }
        ...
        public static void OnFractionCityhall(ExtPlayer player, int interact)
        {
            try
            {
                ...
                switch (interact)
                {
                    ...
                    case 3:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NotGOV), 3000);
                            return;
                        }
                        ...
                        Chars.Repository.LoadOtherItemsData(player, "Fraction", "6", 5,
            ...
            {
                Log.Write($"interactPressed Exception: {e}");
            }
        }
        ...
        public static void UseCityhallLift(ExtPlayer player, byte lift, int index)
        {
            try
            {
                ...
                if (lift == 1)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AlreadyAtFloor), 3000);
                        return;
                    }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.GoingToFloor, index), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"UseCityhallLift Exception: {e}");
            }
        }
        ...
        public static void OnFractionCityhallBeginWorkDay(ExtPlayer player)
        {
            try
            {
                if (player.GetFractionId() == (int)Models.Fractions.CITY)
            ...
            {
                Log.Write($"beginWorkDay Exception: {e}");
            }
        }
        ...
        public static void OnFractionCityhallGunMenu(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OnFractionCityhallGunMenu Exception: {e}");
            }
        }
        ...
        public static void callback_cityhallGuns(ExtPlayer player, int index)
        {
            try
            {
                ...
                switch (index)
                {
                    ...
                    case 9:
                        ...
                        fractionData.UpdateLabel();
                        Chars.Repository.RemoveIndex(player, "inventory", aItem.Index);
            ...
            {
                Log.Write($"callback_cityhallGuns Exception: {e}");
            }
        }
    }
}