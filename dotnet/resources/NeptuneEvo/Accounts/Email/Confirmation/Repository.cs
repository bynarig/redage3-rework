    ...
    public class Repository
    {
        ...
        public static async Task<EmailConfirmEnum> Confirm(ExtPlayer player, string email)
        {
            try
            {
                ...
                var hash = await Email.Repository.Add(player, string.Empty, string.Empty, email, string.Empty, false,
                    1);

                HelperThread.AddUrl(
                    $"verify?email={email}&name={accountData.Login}&hash={hash}&sid={Main.ServerNumber}");
        ...
        {
            var emailVerification = Email.Repository.GetVerification(hash, false);
            ...
            if (emailVerification != null)
            {
                ...
                Chars.Repository.AddNewItemWarehouse(player, ItemId.Case4);
                ...
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge,
                    "Почта подтверждена успешно. Приз отправлен на склад. (M->GPS - > Склад)", DateTime.Now);
