...
namespace NeptuneEvo.Accounts.Merger
{
    public class Repository
    {
        ...
        public static async Task MergerAuntification(ExtPlayer player, string pass_, int serverId = 2)
        {
            try
            {
                if (Admin.IsServerStoping)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MergeErrorRestart), 3000);
                    return;
                }
                ...
                var SlotsForMerger = serverId == 2 ? 3 : 6;
                ...
                var account = await db.Accounts
                    ...
                    .Where(v => (v.Socialclub == sessionData.SocialClubName ||
                                 v.Socialclub == sessionData.RealSocialClub) && v.Password == pass_)
                    .FirstOrDefaultAsync();
                if (account == null)
                {
                ...
                var characters = await db.Characters
                    ...
                    .Where(v => v.Uuid == account.Character1 || v.Uuid == account.Character2 ||
                                v.Uuid == account.Character3)
                    .ToListAsync();

                var CharsToSlotId = new Dictionary<int, int>();
                var SlotsIdToChar = new Dictionary<int, int>();

                foreach (var character in characters)
                {
                    var SloId = 0;
                    foreach (var SloData in accountData.Chars)
                    {
                        if (SloId >= SlotsForMerger && SlotsForMerger + 3 > SloId)
                            if (SloData < 0 && !CharsToSlotId.ContainsKey(character.Uuid) &&
                                !SlotsIdToChar.ContainsKey(SloId))
                            {
                ...
                if (account.Viplvl > 0)
                {
                    var unixTime = TimeSpan.FromTicks(account.Vipdate.Ticks - DateTime.Now.Ticks).TotalDays;
                    if (unixTime > 0)
                        Chars.Repository.UpdateVipStatus(player, account.Viplvl, (int)unixTime, true, true);
                }
                ...
                foreach (var character in characters)
                {
                    ...
                    var MoneyTo = 0;
                    var MoneyToBiz = await MergerToBiz(db, serverId, character.Biz);
                    if (MoneyToBiz > 0)
                    {
                        MoneyTo += MoneyToBiz;
                        redBucks += 3000;
                    ...
                    if (houseData != null)
                    {
                        ...
                        {
                            var fsd =
                                JsonConvert.DeserializeObject<Dictionary<int, HouseFurniture>>(furniture.Furniture);
                            foreach (var f in fsd.Values) itemsData.Add($"furniture_{houseData.Id}_{f.Id}", null);
                        }
                    }

                    Trigger.ClientEvent(player, "client.merger.progress", num * 24);

                    var OldFirstname = character.Firstname;
                    var OldLastname = character.Lastname;
                    ...
                    var charId = await MergerToChar(player, db, saveDB, serverId, MoneyTo, ch, num);

                    itemsData.Add($"char_{character.Uuid}", $"char_{charId}");

                    await MergerToCustom(db, saveDB, ch, charId);

                    var VehiclesItemsData =
                        await MergerToVehicles(db, saveDB, serverId, $"{OldFirstname}_{OldLastname}", ch);

                    foreach (var vItems in VehiclesItemsData) itemsData.Add(vItems.Key, vItems.Value);

                    Trigger.ClientEvent(player, "client.merger.progress", num * 26);

                    var OrganizationItemsData = await MergerToOrganization(player, db, saveDB, serverId,
                        $"{OldFirstname}_{OldLastname}", MoneyTo, charId, ch);

                    foreach (var vItems in OrganizationItemsData) itemsData.Add(vItems.Key, null);
            ...
            {
                Log.Write($"MergerAuntification Exception: {e}");
            }
        }
        ...
        public static async Task<int> MergerToBiz(ServerBD db, int serverId, string biz)
        {
            try
            {
                var BizIds = JsonConvert.DeserializeObject<List<int>>(biz);
                var MoneyTo = 0;
                if (BizIds.Count > 0)
                {
                    ...
                    if (businessesData != null)
                    {
                        ...
                        var orders = JsonConvert.DeserializeObject<List<Order>>(businessesData.Orders);
                        ...
                        var prodlist = JsonConvert.DeserializeObject<List<Product>>(businessesData.Products);
            ...
            {
                Log.Write($"MergerToBiz(serverId({serverId})) Exception: {e}");
                return 0;
            }
        }

        public static async Task<int> MergerToChar(ExtPlayer player, ServerBD db, ServerBD saveDB, int serverId,
            int MoneyTo, Characters character, int num)
        {
            try
            {
                var OldFirstname = character.Firstname;
                var OldLastname = character.Lastname;
                var renamed = false;
                while (saveDB.Characters.Any(v =>
                           v.Firstname == character.Firstname && v.Lastname == character.Lastname))
                    try
                    {
                        if (character.Lastname.Length >= 25)
                        {
                ...
                if (renamed)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.TopCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CharSuccMerged, OldFirstname, OldLastname), 10000);
                ...
                if (charMoney != null)
                {
                    bankID = await Bank.Create($"{character.Firstname}_{character.Lastname}", 1,
                        Convert.ToInt32(charMoney.Balance / 10));

                    Main.PlayerBankAccs.TryAdd($"{character.Firstname}_{character.Lastname}", bankID);
                }
                else
                {
                    bankID = await Bank.Create($"{character.Firstname}_{character.Lastname}");
                }

                Trigger.ClientEvent(player, "client.merger.progress", num * 25);
                if (character.Hotel != -1) MoneyTo += (int)(Main.HotelRent * character.Hotelleft);

                var newsimint = Players.Phone.Sim.Repository.GenerateSimCard();
                ...
                Main.PlayerUUIDs.TryAdd($"{character.Firstname}_{character.Lastname}", charId);
                Main.PlayerNames.TryAdd(charId, $"{character.Firstname}_{character.Lastname}");
            ...
            {
                Log.Write(
                    $"MergerToChar({character.Uuid} - {character.Firstname}_{character.Lastname}) Exception: {e}");
                return 0;
            }
        }
        ...
        public static async Task MergerToCustom(ServerBD db, ServerBD saveDB, Characters character, int charId)
        {
            ...
            {
                Log.Write($"MergerToCustom({character.Uuid}) Exception: {e}");
            }
        }

        public static async Task<Dictionary<string, string>> MergerToVehicles(ServerBD db, ServerBD saveDB,
            int serverId, string Name, Characters character)
        {
            try
            {
                var ItemsData = new Dictionary<string, string>();
            ...
            {
                Log.Write($"MergerToVehicles({character.Uuid}) Exception: {e}");
                return new Dictionary<string, string>();
            }
        }

        public static async Task<Dictionary<string, string>> MergerToOrganization(ExtPlayer player, ServerBD db,
            ServerBD saveDB, int serverId, string Name, int moneyTo, int charId, Characters character)
        {
            try
            {
                var ItemsData = new Dictionary<string, string>();
                ...
                if (orgRank != null)
                {
                    ...
                    if (orgData != null)
                    {
                        ...
                        if (officeUpgrade >= 2)
                            UpdateData.RedBucks(player, Convert.ToInt32(Main.PricesSettings.SecondOrgPrice / 1),
                                LangFunc.GetText(LangType.Ru, DataName.CashBackOrgMerge));
                        if (Convert.ToBoolean(orgData.Customs))
                            moneyTo += Convert.ToInt32(Main.PricesSettings.CustomsPrice / 1);
                        if (Convert.ToBoolean(orgData.Stock))
                            moneyTo += Convert.ToInt32(Main.PricesSettings.StockPrice / 1);
                        if (Convert.ToBoolean(orgData.CrimeOptions))
                            moneyTo += Convert.ToInt32(Main.PricesSettings.CrimeOptionsPrice / 1);
            ...
            {
                Log.Write($"MergerToOrganization({character.Uuid}) Exception: {e}");
                return new Dictionary<string, string>();
            }
        }

        public static async Task<Dictionary<string, string>> InitItemsData(ServerBD db, ServerBD saveDB, int charId,
            Dictionary<string, string> ItemsData)
        {
            try
            {
                var itemsList = new List<string>();
                foreach (var data in ItemsData) itemsList.Add(data.Key);

                var items = await db.ItemsData
                    .Where(v => v.DataId.In(itemsList))
                    .ToListAsync();

                var ItemsBackpackData = new Dictionary<string, string>();
                if (items != null)
                {
                    var _SlotId = (short)Chars.Repository.GetItemsDataLastId($"warehouse_{charId}", "warehouse");
                    var locationName = $"warehouse_{charId}";
                    var Location = "warehouse";
                    var isWarehouse = true;
                    foreach (var item in items)
                        if ((ItemId)item.ItemId != ItemId.CarKey)
                        {
                            ...
                            if (!Chars.Repository.ItemsData.ContainsKey(locationName))
                                Chars.Repository.ItemsData.TryAdd(locationName,
                            ...
                            if (!Chars.Repository.ItemsData[locationName].ContainsKey(Location))
                                Chars.Repository.ItemsData[locationName].TryAdd(Location,
                                    new ConcurrentDictionary<int, InventoryItemData>());

                            Chars.Repository.ItemsData[locationName][Location][SlotId] =
                                new InventoryItemData(itemSqlID, ItemId, (int)item.ItemCount, item.ItemData, SlotId);

                            if (isWarehouse) _SlotId++;
                            if ((ItemId)item.ItemId == ItemId.Bag)
                                ItemsBackpackData.Add($"backpack_{item.AutoId}", $"backpack_{itemSqlID}");
                        }
            ...
            {
                Log.Write($"InitItemsData({charId}) Exception: {e}");
                return new Dictionary<string, string>();
            }
        }
    }
