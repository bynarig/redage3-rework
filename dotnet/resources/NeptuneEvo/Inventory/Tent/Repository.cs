...
namespace NeptuneEvo.Inventory.Tent
{
    internal class Repository
    {
        ...
        public static void OnResourceStart()
        {
            ...
            {
                var shape = CustomColShape.CreateCylinderColShape(data.tradePosition, 2f, 3, 0, ColShapeEnums.Tent,
                    index);
                var label = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel($"#{index + 1}",
                    data.shopPosition + new Vector3(0, 0, 1f), 7f, 0.5F, 0, new Color(255, 255, 255), true, 0);
                var marker = (ExtMarker)NAPI.Marker.CreateMarker(1, data.tradePosition, new Vector3(), new Vector3(),
                    0.4f, new Color(255, 255, 255, 100), false, 0);
            ...
            Main.CreateBlip(new Main.BlipData(565, LangFunc.GetText(LangType.Ru, DataName.GlavRinok),
                TentList.PositionGps[0], 36, true));
            CustomColShape.CreateCylinderColShape(TentList.PositionGps[0], 90, 30, 0, ColShapeEnums.SafeZoneTent);

            Main.CreateBlip(new Main.BlipData(484, LangFunc.GetText(LangType.Ru, DataName.CherRinok),
                TentList.PositionGps[1], 54, true));
            CustomColShape.CreateCylinderColShape(TentList.PositionGps[1], 90, 30, 0, ColShapeEnums.SafeZoneTent);
        ...
        public static void OpenTent(ExtPlayer player, int index)
        {
            ...
            Chars.Repository.LoadOtherItemsData(player, "tent", targetCharacterData.UUID.ToString(), 11,
                Chars.Repository.InventoryMaxSlots["tent"], isMyTent: false);
        }
        ...
        public static void OpenMenuTent(ExtPlayer player, bool isPhone = false)
        {
            try
            {
                ...
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.RentTentDo, tentData.RentTime),
                    "info"));
                frameList.List.Add(new ListData(
                    LangFunc.GetText(LangType.Ru, DataName.RentTentExp, Wallet.Format(TentPrice)), "rentTime"));
            ...
            {
                Log.Write($"OpenRentcarMenu Exception: {e}");
            }
        }
        ...
        private static void callback_menutent(ExtPlayer player, object listItem) /// Никитос Чини
        {
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "inventory":
                        //Trigger.ClientEvent(player, "client.inventory.SlotToPrice", JsonConvert.SerializeObject(tentData.slotToPrice));
                        Chars.Repository.LoadOtherItemsData(player, "tent", characterData.UUID.ToString(), 11,
                            Chars.Repository.InventoryMaxSlots["tent"], isMyTent: true);
                        break;
                    case "rentTime":
                        if (UpdateData.CanIChange(player, TentPrice, true) != 255) return;

                        GameLog.Money($"player({characterData.UUID})", $"tent({sessionData.TentIndex})", TentPrice,
                            "rentTimeTent");

                        Wallet.Change(player, -TentPrice);

                        EventSys.SendCoolMsg(player, "Рынок", "Аренда палатки",
                            $"{LangFunc.GetText(LangType.Ru, DataName.RentTentExpand)}", "", 7000);
                        ...
                        break;
                    case "name":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.TentName),
                            LangFunc.GetText(LangType.Ru, DataName.NameTentInput), 35, "rentname");
                        break;
                }
            }
            catch (Exception e)
            {
                Log.Write($"callback_menutent Exception: {e}");
            }
        }
        ...
        public static void UpdateTentLabel(int index, string text = "")
        {
            ...
            if (player.IsCharacterData())
                tentData.label.Text = LangFunc.GetText(LangType.Ru, DataName.SellerTent, text, player.Name);
        ...
        public static void OpenRentTent(ExtPlayer player, int index)
        {
            try
            {
                ...
                frameList.List.Add(new ListData(
                    LangFunc.GetText(LangType.Ru, DataName.ArendaTelephone, Wallet.Format(TentPrice)),
                    index)); // почему-то не уверен
        ...
        private static void callback_renttent(ExtPlayer player, object listItem) /// Никитос Чини 
        {
            try
            {
                ...
                var index = Convert.ToInt32(listItem);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AlreadyTentYou), 3000);
                    return;
                }

                if (tentData.player != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AlreadyTent), 3000);
                    return;
                }
                ...
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tent,
                    LangFunc.GetText(LangType.Ru, DataName.YouRentedTent), DateTime.Now);
            ...
            {
                Log.Write($"callback_renttent Exception: {e}");
            }
        }
        ...
        public static void RemoveTent(int index, string type)
        {
            try
            {
                ...
                switch (type)
                {
                    case "time":
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tent,
                            LangFunc.GetText(LangType.Ru, DataName.TentRentGo), DateTime.Now);
                        break;
                    case "remove":
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tent,
                            LangFunc.GetText(LangType.Ru, DataName.TentRentCancel), DateTime.Now);
                        break;
                    /*case "SafeZoneTent":
                ...
                if (Chars.Repository.ItemsData.ContainsKey(locationName) &&
                    Chars.Repository.ItemsData[locationName].ContainsKey(Location))
            ...
            {
                Log.Write($"RemoveTent Exception: {e}");
            }
        }
