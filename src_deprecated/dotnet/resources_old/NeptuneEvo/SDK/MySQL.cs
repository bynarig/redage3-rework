...
namespace NeptuneEvo.SDK
{
    ...
    {
        private static readonly MysqlSettings config = Settings.ReadAsync("mainDB", new MysqlSettings());
        private static readonly nLog Log = new nLog("MySQL");

        private static string Connection;
        private static string LogsConnection;
        ...
        public static void Query(MySqlCommand command)
        {
            try
            {
                ...
                {
                    Log.Write($"BAD Query?: '{command.CommandText}'", nLog.Type.Error);
                }
                else
                {
            ...
            {
                Log.Write($"Query({command.CommandText}) Exception: {e}");
            }
        }
        ...
        public static async Task LogsQueryAsync(MySqlCommand command)
        {
            try
            {
                if (command.CommandText.Length < 1)
                {
                    Log.Write($"BAD LogsQueryAsync?: '{command.CommandText}'", nLog.Type.Error);
                }
                else
                {
                    if (Debug) Log.Debug("Query to LogsDB:\n" + command.CommandText);
                    //qlog.Write($"{DateTime.Now} | Query: {command.CommandText}\n");
                    using (var connection = new MySqlConnection(LogsConnection))
                    {
            ...
            {
                Log.Write($"LogsQuery({command.CommandText}) Exception: {e}");
            }
        }
        ...
        public static async Task QueryAsync(MySqlCommand command)
        {
            try
            {
                if (command.CommandText.Length < 1)
                {
                    Log.Write($"BAD QueryAsync?: '{command.CommandText}'", nLog.Type.Error);
                }
                else
                {
                    if (Debug) Log.Debug("Query to DB:\n" + command.CommandText);
            ...
            {
                Log.Write($"QueryAsync({command.CommandText}) #1 Exception: {e}");
            }
        }
        ...
        public static DataTable QueryRead(MySqlCommand command)
        {
            ...
            {
                Log.Write($"QueryRead({command.CommandText}) Exception: {e}");
                return null;
            }
        }
        ...
        public static async Task<DataTable> QueryReadAsync(MySqlCommand command)
        {
            ...
            {
                Log.Write($"QueryReadAsync({command.CommandText}) Exception: {e}");
                return null;
            }
        }
        ...
        public static int Insert(MySqlCommand command)
        {
            ...
            {
                Log.Write($"Insert({command.CommandText}) Exception: {e}");
                return 0;
            }
        }
