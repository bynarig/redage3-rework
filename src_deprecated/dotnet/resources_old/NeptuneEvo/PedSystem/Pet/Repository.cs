...
namespace NeptuneEvo.PedSystem.Pet
{
    internal class Repository
    {
        ...
        private static readonly List<uint> IsAttack = new List<uint>
        {
            (uint)PedHash.Cat, (uint)PedHash.Rottweiler, (uint)PedHash.Husky, (uint)PedHash.Retriever,
            (uint)PedHash.Shepherd, (uint)PedHash.MountainLion, (uint)PedHash.Coyote, 3877461608
        };
        ...
        private static readonly IReadOnlyDictionary<ItemId, int> PetFood = new Dictionary<ItemId, int>
        {
            { ItemId.Crisps, 30 },
            { ItemId.Pizza, 70 },
            { ItemId.Burger, 50 },
        ...
        public static void LoadInGame()
        {
            ...
            PedSystem.Repository.CreateQuest("a_m_y_soucent_03", new Vector3(268.109, -641.3529, 42.01984), 60.41674f,
                title: "~y~NPC~w~ Виталий\nПродавец питомцев", colShapeEnums: ColShapeEnums.PetShop);
            Main.CreateBlip(new Main.BlipData(141, LangFunc.GetText(LangType.Ru, DataName.PetSeller),
                new Vector3(268.109, -641.3529, 42.01984), 9, true));
        ...
        {
            foreach (var ped in PetsData.Keys.ToList()) Damage(ped, -1);
        }
        ...
        public static async Task<List<Pets>> LoadPlayerPet(ServerBD db, int uuid)
        {
            ...
            {
                Log.Write($"LoadPlayerPet Exception: {e}");
            }
        ...
        public static void InitPlayerPet(ExtPlayer player, List<Pets> pets)
        {
            try
            {
                ...
                foreach (var ped in createPets)
                {
                    ...
                    var petName = ped.GetSharedData<string>("petName");
        ...
        private static void Save(List<PetData> ListSavePet)
        {
            Trigger.SetTask(async () =>
            {
                ...
                {
                    Log.Write($"Save Trigger.SetTask Exception: {e}");
                }
            });
        }
        ...
        public static void Damage(ExtPed ped, int amount)
        {
            ...
            if (petData.Health <= 0)
            {
                ...
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Ems,
                    LangFunc.GetText(LangType.Ru, DataName.PetDying), DateTime.Now);
            }
            else if (player != null)
            {
        ...
        {
            var ped = (ExtPed)NAPI.Ped.CreatePed(petData.Model, petData.Position, petData.Heading, true, true,
                controlLocked: false, dimension: 0);
        ...
        public static void Create(ExtPlayer player, int index)
        {
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AlreadyHavePet), 5000);
                        return;
                    }

                    if (!pet.isDonate && UpdateData.CanIChange(player, pet.Price, true) != 255) return;
                    if (pet.isDonate && accountData.RedBucks < pet.Price)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    ...
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            if (!pet.isDonate)
                            {
                                GameLog.Money($"player({characterData.UUID})", "buyPet", pet.Price,
                                    $"buyPet({pet.Ped.ToString()})");
                                Wallet.Change(player, -pet.Price);
                            }
                            else
                            {
                                UpdateData.RedBucks(player, -pet.Price,
                                    LangFunc.GetText(LangType.Ru, DataName.BoughtPet));
                            }
                        ...
                        {
                            Log.Write($"Create NAPI.Trigger.SetTask Exception: {e}");
                        }
                    });
                }
                catch (Exception e)
                {
                    Log.Write($"Create Trigger.SetTask Exception: {e}");
                }
            });
        }
        ...
        public static void RespawnPet(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.PetIsOk), 5000);
                return;
            }
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouHaveNoPet), 5000);
                        return;
                    }
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Ems,
                            LangFunc.GetText(LangType.Ru, DataName.YouPetIsOk, price), DateTime.Now);
                ...
                {
                    Log.Write($"RespawnPet Trigger.SetTask Exception: {e}");
                }
            });
        }
        ...
        public static void AddBall(ExtPlayer player)
        {
            ...
            if (Chars.Repository.isFreeSlots(player, ItemId.Ball, 1, false, "fastSlots") != 0)
            {
                Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Ball);
                return;
            }

            Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "fastSlots", ItemId.Ball);
        }
        ...
        public static void OnSniff(ExtPlayer player, ExtPlayer target)
        {
            ...
            var locationName = $"char_{targetСharacterData.UUID}";
            ...
            if (Chars.Repository.ItemsData.ContainsKey(locationName))
                ...
                foreach (var itemData in Chars.Repository.ItemsData[locationName][Location])
                    if (Location == "inventory" || Location == "fastSlots")
                    {
                        ...
                        if (Police.IllegalsItems.ContainsKey(item.ItemId))
                        {
        ...
        public static void OnUpdateDim(ExtPlayer player, uint dimension)
        {
            ...
            {
                Log.Write($"OnUpdateDim Exception: {e}");
            }
        }
        ...
        public static void OpenPetMenu(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.Name),
                    LangFunc.GetText(LangType.Ru, DataName.InputPetName), 36, "make_ped_name");
            }
            catch (Exception e)
            {
                Log.Write($"OpenPetMenu Exception: {e}");
            }
        }
        ...
        public static void UpdateName(ExtPlayer player, ExtPed ped, string name)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.PetNameChanged, name), 5000);
            }
            catch (Exception e)
            {
                Log.Write($"UpdateName Exception: {e}");
            }
        }
        ...
        public static void Sell(ExtPlayer player)
        {
            try
            {
                ...
                if (pet.isDonate)
                    Trigger.ClientEvent(player, "openDialog", "sell_pet",
                        LangFunc.GetText(LangType.Ru, DataName.PetSellRb, Wallet.Format(pet.Price / 2)));
                else
                    Trigger.ClientEvent(player, "openDialog", "sell_pet",
                        LangFunc.GetText(LangType.Ru, DataName.PetSellMoney, Wallet.Format(pet.Price / 2)));
            }
            catch (Exception e)
            {
                Log.Write($"callback_cars Exception: {e}");
            }
        }
        ...
        public static void ConfirmSell(ExtPlayer player, ExtPed ped)
        {
            try
            {
                ...
                if (!pet.isDonate)
                {
                    GameLog.Money($"player({characterData.UUID})", "sellPet", Convert.ToInt32(pet.Price / 2),
                        $"sellPet({pet.Ped.ToString()})");
                    Wallet.Change(player, Convert.ToInt32(pet.Price / 2));
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PetGivenRb, Convert.ToInt32(pet.Price / 2)), 5000);
                }
                else
                {
                    UpdateData.RedBucks(player, Convert.ToInt32(pet.Price / 2),
                        LangFunc.GetText(LangType.Ru, DataName.PetSell));
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PetGivenMoney, Convert.ToInt32(pet.Price / 2)), 5000);
                }
                ...
                Trigger.SetTask(async () =>
                {
                    ...
                    {
                        Log.Write($"ConfirmSell Task Exception: {e}");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"ConfirmSell Exception: {e}");
            }
        }
        ...
        public static void SetEat(ExtPlayer player)
        {
            try
            {
                ...
                if (ped.Position.DistanceTo(player.Position) > 3f)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PetTooFar), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PetNoHungry), 5000);
                    return;
                }
                ...
                if (ItemStruct == null || ItemStruct.Item == null || ItemStruct.Index == -1 ||
                    !PetFood.ContainsKey(ItemStruct.Item.ItemId))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveNoFood), 5000);
                    return;
                }
                ...
                if (ItemStruct.Item.ItemId == ItemId.Burger)
                    Attachments.AddAttachment(player, Attachments.AttachmentsName.Burger);
                else if (ItemStruct.Item.ItemId == ItemId.HotDog)
                    Attachments.AddAttachment(player, Attachments.AttachmentsName.HotDog);
                else if (ItemStruct.Item.ItemId == ItemId.Pizza)
                    Attachments.AddAttachment(player, Attachments.AttachmentsName.Pizza);
                else if (ItemStruct.Item.ItemId == ItemId.Sandwich)
                    Attachments.AddAttachment(player, Attachments.AttachmentsName.Sandwich);
                else if (ItemStruct.Item.ItemId == ItemId.Crisps)
                ...
                Timers.StartOnce(5000, () =>
                {
                    try
                    {
                        ...
                        Commands.RPChat("sme", player,
                            "покормил" + (characterData.Gender ? "" : "а") +
                            LangFunc.GetText(LangType.Ru, DataName.Pets));
                    }
                    catch (Exception e)
                    {
