...
namespace NeptuneEvo.Inventory.Drop
{
    public class Repository
    {
        ...
        private static readonly ConcurrentDictionary<Object, DropData> DropsData =
            new ConcurrentDictionary<Object, DropData>();
        ...
        private static void DropControl()
        {
            ...
            foreach (var obj in myObjects) DeleteObject(obj);
        }

        public static void PutToObject(ExtPlayer player, InventoryItemData item, float posX, float posY, float posZ,
            float rotX, float rotY, float rotZ, bool isMy = false)
        {
            try
            {
                ...
                var obj = NAPI.Object.CreateObject(itemInfo.Model, new Vector3(posX, posY, posZ),
                    new Vector3(rotX, rotY, rotZ), 255, dim);
            ...
            {
                Log.Write($"PutToMyObject Exception: {e}");
            }
        }
        ...
        private static void DeleteObject(Object obj)
        {
            try
            {
                ...
                if (dropData.Item != null)
                {
                    ...
                    Chars.Repository.UpdateSqlItemData(locationName, location, -1,
                        new InventoryItemData(dropData.Item.SqlId));
                }
        ...
        public static void ItemRaise(ExtPlayer player, Object obj)
        {
            try
            {
                if (obj == null || !DropsData.ContainsKey(obj))
                ...
                if (characterData.AdminLVL >= 1 && characterData.AdminLVL <= 4)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AdminCantPickupItems), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantPickupInCar), 3000);
                    return;
                }
                ...
                if (dropItem.ItemId == ItemId.BagWithDrill || dropItem.ItemId == ItemId.BagWithMoney)
                {
                    ...
                    {
                        var freeSlotId =
                            Chars.Repository.AddItem(player, $"char_{characterData.UUID}", "inventory", item);
                        if (freeSlotId == -1) return;
                    }

                    if (dropItem.SqlId == 0)
                        Chars.Repository.AddSqlItem(player, $"char_{characterData.UUID}", "accessories",
                            dropItem.ItemId, 8, count == 0 ? dropItem.Count : count, dropItem.Data);
                    else Chars.Repository.SetItemData(player, "accessories", 8, dropItem, true);
                }
                else if (dropItem.SqlId != 0 && count == 0)
                {
                    Chars.Repository.AddItem(player, $"char_{characterData.UUID}", "inventory", dropItem);
                }
                else
                {
                    Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", dropItem.ItemId,
                        count == 0 ? dropItem.Count : count, dropItem.Data);
                }
                ...
                else
                {
                    Commands.RPChat("sme", player,
                        "поднял" + (characterData.Gender ? "" : "а") + $" часть {itemsInfo.Name}");
                ...
                Timers.StartOnce(1700, () =>
                {
                    ...
                    {
                        Log.Write("objectSelected11 ERROR");
                    }
                });
            }
            catch (Exception e)
            {
                Log.Write($"ItemRaise Exception: {e}");
            }
        }
        ...
        public static void OnHookah(ExtPlayer player, Object obj)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.HookahCantUse), 3000);
                    DeleteObject(obj);
                    return;
                }
                ...
                Chars.Repository.UseSmoke(player, ItemId.Hookah);
            }
            catch (Exception e)
            {
                Log.Write($"callback_HookahManageMenu Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                foreach (var obj in myObjects)
                    if (DropsData.ContainsKey(obj))
                    {
                        ...
                        if (dropItem != null && dropItem.ItemId != ItemId.Debug)
                        {
                            var freeSlotId = Chars.Repository.AddItem(player, $"char_{characterData.UUID}", "inventory",
                                dropItem);
                            if (freeSlotId != -1) dropData.Item = null;
                        }
