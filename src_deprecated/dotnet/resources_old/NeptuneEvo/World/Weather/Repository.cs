...
namespace NeptuneEvo.World.Weather
{
    ...
    {
        private static readonly IReadOnlyDictionary<int, List<int>> WeatherList = new Dictionary<int, List<int>>
        {
        ...
        private static readonly IReadOnlyDictionary<int, List<int>> WeatherTemp = new Dictionary<int, List<int>>
        {
        ...
        private static readonly int MaxRandom = 8;
        private static readonly int MaxTimeSeconds = 1800; // 30 мин 
        public static int NextTime;

        private static readonly IReadOnlyDictionary<int, int> WeatherRandomPercent = new Dictionary<int, int>
        {
            { 0, 1500 },
            { 1, 1500 },
            { 2, 1500 },
        ...
        private static void UpdateNextTime()
        {
            ...
            NextTime = (int)dateTime.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
        }
        ...
        public static void Set()
        {
            ...
            {
                if (!new[] { 10, 11, 12, 13 }.Contains(WeatherRandom[1].WeatherId) &&
                    new[] { 10, 11, 12, 13 }.Contains(weatherData.WeatherId))
                ...
                Trigger.ClientEventForAll("SetWeather", WeatherRandom[0].WeatherId, WeatherRandom[1].WeatherId,
                    NextTime);
        ...
        public static bool Update(int weatherId)
        {
            ...
            if (weatherId == -1 && CustomWeather != -1)
            {
                if (new[] { 10, 11, 12, 13 }.Contains(CustomWeather) &&
                    !new[] { 10, 11, 12, 13 }.Contains(weatherData.WeatherId))
                    NAPI.World.SetWeather(GTANetworkAPI.Weather.CLEAR);
            ...
            if (new[] { 10, 11, 12, 13 }.Contains(CustomWeather) && !new[] { 10, 11, 12, 13 }.Contains(weatherId))
                NAPI.World.SetWeather(GTANetworkAPI.Weather.XMAS);

            if (new[] { 10, 11, 12, 13 }.Contains(weatherId) &&
                !new[] { 10, 11, 12, 13 }.Contains(weatherData.WeatherId))
                NAPI.World.SetWeather(GTANetworkAPI.Weather.XMAS);
