    ...
    public class Repository
    {
        public static string Enter(ExtPlayer player, string text)
        {
            ...
            if (!int.TryParse(text, out uuid))
            {
                ...
                if (Main.RefCodes.ContainsKey(text))
                {
                    if (accountData.RefferalId == 0)
                    {
                        uuid = Main.RefCodes[text];
                        if (accountData.Chars[0] == uuid || accountData.Chars[1] == uuid ||
                            accountData.Chars[2] == uuid)
            ...
            if (accountData.RefferalId == 0)
            {
                if (!Main.UUIDs.Contains(uuid)) return LangFunc.GetText(LangType.Ru, DataName.NoCodeThisServ);
