...
namespace NeptuneEvo.Players.Phone.News
{
    ...
    {
        private static readonly int PriceSymbol = 3;

        private static readonly List<List<object>> NewsList = new List<List<object>>();
        ...
        public static void Add(ExtPlayer player, string text, string link, sbyte type, bool isPremium)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.CantGoAdNow), 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.AdAlreadyQueue), 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoSimcard), 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.AdTooShort), 3000);
                return;
            }
            ...
            Trigger.SetTask(async () => { await LsNewsSystem.AddAdvert(player, text, link, type, isPremium, price); });
        }
