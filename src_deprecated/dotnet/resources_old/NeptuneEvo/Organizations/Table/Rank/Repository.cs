    ...
    public class Repository
    {
        ...
        public static void UpdateRanksId(ExtPlayer player, string json)
        {
            try
            {
                ...
                foreach (var rank in ranks) organizationData.Ranks[rank.Key] = rank.Value;
                ...
                foreach (var foreachMemberOrganizationData in Manager.AllMembers[orgId].ToList())
                    if (updateRankId.ContainsKey(foreachMemberOrganizationData.Rank))
                        Organizations.Player.Repository.SetRank(orgId, foreachMemberOrganizationData.UUID,
                            updateRankId[foreachMemberOrganizationData.Rank]);
        ...
        public static void CreateRank(ExtPlayer player, string name, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.MaxRanksAmount), 4500);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.ExistNameRank), 4500);
                    return;
                }

                var index = -1;
                var maxIndex = 0;
                for (var i = 1; i <= NeptuneEvo.Table.Repository.MaxRankCount; i++)
                {
        ...
        public static void RemoveRank(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (!VehicleData.LocalData.Repository.IsVehicleToNumber(VehicleAccess.Organization,
                            vehicleData.Key)) continue;
                ...
                foreach (var vehicleNumber in vehiclesNumber)
                {
                    ...
                    var vehicle =
                        VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Organization, vehicleNumber);
                    var vehicleLocalData = vehicle.GetVehicleLocalData();
                    if (vehicleLocalData == null) continue;

                    if (vehicleLocalData.Access == VehicleAccess.Organization &&
                        vehicleLocalData.Fraction == organizationData.Id && vehicle.NumberPlate == vehicleNumber)
        ...
        public static void UpdateRankScore(ExtPlayer player, int id, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        $"Минимальное значение: {NeptuneEvo.Table.Repository.MinScore}, а максимальное: {NeptuneEvo.Table.Repository.MaxScore}",
                        5500);
                    return;
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    var targetMemberOrganizationData =
                        Manager.GetOrganizationMemberData(uuid, memberOrganizationData.Id);
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCantUvolit), 3000);
                        return;
                    }

                    Organizations.Player.Repository.RemoveOrganizationMemberData(orgId, uuid);

                    Logs.Repository.AddLogs(player, OrganizationLogsType.UnInvite,
                        $"Выгнал {targetMemberOrganizationData.Name} ({targetMemberOrganizationData.UUID})");
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.TiUvolil, targetMemberOrganizationData.Name), 3000);
                }
            ...
            try
            {
                ...
                foreach (var foreachMemberOrganizationData in Manager.AllMembers[orgId].ToList())
                    if (foreachMemberOrganizationData.UUID == organizationData.OwnerUUID)
                        ...
                        Organizations.Player.Repository.SetRank(orgId, foreachMemberOrganizationData.UUID, 0, false);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.SucResetDefaultRanks), 3000);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы сбросили настройки рангов",
                    5000);
                var ranksName = NeptuneEvo.Table.Repository.GetRanksData(organizationData.Ranks);
                Trigger.ClientEvent(player, "client.org.main.setRanks", JsonConvert.SerializeObject(ranksName));
            }
        ...
        public static void SetLeader(ExtPlayer player, int uuid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У Вас не хватает RedBucks для смены лидера.", 3000);
                    return;
                }

                UpdateData.RedBucks(player, -Main.PricesSettings.UpdateOrganizationLeader, "Смена лидера");
                GameLog.Money($"player({player.GetUUID()})", "server", Main.PricesSettings.UpdateOrganizationLeader,
                    $"NewLeader({organizationData.Id})");
                ...
                Organizations.Player.Repository.SetRank(organizationData.Id, targetMemberOrganizationData.UUID,
                    leaderRank);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    $"Вы передали лидерство организации игроку {targetMemberOrganizationData.Name}", 5000);
