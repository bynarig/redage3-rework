...
namespace NeptuneEvo.Organizations.FamilyZones
{
    public class Repository
    {
        ...
        public static Dictionary<byte, FamilyZoneData> FamilyZones = new Dictionary<byte, FamilyZoneData>
        {
            ...
            {
                43,
                new FamilyZoneData(43, "Нефтедобывающее предприятие 1", 0,
                    new Vector3(1229.2949, -2427.5532, 44.548492))
            },
            {
                44,
                new FamilyZoneData(44, "Нефтедобывающее предприятие 2", 0, new Vector3(1522.9775, -2187.4417, 77.42543))
            },
            {
                45,
                new FamilyZoneData(45, "Нефтедобывающее предприятие 3", 0, new Vector3(1551.4207, -1847.17, 92.53648))
            },
            {
                46,
                new FamilyZoneData(46, "Нефтедобывающее предприятие 4", 0,
                    new Vector3(1695.3457, -1667.2291, 112.47394))
            },
        ...
        private static readonly string[] TypeBattle =
        {
            "Больше убийств",
            "Больше очков",
        ...
        private static readonly string[] WeaponsCategory =
        {
            "Без ограничений",
            "Ближний бой",
            "Пистолеты",
        ...
        private static readonly byte MinMinutes = 15;
        private static readonly byte MaxProtection = 1;
        private static readonly byte MaxAttack = 1;
        ...
        public static void OnResource()
        {
            ...
            foreach (var familyZone in FamilyZones.Values.ToList())
                CustomColShape.CreateSphereColShape(familyZone.Position, Range, NAPI.GlobalDimension,
                    ColShapeEnums.FamilyZone, familyZone.Id);
        }

        [Interaction(ColShapeEnums.FamilyZone, true)]
            ...
            try
            {
                ...
                foreach (var familyZone in FamilyZones.Values.ToList())
                {
                    ...
                    else
                    {
                        ...
                        zone.Add(new[]
                            { organizationData.Color.Red, organizationData.Color.Green, organizationData.Color.Blue });
                    ...
                    if (war != null)
                    {
                        ...
                        if (attackingOrganizationData != null)
                            zone.Add(new[]
                            {
                                attackingOrganizationData.Color.Red, attackingOrganizationData.Color.Green,
                                attackingOrganizationData.Color.Blue
                            });
                        else
                            zone.Add(null);
                        ...
                        if (protectingOrganizationData != null)
                            zone.Add(new[]
                            {
                                protectingOrganizationData.Color.Red, protectingOrganizationData.Color.Green,
                                protectingOrganizationData.Color.Blue
                            });
                        else
                            zone.Add(null);
                    }
                ...
                foreach (var top in tops)
                {
                    ...
                    if (organizationData != null)
                        topNames.Add($"{i + 1}. {organizationData.Name} ({top.Count} зон)" +
                                     (top.Id == memberOrganizationData.Id ? " (вы)" : ""));
                    else
                        topNames.Add($"{i + 1}. -");
                    i++;
                }
                ...
                var wars = World.War.Repository.Wars.Values
                    ...
                    .Where(w => w.AttackingId == memberOrganizationData.Id ||
                                w.ProtectingId == memberOrganizationData.Id)
                    .ToList();
                ...
                Trigger.ClientEvent(player, "client.familyZones", JsonConvert.SerializeObject(zones),
                    JsonConvert.SerializeObject(topNames), JsonConvert.SerializeObject(warsJson));
            }
            catch (Exception e)
            {
        ...
        public static bool IsAttack(ExtPlayer player, DateTime time, int addDay, ushort attackingId,
            ushort protectingId)
        {
            try
            {
                ...
                var warsCount = World.War.Repository.Wars.Values
                    .Where(w => w.Type == WarType.OrgWarZone)
                    .Where(w => w.AttackingId == attackingId || w.ProtectingId == attackingId ||
                                w.AttackingId == protectingId || w.ProtectingId == protectingId)
                    .Count(w => World.War.Repository.GetTime(w.Time, time) <= MinMinutes);

                if (warsCount > 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "На данное время вы не можете забить стрелку.", 8000);
                    //Trigger.ClientEvent(player, "client.closeWar");
                    return false;
                }

                var sec = NeptuneEvo.Table.Tasks.Repository.GetTime(addDay);
                OrganizationData organizationDataAttack = null;
                if (attackingId > 0)
                {
                    organizationDataAttack = Manager.GetOrganizationData(attackingId);
                    if (organizationDataAttack == null)
                        return false;

                    if (organizationDataAttack.AttackingCount.ContainsKey(sec) &&
                        organizationDataAttack.AttackingCount[sec] >= MaxAttack)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                ...
                if (protectingId > 0)
                {
                    organizationDataProtecting = Manager.GetOrganizationData(protectingId);
                    if (organizationDataProtecting == null)
                        return false;

                    if (organizationDataProtecting.ProtectingCount.ContainsKey(sec) &&
                        organizationDataProtecting.ProtectingCount[sec] >= MaxProtection)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
        ...
        public static void Attack(ExtPlayer player, byte id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "На данную зону уже идет захват",
                        3000);
                    Trigger.ClientEvent(player, "client.closeWar");
                    return;
                }
                ...
                if (familyZone.OrganizationId == 0)
                {
                    if (war == null)
                    {
                        World.War.Repository.Open(player, id, familyZone.Id, familyZone.Name, familyZone.Position, 150f,
                            WarType.OrgWarZone, 0, (ushort)memberOrganizationData.Id);
                    }
                    else
                    {
                        if (war.ProtectingId == memberOrganizationData.Id) return;
                        var warData = sessionData.WarData;

                        warData.ObjectId = id;

                        var compositionName = war.Composition == -1 ? Composition[0] : Composition[war.Composition];
                        Trigger.ClientEvent(player, "openDialog", "FamilyZones",
                            $"Вы уверены, что хотите напасть на зону {familyZone.Name} в {war.Time.ToString("dd.MM HH:mm")}.<br/>Тип битвы: <b>{TypeBattle[(int)war.GripType]}</b><br/>Кол-во: <b>{compositionName}</b><br/>Оружие: <b>{WeaponsCategory[war.WeaponsCategory]}</b>");
                    }

                    return;
                }

                World.War.Repository.Open(player, id, familyZone.Id, familyZone.Name, familyZone.Position, Range,
                    WarType.OrgWarZone, (ushort)memberOrganizationData.Id, (ushort)familyZone.OrganizationId);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
        ...
        public static void ConfirmAttack(ExtPlayer player)
        {
            try
            {
                ...
                if (familyZone.OrganizationId == memberOrganizationData.Id) return;
