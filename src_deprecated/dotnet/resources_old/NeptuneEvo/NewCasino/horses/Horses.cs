...
namespace NeptuneEvo.NewCasino
{
    internal class Horses : Script
    {
        ...
        public static bool StateInWork;
        ...
        public static void GameInterval()
        {
            try
            {
                ...
                if (StateInWork) return; // Защита от того, что таймер пойдёт быстрее, чем отработает код
                switch (GameStatus)
                {
                    case horsesStatus.None:
                        ...
                        Trigger.ClientEventInRange(pos, radius, "client.horse.START_TIMER",
                            (int)horsesTimers.ChoiceRaters);
                    ...
                    case horsesStatus.ChoiceRaters:
                        ...
                        var horseList = new List<int> { 1, 2, 3, 4, 5, 6 };
                        ...
                        Trigger.ClientEventInRange(pos, radius, "client.horse.START_RACING", seed, curentRandomHours[0],
                            curentRandomHours[1], curentRandomHours[2], curentRandomHours[3], curentRandomHours[4],
                            curentRandomHours[5], 1);
                        curentScreen = 1;
                    ...
                    case horsesStatus.Racing:
                        ...
                        lock (PlayerData)
                        {
                            foreach (var foreachPlayer in PlayerData.Keys)
                            {
                                ...
                                {
                                    var payment = rData.Bet + Convert.ToInt32(rData.Bet * 2);
                                    Wallet.Change(foreachPlayer, payment);
                                    GameLog.CasinoHorsesLog(payment);
                                    GameLog.Money("system", $"player({foreachCharacterData.UUID})", payment,
                                        $"HorseWin({rData.Spot})");
                                    Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.YouWinHorse, Wallet.Format(payment),
                                            rData.Spot), 7000);
                                    bets.Add(payment);
                                }
                                else
                                {
                                    Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.Ru, DataName.YouLoseHorse, rData.Spot), 5000);
                                    bets.Add(-rData.Bet);
                                }

                                gametags.Add(LangFunc.GetText(LangType.Ru, DataName.Horse, foreachSessionData.Name,
                                    rData.Spot));
                            }
                        }

                        Trigger.ClientEventInRange(pos, radius, "client.horse.SHOW_RESULTS",
                            JsonConvert.SerializeObject(gametags), JsonConvert.SerializeObject(bets));
            ...
            {
                Log.Write($"GameInterval Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                else
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.HorseBetAlready, PlayerData[player].Spot), 6000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"sit_slot Exception: {e}");
            }
        }
        ...
        private static void tears_slot(ExtPlayer player)
        {
            ...
            {
                Log.Write($"tears_slot Exception: {e}");
            }
        }
        ...
        public static void CMD_SelectHorse(ExtPlayer player, int horse, int money)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantDoStavka), 3000);
                    return;
                }
                ...
                Trigger.ClientEventInRange(player.Position, 100f, "client.horse.SHOW_HORSE",
                    JsonConvert.SerializeObject(gametags), horse, JsonConvert.SerializeObject(bets));
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.BetOnHorse, horse), 3000);
                tears_slot(player);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_SelectHorse Exception: {e}");
            }
        }
        ...
        public static void Disconnect(ExtPlayer player, DisconnectionType type)
        {
            try
            {
                ...
                {
                    Wallet.Change(player, sData.Bet);
