...
namespace NeptuneEvo.Events.AirDrop
{
    public class Repository : Script
    {
        ...
        private static readonly Dictionary<string, TeamData> TeamsInfo = new Dictionary<string, TeamData>();
        private static List<AirDropData> AirDropsData = new List<AirDropData>();
        private static byte AirDropEventStatus;
        private static DateTime AirDropEndEventTime = DateTime.Now;

        private static string AirdropEndTimer;

        private static readonly List<Vector3> AirDropsCentralPosition = new List<Vector3>
        {
        ...
        private static readonly IReadOnlyDictionary<int, Vector3[]> AirDropsPosition = new Dictionary<int, Vector3[]>
        {
            {
                0,
                new[]
        ...
        private static readonly int minMinutes = 11;
        private static readonly int maxMinutes = 20;
        private static readonly int notificationMinutes = 10;

        [ServerEvent(Event.ResourceStart)]
        public void Event_ResourceStart()
        {
            PedSystem.Repository.CreateQuest("g_m_m_cartelguards_01", new Vector3(-480.33752, -400.36942, 34.5466),
                27.42f, title: "~y~NPC~w~ Juan de cartel\nПродавец AirDrop", colShapeEnums: ColShapeEnums.PedAirDrop);
        }
                ...
                if (hour == 8 || hour == 15 || hour == 21 || needStart)
                {
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        Trigger.SendChatMessage(foreachPlayer,
                            "~o~" + LangFunc.GetText(LangType.Ru, DataName.AirDropZakazan));
                        EventSys.SendCoolMsg(foreachPlayer, "AirDrop", "Скоро начнётся!",
                            LangFunc.GetText(LangType.Ru, DataName.AirDropZakazan), "", 10000);
                        ...
                        Trigger.ClientEvent(foreachPlayer, "phone.notify", (int)DefaultNumber.Informer,
                            $"Гринго!!! В течение 20 минут ожидается сброс ящиков, их будет: {dropCount}. Пиши /airdrop, если туго доходит.",
                            20);
                    }
        ...
        private static void CreateItems(string locationName, string location)
        {
            var rand = new Random();
            // 100%:
            Chars.Repository.AddNewItem(null, locationName, location, ItemId.Revolver, rand.Next(1, 3),
                WeaponRepository.GetAirdropSerial(), true, 100);
            Chars.Repository.AddNewItem(null, locationName, location, ItemId.AssaultRifleMk2, rand.Next(1, 3),
                WeaponRepository.GetAirdropSerial(), true, 100);
            Chars.Repository.AddNewItem(null, locationName, location, ItemId.BodyArmor, rand.Next(20, 40), "100", true,
                100);

            for (var item = 0; item < rand.Next(5, 10); item++)
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.Drugs, 50);

            // Рандом:
            var item_chance = rand.Next(1, 101);
            ...
            if (item_chance >= 80)
            {
                for (var item = 0; item < rand.Next(5, 10); item++)
                {
                    ...
                    Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, 1,
                        $"{maskRand.Variation}_{maskRand.Colors[0]}_True");
                }
            }
            else if (item_chance >= 70)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.APPistol, rand.Next(5, 10),
                    WeaponRepository.GetAirdropSerial(), true, 100);
            }
            else if (item_chance >= 60)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.SweeperShotgun, rand.Next(1, 3),
                    WeaponRepository.GetAirdropSerial(), true, 100);
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.CarbineRifleMk2, rand.Next(1, 3),
                    WeaponRepository.GetAirdropSerial(), true, 100);
            }
            else if (item_chance >= 50)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.Gusenberg, rand.Next(1, 3),
                    WeaponRepository.GetAirdropSerial(), true, 100);
            }
            else if (item_chance >= 40)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.SniperRifle, rand.Next(1, 5),
                    WeaponRepository.GetAirdropSerial(), true, 100);
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.AssaultShotgun, rand.Next(2, 5),
                    WeaponRepository.GetAirdropSerial(), true, 100);
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.TacticalRifle, rand.Next(2, 4),
                    WeaponRepository.GetAirdropSerial(), true, 100);
            }
            else if (item_chance >= 18)
            {
                if (item_chance >= 30)
                {
                    Chars.Repository.AddNewItem(null, locationName, location, ItemId.MarksmanRifle, rand.Next(1, 3),
                        WeaponRepository.GetAirdropSerial(), true, 100);
                    Chars.Repository.AddNewItem(null, locationName, location, ItemId.Wrench, rand.Next(1, 3),
                        WeaponRepository.GetAirdropSerial(), true, 100);
                }
            ...
            else if (item_chance >= 15)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.CarCoupon, 1, "Phoenix");
            }
            else if (item_chance >= 3)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.CombatMG, rand.Next(1, 3),
                    WeaponRepository.GetAirdropSerial(), true, 100);

                Chars.Repository.AddNewItem(null, locationName, location, ItemId.HeavyShotgun, rand.Next(2, 4),
                    WeaponRepository.GetAirdropSerial(), true, 100);
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.HeavySniper, rand.Next(1, 3),
                    WeaponRepository.GetAirdropSerial(), true, 100);

                if (item_chance >= 7)
                    Chars.Repository.AddNewItem(null, locationName, location, ItemId.MarksmanPistol, rand.Next(1, 2),
                        WeaponRepository.GetAirdropSerial(), true, 100);

                if (item_chance >= 8 && item_chance < 10)
                    Chars.Repository.AddNewItem(null, locationName, location, ItemId.CarCoupon, 1, "Neon");
                else if (item_chance >= 6 && item_chance < 8)
                    Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, rand.Next(1, 2),
                        "197_8_True");
            }
            else if (item_chance == 2)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.CarCoupon, 1, "Rapidgt3");
            }
            else if (item_chance == 1)
            {
                Chars.Repository.AddNewItem(null, locationName, location, ItemId.CarCoupon, 1, "Ferrari488");
            }
        }
        ...
        public static void StopAirDropEvent()
        {
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    try
                    {
                        ...
                        Trigger.ClientEvent(foreachPlayer, "phone.notify", (int)DefaultNumber.Informer,
                            "Хола! По моим каналам прошла инфа, что AirDrop закончился. Надеюсь, у тебя всё чики-пуки, до новых встреч!",
                            15);
                    ...
                    {
                        Log.Write($"StopAirDropEvent Players-foreach Exception: {e}");
                    }
            ...
            {
                Log.Write($"StopAirDropEvent Exception: {e}");
            }
        }
                ...
                foreach (var airDropData in AirDropsData)
                {
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        if (airDropData.Status == 1)
                            Trigger.ClientEvent(foreachPlayer, "client.airdropZone.create", airDropData.CentralPosition,
                                100, 1, notificationMinutes);
                        else
                            Trigger.ClientEvent(foreachPlayer, "client.fight.create", airDropData.DropId, 0,
                                airDropData.Position, 2);

                        Trigger.ClientEvent(foreachPlayer, "createWaypoint", airDropData.CentralPosition.X,
                            airDropData.CentralPosition.Y);
                    }
        ...
        public static bool IsPlayerToEvents(ExtPlayer player)
        {
            ...
            {
                Log.Write($"IsPlayerToEvents Exception: {e}");
                return false;
            }
        }
            ...
            try
            {
                ...
                if (state == 1)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirGo), 15000);
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        Trigger.ClientEvent(foreachPlayer, "airdrop.updateTeamsInfo",
                            JsonConvert.SerializeObject(TeamsInfo.OrderBy(u => u.Value.TeamFrags).Reverse()));
                    }
                }
                else if (state == 2)
                {
                    ...
                    if (AirDropEventStatus > 0)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.LeaveAirTer), 3000);
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"AirdropChangePlayerDimension Exception: {e}");
            }
        }
        ...
        public static void CheckAirdropLockStatus(ExtPlayer player, int ShapeId)
        {
                ...
                foreach (var airDropData in AirDropsData)
                    if (airDropData.DropId == ShapeId)
                    {
                        if (airDropData.AirdropLockHealth > 0)
                        {
                            ...
                            Trigger.ClientEvent(player, "client.updateAirdropHackStatus", true,
                                airDropData.AirdropLockHealth);
                        }
                        else
                        {
            ...
            {
                Log.Write($"CheckAirdropLockStatus Exception: {e}");
            }
        }
        ...
        public static void AirdropChangeLockStatus(ExtPlayer player, int ShapeId, int health)
        {
                ...
                foreach (var airDropData in AirDropsData)
                    if (airDropData.DropId == ShapeId)
                    {
                        ...
                        if (airDropData.AirdropLockHealth <= 0)
                        {
                            ...
                            if (count > 0)
                                Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory",
                                    ItemId.ArmyLockpick);
                        }
                    }
            }
            ...
            {
                Log.Write($"AirdropChangeLockStatus Exception: {e}");
            }
        }
                    ...
                    if (airDropData.DropId == ShapeId)
                    {
                        if (airDropData.AirdropLockHealth > 0)
                        {
                            if (airDropData.AirdropHackPlayerInfo != null)
                            {
                                ...
                                else if (airDropData.Position.DistanceTo(airDropData.AirdropHackPlayerInfo.Position) >
                                         3)
                                {
                                    airDropData.AirdropHackPlayerInfo = null;
                                }
        ...
        public static void StartedHack(ExtPlayer player, int ShapeId)
        {
            ...
            {
                Log.Write($"StartedHack Exception: {e}");
            }
        }
        ...
        public static void Open(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    var random_dialog = new Random().Next(0, 2);
        ...
        public static void BuyInfo(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirdropBuyed), 3000);
                    return;
                }

                if (AirDropEventStatus == 2)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirdropAlready), 3000);
                    return;
                }

                if (UpdateData.CanIChange(player, Main.PricesSettings.AirdropInfoPrice, true) != 255) return;

                if ((DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23) ||
                    (DateTime.Now.Hour == 0 && DateTime.Now.Minute <= 10))
                    Trigger.ClientEvent(player, "openDialog", "PAY_AIRDROP_INFO",
                        LangFunc.GetText(LangType.Ru, DataName.AidropOffer, Main.PricesSettings.AirdropInfoPrice));
                else
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AidropTime), 3000);
            }
            catch (Exception e)
            {
        ...
        public static void Perform(ExtPlayer player)
        {
            ...
            {
                if (!player.IsCharacterData()) return;
                if (!FunctionsAccess.IsWorking("PedAirDrop_Perform"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.FunctionOffByAdmins), 3000);
                    return;
                ...
                if (UpdateData.CanIChange(player, Main.PricesSettings.AirdropOrderPrice, true) != 255) return;
                if (AirDropEventStatus > 0)
                {
                    var nextAirDropTime = "Следующий AirDrop можно будет заказать в " +
                                          (AirDropEndEventTime.Hour < 10
                                              ? $"0{AirDropEndEventTime.Hour}"
                                              : $"{AirDropEndEventTime.Hour}") + ":" + (AirDropEndEventTime.Minute < 10
                                              ? $"0{AirDropEndEventTime.Minute}"
                                              : $"{AirDropEndEventTime.Minute}");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirdropWaiting, nextAirDropTime), 3000);
                    return;
                }

                if (DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23)
                    Trigger.ClientEvent(player, "openDialog", "PAY_AIRDROP_ORDER",
                        LangFunc.GetText(LangType.Ru, DataName.AirdropBuyingg, Main.PricesSettings.AirdropOrderPrice));
                else
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AidropTime), 3000);
            }
            ...
            try
            {
                    ...
                    (DateTime.Now.Hour == 0 && DateTime.Now.Minute <= 10))
                {
                    ...
                    if (fracId > (int)Fractions.Models.Fractions.None)
                    {
                        ...
                        foreach (var airDropData in AirDropsData)
                        {
                            if ((airDropData.Status == 0 || airDropData.Status == 1) &&
                                DateTime.Now.AddMinutes(10) > airDropData.DateTime)
                                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                                {
                                    ...
                                    {
                                        Trigger.ClientEvent(foreachPlayer, "client.airdropZone.create",
                                            airDropData.CentralPosition, 100, 1, notificationMinutes);
                                        Trigger.ClientEvent(foreachPlayer, "createWaypoint",
                                            airDropData.CentralPosition.X, airDropData.CentralPosition.Y);
                                    }
                                }
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.AirdropSub), 3000);
                            return;
                        }
                    }

                    Wallet.Change(player, -Main.PricesSettings.AirdropInfoPrice);
                    GameLog.Money($"player({characterData.UUID})", "server", Main.PricesSettings.AirdropInfoPrice,
                        "buyAirDropInfo");

                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirdropSubbed), 3000);

                    if (AirDropEventStatus == 1)
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.AidropFractions), 10000);
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
        ...
        public static void PayAirdropOrder(ExtPlayer player)
        {
            try
            {
                ...
                if (DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23)
                {
                    ...
                    GameLog.Money($"player({characterData.UUID})", "server", Main.PricesSettings.AirdropOrderPrice,
                        "buyAirDropOrder");

                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirdropZakazan), 3000);
        ...
        public static void AirDropDeathHandler(ExtPlayer player, ExtPlayer entityKiller)
        {
            try
            {
                if (!IsPlayerToEvents(player) || !IsPlayerToEvents(entityKiller) || player.Dimension != 100 ||
                    entityKiller.Dimension != 100) return;
                ...
                if (memberOrganizationData != null)
                {
                    ...
                    if (killerMemberOrganizationData != null &&
                        memberOrganizationData.Id == killerMemberOrganizationData.Id)
                        return;
                }
            ...
            {
                Log.Write($"AirDropDeathHandler Exception: {e}");
            }
        }
        ...
        public static void CMD_airlist(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AirNeZapushen), 3000);
                    return;
                }
            ...
            {
                Log.Write($"CMD_airlist Exception: {e}");
            }
        }
        ...
        public static void CMD_starttestairdrop(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.Adminstopaird), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_starttestairdrop Exception: {e}");
            }
        }
        ...
        public static void CMD_stopairdrop(ExtPlayer player)
        {
            try
            {
                ...
                StopAirDropEvent();
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.Adminstopped), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_stopairdrop Exception: {e}");
            }
        }
        ...
        public static void CMD_airdrop(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoUchastnik), 3000);
                    return;
                }

                var airDropsTimeInfo = new List<DateTime>();
                ...
                if (airDropsTimeInfo.Count > 0)
                {
                    ...
                    for (var i = 0; i < airDropsTimeInfo.Count; i++)
                        Trigger.SendChatMessage(player,
                            $"№{i + 1} | " +
                            (airDropsTimeInfo[i].Hour < 10
                                ? $"0{airDropsTimeInfo[i].Hour}"
                                : $"{airDropsTimeInfo[i].Hour}") + ":" + (airDropsTimeInfo[i].Minute < 10
                                ? $"0{airDropsTimeInfo[i].Minute}"
                                : $"{airDropsTimeInfo[i].Minute}"));
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoInfo), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"CMD_airdrop Exception: {e}");
            }
        }
    }
}