...
{
    internal class Drone : Script
    {
        private static readonly nLog Log = new nLog("VehicleModel.Drone");

        private static readonly Dictionary<int, (string, int)> FractionDroneData = new Dictionary<int, (string, int)>
        {
        ...
        public void CreateDrone(ExtPlayer player)
        {
            ...
            var vehicle = VehicleStreaming.CreateVehicle((uint)VehicleHash.Rcbandito, player.Position,
                player.Rotation.Z, 0, 0, acc: VehicleAccess.Admin, numberPlate: number, by: player.Name, petrol: 9999,
                engine: true);
            vehicle.SetSharedData("attachmentsData",
                JsonConvert.SerializeObject(new List<uint> { NAPI.Util.GetHashKey("spec1") }));
            ...
            try
            {
                ...
                if (Configs.FractionDrones[fracId].Count >= FractionDroneData[fracId].Item2)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.Only2DronesAvaible, FractionDroneData[fracId].Item2),
                        5000);
                    return;
                }
                ...
                if (vehicleLocalData.Access == VehicleAccess.Fraction && vehicleLocalData.Fraction == fracId)
                {
                    ...
                    var drone = VehicleStreaming.CreateVehicle((uint)VehicleHash.Rcbandito,
                        vehicle.Position + new Vector3(2f, 2f, 0), player.Rotation.Z, 0, 0, number,
                        acc: VehicleAccess.Fraction, fr: fracId, petrol: 9999, minrank: 0);
                    drone.SetSharedData("attachmentsData",
                        JsonConvert.SerializeObject(new List<uint>
                            { NAPI.Util.GetHashKey(FractionDroneData[fracId].Item1) }));
                    ...
                    Trigger.ClientEvent(player, "client.dron.init", vehicle.Position.X, vehicle.Position.Y,
                        vehicle.Position.Z);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DroneControls), 10000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"CreateDroneFraction Exception: {e}");
            }
        }
        ...
        public void Event_OnPlayerExitVehicle(ExtPlayer player, ExtVehicle vehicleDrone)
        {
            try
            {
                ...
                if (vehicleLocalData.Access == VehicleAccess.Fraction &&
                    ...
                    Configs.FractionDrones[vehicleLocalData.Fraction].ContainsKey(vehicleDrone))
                {
                    var number = Configs.FractionDrones[vehicleLocalData.Fraction][vehicleDrone];

                    VehicleStreaming.DeleteVehicle(vehicleDrone);
                    var characterData = player.GetCharacterData();
                    if (characterData == null) return;
                    var vehicle = Repository.GetVehicleToNumber(vehicleLocalData.Access, number);
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        if (emptySlots.Contains(foreachPlayer.VehicleSeat))
                            emptySlots.Remove(foreachPlayer.VehicleSeat);
                    }

                    characterData.ExteriorPos = new Vector3();
                    if (emptySlots.Count == 0)
                    {
                        player.Position = new Vector3(vehicle.Position.X + 3f, vehicle.Position.Y + 3f,
                            vehicle.Position.Z);
                        return;
                    }
                ...
                else if (vehicleLocalData.Access == VehicleAccess.Admin &&
                         vehicleDrone.Model == NAPI.Util.GetHashKey("rcbandito") &&
                         vehicleDrone.HasSharedData("isDrone"))
                {
                    VehicleStreaming.DeleteVehicle(vehicleDrone);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DroneDeleted), 3000);
                }
            }
            catch (Exception e)
            {
