...
namespace NeptuneEvo.Table.Tasks.Player
{
    ...
    {
        public static void InitTableFraction(this ExtPlayer player, string fractionMyTasksData, bool isMyUpdate = false,
            bool isUpdate = false)
        {
            try
            {
                ...
                if (!Tasks.Repository.FractionTaskIds.ContainsKey(fracId))
                    return;

                var ids = Tasks.Repository.FractionTaskIds[fracId].ToList();

                memberFractionData.TasksData =
                    Tasks.Repository.GetData(memberFractionData.TasksData, fractionData.TasksData, isUpdate);
                player.FractionTasksData = Tasks.Repository.GetData(fractionMyTasksData, ids, isMyUpdate);
            }
            catch (Exception e)
            {
        ...
        public static void AddTableScore(this ExtPlayer player, TableTaskId tableTaskId)
        {
            try
            {
                ...
                if (!Tasks.Repository.TasksList.ContainsKey(tableTaskId))
                    return;

                var task = Tasks.Repository.TasksList[tableTaskId];
                ...
                if (player.OrganizationTasksData != null)
                {
                    ...
                    if (tableTaskData != null && !tableTaskData.Success)
                    {
                        ...
                        if (tableTaskData.Count >= task.MaxCount)
                        {
                            ...
                            foreach (var award in task.Awards) Tasks.Repository.GiveBonus(player, award);
                        }
                    }
                }
                //
                    ...
                    if (tableTaskData != null && !tableTaskData.Success)
                    {
                        ...
                        if (tableTaskData.Count >= task.MaxCount)
                        {
                            ...
                            if (fractionTasksData != null && !fractionTasksData.Success)
                            {
                                ...
                                if (fractionTasksData.Count >= task.MaxCount * Tasks.Repository.MaxCountPlayerSuccess)
