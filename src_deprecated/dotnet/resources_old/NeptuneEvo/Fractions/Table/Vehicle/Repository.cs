    ...
    public class Repository
    {
        public static void GetVehicles(ExtPlayer player)
        {
            ...
            foreach (var v in fractionData.Vehicles)
            {
                ...
                if (fractionData.Id == (int)Models.Fractions.MERRYWEATHER &&
                    (model.Equals("SUBMERSIBLE") || model.Equals("THRUSTER"))) continue;
                var vehicleData =
            ...
            Trigger.ClientEvent(player, "client.frac.main.vehicles", isVehicleUpdateRank,
                JsonConvert.SerializeObject(vehiclesList));
        }
        ...
        public static void Evacuation(ExtPlayer player, string number)
        {
            try
            {
                ...
                if (vehicleLocalData.Fraction == memberFractionData.Id &&
                    memberFractionData.Rank >= vehicleLocalData.MinRank)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CarCantOtognat), 3000);
                        return;
                    }
        ...
        public static void DefaultRanks(ExtPlayer player)
        {
            try
            {
                ...
                {
                    if (!VehicleData.LocalData.Repository.IsVehicleToNumber(VehicleAccess.Fraction, vehicleNumber))
                        continue;
                ...
                foreach (var vehicleNumber in vehiclesNumber)
                {
                    ...
                    var vehicle =
                        VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Fraction, vehicleNumber);
