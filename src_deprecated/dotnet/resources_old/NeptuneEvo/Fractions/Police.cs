...
namespace NeptuneEvo.Fractions
{
    internal class Police : Script
    {
        ...
        private static int PedSecretaryId;
        ...
        private static readonly Vector3 ArrestPosition = new Vector3(481.40088, -1010.83453, 26.273132);
        ...
        private static readonly Vector3 CloakroomSpecialPosition = new Vector3(463.5043, -999.32587, 30.68949);
        ...
        private static readonly Vector3 StockPosition = new Vector3(486.9647, -995.9626, 30.68962);

        private static readonly Vector3
            VehicleBoostPosition = new Vector3(1408.2571, 183.5331, -49.280865); //463.32733, -1019.0836, 28.10166

        private static readonly Vector3 VehicleRepairPosition = new Vector3(447.07394, -1013.2826, 28.541023);
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.Marker.CreateMarker(30, CloakroomPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));

                CustomColShape.CreateCylinderColShape(CloakroomSpecialPosition, 1, 2, 0, ColShapeEnums.FractionPolic,
                    3);
                NAPI.Marker.CreateMarker(30, CloakroomSpecialPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));
                ...
                NAPI.Marker.CreateMarker(20, StockPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));

                CustomColShape.CreateCylinderColShape(VehicleBoostPosition, 3, 5, 1, ColShapeEnums.FractionPolic, 6);
                NAPI.Marker.CreateMarker(44, VehicleBoostPosition, new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220), dimension: 1);


                CustomColShape.CreateCylinderColShape(VehicleRepairPosition, 3, 5, 0, ColShapeEnums.FractionPolic,
                    9); // repair
                NAPI.Marker.CreateMarker(1,
                    new Vector3(VehicleRepairPosition.X, VehicleRepairPosition.Y, VehicleRepairPosition.Z - 1.75),
                    new Vector3(), new Vector3(), 3f, new Color(255, 255, 255, 220));
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Ремонт наземного транспорта"),
                    VehicleRepairPosition, 5F, 0.3F, 0, new Color(255, 255, 255));

                Ped ped = Repository.CreateQuest("s_m_y_cop_01", new Vector3(442.6866f, -981.88605f, 30.689486f),
                    89.897484f, title: "~y~NPC~w~ Офицер Энтони\nДежурный", colShapeEnums: ColShapeEnums.FracPolic);
                PedSecretaryId = ped.Value;

                Repository.CreateQuest("s_m_m_fibsec_01", new Vector3(480.34195, -996.7015, 30.690102), 88.88336f,
                    title: "~y~NPC~w~ Агент Роберт\nСкладской", colShapeEnums: ColShapeEnums.FracPolic);
                Repository.CreateQuest("s_f_y_cop_01", new Vector3(442.5702, -978.62854, 30.689486), 100.0675f,
                    title: "~y~NPC~w~ Офицер Лиса\nВызвать сотрудника", colShapeEnums: ColShapeEnums.CallPoliceMember);
            }
            catch (Exception e)
            {
        ...
        public static void OpenCallPoliceMemberDialog(ExtPlayer player, int _)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "CallPoliceMemberDialog",
                    LangFunc.GetText(LangType.Ru, DataName.AreYouWantToCallGov));
            }
            catch (Exception e)
            {
                Log.Write($"OpenCallPoliceMemberDialog Exception: {e}");
            }
        }
        ...
        public static void Open(ExtPlayer player, int index)
        {
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoPolice), 3000);
                    return;
                }
        ...
        public static void Perform(ExtPlayer player)
        {
            ...
            var Bags = Chars.Repository.GetItemData(player, "accessories", 8);
            if (Bags.ItemId != ItemId.BagWithDrill && Bags.ItemId != ItemId.BagWithMoney)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoBagDrillMoney), 3000);
                return;
            }

            Chars.Repository.RemoveIndex(player, "accessories", 8);
            Wallet.Change(player, Main.PoliceAward);
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.Reward, Main.PoliceAward), 3000);
        }
        ...
        public void Event_OnPlayerExitVehicle(ExtPlayer player, Vehicle vehicle)
        {
            try
            {
                ...
                if (player.VehicleSeat != (int)VehicleSeat.Driver ||
                    player.VehicleSeat != (int)VehicleSeat.RightFront) return;
        ...
        public static string OnCallPolice(ExtPlayer player, string reason)
        {
            try
            {
                ...
                if (Manager.FractionMembersCount(new[]
                        { (int)Models.Fractions.POLICE, (int)Models.Fractions.FIB, (int)Models.Fractions.SHERIFF }) ==
                    0)
                ...
                var blip = (ExtBlip)NAPI.Blip.CreateBlip(0, player.Position, 1, 70,
                    $"Police Call from {player.Name.Replace('_', ' ')} [{player.Value}]", 0, 0, true, 0, 0);
                blip.Transparency = 0;
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        Log.Write($"OnCallPolice Task Exception: {e}");
                    }
                }, 60000);
                ...
                Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                    "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.PoliceCallFrom, player.Name, player.Value,
                        reason), true);
            ...
            {
                Log.Write($"OnCallPolice Exception: {e}");
            }
        ...
        public static void acceptCall(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (target != null && target.IsCharacterData() && PoliceCalls.ContainsKey(target))
                {
                    ...
                    if (Configs.IsFractionPolic(fracId))
                    {
                        Manager.sendFractionMessage(fracId,
                            LangFunc.GetText(LangType.Ru, DataName.CallAccept, player.Name.Replace("_", " "),
                                target.Value));
                        Manager.sendFractionMessage(fracId,
                            "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.CallAccept,
                                player.Name.Replace("_", " "), target.Value), true);
                ...
                else if (PoliceHouseCalls.ContainsKey(id))
                {
                    ...
                    {
                        Manager.sendFractionMessage(fracId,
                            $"{player.Name.Replace('_', ' ')} принял вызов из дома №{id}.");
                        Manager.sendFractionMessage(fracId,
                            "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} принял вызов из дома №{id}", true);
                        player.AddTableScore(TableTaskId.Item5);
                    }
                }
                else if (PoliceCarsCalls.ContainsKey(id))
                {
                    ...
                    {
                        Manager.sendFractionMessage(fracId,
                            $"{player.Name.Replace('_', ' ')} принял вызов сигнализации машины №{id}.");
                        Manager.sendFractionMessage(fracId,
                            "!{#F08080}[F] " +
                            $"{player.Name.Replace('_', ' ')} принял вызов сигнализации машины №{id}", true);
                        player.AddTableScore(TableTaskId.Item5);
                ...
                else if (PoliceSafesCalls.ContainsKey(id))
                {
                    ...
                    {
                        Manager.sendFractionMessage(fracId,
                            $"{player.Name.Replace('_', ' ')} принял вызов по ограблению сейфа.");
                        Manager.sendFractionMessage(fracId,
                            "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} принял вызов по ограблению сейфа.",
                            true);
                        player.AddTableScore(TableTaskId.Item5);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CantFindCall), 3000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void clearWantedLvl(ExtPlayer sender, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight,
                            LangFunc.GetText(LangType.Ru, DataName.PassNumberDoesnt), 3000);
                        return;
                    }
                ...
                catch
                {
                    ...
                    if (!Main.PlayerNames.Values.Contains(target))
                    {
                        ...
                        {
                            Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight,
                                LangFunc.GetText(LangType.Ru, DataName.CantFindPlayerOrVeh), 3000);
                            return;
                        }
                    }
                ...
                if (vehiclenumb.Equals("null"))
                {
                    ...
                    var tauuid = Main.PlayerUUIDs[target];
                    ...
                    Notify.Send(sender, NotifyType.Success, NotifyPosition.TopCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouRemoveWantedPass, target), 3000);
                    Table.Logs.Repository.AddLogs(sender, FractionLogsType.Su, $"Снял розыск с {target} ({tauuid})");
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        if (foreachMemberFractionData.Id == (int)Models.Fractions.POLICE ||
                            ...
                            Trigger.SendChatMessage(foreachPlayer,
                                "!{#FF8C00}" + LangFunc.GetText(LangType.Ru, DataName.RemoveWantedFrom, sender.Name,
                                    target));
                    }
                }
                else
                {
                    if (WantedVehicles.ContainsKey(vehiclenumb))
                    {
                        ...
                        Notify.Send(sender, NotifyType.Success, NotifyPosition.TopCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouRemoveWantedVeh, vehiclenumb), 3000);
                        Table.Logs.Repository.AddLogs(sender, FractionLogsType.Su,
                            $"Снял розыск с т/с ({vehiclenumb})");
                    }
                    else
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight,
                            LangFunc.GetText(LangType.Ru, DataName.PersonalVehWanted), 3000);
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"clearWantedLvl Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (vehicleData == null)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoCarWithThisNumber), 3000);
                    return;
                }

                Trigger.ClientEvent(sender, "executeCarInfo", Convert.ToString(vehicleData.Model),
                    vehicleData.Holder.Replace('_', ' '));
            }
            catch (Exception e)
            {
                Log.Write($"checkNumber Exception: {e}");
            }
        }
        ...
        public static void checkPerson(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    var pasport = Convert.ToInt32(data);
                    if (!Main.PlayerNames.ContainsKey(pasport))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomRight,
                            LangFunc.GetText(LangType.Ru, DataName.PassNumberDoesnt), 3000);
                        return;
                    }
                ...
                catch
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomRight,
                            LangFunc.GetText(LangType.Ru, DataName.CantFindPlayer), 3000);
                        return;
                    }
                ...
                try
                {
                    ...
                    var numberInfo = targetCharacterData.Sim == -1 ? "Отсутствует" : targetCharacterData.Sim.ToString();

                    var wantedLvl = targetCharacterData.WantedLVL == null ? 0 : targetCharacterData.WantedLVL.Level;
                    var gender = targetCharacterData.Gender
                    ...
                    Trigger.ClientEvent(player, "executePersonInfo", $"{targetCharacterData.FirstName}",
                        $"{targetCharacterData.LastName}", $"{targetCharacterData.UUID}", $"{fraction_name}",
                        $"{numberInfo}", $"{gender}", $"{wantedLvl}", $"{lic}", $"{houseInfo}");
                }
                catch
                {
                    ...
                    var targetuuid = Main.PlayerUUIDs[data];
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            Trigger.ClientEvent(player, "executePersonInfo", $"{character.Firstname}",
                                $"{character.Lastname}", $"{character.Uuid}", $"{fraction_name}", $"{numberInfo}",
                                $"{gender}", $"{wantedLvl}", $"{lic}", $"{houseInfo}");
                        }
                        catch
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Человек не найден",
            ...
            {
                Log.Write($"checkPerson Exception: {e}");
            }
        }
        ...
        public static void checkWantedList(ExtPlayer sender)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                    ...
                    {
                        var _WantedList = new List<string>();
                        foreach (var foreachPlayer in Character.Repository.GetPlayers())
                        {
                            var foreachCharacterData = foreachPlayer.GetCharacterData();
                            if (foreachCharacterData == null) continue;
                            var wantedLvl = foreachCharacterData.WantedLVL == null
                                ? 0
                                : foreachCharacterData.WantedLVL.Level;
                            if (wantedLvl != 0)
                                _WantedList.Add(
                                    $"{foreachCharacterData.FirstName} {foreachCharacterData.LastName} - {wantedLvl}*");
                        }

                        lock (WantedVehicles)
                        {
                            if (WantedVehicles.Count >= 1)
                                foreach (var p in WantedVehicles)
                                    _WantedList.Add($"{p.Value.Item2} [{p.Key}]");
                        }

                        var json = JsonConvert.SerializeObject(_WantedList);
                        Log.Debug(json);
            ...
            {
                Log.Write($"checkWantedList Exception: {e}");
            }
        }
                ...
                if (vehicleLocalData != null)
                    if (vehicleLocalData.Access == VehicleAccess.Fraction &&
                        ...
                        if (Configs.IsFractionPolic(sender.GetFractionId()))
                        {
                            ...
                            if (characterData.Gender)
                                Commands.RPChat("sme", sender, LangFunc.GetText(LangType.Ru, DataName.HeOnComputer));
                            else Commands.RPChat("sme", sender, LangFunc.GetText(LangType.Ru, DataName.SheOnComputer));
                        }
            }
            catch (Exception e)
            {
                Log.Write($"openCopcarmenu Exception: {e}");
            }
        }
        ...
        public static void PlayerSoloArrest(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouAlreadyInPrison), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouNotWanted), 3000);
                    return;
                }
                ...
                characterData.ArrestTime =
                    Convert.ToInt32(
                        (characterData.WantedLVL.Level >= 5 ? 3600 : characterData.WantedLVL.Level * 10 * 60) / 2);
                ...
                var minutes = Convert.ToInt32(characterData.ArrestTime / 60);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouArrested, minutes), 3000);

                FractionCommands.arrestPlayer(player);

                Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                    "!{#FF8C00}[F] " + LangFunc.GetText(LangType.Ru, DataName.PoliceSdalsya, player.Name, minutes),
                    true);
                Manager.sendFractionMessage((int)Models.Fractions.SHERIFF,
                    "!{#FF8C00}[F] " + LangFunc.GetText(LangType.Ru, DataName.PoliceSdalsya, player.Name, minutes),
                    true);
                ...
                Table.Logs.Repository.AddLogs(player, FractionLogsType.Arrest,
                    $"{player.Name} ({characterData.UUID}) сдался с поличным и был посажен в КПЗ на {minutes} минут.");
            }
            catch (Exception e)
            {
                Log.Write($"PlayerSoloArrest Exception: {e}");
            }
        }
            ...
            try
            {
                ...
                if (Chars.Repository.ItemsData.ContainsKey(locationName))
                    ...
                    foreach (var itemData in Chars.Repository.ItemsData[locationName][Location])
                        ...
                        {
                            if (itemData.Value.ItemId == ItemId.Debug || itemData.Value.ItemId == ItemId.Wrench ||
                                itemData.Value.ItemId == ItemId.Flashlight || itemData.Value.ItemId == ItemId.Ball ||
                                itemData.Value.ItemId == ItemId.Bag) continue;

                            if (itemData.Value.Data.Contains("_") &&
                                WeaponsMaxHP.ContainsKey((int)itemData.Value.ItemId))
                            {
                                var weaponHP = Convert.ToInt32(itemData.Value.Data.Split("_")[1]);
                                var condition =
                                    Convert.ToInt32((double)weaponHP / WeaponsMaxHP[(int)itemData.Value.ItemId] * 100);
                            ...
                            {
                                if (item.ItemId == ItemId.PistolAmmo || item.ItemId == ItemId.SMGAmmo ||
                                    item.ItemId == ItemId.RiflesAmmo || item.ItemId == ItemId.SniperAmmo ||
                                    item.ItemId == ItemId.ShotgunsAmmo)
                                {
                                    rewardSumm += IllegalsItems[item.ItemId] * item.Count;
                                }
                                else if (item.ItemId == ItemId.Drugs)
                                {
                                    var dCount = Chars.Repository.getCountItem($"char_{characterData.UUID}",
                                        ItemId.Drugs, false);
                                    rewardSumm += IllegalsItems[item.ItemId] * dCount;
                                }
                                else
                                {
                                    rewardSumm += IllegalsItems[item.ItemId];
                                }
                            }
                        }
                ...
                if (rewardSumm > 0)
                {
                    Chars.Repository.RemoveAllIllegalStuff(player, false);
                    Wallet.Change(player, rewardSumm);
                    GameLog.Money("server", $"player({characterData.UUID})", rewardSumm,
                        $"TakeIllegalStuff({rewardSumm})");
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SucSdalNelegal, rewardSumm), 3000);
                    Manager.sendFractionMessage((int)Models.Fractions.POLICE,
                        "!{#FF8C00}[F] " + LangFunc.GetText(LangType.Ru, DataName.SdalNelegal, player.Name, rewardSumm),
                        true);
                    Manager.sendFractionMessage((int)Models.Fractions.SHERIFF,
                        "!{#FF8C00}[F] " + LangFunc.GetText(LangType.Ru, DataName.SdalNelegal, player.Name, rewardSumm),
                        true);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoNelegal), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"TakeIllegalStuff Exception: {e}");
            }
        }
        ...
        public static void OnFractionPolic(ExtPlayer player, int interact)
        {
            try
            {
                ...
                switch (interact)
                {
                    ...
                    case 3:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoEmergency), 3000);
                            return;
                        }
                    ...
                    case 5:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.Warehouse2Closed), 3000);
                            return;
                        }

                        if (!player.IsFractionAccess(RankToAccess.OpenWeaponStock)) return;
                        sessionData.OnFracStock = 7;
                        Chars.Repository.LoadOtherItemsData(player, "Fraction", "7", 5,
                            Chars.Repository.InventoryMaxSlots["Fraction"]);
                        return;
                    case 6:
                        if (!sessionData.WorkData.OnDuty)
                        {
                        ...
                        if (!player.IsInVehicle || (player.Vehicle.Model != NAPI.Util.GetHashKey("police") &&
                                                    player.Vehicle.Model != NAPI.Util.GetHashKey("police2") &&
                                                    player.Vehicle.Model != NAPI.Util.GetHashKey("police3") &&
                                                    player.Vehicle.Model != NAPI.Util.GetHashKey("police4")))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.MustCarPolice), 3000);
                            return;
                        }

                        player.Vehicle.SetSharedData("BOOST", 20);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CarBoost), 3000);
                        return;
                    case 9:
                        if (!sessionData.WorkData.OnDuty)
                        {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.RepairFlatVeh), 3000);
                            return;
                        }

                        if (DateTime.Now < Main.NextFixcarPoliceVeh)
                        {
                            var ticks = Main.NextFixcarPoliceVeh.Ticks - DateTime.Now.Ticks;
                            if (ticks <= 0) return;
                            var g = new DateTime(ticks);
                            if (g.Hour >= 1)
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.NextRepair1h, g.Hour, g.Minute, g.Second),
                                    3000);
                            else
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.NextRepair, g.Minute, g.Second), 3000);
                            return;
                        }
                        ...
                        {
                            if (vehicleLocalData.Access == VehicleAccess.Fraction &&
                                vehicleLocalData.Fraction == (int)Models.Fractions.POLICE)
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.SucRepairVeh), 3000);
                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
            ...
            {
                Log.Write($"OnFractionPolic Exception: {e}");
            }
        }
        ...
        public static void setPlayerWantedLevel(ExtPlayer player, WantedLevel wantedlevel)
        {
            ...
            {
                Log.Write($"setPlayerWantedLevel Exception: {e}");
            }
        }

        #region shapes

        [Interaction(ColShapeEnums.FractionPolicArrest, true)]
        public static void InFractionPolicArrest(ExtPlayer player, int index)
        {
            ...
            {
                Log.Write($"InFractionPolicArrest Exception: {e}");
            }
        }
        ...
        public static void OutFractionPolicArrest(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OutFractionPolicArrest Exception: {e}");
            }
        }
        ...
        public static void OpenPoliceGunMenu(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OpenPoliceGunMenu Exception: {e}");
            }
        }
        ...
        public static void OpenSpecialPoliceMenu(ExtPlayer player)
        {
            ...
            {
                Log.Write($"OpenSpecialPoliceMenu Exception: {e}");
            }
        }
