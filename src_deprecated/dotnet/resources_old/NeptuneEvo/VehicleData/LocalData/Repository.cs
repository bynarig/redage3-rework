...
namespace NeptuneEvo.VehicleData.LocalData
{
    ...
    {
        public static ConcurrentDictionary<VehicleAccess, ConcurrentDictionary<string, ExtVehicle>>
            VehicleNumberToHandle = new ConcurrentDictionary<VehicleAccess, ConcurrentDictionary<string, ExtVehicle>>();
        ...
        {
            var vehicle = GetVehicleToNumber(vehicleAccess, number);
            if (vehicle != null) return vehicle.VehicleLocalData;
            return null;
        }
        ...
        {
            var vehiclesAccess = new List<VehicleAccess>();
            ...
            foreach (var access in vehiclesAccess)
                if (VehicleNumberToHandle[access].ContainsKey(number))
                    return VehicleNumberToHandle[access][number];
        ...
        public static void Delete(VehicleAccess vehicleAccess, string number)
        {
            ...
            foreach (var access in vehiclesAccess)
                if (VehicleNumberToHandle[access].ContainsKey(number))
                    VehicleNumberToHandle[access].TryRemove(number, out _);
        }
    }
}