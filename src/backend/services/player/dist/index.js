...
import { Topics, LoadCharacterCommand, SaveCharacterCommand, CreateCharacterCommand, SaveCustomizationCommand, } from '@redage/contracts';
...
async function main() {
    ...
    consumer.on(Topics.PlayerCommands, 'player.create_character', CreateCharacterCommand, async (cmd, env) => {
        await handleCreateCharacter(cmd, env, { repo, replier, logger });
    });
    consumer.on(Topics.PlayerCommands, 'player.save_customization', SaveCustomizationCommand, async (cmd, env) => {
        await handleSaveCustomization(cmd, env, { repo, logger });
    });
