...
import type { SaveCustomizationCommandMsg, Envelope } from '@redage/contracts';
...
const cmd: SaveCustomizationCommandMsg = {
  type: 'player.save_customization',
...
describe('handleSaveCustomization', () => {
  it('delegates to repo.saveCustomization', async () => {
    ...
    await handleSaveCustomization(cmd, env, { repo: repo as never, logger: logger as never });
  ...
  it('swallows errors (fire-and-forget) but logs them', async () => {
    ...
    await expect(handleSaveCustomization(cmd, env, { repo: repo as never, logger: logger as never })).resolves.toBeUndefined();
