...
import { CreateCharacterInput, Appearance, } from '@redage/validators';
/**
...
export function registerCharacterBridge(deps) {
    ...
    mp.events.add('redage:customization:save', (...args) => {
        ...
        try {
            ...
            const raw = typeof args[1] === 'string' ? JSON.parse(args[1]) : args[1];
        ...
        void (async () => {
            try {
                ...
                const gender = player.model === mp.joaat('mp_f_freemode_01')
                    ? 'FEMALE'
                    : 'MALE';
 ...
 */
function applyAppearance(player, appearance, gender, logger) {
    const p = player;
    try {
...
function applyOverlay(player, overlayId, drawableId, opacity, colorType) {
