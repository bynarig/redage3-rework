...
export function registerPlayerEventBridge(deps) {
    ...
    mp.events.add('playerQuit', (...args) => {
        ...
        if (!session)
            return;
        const reason = reasonRaw === 'kicked' ? 'kicked' : reasonRaw === 'timeout' ? 'timeout' : 'quit';
        void producer
            .publish({
            topic: Topics.PlayerEvents,
            key: String(player.id),
    ...
    mp.events.add('redage:position', (...args) => {
        ...
        if (!session?.characterId)
            return;
        const now = Date.now();
        if (now - session.lastPositionSentAt < positionThrottleMs)
            return;
...
//# sourceMappingURL=player-events.js.map