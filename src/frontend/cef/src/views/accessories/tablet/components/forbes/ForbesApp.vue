                    ...
                    <div>
                        <template v-if="(selectedItem.houses?.length ?? 0) > 0 || (selectedItem.biz?.length ?? 0) > 0">
                            ...
                            <div v-for="h in selectedItem.houses" :key="h.Name">
                                <div>{{ h.Name }}<div><div>Стоимость:</div><div>${{ formatMoney(h.Money) }}</div></div></div>
                            </div>
                            <div v-for="b in selectedItem.biz" :key="b.Name">
                                <div>{{ b.Name }}<div><div>Стоимость:</div><div>${{ formatMoney(b.Money) }}</div></div></div>
                            </div>
                        </template>
                        <template v-if="(selectedItem.vehicles?.length ?? 0) > 0">
                            <div>Транспорт:</div>
                            <div v-for="v in selectedItem.vehicles" :key="v.Name">
                                <div>{{ v.Name }}<div><div>Стоимость:</div><div>${{ formatMoney(v.Money) }}</div></div></div>
                            </div>
                        </template>
                    </div>
                </template>
