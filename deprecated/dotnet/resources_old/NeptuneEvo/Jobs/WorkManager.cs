...
{
    internal class WorkManager : Script
    {
        ...
        {
            { 0, Cityhall.JobPosition }, // Employment center
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(354, LangFunc.GetText(LangType.Ru, DataName.Electrostanciya),
                    new Vector3(724.9625, 133.9959, 79.83643), 70, true, 1.2f));
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (characterData.WorkID != 0)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouLeaveJob), 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouNotJober), 3000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"Layoff Exception: {e}");
            }
        }
        ...
        public static void JobJoin(ExtPlayer player, int job)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BeforeLeaveLastJob), 3000);
                    return;
                }
                ...
                if (characterData.WorkID == job)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouAlreadyJob, JobList[job]), 3000);
                }
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.NeedJobsLvl, JobsMinLvl[job]), 3000);
                        return;
                    }

                    if ((job == (int)JobsId.Postman || job == (int)JobsId.Taxi || job == (int)JobsId.CarMechanic) &&
                        !characterData.Licenses[1])
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.JobLicB), 10000);
                        return;
                    }

                    if ((job == (int)JobsId.Bus || job == (int)JobsId.Trucker || job == (int)JobsId.CashCollector) &&
                        !characterData.Licenses[2])
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.JobLicC), 10000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouStartedJob, JobList[job]), 8000);
        ...
        public static void InJobSelect(ExtPlayer player)
        {
            ...
            {
                Log.Write($"JobMenu_onEntityEnterColShape Exception: {e}");
            }
        }
        ...
        public static void callback_jobsSelecting(ExtPlayer player, int act)
        {
            ...
            {
                Log.Write($"callback_jobsSelecting Exception: {e}");
            }
        }
    }
}