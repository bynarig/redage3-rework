...
{
    internal class Collector : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                NAPI.Marker.CreateMarker(1, TakeMoneyPos - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220), false, 0);
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16(LangFunc.GetText(LangType.Ru, DataName.TakeMeshki)),
                    TakeMoneyPos + new Vector3(0, 0, 0.3), 30f, 0.4f, 0, new Color(255, 255, 255), true, 0);
            }
            catch (Exception e)
            {
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.InkassStart), 3000);
        ...
        private static void SetAtmPoint(ExtPlayer player)
        {
            ...
            var rand = new Random();
            var atmId = WorkManager.rnd.Next(0, ATM.ATMs.Count - 1);
            while (atmId == 58 || atmId == 36 || ATM.ATMs[atmId].DistanceTo2D(playerPosition) < 200)
                atmId = rand.Next(0, ATM.ATMs.Count - 1);

            var atm = ATM.ATMs[atmId];
        ...
        public static void OnTakeMoney(ExtPlayer player)
        {
            try
            {
                ...
                if (player.IsInVehicle || characterData.WorkID != (int)JobsId.CashCollector ||
                    !sessionData.WorkData.OnWork) return;
                if (sessionData.WorkData.Packages != 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveBags, sessionData.WorkData.Packages), 3000);
                    return;
                }
                ...
                SetAtmPoint(player);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.GotNewBags), 3000);
            }
            catch (Exception e)
            {
        ...
        [Interaction(ColShapeEnums.Atm, true)]
        public static void InAtm(ExtPlayer player, int Index)
        {
            try
            {
                ...
                if (rentData == null || player.IsInVehicle || characterData.WorkID != (int)JobsId.CashCollector ||
                    !sessionData.WorkData.OnWork || sessionData.WorkData.WorkCheck != Index) return;

                if (sessionData.WorkData.Packages == 0 ||
                    !Attachments.HasAttachment(player, Attachments.AttachmentsName.MoneyBag))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        "У вас нет сумки с деньгами в руке", 3000);
                    return;
                }

                var coef = Convert.ToInt32(player.Position.DistanceTo2D(sessionData.WorkData.Position) / 100);
                var lastTime = sessionData.WorkData.Time;
                if (Main.ServerNumber != 0 && DateTime.Now < lastTime.AddSeconds(coef * 2))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AtmFull), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.JobVehFar), 3000);
                    return;
                }

                sessionData.WorkData.Packages -= 1;

                var payment = Convert.ToInt32(coef * Main.CollectorPayment * Group.GroupPayAdd[accountData.VipLvl] *
                ...
                var jobLevelInfo = characterData.JobSkills.ContainsKey((int)JobsId.CashCollector)
                ...
                if (sessionData.WorkData.Packages == 0)
                {
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.BackToBaza), 3000);
                ...
                else
                {
                    SetAtmPoint(player);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.GoNextAtm), 3000);
                }
                ...
                {
                    characterData.JobSkills.Add((int)JobsId.CashCollector, 1);
                }
            }
            catch (Exception e)
            {
