...
{
    internal class LsNewsSystem : Script
    {
        private static readonly nLog Log = new nLog("Fractions.LSNews");

        private static readonly Queue<CompletedAdvert> CompletedAdverts = new Queue<CompletedAdvert>();
        ...
        public void OnResourceStartHandler()
        {
            try
            {
                ...
                NAPI.Marker.CreateMarker(30, LSNewsCoords[0], new Vector3(), new Vector3(), 1,
                    new Color(255, 255, 255, 220));

                for (var i = 0; i < FirstNewsLiftTP.Length; i++)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Лифт"),
                        new Vector3(FirstNewsLiftTP[i].X, FirstNewsLiftTP[i].Y, FirstNewsLiftTP[i].Z), 5F, 0.3F, 0,
                        new Color(255, 255, 255));
                    NAPI.Marker.CreateMarker(1, FirstNewsLiftTP[i] - new Vector3(0, 0, 1.25), new Vector3(),
                        new Vector3(), 1f, new Color(255, 255, 255, 220));
                }

                for (var i = 0; i < SecondNewsLiftTP.Length; i++)
                {
                    CustomColShape.CreateCylinderColShape(SecondNewsLiftTP[i], 1, 2, 0, ColShapeEnums.FractionLSNews,
                        3);
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Лифт"),
                        new Vector3(SecondNewsLiftTP[i].X, SecondNewsLiftTP[i].Y, SecondNewsLiftTP[i].Z), 5F, 0.3F, 0,
                        new Color(255, 255, 255));
                    NAPI.Marker.CreateMarker(1, SecondNewsLiftTP[i] - new Vector3(0, 0, 1.25), new Vector3(),
                        new Vector3(), 1f, new Color(255, 255, 255, 220));
                }

                Repository.CreateQuest("ig_molly", new Vector3(-586.7227, -921.56085, 23.869083), 137.71083f,
                    title: "~y~NPC~w~ Дженнифер", colShapeEnums: ColShapeEnums.FracLSNews);
                Repository.CreateQuest("ig_englishdave_02", new Vector3(-588.256, -936.6138, 23.867977), -4.159257f,
                    title: "~y~NPC~w~ Журналист Шаманчик\nВызвать сотрудника",
                    colShapeEnums: ColShapeEnums.CallNewsMember);
            }
            catch (Exception e)
            {
        ...
        public static void OnFractionLSNews(ExtPlayer player, int interact)
        {
                    ...
                    case 1:
                        if (player.GetFractionId() == (int)Models.Fractions.LSNEWS)
                            ...
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.NoNEWS), 3000);
                        return;
                    case 2:
                    case 3:
            ...
            {
                Log.Write($"OnFractionLSNews Exception: {e}");
            }
        }
        ...
        public static void UseNewsLift(ExtPlayer player, byte lift, int index)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.GoingToFloor, index + 1), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"UseNewsLift Exception: {e}");
            }
        }
        ...
        public static bool IsAdvertToName(string Name)
        {
            ...
            {
                Log.Write($"IsAdvertToName Exception: {e}");
                return false;
            }
        }
        ...
        public static void onLSNPlayerLoad(ExtPlayer player)
        {
            ...
            {
                Log.Write($"onLSNPlayerLoad Exception: {e}");
            }
        }
        ...
        public static void OpenCallNewsMemberDialog(ExtPlayer player, int _)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "CallNewsMemberDialog",
                    LangFunc.GetText(LangType.Ru, DataName.AreYouWantToCallGov));
            }
            catch (Exception e)
            {
                Log.Write($"OpenCallNewsMemberDialog Exception: {e}");
            }
        }

        public static async Task AddAdvert(ExtPlayer player, string text, string link, int type, bool isPremium,
            int price)
        {
            try
            {
                ...
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.News,
                    LangFunc.GetText(LangType.Ru, DataName.AdDone), DateTime.Now);
                ...
                var itemSqlID = await db.InsertWithInt32IdentityAsync(new Advertiseds
                {
                    Author = sessionData.Name,
                    AuthorSIM = characterData.Sim,
                    AD = text,
            ...
            {
                Log.Write($"AddAdvert Exception: {e}");
            }
        }
        ...
        public static void UpdateAnswer(ExtPlayer player, int repID, string response, bool deleted = false)
        {
            try
            {
                ...
                if (characterData.AdminLVL == 0 && memberFractionData.Id == (int)Models.Fractions.LSNEWS &&
                    memberFractionData.Rank <= 1)
                {
                    AdvertTake(player, repID);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoAccess), 3000);
                    return;
                }
                ...
                if (!AdvertList.ContainsKey(repID))
                {
                    if (deleted)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.CantFindAdNumber), 3000);
                    return;
                }
                ...
                if (response.Length >= 150)
                {
                    AdvertTake(player, repID);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AdMustBe150), 3000);
                    return;
                }
                ...
                if (!deleted)
                {
                    var moneyad = Convert.ToInt32(advert.AD.Length / 7 * Main.AdSymbCost * Main.AdEditorCost);
                    Bank.Change(characterData.Bank, moneyad, false);
                    CompletedAdverts.Enqueue(new CompletedAdvert(advert.Author.Replace('_', ' '), advert.AuthorSIM,
                        response, player.Name.Replace('_', ' '), advert.IsPremium));
                    Players.Phone.News.Repository.AddList(advert, response);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AdToList, advert.Author, CompletedAdverts.Count), 3000);
                    ...
                    if (target.IsCharacterData())
                        Players.Phone.Messages.Repository.AddSystemMessage(target, (int)DefaultNumber.News,
                            LangFunc.GetText(LangType.Ru, DataName.AdToList, advert.Author, CompletedAdverts.Count),
                            DateTime.Now);
                }
                else
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.DelAd, advert.Author), 3000);
                    var target = (ExtPlayer)NAPI.Player.GetPlayerFromName(advert.Author);
                    response += LangFunc.GetText(LangType.Ru, DataName.deletedad);
                    if (target.IsCharacterData())
                        Players.Phone.Messages.Repository.AddSystemMessage(target, (int)DefaultNumber.News,
                            LangFunc.GetText(LangType.Ru, DataName.DelAdReason, player.Name, response), DateTime.Now);
                }
            ...
            try
            {
                ...
                if (AdvertList.Count > 0)
                    foreach (var ad in AdvertList.Values)
                        ...
                        {
                            if (ad == null) continue;
                            if (ad.Editor != null && ad.Editor.Equals(player.Name))
                            {
                                AdvertTake(player, ad.ID);
                                break;
                            }

                            if (ad.Status) Remove(ad.ID);
                        }
                        catch (Exception e)
                        {
                            Log.Write($"OnDisconnect Foreach Exception: {e}");
                        }
            }
            catch (Exception e)
            {
                Log.Write($"OnDisconnect Exception: {e}");
            }
        }
        ...
        public static void CheckCompletedAds()
        {
            try
            {
                if (CompletedAdverts.Count >= 1)
                {
                    ...
                    if (ad.AuthorSIM >= 1)
                        NAPI.Chat.SendChatMessageToAll(color + LangFunc.GetText(LangType.Ru, DataName.AdWithPhone,
                            ad.EditedAD, ad.AuthorSIM));
                    else
                        NAPI.Chat.SendChatMessageToAll(color + LangFunc.GetText(LangType.Ru, DataName.Ad, ad.EditedAD));
                    NAPI.Chat.SendChatMessageToAll(color2 +
                                                   LangFunc.GetText(LangType.Ru, DataName.AdRewriter, ad.Editor));
                }
            ...
            {
                Log.Write($"CheckCompletedAds Exception: {e}");
            }
        }
        ...
        private static void AddPlayer(string advert)
        {
            try
            {
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    ...
                    {
                        Log.Write($"AddPlayer Foreach Exception: {e}");
                    }
            }
            catch (Exception e)
            {
                Log.Write($"AddPlayer Exception: {e}");
            }
        }
        ...
        private class CompletedAdvert
        {
            ...
            public int AuthorSIM { get; }
            public string EditedAD { get; }
        ...
        public static void AdvertTake(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (AdvertList.Values.Any(a => a.Editor == player.Name && a.ID != index))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.AdAlreadyTaken), 3000);
                    return;
                }
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    ...
                    {
                        Log.Write($"AdvertTake Foreach Exception: {e}");
                    }
            }
            catch (Exception e)
            {
                Log.Write($"AdvertTake Exception: {e}");
            }
        }
        ...
        public static void AdvertSend(ExtPlayer player, int ID, string answer)
        {
            try
            {
                ...
                if (!AdvertList[ID].Status)
                {
                    UpdateAnswer(player, ID, answer);
                }
                else
                {
            ...
            {
                Log.Write($"AdvertSend Exception: {e}");
            }
        }
        ...
        public static void AdvertDelete(ExtPlayer player, int ID, string reason)
        {
            try
            {
                ...
                else if (CommandsAccess.CanUseCmd(player, AdminCommands.Delad))
                {
                    UpdateAnswer(player, ID, reason, true);
                }
            }
            ...
            {
                Log.Write($"AdvertDelete Exception: {e}");
            }
        }
