...
{
    internal class Roullete : Script
    {
        /// <summary>
        ///     Максимальное кол-во ставок
        /// </summary>
        private static readonly int MaxBetsLimit = 3;
        ...
        /// </summary>
        public static Dictionary<int, string> Roulette =
            new Dictionary<int, string> // Справа правильные цифры рулетки, слева то, что видит система
            {
                [1] = "00", // 00
                [2] = "27",
                [3] = "10",
        ...
        ///     Данные игрока
        /// </summary>
        public static Dictionary<ExtPlayer, RoulettePlayerData> PlayerData =
            new Dictionary<ExtPlayer, RoulettePlayerData>();
        ...
        public static void StartGame(int index)
        {
            try
            {
                var table = RouletteTables[index];
                ...
                if (table.WaitTimeout == null)
                {
                    ...
                    table.WaitTimeout = Timers.Start(1000, () =>
                    {
                        if (--timer > 0)
                        {
                            lock (table.Seats)
                            {
                                foreach (var foreachPlayer in table.Seats)
                                    if (foreachPlayer.IsCharacterData() && PlayerData.ContainsKey(foreachPlayer))
                                        Trigger.ClientEvent(foreachPlayer, "client.roullete.timer", timer);
                            }
                        }
                        else
                        {
            ...
            {
                Log.Write($"StartGame Exception: {e}");
            }
        }
        ...
        public static void ContinueGame(int index)
        {
            try
            {
                ...
                var current_players_on_table = 0;

                /** Количество ставок, которые находятся на столе */
                var current_players_bets_on_table = 0;
                ...
                lock (table.Seats)
                {
                    foreach (var foreachPlayer in table.Seats)
                        if (foreachPlayer.IsCharacterData() && PlayerData.ContainsKey(foreachPlayer))
                        {
                            var rData = PlayerData[foreachPlayer];
                ...
                if (current_players_on_table > 0 && current_players_bets_on_table > 0)
                {
                    ...
                    while (!Roulette.ContainsKey(table.Win)) table.Win = new Random().Next(1, 39);
                    var bet_string = $"exit_{table.Win.ToString()}_";
                    for (var i = 0; i < TableChipsOffsets.Count; i++)
                        ...
                        foreach (var foreachPlayer in table.Seats)
                            if (foreachPlayer.IsCharacterData())
                            {
                                if (PlayerData.ContainsKey(foreachPlayer))
                                    PlayerData[foreachPlayer].Time = DateTime.Now.AddSeconds(5);
            ...
            {
                Log.Write($"ContinueGame Exception: {e}");
            }
        }
        ...
        public static void DestroyAllPlayerBets(ExtPlayer player)
        {
            ...
            {
                Log.Write($"DestroyAllPlayerBets Exception: {e}");
            }
        }
        ...
        public static void SendRouletteNotify(int index, string message)
        {
            ...
            {
                Log.Write($"SendRouletteNotify Exception: {e}");
            }
        }
        ...
        public static void OnCharacterOccupyPlace(ExtPlayer player, int tableIndex, int placeID)
        {
            try
            {
                ...
                var table = RouletteTables[tableIndex];
                ...
                Trigger.ClientEvent(player, "client.roullete.CHARACTER_OCCUPY_PLACE", tableIndex, placeID,
                    table.Process ? 0 : 1);
            ...
            {
                Log.Write($"OnCharacterOccupyPlace Exception: {e}");
            }
        }
        ...
        public static void OnCharacterLeavePlace(ExtPlayer player)
        {
            try
            {
                ...
                var table = RouletteTables[rData.SelectedTable];
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NowDeleteStavkas), 3000);
                    return;
                }

                PlayerData.Remove(player);

                var seat_index = table.Seats.IndexOf(player);
            ...
            {
                Log.Write($"OnCharacterLeavePlace Exception: {e}");
            }
        }
        ...
        public static void OnCreateBet(ExtPlayer player, int bet, int spot, float x, float y, float z)
        {
            try
            {
                ...
                if (UpdateData.CanIChange(player, bet, true) != 255) return;
                if (!RouletteWorking)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.FunctionOffByAdmins), 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.OneGameMaxBets, MaxBetsLimit), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.WaitUntilGameEnds), 3000);
                    return;
                }

                for (var i = 0; i < rData.AllBets.Count; i++)
                    if (rData.AllBets[i].Spot == spot)
                        return;
                var casenum = rData.AllBets.Count;
                switch (rData.AllBets.Count)
                {
                    case 0:
                        rData.FBetObject = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("vw_prop_chip_100dollar_x1"),
                            new Vector3(x, y, z), new Vector3());
                        break;
                    case 1:
                        rData.SBetObject = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("vw_prop_chip_100dollar_x1"),
                            new Vector3(x, y, z), new Vector3());
                        break;
                    case 2:
                        rData.TBetObject = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("vw_prop_chip_100dollar_x1"),
                            new Vector3(x, y, z), new Vector3());
                        break;
                }

                Wallet.Change(player, -bet);
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.YouBet, bet), 3000);
                StartGame(rData.SelectedTable);
            }
            catch (Exception e)
            {
                Log.Write($"OnCreateBet Exception: {e}");
            }
        }
        ...
        public static void OnDestroyLastBet(ExtPlayer player)
        {
            try
            {
                ...
                var mybet = PlayerData[player].AllBets[rData.AllBets.Count - 1].Bet;
                Wallet.Change(player, mybet);
            ...
            {
                Log.Write($"OnDestroyLastBet Exception: {e}");
            }
        }
        ...
        public static void OnClearTable(ExtPlayer player)
        {
            ...
            {
                if (!PlayerData.ContainsKey(player)) return;
                var index = PlayerData[player].SelectedTable;
                if (RouletteTables.Count <= index) return;
                var table = RouletteTables[index];
                ...
                lock (table.Seats)
                {
                    foreach (var foreachPlayer in table.Seats)
                    {
                        ...
                        {
                            var targetrData = PlayerData[foreachPlayer];
                            if (targetrData.AllBets.Count > 0)
                            {
                                targetrData.WinMoney = 0;

                                var WinSpots = new List<int>();

                                for (var x = 0; x < targetrData.AllBets.Count; x++)
                                {
                                    var target_data = targetrData.AllBets[x];

                                    if (table.WinSpots.Contains(target_data.Spot) &&
                                        !WinSpots.Contains(target_data.Spot))
                                    {
                                        WinSpots.Add(target_data.Spot);
                                        targetrData.WinMoney += Convert.ToInt32(target_data.Bet *
                                            (coefficientData.ContainsKey(TableChipsOffsets[target_data.Spot].Count)
                                                ? coefficientData[TableChipsOffsets[target_data.Spot].Count]
                                                : 1.25f));
                                    }
                                }

                                var message =
                                    $"Выпало [{table.WinNum}]. Вы {(targetrData.WinMoney > 0 ? $"выиграли ${Wallet.Format(targetrData.WinMoney)}" : "проиграли.")}"; // harddd
                                ...
                                if (targetrData.WinMoney > 0)
                                {
                                    ...
                                    Wallet.Change(foreachPlayer, targetrData.WinMoney);
                                    GameLog.Money("system", $"player({foreachCharacterData.UUID})",
                                        targetrData.WinMoney, "RouletteWin");
                                }
                                else
                                {
            ...
            {
                Log.Write($"OnClearTable Exception: {e}");
            }
        }
        ...
        public static void Disconnect(ExtPlayer player, DisconnectionType type)
        {
            try
            {
                ...
                var table = RouletteTables[rData.SelectedTable];
                var money = 0;
                for (var x = 0; x < rData.AllBets.Count; x++) money += rData.AllBets[x].Bet;
