    ...
    public class Voice : Script
    {
        ...
        public static void PlayerQuit(ExtPlayer player, string reson)
        {
            try
            {
                ...
                {
                    var target = playerPhoneMeta.Target;
                    ...
                    var targetPhoneMeta = targetSessionData.VoiceData;

                    var pSim = characterData.Sim;
                    var playerName = targetCharacterData.Contacts.ContainsKey(pSim)
                        ? targetCharacterData.Contacts[pSim]
                        : pSim.ToString();

                    Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PlayerHangPhone, playerName), 3000);
            ...
            {
                Log.Write($"PlayerQuit Exception: {e}");
            }
        }
        ...
        public void voiceDebugReload(ExtPlayer player)
        {
            ...
            {
                Log.Write($"voiceDebugReload Exception: {e}");
            }
        }
        ...
        public static void add_voice_listener(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (sessionData.LoggedIn) player.EnableVoiceTo(target);
            ...
            {
                Log.Write($"add_voice_listener Exception: {e}");
            }
        }
        ...
        public static void add_voice_listener_sync(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (sessionData == null) return;
                if (!target.IsCharacterData()) return;
                if (sessionData.LoggedIn) target.EnableVoiceTo(player);
            }
            catch (Exception e)
            {
                Log.Write($"add_voice_listener Exception: {e}");
        ...
        public static void remove_voice_listener_sync(ExtPlayer player, ExtPlayer target)
        {
            ...
            {
                Log.Write($"remove_voice_listener Exception: {e}");
            }
        }
        ...
        public void set_polygon(ExtPlayer player, int idPolygon)
        {
            ...
            {
                Log.Write($"set_polygon Exception: {e}");
            }
        }
        ...
        public void set_whisper(ExtPlayer player, bool toggled)
        {
            ...
            {
                Log.Write($"set_whisper Exception: {e}");
            }
        }

        public static bool isGov(int fracId)
        {
            return fracId == (int)Fractions.Models.Fractions.CITY || fracId == (int)Fractions.Models.Fractions.POLICE ||
                   fracId == (int)Fractions.Models.Fractions.EMS || fracId == (int)Fractions.Models.Fractions.FIB ||
                   fracId == (int)Fractions.Models.Fractions.ARMY ||
        ...
        public static void TakeWalkieTalkie_server(ExtPlayer player)
        {
            try
            {
                ...
                if (access > 0)
                    Trigger.ClientEvent(player, "openWalkieTalkieMenu", access == 1, sessionData.WalkieTalkieFrequency);
                else
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.NoWalkieTalkie), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"TakeWalkieTalkie_server Exception: {e}");
            }
        }
        ...
        public static void PressedDangerButton(ExtPlayer player)
        {
            try
            {
                ...
                if (memberFractionData.Id == 7 || memberFractionData.Id == 9 || memberFractionData.Id == 18)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.Panic15sec), 3000);
                        return;
                    }
                    ...
                    foreach (var foreachPlayer in Repository.GetPlayers())
                    {
                        ...
                        if (foreachMemberFractionData.Id == (int)Fractions.Models.Fractions.POLICE ||
                            foreachMemberFractionData.Id == (int)Fractions.Models.Fractions.FIB ||
                            foreachMemberFractionData.Id == (int)Fractions.Models.Fractions.SHERIFF)
                        {
                            ...
                            Trigger.SendChatMessage(foreachPlayer,
                                "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.PanicActivator, player.Name,
                                    player.Value));
                        }
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        {
                            Log.Write($"PressedDangerButton Task Exception: {e}");
                        }
                    }, 15000);
                }
            }
            ...
            {
                Log.Write($"PressedDangerButton Exception: {e}");
            }
        }
        ...
        public static void CMD_to(ExtPlayer player, int sender_id)
        {
            try
            {
                ...
                if (memberFractionData.Id == 7 || memberFractionData.Id == 9 || memberFractionData.Id == 18)
                {
                    ...
                    var targetSessionData = target.GetSessionData();
                    if (targetSessionData == null) return;
                    if (!targetSessionData.IsPressedDangerButton) return;
                    ...
                    foreach (var foreachPlayer in Repository.GetPlayers())
                    {
                        ...
                        if (foreachMemberFractionData.Id == (int)Fractions.Models.Fractions.POLICE ||
                            ...
                            foreachMemberFractionData.Id == (int)Fractions.Models.Fractions.SHERIFF)
                            Trigger.SendChatMessage(foreachPlayer,
                                "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.Code0accept, player.Name,
                                    player.Value, sender_id));
        ...
        public static void UpdateWalkieTalkieFrequency(ExtPlayer player, int value)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.WalkieWave, value), 3000);
            }
            catch (Exception e)
            {
                Log.Write($"UpdateWalkieTalkieFrequency Exception: {e}");
            }
        }
        ...
        public static void AddRadio(ExtPlayer player)
        {
            try
            {
                ...
                foreach (var foreachPlayer in Repository.GetPlayers())
                {
                    ...
                    else if (_isGov && fracId > (int)Fractions.Models.Fractions.None &&
                             fracId == foreachMemberFractionData?.Id)
                    {
            ...
            {
                Log.Write($"addRadio Exception: {e}");
            }
        }
        ...
        public void Event_PlayerExitVehicle(ExtPlayer player, ExtVehicle veh)
        {
            ...
            {
                Log.Write($"Event_PlayerExitVehicle Exception: {e}");
            }
        }
        ...
        [Interaction(ColShapeEnums.VoiceZone, true)]
        ...
        public static void CMD_setmicrophone(ExtPlayer player, int id, int value)
        {
            ...
            {
                Log.Write($"setmicrophone Exception: {e}");
            }
        }
    }
}