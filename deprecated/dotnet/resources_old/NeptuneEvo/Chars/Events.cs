...
namespace NeptuneEvo.Chars
{
    internal class Events : Script
    {
            ...
            try
            {
                ...
                if (DateTime.Now <= sessionData.RequestData.Time && sessionData.RequestData.IsRequested)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveActiveOrders), 3000);
                    return;
                }
                ...
                if (DateTime.Now <= targetSessionData.RequestData.Time && targetSessionData.RequestData.IsRequested)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                ...
                if (type == "vehicle")
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PersonHaveNoCar), 3000);
                        return;
                    }
                    ...
                    EventSys.SendCoolMsg(player, "Предложение", "Обмен",
                        $"{LangFunc.GetText(LangType.Ru, DataName.YouWantTradeVeh)}", "", 10000);
                    EventSys.SendCoolMsg(target, "Предложение", "Обмен",
                        $"{LangFunc.GetText(LangType.Ru, DataName.PersonWantTradeVeh, player.Value)}", "", 10000);
                }
                else if (type == "house")
                {
                    ...
                    var houseTarget = HouseManager.GetHouse(target, true);

                    if (houseTarget == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.PersonNoHome), 3000);
                        return;
                    }
                    ...
                    EventSys.SendCoolMsg(player, "Предложение", "Обмен",
                        $"{LangFunc.GetText(LangType.Ru, DataName.YouWantTradeHouse)}", "", 10000);
                    EventSys.SendCoolMsg(target, "Предложение", "Обмен",
                        $"{LangFunc.GetText(LangType.Ru, DataName.PersonWantTradeHouse)}", "", 10000);
                }
                else if (type == "business")
                {
                    ...
                    EventSys.SendCoolMsg(player, "Предложение", "Обмен",
                        $"{LangFunc.GetText(LangType.Ru, DataName.YouWantTradeBiz)}", "", 10000);
                    EventSys.SendCoolMsg(target, "Предложение", "Обмен",
                        $"{LangFunc.GetText(LangType.Ru, DataName.PersonWantTradeBiz)}", "", 10000);
                }
            }
            catch (Exception e)
            {
                Log.Write($"TryTrade Exception: {e}");
            }
        }
        ...
        public void RoulleteBuy(ExtPlayer player, int caseid, int count)
        {
            ...
            {
                Log.Write($"RoulleteOpen Exception: {e}");
            }
        }
        ...
        public static void RoulleteConfirm(ExtPlayer player, bool type, int IndexList)
        {
            try
            {
                if (IndexList == -1)
                {
                    ...
                    var _DoneListId = new List<int>();
                    foreach (var playerData in sessionData.RouletteData)
                        if (!playerData.Done)
                            _DoneListId.Add(playerData.IndexList);
                    foreach (var index in _DoneListId) Repository.RouletteConfirmCase(player, type, index);
                }
                else
                {
                    Repository.RouletteConfirmCase(player, type, IndexList);
                }
            }
            catch (Exception e)
            {
                Log.Write($"RoulleteConfirm Exception: {e}");
            }
        }

        [RemoteEvent("server.gamemenu.inventory.move")]
        public void InventoryMove(ExtPlayer player, string selectArrayName, int selectIndex, string hoverArrayName,
            int hoverIndex)
        {
            try
            {
            ...
            {
                Log.Write($"InventoryMove Exception: {e}");
            }
        }

        [RemoteEvent("server.gamemenu.inventory.move.stack")]
        public void InventoryMoveStack(ExtPlayer player, string selectArrayName, int selectIndex, string hoverArrayName,
            int hoverIndex, int count)
        {
            try
            {
                if (!player.IsCharacterData()) return;
            ...
            {
                Log.Write($"InventoryMoveStack Exception: {e}");
            }
        }
        ...
        public void InventoryUse(ExtPlayer player, string ArrayName, int Index)
        {
            ...
            {
                Log.Write($"InventoryUse Exception: {e}");
            }
        }
        ...
        public void InventoryDrop(ExtPlayer player, string ArrayName, int Index, float posZ)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.CooldownItemDrop), 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouCantDrop), 3000);
                    return;
                }
            ...
            {
                Log.Write($"InventoryDrop Exception: {e}");
            }
        }
        ...
        public void InventoryTrade(ExtPlayer player, int status)
        {
            ...
            {
                Log.Write($"InventoryTrade Exception: {e}");
            }
        }
        ...
        public void InventoryTradeMoney(ExtPlayer player, int money)
        {
            ...
            {
                Log.Write($"InventoryTradeMoney Exception: {e}");
            }
        }
        ...
        public void InventoryClose(ExtPlayer player)
        {
            ...
            {
                Log.Write($"InventoryClose Exception: {e}");
            }
        }
        ...
        public void InventoryOtherClose(ExtPlayer player)
        {
            ...
            {
                Log.Write($"InventoryOtherClose Exception: {e}");
            }
        }
        ...
        public void InventoryStack(ExtPlayer player, string ArrayName, int Index, int Id, int Value)
        {
            ...
            {
                Log.Write($"InventoryStack Exception: {e}");
            }
        }
        ...
        public void InventoryBuy(ExtPlayer player, string ArrayName, int Index, int Value)
        {
            try
            {
                ...
                var Item = Repository.GetItemData(player, ArrayName, Index);
                ...
                Trigger.ClientEvent(player, "openDialog", "buy_tent",
                    LangFunc.GetText(LangType.Ru, DataName.DialogBuyFromTent, Repository.ItemsInfo[Item.ItemId].Name,
                        Wallet.Format(Item.Price)));
        ...
        public void UpdateStats(ExtPlayer player)
        {
            ...
            {
                Log.Write($"UpdateStats Exception: {e}");
            }
        }
        ...
        public void InventoryToput(ExtPlayer player, string ArrayName, int Index)
        {
            ...
            {
                Log.Write($"InventoryToput Exception: {e}");
            }
        }
        ...
        public void animationEvent(ExtPlayer player, bool toggle, string dirt, string name, int flag)
        {
            ...
            {
                Log.Write($"animationEvent Exception: {e}");
            }
        }

        [RemoteEvent("server.dropeditor.finish")]
        public void dropEditorFinish(ExtPlayer player, string arrayName, int index, float posX, float posY, float posZ,
            float rotX, float rotY, float rotZ)
        {
            ...
            {
                Log.Write($"dropEditorFinish Exception: {e}");
            }
        }
    }
}