...
namespace NeptuneEvo.Events.HeliCrash
{
    ...
    {
        private static readonly uint _heliHash = NAPI.Util.GetHashKey("p_crahsed_heli_s");
        private static readonly uint _boxHash = NAPI.Util.GetHashKey("prop_box_ammo03a");

        private static readonly int _startBlockHour = 12;
        private static readonly int MaxHealth = 1000;

        private static readonly Dictionary<HeliType, int> HeliChances = new Dictionary<HeliType, int>
        {
            { HeliType.Medical, 60 },
            { HeliType.Army, 40 }
        };

        private static readonly Dictionary<HeliType, string> HeliDisplayName = new Dictionary<HeliType, string>
        {
            { HeliType.Medical, "спасательный" },
            { HeliType.Army, "военный" }
        };

        private static readonly List<HeliCords> Cords = new List<HeliCords>
        {
            new HeliCords("Pacific Bluffs", new Vector3(-2219.26855, 113.428825, 159.576553),
                new Vector3(-4.92, 12.4, -0.87),
                new[]
                {
            ...
            new HeliCords("VineWood Hills", new Vector3(-1263.03662, 1142.36719, 280.522),
                new Vector3(-0.27, 8.07, 0.15),
                new[]
                {
            ...
            new HeliCords("VineWood Hills", new Vector3(427.456573, 1467.124, 335.605652),
                new Vector3(14.29, 10.41, 2.26),
                new[]
                {
            ...
            new HeliCords("Land Act DAM", new Vector3(1795.37866, -249.620255, 291.150543),
                new Vector3(0.48, 4.07, -0.25),
                new[]
                {
            ...
            new HeliCords("Palomino Highlands", new Vector3(1912.22266, -1704.93311, 197.946442),
                new Vector3(-5.65, -1.90, -0.69),
                new[]
                {
            ...
            new HeliCords("Los Santos Airport", new Vector3(-1306.67957, -2515.31787, 12.7466946),
                new Vector3(0, 1.1, 135.97),
                new[]
                {
        ...
        private static readonly List<HeliBox> HeliBoxs = new List<HeliBox>();
        private static readonly List<string> TimersList = new List<string>();
        ...
        public static void Create()
        {
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    EventSys.SendCoolMsg(foreachPlayer, "HeliCrash", "Через 10 минут!",
                        $"Над {(foreachSessionData.IsRadioInterceptor ? _heliCords.Name : "штатом")} пролетит {HeliDisplayName[_heliType]} вертолёт, который передает сигнал бедствия.",
                        "", 10000);
                }
                ...
                MainTimer = Timers.StartOnce(Main.ServerNumber == 0 ? 1000 * 60 : 1000 * 60 * 10, () =>
                {
                    try
                    {
                        ...
                        _heliObj = (ExtObject)NAPI.Object.CreateObject(NAPI.Util.GetHashKey("bkr_prop_meth_ammonia"),
                            new Vector3(
        ...
        private static void SpawnObjects()
        {
            try
            {
                var heli = (ExtObject)NAPI.Object.CreateObject(_heliHash, _heliCords.HeliPosition,
                    _heliCords.HeliRotation, dimension: 0);
                ...
                boxRandom = Horses.Shuffle(boxRandom);
                ...
                for (var i = 0; i < _heliCords.BoxPositions.Length; i++)
                {
                    ...
                    heliBox.Obj = (ExtObject)NAPI.Object.CreateObject(_boxHash, _heliCords.BoxPositions[i],
                        _heliCords.BoxRotations[i],
                        dimension: 0);

                    heliBox.Shape = CustomColShape.CreateCylinderColShape(_heliCords.BoxPositions[i], 1.5f, 2, 0,
                        ColShapeEnums.HeliCrash, i + 1);
        ...
        private static void CreateItems(string locationName, string location, int index)
        {
            try
            {
                ...
                if (_heliType == HeliType.Medical)
                {
                    if (index == 0)
                    {
                        for (var item = 0; item < rand.Next(5, 20); item++)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.HealthKit, stack: false);
                    }
                    else if (index == 1)
                    {
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.FlareGun, rand.Next(1, 3),
                            WeaponRepository.GetHaliSerial(), false, 100);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, rand.Next(1, 2),
                            "186_0_True", false);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, 1, "38_0_True", false);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, 1, "46_0_True", false);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, 1, "177_0_True", false);

                        if (rand.Next(0, 100) >= 97)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.Mask, 1, "36_0_True",
                                false);
                    }
                    else
                    {
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.GasCan, rand.Next(3, 10),
                            stack: false);

                        for (var item = 0; item < rand.Next(0, 3); item++)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.Epinephrine, 10,
                                stack: false);
                    }
                }
                else
                {
                    if (index == 0)
                    {
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.BodyArmor, rand.Next(3, 10),
                            "100", false, 100);

                        var ammo = rand.Next(0, 6);

                        if (ammo == 0)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.PistolAmmo,
                                rand.Next(20, 100), "", false, 100);
                        else if (ammo == 1)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.RiflesAmmo,
                                rand.Next(20, 250), "", false, 100);
                        else if (ammo == 2)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.ShotgunsAmmo,
                                rand.Next(20, 50), "", false, 100);
                        else if (ammo == 3)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.SMGAmmo,
                                rand.Next(20, 300), "", false, 100);
                        else if (ammo == 4)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.SniperAmmo,
                                rand.Next(20, 48), "", false, 100);

                        if (rand.Next(0, 100) >= 97)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.CarCoupon, 1, "Winky",
                                false);
                    }
                    else if (index == 1)
                    {
                        for (var item = 0; item < rand.Next(4, 12); item++)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.Material, 170,
                                stack: false);

                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.HeavyShotgun, 1,
                            WeaponRepository.GetHaliSerial(), false, 100);

                        if (rand.Next(0, 100) >= 98)
                            Chars.Repository.AddNewItem(null, locationName, location, ItemId.HeavySniperMk2, 1,
                                WeaponRepository.GetHaliSerial(), false, 100);
                    }
                    else
                    {
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.Revolver, rand.Next(2, 8),
                            WeaponRepository.GetHaliSerial(), false, 100);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.MG, rand.Next(1, 3),
                            WeaponRepository.GetHaliSerial(), false, 100);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.AssaultRifle, rand.Next(3, 5),
                            WeaponRepository.GetHaliSerial(), false, 100);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.MilitaryRifle, rand.Next(1, 3),
                            WeaponRepository.GetHaliSerial(), false, 100);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.AssaultShotgun,
                            rand.Next(1, 2), WeaponRepository.GetHaliSerial(), false, 100);
                        Chars.Repository.AddNewItem(null, locationName, location, ItemId.MarksmanRifle, rand.Next(1, 2),
                            WeaponRepository.GetHaliSerial(), false, 100);
                    }
                }
            }
        ...
        [Interaction(ColShapeEnums.HeliCrash, true)]
        public void InHeliCrash(ExtPlayer player, int index)
        {
            try
            {
                ...
                keyClampData.ColShapesData =
                    new ExtColShapeData(ColShapeEnums.HeliCrash, index, (int)ColShapeData.Error);
            ...
            try
            {
                ...
                if (heliBox.Health == 0)
                {
                    ...
                    if (hack_chance <= 50)
                    {
                        ...
                        KeyClamp.ClearKeyClamp(keyClampData.ColShapesData.ColShapeId, keyClampData.ColShapesData.Index,
                            keyClampData.ColShapesData.ListId);
                    }
                    else
                    {
                        heliBox.Health = MaxHealth;
                    ...
                    if (count > 0)
                        Chars.Repository.Remove(player, $"char_{characterData.UUID}", "inventory", ItemId.ArmyLockpick);
                }
