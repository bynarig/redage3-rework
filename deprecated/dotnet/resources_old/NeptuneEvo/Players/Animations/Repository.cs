...
namespace NeptuneEvo.Players.Animations
{
    ...
    {
        private static readonly Dictionary<AnimationList, AnimationData> Animations =
            new Dictionary<AnimationList, AnimationData>
            {
                {
        ...
        public static void Stop(ExtPlayer player, AnimationList animationList)
        {
            ...
            {
                Trigger.StopAnimation(player, anim.StopAnimDict == string.Empty);
        ...
        public static void PlayScenario(ExtPlayer player, string name)
        {
            Trigger.SetMainTask(() =>
            {
                if (player.IsCharacterData()) player.SetSharedData("cSen", name);
            });
        }
        ...
        public static void StopScenario(ExtPlayer player)
        {
            Trigger.SetMainTask(() =>
            {
                if (player.IsCharacterData()) player.ResetSharedData("cSen");
            });
        }
    }
}