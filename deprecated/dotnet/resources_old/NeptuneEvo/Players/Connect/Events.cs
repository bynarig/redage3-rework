...
namespace NeptuneEvo.Players.Connect
{
    internal class Events : Script
    {
        ...
        private void OnPlayerConnected(ExtPlayer player)
        {
            ...
            {
                Log.Write(
                    $"OnPlayerConnected error, player ({player.Name} | {player.Value} | {player.SocialClubName}) already contains somehow?!");
                player.Kick();
                return;
            }
            ...
            {
                Trigger.ClientEvent(player, "queue.text", true,
                    "Ошибка входа на сервер #9461, попробуйте перезайти в игру.");
                return;
            }
            ...
            Trigger.SetTask(async () => { await Repository.OnPlayerConnected(player); });
        }
    }
}