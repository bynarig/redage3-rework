...
namespace NeptuneEvo.Players.Disconnect
{
    public class Repository
    {
        ...
        public static void OnPlayerDisconnect(ExtPlayer player, DisconnectionType type, string reason,
            bool isSave = true)
        {
            try
            {
                ...
                Miner.OnPlayerDisconnected(player);
                ...
                try
                {
                    ...
                    {
                        if (Commands.ActionLabels[player.Value].Item1 != null &&
                            Commands.ActionLabels[player.Value].Item1.Exists)
                            Commands.ActionLabels[player.Value].Item1.Delete();
                ...
                {
                    Log.Write($"Event_OnPlayerDisconnected - ActionLabels.Delete() Exception: {e}");
                }
                ...
                if (characterData != null)
                {
                    ...
                    {
                        var selveh = sessionData.SelectData.SelectedVeh;
                    ...
                    {
                        var target = sessionData.DicePlayingWith;
                        ...
                        if (targetSessionData != null)
                        {
                            ...
                            Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.Ru, DataName.ChelVishelNoGame), 3000);
                        }
                        ...
                        Chars.Repository.TradeGet(player))
                    {
                        ...
                        var buyer = sessionData.SellItemData.Buyer;
                        ...
                        if (seller == player)
                        {
                            if (buyerSessionData != null)
                            {
                                ...
                                Notify.Send(buyer, NotifyType.Error, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.DisconnectedSellClose), 3000);
                            }
                        }
                    ...
                    {
                        var cufdata = sessionData.CuffedData;
                        if (cufdata.Cuffed && (cufdata.CuffedByMafia || cufdata.CuffedByCop) &&
                            characterData.DemorganTime <= 0)
                        {
                            if (cufdata.CuffedByCop)
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.DemorganCuffs,
                                    player.Name));
                                GameLog.Admin("server", "demorgan(4ч,выход из игры в наручниках)", $"{player.Name}");
                            }
                            else if (cufdata.CuffedByMafia)
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.Ru, DataName.DemorganStyzahka,
                                    player.Name));
                            ...
                            Chars.Repository.RemoveAllWeapons(player, true, true, true);
                        }
                    }
                    ...
                    foreach (var numberDell in vehiclesNumber)
                        VehicleStreaming.DeleteVehicle(
                            VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Personal, numberDell));
                    ...
                    TankRoyale.PlayerExitFromTR(player, true, true, true);
                    ...
                    Blackjack.Disconnect(player, type);
                    Roullete.Disconnect(player, type);
                    Horses.Disconnect(player, type);
                    Spin.Disconnect(player, type);
                    Voice.Voice.PlayerQuit(player, reason);
                    GameLog.Disconnected(characterData.UUID, player.Value,
                        Enum.GetName(typeof(OwnDisconnectionType), type), login);
                    ...
                    if (characterData.AdminLVL >= 1 && characterData.AdminLVL <= 5)
                        Trigger.SendToAdmins(1,
                            $"!{{#FFB833}}[A] {player.Name} отключился ({characterData.AdminLVL} lvl)");

                    Commands.RPChat("sb", player, LangFunc.GetText(LangType.Ru, DataName.PlayerDisconnect),
                        optName: player.Name + $" ({player.Value})");
                ...
                foreach (var key in NAPI.Data.GetAllEntityData(player))
                ...
                if (characterData != null && Chars.Repository.ItemsData.ContainsKey($"char_{characterData.UUID}") &&
                    Chars.Repository.ItemsData[$"char_{characterData.UUID}"].ContainsKey("accessories") &&
                    Chars.Repository.ItemsData[$"char_{characterData.UUID}"]["accessories"].ContainsKey(7))
                {
