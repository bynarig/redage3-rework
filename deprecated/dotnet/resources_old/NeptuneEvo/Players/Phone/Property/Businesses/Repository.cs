...
namespace NeptuneEvo.Players.Phone.Property.Businesses
{
    public class Repository
    {
        public static void GetData(ExtPlayer player, int bizId)
        {
            ...
            Trigger.ClientEvent(player, "client.phone.business.init", JsonConvert.SerializeObject(stats),
                JsonConvert.SerializeObject(stocks), JsonConvert.SerializeObject(orders));
        }
        ...
        private static int CountMinOrder(int bizType)
        {
            return bizType == 14 || (bizType >= 2 && bizType <= 5) ? 1 : 10;
        }
        ...
        public static void AddOrder(ExtPlayer player, string name, int value)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.AlreadyOrderedThis), 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.Enter10To, productsData.MaxCount - product.Lefts), 5000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.MaxCol, productsData.MaxCount - product.Lefts), 5000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.OrderedAmount, product.Name, value, order.UID), 3000);
        ...
        public static void MaxProducts(ExtPlayer player)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (count > 0)
                {
                    ...
                    message += LangFunc.GetText(LangType.Ru, DataName.AmountCosts, product.Name, count,
                        count * itemPrice);
                    success = true;
                }
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoTovarZakaz), 3000);
                return;
            }
            ...
            Trigger.ClientEvent(player, "openDialog", "BIZ_CONFIRM_BUY_ORDERS",
                $"Вы желаете заказать товаров в количестве <font style='color: red'>{whCount}</font> на сумму <font style='color: green'>{whCost}$</font>.<br/><br/>{message}");
        }
        ...
        public static void MaxProductsConfirm(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.AllTovarSucOrder), 3000);
        ...
        public static void CancelOrder(ExtPlayer player, int uid)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    "Вы не можете отменить заказ, пока его доставляют", 3000);
                return;
            }
            ...
            biz.Zatratq -= price;
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы отменили заказ на {order.Name}",
                3000);
        }
        ...
        public static void ExtraCharge(ExtPlayer player, string name, int value)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.CantLotteryPriceChange), 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.CantSetThisPrice), 3000);
                return;
            }

            product.Price = value;

            var ch = biz.Type == 7 || biz.Type == 11 || biz.Type == 12 || product.Name == "Татуировки" ||
                     product.Name == "Парики"
                ? "%"
                : "$";
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                LangFunc.GetText(LangType.Ru, DataName.ThenThisCosts, product.Name, product.Price, ch), 3000);
        ...
        public static void OnSell(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "openDialog", "BIZ_SELL_TOGOV",
        ...
        public static void OnSellConfirm(ExtPlayer player)
        {
            ...
            Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank,
                LangFunc.GetText(LangType.Ru, DataName.SellBizGos, Wallet.Format(price)), DateTime.Now);
        }
    }
}