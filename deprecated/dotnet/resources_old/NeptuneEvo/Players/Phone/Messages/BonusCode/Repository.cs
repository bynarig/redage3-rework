    ...
    public class Repository
    {
        public static string Enter(ExtPlayer player, string text)
        {
            ...
            if (Main.BonusCodes.ContainsKey(text))
                ...
                {
                    if (accountData.BonusCodes.Contains(text)) return LangFunc.GetText(LangType.Ru, DataName.BonusUsed);
                    ...
                    if (pcdata.UsedLimit == 0 || pcdata.UsedTimes < pcdata.UsedLimit)
                    {
                        ...
                        if (pcdata.RewardVipLvl != 0)
                        {
                            if (accountData.VipLvl == 0)
                            {
                                switch (pcdata.RewardVipLvl)
                                {
                                    case 1:
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.BronzeBonus, pcdata.RewardVipDays),
                                            15000);
                                        break;
                                    case 2:
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.SilverBonus, pcdata.RewardVipDays),
                                            15000);
                                        break;
                                    case 3:
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    ...
                                    case 4:
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.PlatinumBonus, pcdata.RewardVipDays),
                                            15000);
                                        break;
                                    default:
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.Ru, DataName.ErrorBonus, pcdata.RewardVipDays),
                                            15000);
                                        break;
                                }
                        ...
                        if (characterData.Gender)
                        {
                            if (pcdata.RewardItemsMale.Count != 0)
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.BonusItems, pcdata.RewardItemsMale.Count),
                                    15000);
                            }
                        }
                        else
                        {
                            if (pcdata.RewardItemsFemale.Count != 0)
                            {
                                foreach (var item in pcdata.RewardItemsFemale)
                                    WeaponRepository.GiveWeapon(player, item.ItemId, item.Data, item.Count);
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                    LangFunc.GetText(LangType.Ru, DataName.BonusItems, pcdata.RewardItemsFemale.Count),
                                    15000);
                            }
                        }
