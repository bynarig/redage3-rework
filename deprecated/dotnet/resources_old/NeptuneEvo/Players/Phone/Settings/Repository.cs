...
namespace NeptuneEvo.Players.Phone.Settings
{
    ...
    {
        private static readonly List<string> SoundsCall = new List<string>
        {
        ...
        private static readonly List<string> SoundsNotify = new List<string>
        {
        ...
        public static void OnRemoveSim(ExtPlayer player)
        {
            ...
            if (Chars.Repository.isFreeSlots(player, ItemId.SimCard) != 0)
                return;
        ...
        public static void OnUpdateAir(ExtPlayer player)
        {
            ...
            if (phoneData.Settings.IsAirAntiFlood > DateTime.Now)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    "Авиа режим можно менять раз в 5 минут", 3000);
                return;
            }
        ...
        public static void OnUpdateForbesVisible(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "phone.notify", (int)DefaultNumber.RedAge,
                "Статус приватности Forbes изменится со следующим обновлением ТОПа.", 5);
        }
