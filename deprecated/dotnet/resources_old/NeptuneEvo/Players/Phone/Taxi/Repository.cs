...
namespace NeptuneEvo.Players.Phone.Taxi
{
    public class Repository
    {
        ...
        public static void OnOrder(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.AlreadyTaxiOrdered), 3000);
                return;
            }
            ...
            foreach (var foreachPlayer in GetListTaxi())
            {
                ...
                taxiCount++;
                Trigger.ClientEvent(foreachPlayer, "client.phone.taxijob.add", characterData.UUID, player.Name,
            ...
            if (taxiCount > 0)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.WaitForTaxi, taxiCount), 3000);
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NoTaxiHere), 3000);
            }
        }
                ...
                bool isNotRemove = true) //Отмена для таксиста или вызывающего
        {
            ...
            if (taxiData == null)
            {
                if (!isDisconnect)
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouDoesntCallTaxi), 3000);
                return;
            }
            ...
            else
            {
                if (isNotRemove)
                {
                    ...
                    if (!isDisconnect)
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.YouCancelTrip), 3000);
                }
                ...
                if (isDriver)
                {
                    if (isNotRemove)
                    {
                        foreach (var foreachPlayer in GetListTaxi())
                        {
                            ...
                            Trigger.ClientEvent(foreachPlayer, "client.phone.taxijob.add", targetCharacterData.UUID,
