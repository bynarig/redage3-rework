...
namespace NeptuneEvo.Players.Phone.Taxi.Orders
{
    ...
    {
        private static readonly int OneMileagePrice = 30; //Цена за милю
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            foreach (var order in Taxi.Repository.OrdersList)
            {
        ...
        public static void OnTake(ExtPlayer player, int id)
        {
            ...
            {
                var taxiData = Taxi.Repository.OrdersList.FirstOrDefault(ol => ol.PlayerUUID == id);

                if (taxiData != null && taxiData.Player.IsCharacterData())
                {
                    taxiData.Driver = player;

                    GetSelect(player, true);
                    Trigger.ClientEvent(taxiData.Player, "client.phone.taxi.updateOrder", sessionData.Name,
                        vehicle.NumberPlate, player.Position.X, player.Position.Y, player.Position.Z);

                    taxiData.DriverTimer = Timers.Start(2500, () => OnUpdatePos(player, taxiData.Player), true);

                    foreach (var foreachPlayer in Taxi.Repository.GetListTaxi())
                    {
        ...
        public static void OnUpdatePos(ExtPlayer player, ExtPlayer target)
        {
            ...
            if (player.Position.DistanceTo2D(target.Position) < 10f)
            {
                ...
                Taxi.Repository.OnCancel(target, isNotRemove: false);
                return;
            }
            ...
            if (vehicle != null && characterData.WorkID == (int)JobsId.Taxi &&
                (sessionData.RentData != null || sessionData.WorkData.OnWork))
                Trigger.ClientEvent(target, "client.phone.taxi.updatePosOrder", player.Position.X, player.Position.Y,
                    player.Position.Z);
        }
        ...
        public static void GetSelect(ExtPlayer player, bool isTake = false)
        {
            ...
            if (characterData.WorkID == (int)JobsId.Taxi &&
                (sessionData.RentData != null || sessionData.WorkData.OnWork))
            {
                var taxiData = Taxi.Repository.OrdersList.FirstOrDefault(ol => ol.Driver == player);
            ...
            else
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.NotWorkTaxi), 3000);
            }

            Trigger.ClientEvent(player, "client.phone.taxijob.initSelect", JsonConvert.SerializeObject(selectedOrders),
                isTake);
        }
        ...
        public static void OnPlayerEnterVehicle(ExtPlayer player, ExtVehicle vehicle, sbyte seatId)
        {
            ...
            if (vehicleLocalData.WorkDriver != -1)
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.Ru, DataName.GiveTochkaZ), 5000);
                ...
                {
                    var taxiData = Taxi.Repository.OrdersList.FirstOrDefault(ol => ol.Driver == driverPlayer);
                    ...
                    if (taxiData != null)
                    {
                        ...
                        Taxi.Repository.OnCancel(player, isNotRemove: false);
                    }
        ...
        public static void OnPlayerDisconnect(ExtPlayer player)
        {
            ...
            if (sessionData.TaxiData.Driver != null)
            {
                ...
                if (driverSessionData != null)
                {
                    ...
                    Notify.Send(driver, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.PassengerCancel), 5000);
                }
            }
                ...
                if (sessionData.TaxiData.Passager != null)
                {
                    ...
                    if (targetSessionData != null)
                    {
                        ...
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.Ru, DataName.DriverCancel), 10000);
                    }
                }
        }
        ...
        public static void EndWork(ExtPlayer player)
        {
            ...
            if (sessionData.WorkData.OnWork)
            {
                ...
                if (sessionData.TaxiData.Passager != null)
                {
                    ...
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.TaxiDriverLost), 3000);
                }
            }
        }
        ...
        public static void TaxiPay(ExtPlayer player, int mileage)
        {
            ...
            if (sessionData.WorkData.LastClientTime < DateTime.Now)
            {
                ...
                {
                    characterData.JobSkills.Add((int)JobsId.Taxi, 1);
                }

                sessionData.WorkData.LastClientTime = DateTime.Now.AddMinutes(5);
            }

            if (UpdateData.CanIChange(target, price, true) != 255)
            {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                    "У игрока не хватает денег для оплаты.", 6000);
                return;
            }
